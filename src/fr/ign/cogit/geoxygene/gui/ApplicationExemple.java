...
public class ApplicationExemple {
	...
	public static void main(String[] args) {
		...
		menuItemDernierChargement.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				if (chargement != null) {
					...
					logger.info("Chargement de "+chargement.getFichiers().size()+" fichiers");
					for(Entry<String,String> entry:chargement.getFichiers().entrySet()) {
						logger.info("Chargement du fichier "+entry.getValue()+" pour la population "+entry.getKey());
		...
		menu.add(menuItem);
		JMenuItem menuItemFond = new JMenuItem("Editeur la couleur du fond de carte");
		menuItemFond.addActionListener(new ActionListener(){
			@Override
			public void actionPerformed(ActionEvent e) {
				Color newColor = JColorChooser.showDialog(fr, "Choisir une couleur pour le trait", fr.getPanelVisu().getBackground());
