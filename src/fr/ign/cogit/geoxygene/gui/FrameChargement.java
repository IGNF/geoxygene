...
public class FrameChargement extends JFrame {
  ...
  /**
   * ComboBox utilisée pour éditer les valeurs des types associés aux
   * shapefiles.
   */
    ...
    this.dataModel = new AbstractTableModel() {
      ...
      public void setValueAt(Object value, int row, int col) {
        ...
        if (col == 1) {
          boolean found = false;
          int index = 0;
          for (index = 0; index < FrameChargement.this.layerNames.size()
              && !found; index++) {
  ...
  void addShapeFiles() {
    ...
    this.updateLayout(files);
  }

  /**
   * Ajout de répertoires contenant des fichiers shapefiles. Les répertoires
  ...
  void addDirectories() {
    ...
    this.updateLayoutDirectories(files);
  }

  private void updateLayoutDirectories(File[] files) {
    for (File file : files) {
      if (file.isFile()) {
        if (file.getName().endsWith(".shp") || file.getName().endsWith(".SHP")) {
          this.updateLayout(file);
        }
      } else {
        this.updateLayoutDirectories(file.listFiles());
      }
    }
  }
   ...
   * @param shapefile fichier shapefile à ajouter à la table des fichiers à
   *          charger.
   */
  private void updateLayout(File shapefile) {
    String shapefileName = shapefile.getName();
    String layerName = this.getLayerNameFromFileName(shapefileName);
    ...
    if (layerName != null) {
      ...
      for (index = 0; index < this.layerNames.size() && !found; index++) {
        if (this.layerNames.get(index).equals(layerName)) {
          if (FrameChargement.logger.isTraceEnabled()) {
            FrameChargement.logger.trace(shapefileName + " - layer = "
                + layerName);
          }
  ...
  private JDialog createDialog(Frame parent) {
    ...
    col.setCellRenderer(new MyComboBoxRenderer(this.layerNames
    ...
    JButton addDirectoryButton = new JButton(
        "Ajouter des répertoires contenant des Shapefiles",
  ...
  /**
   * TableCellRenderer utilisé pour la colonne contenant les noms des fichiers
   * chargés.
   */
  ...
  /**
   * TableCellRenderer utilisé pour la colonne contenant le type des objets
   * chargés. Cette colonne est éditable grace à une comboBox.
   */
    ...
    public Component getTableCellRendererComponent(JTable table, Object value,
      ...
      if (isSelected) {
        ...
        if (value.equals(new Integer(0))) {
          this.setBackground(new Color(230, 30, 30));
        } else {
          if (row % 2 == 0) {
            this.setBackground(new Color(230, 230, 230));
          } else {
      ...
      // if ((selectedIndex<0) || (selectedIndex>=sld.getLayers().size()) ||
      // (sld.getLayers().get(selectedIndex)==null)) return this;
      ...
      this.setSelectedItem(layerName);
      return this;
    }
  }
   ...
   * Ce n'est qu'un heuristique simple : on considère que le fichier contient
   * des bâtiments si son nom contient "batiment", etc.
  ...
  private String getLayerNameFromFileName(String shapefileName) {
    ...
    if (fileName.contains("batiment") && !fileName.contains("lin")
        && !fileName.contains("pct")) {
      return "Batiment";
      ...
      return "SurfaceRoute";
    } else if (fileName.contains("surface_eau")
        || fileName.contains("res_hydro_surf")) {
      return "SurfaceEau";
    } else if (fileName.contains("terrain_sport")
        || fileName.contains("eq_sport_surf")) {
      return "TerrainSport";
    } else if (fileName.contains("troncon_chemin")
        || fileName.contains("autre_voie_com")) {
      return "TronconChemin";
    } else if (fileName.contains("troncon_cours_eau")
        || fileName.contains("res_hydro_lin")) {
    ...
    } else if (fileName.contains("vegetation")
        || fileName.contains("occ_sol_surf")
        || fileName.contains("zone_arboree")) {
      return "Vegetation";
