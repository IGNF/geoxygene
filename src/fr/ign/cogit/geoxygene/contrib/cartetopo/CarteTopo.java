...
public class CarteTopo extends DataSet {
    ...
    public Population<Arc> getPopArcs() {return (Population<Arc>)this.getPopulation(I18N.getString("CarteTopo.Edge"));} //$NON-NLS-1$
    /** Population des noeuds de la carte topo. */
    @SuppressWarnings("unchecked")
    public Population<Noeud> getPopNoeuds() {return (Population<Noeud>)this.getPopulation(I18N.getString("CarteTopo.Node"));} //$NON-NLS-1$
    /** Population des faces de la carte topo. */
    @SuppressWarnings("unchecked")
    public Population<Face> getPopFaces() {return (Population<Face>)this.getPopulation(I18N.getString("CarteTopo.Face"));} //$NON-NLS-1$
    /** Population des groupes de la carte topo. */
    @SuppressWarnings("unchecked")
    public Population<Groupe> getPopGroupes() {return (Population<Groupe>)this.getPopulation(I18N.getString("CarteTopo.Group"));} //$NON-NLS-1$
    ...
    public CarteTopo(String nomLogique) {
        ...
        this.addPopulation(new Population<Arc>(false, I18N.getString("CarteTopo.Edge"), fr.ign.cogit.geoxygene.contrib.cartetopo.Arc.class,true)); //$NON-NLS-1$
        this.addPopulation(new Population<Noeud>(false, I18N.getString("CarteTopo.Node"), fr.ign.cogit.geoxygene.contrib.cartetopo.Noeud.class,true)); //$NON-NLS-1$
        this.addPopulation(new Population<Face>(false, I18N.getString("CarteTopo.Face"), fr.ign.cogit.geoxygene.contrib.cartetopo.Face.class,true)); //$NON-NLS-1$
        this.addPopulation(new Population<Groupe>(false, I18N.getString("CarteTopo.Group"), fr.ign.cogit.geoxygene.contrib.cartetopo.Groupe.class,false)); //$NON-NLS-1$
    }
     ...
     * <strong>ATTENTION: ne fonctionne bien que pour une carteTopo non spécialisée
    ...
    public void enleveArcs(List<Arc> arcsAEnlever) {
        for(Arc arc:arcsAEnlever) {
            if (!this.getPopArcs().remove(arc)) {
                if (logger.isDebugEnabled()) logger.debug(I18N.getString("CarteTopo.DeletionFailed")+arc); //$NON-NLS-1$
            }
     ...
     *  localisé à son extrémité, il n'aura pas de noeud initial (ou final).</strong>
    ...
    public void creeNoeudsManquants(double tolerance) {
        ...
        fireActionPerformed(new ActionEvent(this,0,I18N.getString("CarteTopo.MissingNodesCreation"),this.getPopArcs().size())); //$NON-NLS-1$
        int index=0;
        ...
        for(Object a:this.getPopArcs()) {
            ...
            fireActionPerformed(new ActionEvent(this,1,I18N.getString("CarteTopo.EdgeHandled"),++index)); //$NON-NLS-1$
        }
        this.getPopArcs().removeAll(arcsVides);
        fireActionPerformed(new ActionEvent(this,4,I18N.getString("CarteTopo.MissingNodesCreated"))); //$NON-NLS-1$
    }
     ...
     * <li>Cette méthode gère aussi les conséquences sur les correspondants.
    ...
    public void filtreArcsDoublons() {
        ...
        fireActionPerformed(new ActionEvent(this,0,I18N.getString("CarteTopo.DoubleEdgesFiltering"),arcs.size()-1)); //$NON-NLS-1$
        for (int i=0; i<arcs.size()-1;i++) {
            Arc arci = arcs.get(i);
            if (arcsAEnlever.contains(arci)) continue;// on a déjà décidé d'enlever cet arc
            ...
            fireActionPerformed(new ActionEvent(this,1,I18N.getString("CarteTopo.EdgeHandled"),i+1)); //$NON-NLS-1$
        }
        ...
        if (logger.isDebugEnabled()) logger.debug(arcsAEnlever.size()+I18N.getString("CarteTopo.NullEdgesToRemove")); //$NON-NLS-1$
        this.enleveArcs(arcsAEnlever);
        //if (!this.getPopArcs().removeAll(arcsAEnlever)) logger.warn("Attention : des arcs n'ont pas pu être enlevés");
        fireActionPerformed(new ActionEvent(this,4,I18N.getString("CarteTopo.DoubleEdgesFiltered"))); //$NON-NLS-1$
    }
    ...
    public void rendPlanaire(double tolerance) {
        ...
        fireActionPerformed(new ActionEvent(this,0,I18N.getString("CarteTopo.PlanarGraphCreation"),this.getPopArcs().size())); //$NON-NLS-1$
        for(int indexArc=0;indexArc<this.getPopArcs().size();indexArc++) {
            Arc arc = this.getPopArcs().get(indexArc);
            if (logger.isDebugEnabled()) logger.debug(I18N.getString("CarteTopo.Handling")+arc); //$NON-NLS-1$
            ...
            for(Arc arcSel:selection) {
                //On construit un multipoint contenant les extrémités de l'arc de la sélection
            ...
            if (listeInter.size() == 0) {
                fireActionPerformed(new ActionEvent(this,1,I18N.getString("CarteTopo.EdgeHandled"),indexArc+1)); //$NON-NLS-1$
                continue; //pas d'intersection avec cet arc
            }
            //on découpe tout
            GM_Object nodedLineStrings = arc.getGeometrie();
            for(Arc a:listeInter) {
                if (a.getGeometrie().isEmpty()) logger.error(I18N.getString("CarteTopo.EmptyGeometryEdge")+a.getGeometrie().toString()); //$NON-NLS-1$
            ...
            if (nodedLineStrings instanceof GM_LineString ) {
                ...
                if (toutesEgales) {
                    if (logger.isDebugEnabled()) logger.debug(I18N.getString("CarteTopo.EqualGeometries")); //$NON-NLS-1$
                    ...
                    for(Arc arcSel:listeInter) {
                        ...
                        if (premierArc) {
                            // on affecte la valeur initiale de l'orientation avec le premier arc rencontré
                    ...
                    logger.error(I18N.getString("CarteTopo.IntersectionProblem") ); //$NON-NLS-1$
                    logger.error(I18N.getString("CarteTopo.EdgeProblem")+ arc ); //$NON-NLS-1$
                    logger.error(I18N.getString("CarteTopo.UnionWithSeveralEdges")); //$NON-NLS-1$
                    for(Arc a:listeInter) {
            ...
            if (nodedLineStrings instanceof GM_MultiCurve ) { //cas où il faut découper
                ...
                for(GM_LineString ligneDecoupe:(GM_MultiCurve<GM_LineString>)nodedLineStrings) {
                    ...
                    if (logger.isDebugEnabled()) logger.debug(I18N.getString("CarteTopo.NewEdge")+arcNouveau); //$NON-NLS-1$
                }
                //2: on virera les arcs initiaux qui ont été découpés
                for(Arc arcSel:listeInter) {
                    if (logger.isDebugEnabled()) logger.debug(I18N.getString("CarteTopo.IntersectionFound")+arcSel); //$NON-NLS-1$
            ...
            //cas imprévu: OUPS
            logger.error(I18N.getString("CarteTopo.PlanarGraphProblem")); //$NON-NLS-1$
            logger.error(I18N.getString("CarteTopo.UnionBug")+nodedLineStrings.getClass() ); //$NON-NLS-1$
            logger.error(I18N.getString("CarteTopo.EdgeProblem")+arc.getGeom().coord() ); //$NON-NLS-1$
        }
    ...
    public void fusionNoeuds(double tolerance) {
        ...
        fireActionPerformed(new ActionEvent(this,0,I18N.getString("CarteTopo.NodesFusion"),this.getPopNoeuds().size())); //$NON-NLS-1$
        ...
        for(int index=0;index<this.getPopNoeuds().size();index++) {
            ...
            //On crée un nouveau noeud dont la géométrie est le centroïde calculé
            ...
            fireActionPerformed(new ActionEvent(this,1,I18N.getString("CarteTopo.NodeFused"),index+1)); //$NON-NLS-1$
        }

        //on enleve tous les anciens noeuds
        if (!aEnlever.isEmpty()&&!this.getPopNoeuds().removeAll(aEnlever)) logger.warn(I18N.getString("CarteTopo.RemovalFailed")); //$NON-NLS-1$
        List<Arc> listeArcsAEnlever = new ArrayList<Arc>();
        // suppression des arcs trop petits
        fireActionPerformed(new ActionEvent(this,2,I18N.getString("CarteTopo.SmallEdgesDeletion"),this.getPopArcs().size())); //$NON-NLS-1$
        int index=0;
        for (Arc arc:this.getListeArcs()) {
            ...
            fireActionPerformed(new ActionEvent(this,3,I18N.getString("CarteTopo.EdgeHandled"),++index)); //$NON-NLS-1$
        }
        fireActionPerformed(new ActionEvent(this,4,I18N.getString("CarteTopo.NodesFused"))); //$NON-NLS-1$
        enleveArcs(listeArcsAEnlever);
    }
    ...
    public void creeTopologieFaces() {
        ...
        fireActionPerformed(new ActionEvent(this,0,I18N.getString("CarteTopo.FaceTopologyEdges"),this.getPopArcs().size())); //$NON-NLS-1$
        int iteration=0;
        ...
        for(Arc arc:this.getPopArcs()) {
            ...
            if ( !arcsDejaTraitesADroite.contains(arc) ) {
                ...
                //face = null;
                if ( cycle == null ) {logger.error(I18N.getString("CarteTopo.RightNullCycle")+arc.getId());continue;} //$NON-NLS-1$
            ...
            if ( !arcsDejaTraitesAGauche.contains(arc) ) {
                ...
                face = null;
                if ( cycle == null ) {if (logger.isDebugEnabled()) logger.debug(I18N.getString("CarteTopo.LeftNullCycle")+arc.getId());continue;} //$NON-NLS-1$
                arcsDuCycle = cycle.getArcs();
                orientationsArcsDuCycle = cycle.getOrientationsArcs();
                geometrieDuCycle = cycle.getGeometrie();
                boolean multiGeometrie=false;
            ...
            fireActionPerformed(new ActionEvent(this,1,I18N.getString("CarteTopo.FaceTopologyEdge"),iteration++)); //$NON-NLS-1$
        }
    ...
    public static List<Cycle> construireGeometrieCycleExterieur(Arc arcInitial, boolean aGauche) {
        ...
        while ( true ) {// ajout de l'arc en cours au cycle...
            if (sensEnCours) {  // arc dans le bon sens
                ...
                if (pilePoints.contains(premierPoint)) {
                    ...
                    for (int i = index ; i < pilePoints.size(); i++) {
                        for (int j = 0 ; j < pileListesPoints.get(i).size() ; j++) {
                            if (!pointsCycle.contains(pileListesPoints.get(i).get(j))) pointsCycle.add(pileListesPoints.get(i).get(j));
                                ...
                                /* si le point est le même que le dernier du cycle en construction, on ne fait rien (le point est doublé dans la géométrie)
                    ...
                    if (pointsCycle.size()>2&&!dangle) { // si on a plus de 3 points et que le cycle n'est pas marqué comme étant une impasse, on l'ajoute
            ...
            if ( arcOriente == null ) {logger.error(I18N.getString("CarteTopo.Error"));return null;} //$NON-NLS-1$
            // au suivant...
        ...
        if (pilePoints.isEmpty()) {logger.error(I18N.getString("CarteTopo.EmptyBoundary"));return null;} //$NON-NLS-1$
     ...
     *   pour chaque segment reliant des points intermédiaires des surfaces.
    ...
    /** 
     * Charge en mémoire les éléments de la classe 'nomClasseGeo' et remplit 'this' avec des correspondants de ces éléments.
     ...
     * @param face face à affecter aux arcs (du côté déterminé par l'orientation et la valeur de aGauche)
     * @param arcsDejaTraitesAGauche liste des arcs déjà traités à gauche à maintenir
     * @param arcsDejaTraitesADroite liste des arcs déjà traités à droite à maintenir
     */
    public void marquerCycle(List<Arc> arcs, List<Boolean> orientations, boolean aGauche, Face face, List<Arc> arcsDejaTraitesAGauche, List<Arc> arcsDejaTraitesADroite) {
        ...
        while (itArcsCycle.hasNext()) {
            ...
            if ( ( orientationOk.booleanValue() && !aGauche) || ( !orientationOk.booleanValue() && aGauche) ){
