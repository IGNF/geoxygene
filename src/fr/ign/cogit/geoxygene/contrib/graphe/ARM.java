...
public class ARM {
	 ...
	 */
	public static CarteTopo creeARM(FT_FeatureCollection points) {
		...
		GM_LineString trait;
		FT_FeatureCollection pointsCopie = new FT_FeatureCollection(points);
		...
		nouveauNoeud= (Noeud)arm.getPopNoeuds().nouvelElement();
		...
		while(true) {
			...
			for(i=0;i<pointsCopie.size();i++) {
				point = (FT_Feature) pointsCopie.get(i);
				if ( ! (point.getGeom() instanceof GM_Point) ) {
					System.out.println("Un des objets en entrée n'est pas un point, renvoie Null");
					return null;
				}
				...
				for(j=0;j<arm.getPopNoeuds().size();j++) {
					noeud = (Noeud)arm.getPopNoeuds().get(j);
			...
			point = (FT_Feature) pointsCopie.get(imin);
			noeud = (Noeud)arm.getPopNoeuds().get(jmin);
			...
			nouveauNoeud.setGeom(point.getGeom());
			nouveauNoeud.addCorrespondant(point); 
			arc = (Arc)arm.getPopArcs().nouvelElement();
	 ...
	 * 
	 */
	public static CarteTopo creeARMsurObjetsQuelconques(FT_FeatureCollection objets) {
		FT_FeatureCollection points=new FT_FeatureCollection();
		
		Iterator itObjets = objets.getElements().iterator();
		while (itObjets.hasNext()) {
			FT_Feature objet = (FT_Feature) itObjets.next();
			if (objet.getGeom() == null) {
				System.out.println("Un des objets en entrée n'a pas de géométrie, renvoie Null");
				return null;
			}
			Noeud objet2 = new Noeud();
			objet2.setGeom(objet.getGeom().centroid());
			points.add(objet2);
		}
