...
public abstract class Distances {
	...
	public static double hausdorff(GM_LineString L, GM_Point P)   {
		Iterator itPts = L.coord().getList().iterator();
		DirectPosition point;
		double distmax = 0, dist;
   
   		while (itPts.hasNext()) {
			point = (DirectPosition) itPts.next();
	 ...
	 *     (bug en particulier si les surfaces sont dégénérées ou trop complexes).
	 */ 
	public static double distanceSurfacique(GM_MultiSurface A, GM_MultiSurface B)   {
		GM_Object inter = A.intersection(B);
	 ...
	 *     (bug en particulier si les surfaces sont dégénérées ou trop complexes).
	 */ 
	public static double distanceSurfaciqueRobuste(GM_MultiSurface A, GM_MultiSurface B)   {
	 ...
	 *     (bug en particulier si les surfaces sont dégénérées ou trop complexes).
	 */
	public static double surfaceIntersection(GM_MultiSurface A, GM_MultiSurface B) {
		Iterator itA = A.getList().iterator();
		Iterator itB;
		double inter=0;
	 ...
	 *     (bug en particulier si les surfaces sont dégénérées ou trop complexes).
	 */
	public static double surfaceUnion(GM_MultiSurface A, GM_MultiSurface B) {
		double inter = surfaceIntersection(A,B);
		if (inter == -1) {
	 ...
	 * Définition : Surface(A inter B) / Surface(A)
	 */
	public static double exactitude(GM_MultiSurface A, GM_MultiSurface B) {
		GM_Object inter = A.intersection(B);
		if ( inter == null ) return 0;
		return inter.area()/A.area();
	}
	 ...
	 * Définition : Surface(A inter B) / Surface(B)
	 */
	public static double completude(GM_MultiSurface A, GM_MultiSurface B) {
		return exactitude(B,A);
	}
