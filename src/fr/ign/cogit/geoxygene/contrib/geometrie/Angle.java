 ...
 * possede un attribut angle qui est la valeur de l'angle (en radians) comprise
 * entre 0 et 2*pi. La methode setAngle(a) ramene a entre en 0 et 2*pi.
 * 
 * English: Class for computations on angles (in radian). The attribute angle is
 * the value of the angle (in radian) between 0 and 2*pi.
 * 
...
public class Angle {
  ...
  public static Angle angleAPiPres(Angle angle2pi) {
    if (angle2pi.valeur > Math.PI) {
      return new Angle(angle2pi.valeur - Math.PI);
    }
  ...
  public Angle angleAPiPres() {
    if (this.valeur > Math.PI) {
      return new Angle(this.valeur - Math.PI);
    }
    return new Angle(this.valeur);
  }

  /** ajoute a à l'angle */
  public void ajoute(Angle a) {
    this.setValeur(this.valeur + a.valeur);
  }
  ...
  /**
   * Angle de la "bissectrice" des deux angles a et b. L'angle est au milieu
   * entre a et b. NB: bissectrice(a,b) = bissectrice(b,a) + PI
   */
  public static Angle bissectrice(Angle a, Angle b) {
    return new Angle(a.valeur + Angle.ecarttrigo(a, b).valeur / 2);
  }

  /** Moyenne de deux angles (défini à pi près). */
  public static Angle moyenne(Angle a, Angle b) {
    return Angle.angleAPiPres(new Angle((a.valeur + b.valeur) / 2));
  }
