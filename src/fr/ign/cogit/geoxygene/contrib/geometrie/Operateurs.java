 ...
 * projections, abscisse curviligne, décalage, orientation... CONTIENT des
 * méthodes de : Projections d'un point Manipulation de l'abscisse curviligne
 * d'une ligne Mesures sur un polygone Offset d'une ligne (décalage)
 * Echantillonage d'une ligne Regression linéaire et beaucoup d'autres choses
 * très diverses ATTENTION: certaines méthodes n'ont pas été conçues ni testées
 * pour des coordonnées 3D English: Very very diverse set of methods on
 * geometries
...
public abstract class Operateurs {
  ...
  /**
   * Projection de M sur le segment [A,B] English: Projects M on a [A,B] author
   * Mustière
   */
   ...
   * premier dans le sens de parcours de la ligne). English: Projects M on the
   * lineString author Mustière
   */
  public static DirectPosition projection(DirectPosition dp, GM_LineString LS) {
    ...
    for (int i = 0; i < listePoints.size() - 1; i++) {
      pt = Operateurs
          .projection(dp, listePoints.get(i), listePoints.get(i + 1));
      d = dp.distance(pt);
      if (d < dmin) {
        ptmin = pt;
        dmin = d;
   ...
   * ligne English: Projects M on the lineString and return the line with the
   * projected point inserted author Mustière
   */
   ...
   * renvoie qu'un seul. English: Projects M on the agregate author Mustière
   */
  public static DirectPosition projection(DirectPosition dp,
    ...
    while (itComposants.hasNext()) {
      ...
      if (composant instanceof GM_LineString) {
        pt = Operateurs.projection(dp, (GM_LineString) composant);
        d = pt.distance(dp);
        geomOK = true;
      }
      if (!geomOK) {
   ...
   * longueur de la ligne. English: Point located at the curvilinear abscisse
   * author Mustière
   */
  ...
  /**
   * Abscisse curviligne du ieme point de la ligne ls. English: curvilinear
   * abscisse of the ith point author Mustière
   */
  ...
  /**
   * coordonnées du point situé sur au milieu de la ligne. English: Point in the
   * middle of the line author Mustière
   */
  ...
  /**
   * renvoie le milieu de [A,B]. English: Point in the middle of [A,B] author
   * Mustière
   */
   ...
   * longueur est 0. Renvoie tous les points si la longueur est supérieure à la
   * longueur de la ligne NB: les points sont renvoyés dans l'ordre en partant
   * du premier point. English: First points of the line. author Mustière
   */
   ...
   * longueur curviligne passée en paramètre du point final. Renvoie null si la
   * longueur est négative. Renvoie le dernier point seulement si la longueur
   * est 0. Renvoie tous les points si la longueur est supérieure à la longueur
   * de la ligne. NB: les points sont renvoyés dans l'ordre en partant du
   * dernier point (ordre inverse par rapport à la géométrie initiale). English:
   * Last points of the line. author Mustière
   */
   ...
   * intermédiaires du contour, ce qui est très approximatif English: Center of
   * the points of the polygon. author Mustière
   */
   ...
   * Le paramètre offset est la taille du décalage. English: shift of a line on
   * the left author Bonin, Rousseaux.
   */
   ...
   * droite). Le paramètre offset est la taille du décalage. English: shift of a
   * line on the right author Bonin, Rousseaux.
   */
   ...
   * premier point de chaque segment. (voir aussi echantillonePasVariable pour
   * une autre méthode ) English: Resampling of a line author Bonin, Rousseaux.
   */
   ...
   * "echantillone", le pas d'echantillonage diffère sur chaque segment de
   * manière à ce que l'on échantillone chaque segment en différents
   * mini-segments tous de même longueur. Le pas en entrée est le pas maximum
   * autorisé. English : Resampling of a line author Grosso.
   */
    ...
    for (int j = 1; j < listePoints.size(); j++) {
      ...
      for (i = 0; i < nseg - 1; i++) {
        Xins = new DirectPosition(point1.getX() + (i + 1) * nouveauPas
            * u1.vectNorme().getX(), point1.getY() + (i + 1) * nouveauPas
            * u1.vectNorme().getY(), point1.getZ() + (i + 1) * nouveauPas
            * u1.vectNorme().getZ());
        listePointsEchant.add(Xins);
      }
    }
  ...
  /**
   * Méthode pour rééchantillonner une GM_LineString. English: Resampling of a
   * line
   */
  ...
  /**
   * Renvoie le point translaté de P avec le vecteur V; Contrairement au "move"
   * de DirectPosition, on ne deplace pas le point P English : Shift of a point
   */
   ...
   * English: Combination of lines author: Mustière
   */
   ...
   * Si JTS plante au calcul d'intersection, on filtre les surfaces avec Douglas
   * et Peucker, progressivement avec 10 seuils entre min et max. English:
   * Robust intersection of objects (to bypass JTS bugs)
   * @param A a geometry
  ...
  public static GM_Object intersectionRobuste(GM_Object A, GM_Object B,
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (intersection != null) {
        if (Operateurs.logger.isDebugEnabled()) {
          Operateurs.logger
              .debug("Calcul d'intersection fait après filtrage avec Douglas Peucker à "
    ...
    Operateurs.logger
        .error("ATTENTION : Plantage du calcul d'intersection, même après nettoyage de la géométrie avec Douglas Peucker");
    return null;
  }
   ...
   * progressivement avec 10 seuils entre min et max. English: Robust union of
   * objects (to bypass JTS bugs) author: Mustière
   */
   ...
   * important, c'est lui qui permet de donner l'angle à 2.pi près. Exemple: la
   * liste des points peut correspondre à n points d'un arc, l'angle représente
   * alors l'orientation générale de ces points, en prenant le premier pour
   * point de départ. English: Linear approximation author: grosso
   */
  ...
  public static boolean superposes(DirectPosition pt1, DirectPosition pt2) {
    return (pt1.getX() == pt2.getX())
        && (pt1.getY() == pt2.getY())
        && (Double.isNaN(pt1.getZ()) ? Double.isNaN(pt2.getZ())
            : (pt1.getZ() == pt2.getZ()));
  }

  /**
   * Teste si 2 <code>DirectPosition</code>s ont les mêmes coordonnées. Le test
   * est effectué en 2D : aucun Z n'est considéré. English: Tests the equality
   * of geometries in 2D
   * @param pt1 une position
   * @param pt2 une position
   * @return vrai si les deux <code>DirectPosition</code>s ont les mêmes
  ...
  /**
   * Teste si 2 <code>GM_Point</code>s ont les mêmes coordonnées. English: Tests
   * the equality of geometries
   */
   ...
   * des extrémités. English: Z interpolation author : Arnaud Lafragueta
   */
   ...
   * géométrie est remplacée par celle fusionée. English: aggregation of
   * surfaces
   */
  public static void fusionneSurfaces(Population<? extends FT_Feature> pop) {
    ...
    for (FT_Feature feature : pop) {
      ...
      while (changed) {
        ...
        if (Operateurs.logger.isDebugEnabled()) {
          Operateurs.logger.debug("intersercting "
              + intersectingFeatures.size() + " features");
        }
   ...
   * de JTS qui n'accèpte pas les géométries dégénérées. Le calcul est effectué
   * dans un repère local centré sur le premier point de la surface, ce qui est
   * utile pour minimiser les erreurs de calcul si on manipule de grandes
   * coordonnées). English: surface of a polygon
   */
  ...
  /**
   * Surface d'un polygone (liste de points supposée fermée). English: surface
   * of a polygon
   */
  ...
  /**
   * détermine si une liste de points tourne dans le sens direct ou non. NB : La
   * liste de points est supposée fermée (premier point = dernier point). NB :
   * renvoie true pour une surface dégénérée. English : orientation of a polygon
   * (direct rotation?)
   */
   ...
   * @return a polygon
   */
  public static GM_Polygon surfaceFromLineStrings(GM_LineString lineString1,
    ...
    double sommeDistance2 = lineString1.startPoint().distance(
        lineString2.endPoint())
        + lineString1.endPoint().distance(lineString2.startPoint());

    // Construction du périmètre sur le JDD de référence
    Iterator<DirectPosition> itPoints = lineString1.coord().getList()
        .iterator();
   ...
   */
  public static GM_LineString union(List<GM_LineString> linestringList) {
