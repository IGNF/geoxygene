...
public abstract class Operateurs {
	...
	public static double abscisseCurviligne(GM_LineString ls, int i) {
		double abs = 0;
		for(int j=0;j<i;j++) {
			abs = abs + Distances.distance(ls.getControlPoint(j),ls.getControlPoint(j+1));
		}
		return abs;
	}
	...
	public static DirectPositionList premiersPoints(GM_LineString ls, double longueur) {
		...
		for(i=1; i<ls.coord().size() ;i++) {
			l = l+Distances.distance(ls.getControlPoint(i-1),ls.getControlPoint(i));
			if ( l > longueur ) break;
	...
	public static DirectPositionList derniersPoints(GM_LineString ls, double longueur) {
		...
		for(i=nbPts-2; i>=0 ;i--) {
			l = l+Distances.distance(ls.getControlPoint(i),ls.getControlPoint(i+1));
			if ( l > longueur ) break;
			listePts.add(ls.getControlPoint(i));
		}
		return listePts;
	...
	public static void fusionneSurfaces(Population popSurf) {
		...
		while (itSurf.hasNext()) {
			...
			List surfAdjacentes = popSurf.select(objSurf.getGeom()).getElements();
			...
			// ATTENTION: bidouille ci-dessous pour pallier l'absence de "copie" générique de géométrie
			surfFusionnee = new GM_Polygon(((GM_Polygon)objSurf.getGeom()).boundary());
