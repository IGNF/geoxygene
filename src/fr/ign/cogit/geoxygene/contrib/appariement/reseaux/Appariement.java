 ...
 * <b>
 * For data matching of complete sets of data, use {@link AppariementIO}.
 * </b>
 *
 * <p>
 ...
 * cartes topo ({@link ReseauApp}).
 * Pour un appariement complet de jeux géo (création carte topo, appariement,
 * export), voir la classe {@link AppariementIO}.</b>
 *
 * @author Mustiere
 * @see AppariementIO
 * @see ReseauApp
...
public abstract class Appariement {
    ...
    .getLogger(Appariement.class.getName());
    /**
     * Default tile size.
     */
     ...
     * ({@link ReseauApp}).
     * <b>The networks given as arguments are modified during this process:
     * groups are created</b>
     * <p>
     * Appariement entre deux réseaux représentés par des carte topo.
     * Processus largement inspiré de celui défini dans la thèse de
     * Thomas Devogèle (1997).
     *
     ...
     * @see ParametresApp
     */
    public static EnsembleDeLiens appariementReseaux(
            final ReseauApp reseau1, final ReseauApp reseau2,
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.NodePrematching") + //$NON-NLS-1$
                    (new Time(System.currentTimeMillis())).toString());
        }
        EnsembleDeLiens liensPreAppNN = preAppariementNoeudNoeud(
        ...
        if (liensPreAppNN.size() != 0) {
            ...
            if (LOGGER.isDebugEnabled()) {
                ...
                "Appariement.EdgePrematching") + //$NON-NLS-1$
                (new Time(System.currentTimeMillis())).toString());
            }
            liensPreAppAA = preAppariementArcArc(reseau1, reseau2, param);
        } else {
            liensPreAppAA = new EnsembleDeLiens(LienReseaux.class);
            liensPreAppAA.setNom(I18N
                    .getString("Appariement.EdgePrematching")); //$NON-NLS-1$
        }
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.NodeMatching") + //$NON-NLS-1$
                    (new Time(System.currentTimeMillis())).toString());
        }
        EnsembleDeLiens liensAppNoeuds = appariementNoeuds(
                reseau1, reseau2, liensPreAppNN, liensPreAppAA, param);
        // cleaning-up
        nettoyageLiens(reseau1, reseau2, liensPreAppNN);
        if (param.varianteRedecoupageNoeudsNonApparies) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N.getString(
                "Appariement.ReprojectionOfNonMatchedNodes") + //$NON-NLS-1$
                (new Time(System.currentTimeMillis())).toString());
            }
            decoupeNoeudsNonApparies(reseau1, reseau2, liensAppNoeuds, param);
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N
                        .getString("Appariement.LinkCleanup") + //$NON-NLS-1$
                        (new Time(System.currentTimeMillis())).toString());
            }
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N.getString(
                "Appariement.NodePrematching") + //$NON-NLS-1$
                (new Time(System.currentTimeMillis())).toString());
            }
            liensPreAppNN = preAppariementNoeudNoeud(reseau1, reseau2, param);
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N.getString(
                "Appariement.NumberOfPrematchedNodes") + //$NON-NLS-1$
                liensPreAppNN.size());
            }
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N.getString(
                "Appariement.NodeMatching") + //$NON-NLS-1$
                (new Time(System.currentTimeMillis())).toString());
            }
            liensAppNoeuds = appariementNoeuds(
                    reseau1, reseau2, liensPreAppNN, liensPreAppAA, param);
            nettoyageLiens(reseau1, reseau2, liensPreAppNN);
        }
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N.
                    getString("Appariement.EdgeMatching") + //$NON-NLS-1$
                    (new Time(System.currentTimeMillis())).toString());
        }
        EnsembleDeLiens liensAppArcs = appariementArcs(
                reseau1, reseau2, liensPreAppAA, liensAppNoeuds, param);
        EnsembleDeLiens tousLiens = EnsembleDeLiens.compile(
                liensAppNoeuds, liensAppArcs);

        if (param.varianteRedecoupageArcsNonApparies) {
            // NB: pas optimal du tout, on recalcule tout après avoir redécoupé
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N.getString(
                "Appariement.ResplittingNonMatchedEdges") + //$NON-NLS-1$
                (new Time(System.currentTimeMillis())).toString());
            }
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.GlobalControl") + //$NON-NLS-1$
                    (new Time(System.currentTimeMillis())).toString());
        }
     ...
     * <p>
     * Enlève tous les liens "liens" des cartes topo en entrée, et
     * vide les liens "liens".
     * @param reseau1 network 1
     * @param reseau2 network 2
     * @param liens links to empty
     */
    public static void nettoyageLiens(
            final ReseauApp reseau1, final ReseauApp reseau2,
     ...
     * @param liens links to empty
     */
    private static void nettoyageLiens(
            final ReseauApp res, final EnsembleDeLiens liens) {
     ...
     * <p>
     * Enlève tous les liens des cartes topo en entrée, et
     * détruit les groupes.
     * @param reseau1 network 1
     * @param reseau2 network 2
     */
     ...
     * <p>
     * Enlève tous les liens de la carte topo en entrée, et
     * détruit les groupes.
     * @param res network
     */
     ...
     *
     * On crée des liens 1-n
     * (n candidats noeuds de BDcomp pour chaque noeud de BDref).
     *
    ...
    public static EnsembleDeLiens preAppariementNoeudNoeud(
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.Assessment") //$NON-NLS-1$
                    + nbCandidats + I18N
                    .getString("Appariement.CandidateCompNodes") //$NON-NLS-1$
                    + nbRef + I18N
                    .getString("Appariement.RefNodesToProcess")); //$NON-NLS-1$
        }
        return liens;
    }
     ...
     *
     * Pour chaque arc du reseau 2, on garde les arcs du reseau 1 qui sont
     * à la fois :
     ...
     *
     * NB: ce pré-appariement est différent de ce qui est proposé dans
     * [Devogèle 97], pour minimiser la sensibilité aux seuils.
     *
     * On crée des liens 1-n: (1 arc de comp) - (n arcs de ref).
     * Un arc de ref peut être alors concerné par plusieurs liens différents.
     * Au total on a donc des relations n-m codées sous la forme de n
     * relations 1-m.
     * @param reseau1 network 1
     * @param reseau2 network 2
     ...
     */
    public static EnsembleDeLiens preAppariementArcArc(
            final CarteTopo reseau1, final CarteTopo reseau2,
            final ParametresApp param) {
        int nbCandidats = 0;
        ...
        while (itArcs.hasNext()) {
            ...
            double dmin = arcComp.premiereComposanteHausdorff(
                    arcRef, param.distanceArcsMax);
            distances.add(new Double(dmin));
            while (itArcsProches.hasNext()) {
                arcRef = (ArcApp) itArcsProches.next();
                double d = arcComp.premiereComposanteHausdorff(
                        arcRef, param.distanceArcsMax);
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("  Bilan : " //$NON-NLS-1$
                    + nbCandidats + I18N.getString(
                    "Appariement.CandidateCompEdges") + //$NON-NLS-1$
                    reseau1.getListeArcs().size() + I18N.getString(
                    "Appariement.RefEdgesToProcess")); //$NON-NLS-1$
        }
        return liens;
    }
     ...
     *  Une évaluation de l'appariement est stockée sur les liens
     *  (note de 0, 0.5 ou 1). Une explication plus détaillée du résultat est
     *  stockée sur les noeuds ref et comp.
     * @param reseau1 network 1
     * @param reseau2 network 2
    ...
    public static EnsembleDeLiens appariementNoeuds(
            final CarteTopo reseau1, final CarteTopo reseau2,
            final EnsembleDeLiens liensPreAppNN,
            final EnsembleDeLiens liensPreAppAA, final ParametresApp param) {
        Population groupesComp = reseau2.getPopGroupes();
        int nbSansHomologue = 0, nbNonTraite = 0,
        nbPlusieursNoeudsComplets = 0, nbPlusieursGroupesComplets = 0,
        nbNoeudNoeud = 0, nbNoeudGroupe = 0,
        ...
        while (itNoeuds.hasNext()) {
            ...
            noeudComp.setResultatAppariement(I18N.getString(
            "Appariement.Unmatched")); //$NON-NLS-1$
        }
        ...
        while (itNoeuds.hasNext()) {
            ...
            noeudRef.setResultatAppariement(I18N.getString(
            "Appariement.NA")); //$NON-NLS-1$
            // On ne traite pas les noeuds isolés
            if (noeudRef.arcs().size() == 0) {
                noeudRef.setResultatAppariement(I18N.getString(
                "Appariement.IsolatedNode")); //$NON-NLS-1$
                nbNonTraite++;
                continue;
            }
            List<LienReseaux> liensDuNoeudRef = new ArrayList(
            ...
            if (liensDuNoeudRef.size() == 0) {
                noeudRef.setResultatAppariement(I18N.getString(
                "Appariement.NoCandidateForMatching")); //$NON-NLS-1$
                nbSansHomologue++;
                continue;
            }
            ...
            List<Noeud> noeudsCompCandidats =
                (liensDuNoeudRef.get(0)).getNoeuds2();
            List<ElementCarteTopo> complets =
                new ArrayList<ElementCarteTopo>();
            List<ElementCarteTopo> incomplets =
                new ArrayList<ElementCarteTopo>();
            ...
            while (itNoeudsComp.hasNext()) {
                ...
                int correspondance = noeudRef.correspCommunicants(
                        noeudComp,
                        liensPreAppAA);
                if (correspondance == 1) {
            ...
            if (complets.size() == 1) {
                ...
                noeudRef.setResultatAppariement(I18N.getString(
                "Appariement.MatchedWithANode")); //$NON-NLS-1$
                noeudComp.setResultatAppariement(I18N.getString(
                "Appariement.MatchedWithANode")); //$NON-NLS-1$
                nbNoeudNoeud++;
                LienReseaux lien = (LienReseaux) liens.nouvelElement();
            ...
            if (complets.size() > 1) {
                ...
                if (complets.size() == 1) {
                    ...
                    lien.addNoeuds2(noeudComp);
                    noeudComp.addLiens(lien);
                    lien.setEvaluation(ZERO_POINT_FIVE);
                    continue;
                }
                ...
                noeudRef.setResultatAppariement(I18N.getString(
                "Appariement.UncertainMatchedWithSeveralNodes")); //$NON-NLS-1$
                nbPlusieursNoeudsComplets++;
                LienReseaux lien = (LienReseaux) liens.nouvelElement();
                lien.addNoeuds1(noeudRef);
                noeudRef.addLiens(lien);
                for (int j = 0; j < complets.size(); j++) {
                    ...
                    noeudComp.setResultatAppariement(I18N.getString(
                    "Appariement.UncertainCompetingNodes")); //$NON-NLS-1$
                    lien.addNoeuds2(noeudComp);
                    noeudComp.addLiens(lien);
                }
            ...
            if (incomplets.size() == 1) {
                ...
                noeudComp.setResultatAppariement(I18N.getString(
                "Appariement.UncertainIncompleteNode")); //$NON-NLS-1$
            ...
            if (incomplets.size() == 0) {
                noeudRef.setResultatAppariement(I18N.getString(
                "Appariement.UnmatchedNoPossibleCandidate")); //$NON-NLS-1$
                nbSansHomologue++;
                continue;
            }
            ...
            if (param.varianteForceAppariementSimple) {
                NoeudApp noeudComp = noeudLePlusProche(incomplets, noeudRef);
                noeudRef.setResultatAppariement(I18N.getString(
                        "Appariement." + //$NON-NLS-1$
                "UncertainMatchedWithIncompleteNode")); //$NON-NLS-1$
                noeudComp.setResultatAppariement(I18N.getString(
            ...
            List<ElementCarteTopo> noeudsIncomplets =
                new ArrayList<ElementCarteTopo>(incomplets);
            ...
            for (int j = 0; j < incomplets.size(); j++) {
                ...
                for (int k = 0; k < entrants.size(); k++) {
                    NoeudApp noeudDeb = (NoeudApp)
                        ((ArcApp) entrants.get(k)).getNoeudIni();
            ...
            if (complets.size() == 1) {
                ...
                groupeComp.filtrageGroupePendantAppariementDesNoeuds(
                        noeudRef, liensPreAppAA);
                ...
                if ((groupeComp.getListeNoeuds().size() == 1)
                    ...
                    NoeudApp noeudComp =
                        (NoeudApp) groupeComp.getListeNoeuds().get(0);
                    noeudRef.setResultatAppariement(I18N.getString(
                    "Appariement.MatchedWithANode")); //$NON-NLS-1$
                    noeudComp.setResultatAppariement(I18N.getString(
                    "Appariement.MatchedWithANode")); //$NON-NLS-1$
                ...
                noeudRef.setResultatAppariement(I18N.getString(
                "Appariement.MatchedWithAGroup")); //$NON-NLS-1$
                groupeComp.setResultatAppariement(I18N.getString(
                "Appariement.MatchedWithANode")); //$NON-NLS-1$
                nbNoeudGroupe++;
                for (int j = 0; j < groupeComp.getListeArcs().size(); j++) {
                    ((ArcApp) groupeComp.getListeArcs().get(j))
                    .setResultatAppariement(I18N.getString(
                    "Appariement.MatchedWithANodeInAGroup")); //$NON-NLS-1$
                }
            ...
            if (complets.size() > 1) {
                ...
                List<ElementCarteTopo> complets2 =
                    new ArrayList<ElementCarteTopo>();
                for (int j = 0; j < complets.size(); j++) {
                ...
                if (complets.size() == 1) {
                    ...
                    groupeComp = (GroupeApp) complets.get(0);
                    groupeComp.setResultatAppariement(I18N.getString(
                    "Appariement.UncertainOnlyCompleteGroup")); //$NON-NLS-1$
                    ...
                    for (int k = 0;
                    k < groupeComp.getListeNoeuds().size(); k++) {
                        ((NoeudApp) groupeComp.getListeNoeuds().get(k))
                        .setResultatAppariement(I18N.getString(
                                "Appariement.Uncertain" + //$NON-NLS-1$
            ...
            if (incomplets.size() == 1) {
                ...
                if (groupeComp.getListeNoeuds().size() == 0) {
                    noeudRef.setResultatAppariement(I18N.getString(
                            "Appariement.UnmatchedOnly" + //$NON-NLS-1$
                ...
                groupeComp.setResultatAppariement(I18N.getString(
                "Appariement.UncertainIncompleteGroup")); //$NON-NLS-1$
                for (int k = 0; k < groupeComp.getListeArcs().size(); k++) {
                    ((ArcApp) groupeComp.getListeArcs().get(k))
                    .setResultatAppariement(I18N.getString(
                    "Appariement.UncertainInAnIncompleteGroup")); //$NON-NLS-1$
                }
            ...
            if (incomplets.size() > 1) {
                List<ElementCarteTopo> incomplets2 =
                    new ArrayList<ElementCarteTopo>();
                for (int j = 0; j < incomplets.size(); j++) {
                ...
                if (incomplets.size() == 1) {
                    ...
                    groupeComp = (GroupeApp) incomplets.get(0);
                    groupeComp.setResultatAppariement(I18N.getString(
                            "Appariement.UncertainInco" + //$NON-NLS-1$
                    "mpleteGroupEmptiedIncompleteGroups")); //$NON-NLS-1$
                    for (int k = 0;
                    k < groupeComp.getListeArcs().size(); k++) {
                        ((ArcApp) groupeComp.getListeArcs().get(k))
                        .setResultatAppariement(I18N.getString(
                                "Appariement.UncertainInInco" + //$NON-NLS-1$
                        "mpleteGroupEmptiedIncompleteGroups")); //$NON-NLS-1$
                    }
                    for (int k = 0;
                    k < groupeComp.getListeNoeuds().size(); k++) {
                        ((NoeudApp) groupeComp.getListeNoeuds().get(k))
                        .setResultatAppariement(I18N.getString(
                        "Appariement.UncertainInIncomplete" + //$NON-NLS-1$
        ...
        if (param.varianteChercheRondsPoints) {
            rondsPoints(
                    reseau1, reseau2, liens, liensPreAppAA, param);
        }
        // Fin, affichage du bilan
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.NodeAssessment")); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.CorrectMatches")); //$NON-NLS-1$
            LOGGER.debug("      " + nbNoeudNoeud + I18N //$NON-NLS-1$
                    .getString("Appariement." + //$NON-NLS-1$
                    "NodesMatchedWithASingleNode")); //$NON-NLS-1$
            LOGGER.debug("      " + nbNoeudGroupe + I18N //$NON-NLS-1$
                    .getString("Appariement." + //$NON-NLS-1$
                    "NodesMatchedWithASingleGroup")); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.UncertainMatches")); //$NON-NLS-1$
            LOGGER.debug("      " + nbNoeudNoeudIncertain + I18N //$NON-NLS-1$
                    .getString("Appariement." + //$NON-NLS-1$
                    "NodesMatchedWithASingleIncompleteNode")); //$NON-NLS-1$
            LOGGER.debug("      " + nbPlusieursNoeudsComplets //$NON-NLS-1$
                    + I18N.getString("Appariement." + //$NON-NLS-1$
                    "NodesMatchedWithASeveralNodes")); //$NON-NLS-1$
            LOGGER.debug("      " + nbNoeudGroupeIncertain + I18N //$NON-NLS-1$
                    .getString("Appariement." + //$NON-NLS-1$
                    "NodesMatchedWithASingleIncompleteGroup")); //$NON-NLS-1$
            LOGGER.debug(I18N.getString(
                    "Appariement.IncoherentMatches")); //$NON-NLS-1$
            LOGGER.debug("      " + nbSansHomologue + I18N //$NON-NLS-1$
                    .getString("Appariement." + //$NON-NLS-1$
                    "NodesWithoutMatchFound")); //$NON-NLS-1$
            LOGGER.debug("      " + nbPlusieursGroupesComplets //$NON-NLS-1$
                    + I18N.getString("Appariement." + //$NON-NLS-1$
                    "NodesWithSeveralHomologousGroups")); //$NON-NLS-1$
            LOGGER.debug(I18N.getString(
                    "Appariement.UnprocessedNodes")); //$NON-NLS-1$
            LOGGER.debug("      " + nbNonTraite + I18N //$NON-NLS-1$
     ...
     * @param param matching parameters
     */
    private static void rondsPoints(
            final CarteTopo reseau1, final CarteTopo reseau2,
            final EnsembleDeLiens liensNoeuds,
            final EnsembleDeLiens liensPreAppAA, final ParametresApp param) {
        ...
        while (itFaces.hasNext()) {
            ...
            if (compacite < MIN_COMPACITY) { continue; }
            if (face.getGeometrie().area() > MAX_ROUNDABOUT_SURFACE) {
                continue;
            }
            ...
            while (itNoeudsDuRondPoint.hasNext()) {
                ...
                List <LienReseaux> liensDuNoeudDuRondPoint =
                    noeud.getLiens(liensNoeuds.getElements());
                Iterator<?> itLiensDuNoeudsDuRondPoint =
                    liensDuNoeudDuRondPoint.iterator();
                ...
                while (itLiensDuNoeudsDuRondPoint.hasNext()) {
                    LienReseaux lien =
                        (LienReseaux) itLiensDuNoeudsDuRondPoint.next();
                    NoeudApp noeudRef = (NoeudApp) lien.getNoeuds1().get(0);
                    if (lien.getNoeuds2().size() == 1) { // cas 1
                        GroupeApp rondPoint = (GroupeApp) reseau2
                        .getPopGroupes().nouvelElement();
                        rondPoint.addAllNoeuds(
                                new ArrayList<Noeud>(noeudsDuRondPoint));
                        ...
                        if (noeudRef.correspCommunicants(
                                rondPoint, liensPreAppAA) == 1) {
                            lien.setEvaluation(1);
                            rondPoint.setResultatAppariement(I18N.getString(
                                    "Appariement." + //$NON-NLS-1$
                                    "MatchedWithANode" + //$NON-NLS-1$
                                    "-Roundabout")); //$NON-NLS-1$
                            for (int k = 0;
                            k < rondPoint.getListeArcs().size(); k++) {
                            ...
                            for (int k = 0;
                            k < rondPoint.getListeNoeuds().size(); k++) {
                ...
                Iterator<?> itGroupesDuNoeud =
                    noeud.getListeGroupes().iterator();
                while (itGroupesDuNoeud.hasNext()) {
                    GroupeApp groupe = (GroupeApp) itGroupesDuNoeud.next();
                    List<?> liens =
                        groupe.getLiens(liensNoeuds.getElements());
                    if ((liens.size() == 0)
                            || (groupe.getLiens().size() == 0)) {
                        continue;
                    }
                    if (liensNoeuds.getElements().
                            contains(groupe.getLiens().get(0))) {
                        Iterator<?> itNoeudsDuRondPoint2 =
                            noeudsDuRondPoint.iterator();
                        while (itNoeudsDuRondPoint2.hasNext()) {
                            NoeudApp noeud2 =
                                (NoeudApp) itNoeudsDuRondPoint2.next();
                    ...
                    NoeudApp noeudRef = (NoeudApp) lien.getNoeuds1().get(0);
                    if (noeudRef.correspCommunicants(groupe,
                            liensPreAppAA) == 1) {
                        lien.setEvaluation(1);
                        groupe.setResultatAppariement(I18N
                                .getString("Appariement." + //$NON-NLS-1$
                                "MatchedWithANode-Roundabout")); //$NON-NLS-1$
                        for (int k = 0; k < groupe.getListeArcs().size();
                        k++) {
                        ...
                        for (int k = 0; k < groupe.getListeNoeuds().size();
                        k++) {
                        ...
                        for (int k = 0;
                        k < groupe.getListeNoeuds().size(); k++) {
                            ((NoeudApp) groupe.getListeNoeuds().get(k))
                            .setResultatAppariement(I18N
                                    .getString("Appariement." + //$NON-NLS-1$
                                    "UncertainMatchedWithANode" + //$NON-NLS-1$
    ...
    private static void filtrageNoeudsComplets(
        ...
        for (int i = 1; i < complets.size(); i++) {
            ...
            if (d < dmin) { dmin = d; noeudPlusProche = noeudComp; }
        }
        complets.clear();
    ...
    private static NoeudApp noeudLePlusProche(
        ...
        while (itNoeuds.hasNext()) {
            ...
            if (d < dmin) { dmin = d; plusProche = noeud; }
        }
        return plusProche;
    }

    /**
     * Appariement des arcs, s'appuyant sur un appariement préalable des
     * noeuds, et sur un pré-appariement des arcs.
     * S'appuie essentiellement sur la notion de 'plus proche chemin d'un arc',
     * défini comme le chemin minimisant la surface entre le chemin et l'arc.
     * @param reseau1 network 1
     * @param reseau2 network 2
     * @param liensPreAppAA prematching edge links
     ...
     * @return resulting set of links
     */
    public static EnsembleDeLiens appariementArcs(
            final CarteTopo reseau1, final CarteTopo reseau2,
            final EnsembleDeLiens liensPreAppAA,
        ...
        while (itArcs.hasNext()) {
            ...
            if (arcRef.getNoeudIni() == arcRef.getNoeudFin()) {
                ...
                longSansHomologuePbPCC =
                    longSansHomologuePbPCC + arcRef.longueur();
                continue;
            }
            ...
            List<List<Noeud>> noeudsInOut = arcRef
                .noeudsEnCorrespondanceAuxExtremites(liensAppNoeuds,
                        liensPreAppAA);
            ...
            if (((noeudsFinIn.size() == 0) && (noeudsFinOut.size() == 0))
                    || ((noeudsDebutIn.size() == 0)
                            && (noeudsDebutOut.size() == 0))) {
            ...
            GroupeApp tousCandidats =
                (GroupeApp) reseau2.getPopGroupes().nouvelElement();
            tousCandidats.setListeArcs(new ArrayList<Arc>(tousArcs));
            Iterator<Arc> itTousArcsComp =
                tousCandidats.getListeArcs().iterator();
            ...
            double longMaxRecherche =
                arcRef.getGeometrie().length() * param.distanceArcsMax;
            ...
            GroupeApp pccMin1 = tousCandidats.plusCourtChemin(
                    noeudsDebutOut, noeudsFinIn, longMaxRecherche);
            GroupeApp pccMin2 = tousCandidats.plusCourtChemin(
                    noeudsFinOut, noeudsDebutIn, longMaxRecherche);
            ...
            if (pccMin1 == null) {
                ...
                if (pccMin2.getListeArcs().size() == 0) {
                    ...
                    NoeudApp noeudComp =
                        (NoeudApp) pccMin2.getListeNoeuds().get(0);
                    lien.addNoeuds2(noeudComp);
                    lien.setEvaluation(ZERO_POINT_FIVE);
                    noeudComp.addLiens(lien);
                    arcRef.setResultatAppariement(I18N
                    ...
                    nbDouteuxPbNoeud++;
                    longDouteuxPbNoeud =
                        longDouteuxPbNoeud + arcRef.longueur();
                    continue;
                }
                ...
                if (arcRef.getOrientation() == -1) {
                    ...
                    pccMin2.setResultatAppariementGlobal(I18N
                            .getString("Appariement." + //$NON-NLS-1$
            ...
            if (pccMin2 == null) {
                ...
                if (pccMin1.getListeArcs().size() == 0) {
                    ...
                    NoeudApp noeudComp =
                        (NoeudApp) pccMin1.getListeNoeuds().get(0);
                    lien.addNoeuds2(noeudComp);
                    lien.setEvaluation(ZERO_POINT_FIVE);
                    noeudComp.addLiens(lien);
                    arcRef.setResultatAppariement(I18N.getString(
                            "Appariement." + //$NON-NLS-1$
                    "UncertainMatchedWithACompNode")); //$NON-NLS-1$
                    noeudComp.setResultatAppariement(I18N.getString(
                                    "Appariement." + //$NON-NLS-1$
                ...
                if (arcRef.getOrientation() == 1) {
                    ...
                    pccMin1.setResultatAppariementGlobal(I18N.getString(
                                    "Appariement." + //$NON-NLS-1$
                                    "MatchedWithARefEdge")); //$NON-NLS-1$
                    nbOkUneSerie++;
                    longOkUneSerie = longOkUneSerie + arcRef.longueur();
            ...
            if (pccMin1.contientMemesArcs(pccMin2)) {
                ...
                pccMin1.addLiens(lien);
                arcRef.setResultatAppariement(I18N.getString(
                                "Appariement.MatchedWithAnEdge" //$NON-NLS-1$
                        ));
                pccMin1.setResultatAppariementGlobal(I18N.getString(
                                "Appariement.MatchedWithAnEdge" //$NON-NLS-1$
                        ));
            ...
            if (pccMin2.getListeArcs().size() == 0) {
                ...
                if (lien.getEvaluation() == 1) {
                    nbOkPlusieursSeries++;
                    longOkPlusieursSeries =
                        longOkPlusieursSeries + arcRef.longueur();
                } else {
                    nbDouteuxPbNoeud++;
                    longDouteuxPbNoeud = longDouteuxPbNoeud + arcRef.longueur();
                }
        ...
        // Fin, affichage du bilan
        longTot =
            longSansHomologuePbNoeud + longSansHomologuePbPCC + longOkUneSerie
            + longOkPlusieursSeries + longDouteuxPbNoeud + longDouteuxPbSens;
        nbTot = reseau1.getPopArcs().getElements().size();
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.EdgeAssessment")); //$NON-NLS-1$
            LOGGER.debug(I18N.
                    getString("Appariement.Network1TotalLength") //$NON-NLS-1$
                    + Math.round(longTot / THOUSAND) + I18N
                    .getString("Appariement.KmIfMetric")); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.CorrectMatches")); //$NON-NLS-1$
            LOGGER.debug("      " + nbOkUneSerie + I18N //$NON-NLS-1$
                    .getString(
                            "Appariement." + //$NON-NLS-1$
                            "EdgesMatches" + //$NON-NLS-1$
                            "WithOneOrSeveralCompEdges") //$NON-NLS-1$
                    + nbOkUneSerie * HUNDRED / nbTot + "%nb, " //$NON-NLS-1$
                    + Math.round(longOkUneSerie * HUNDRED / longTot)
                    + "%long)"); //$NON-NLS-1$
            LOGGER.debug("      " + nbOkPlusieursSeries + I18N //$NON-NLS-1$
                    .getString(
                            "Appariement." + //$NON-NLS-1$
                            "EdgesMatchesWithTwoSetsOfCompEdges") //$NON-NLS-1$
                    + nbOkPlusieursSeries * HUNDRED / nbTot
                    + "%nb, " + Math.round(longOkPlusieursSeries //$NON-NLS-1$
                            * HUNDRED / longTot) + "%long)"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.UncertainMatches")); //$NON-NLS-1$
            LOGGER.debug("      " + nbDouteuxPbSens + I18N //$NON-NLS-1$
                    .getString(
                            "Appariement.EdgesMatchedOnlyOneWay") //$NON-NLS-1$
                    + nbDouteuxPbSens * HUNDRED / nbTot + "%nb, " //$NON-NLS-1$
                    + Math.round(longDouteuxPbSens * HUNDRED / longTot)
                    + "%long)"); //$NON-NLS-1$
            LOGGER.debug("      " + nbDouteuxPbNoeud + I18N //$NON-NLS-1$
                    .getString(
                            "Appariement.EdgesMatchesWithANode") //$NON-NLS-1$
                    + nbDouteuxPbNoeud * HUNDRED / nbTot
                    + "%nb, " + Math.round(longDouteuxPbNoeud //$NON-NLS-1$
                            * HUNDRED / longTot) + "%long)"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.UnmatchedEdges")); //$NON-NLS-1$
            LOGGER.debug("      " + nbSansHomologuePbNoeud + I18N //$NON-NLS-1$
                    .getString(
                            "Appariement." + //$NON-NLS-1$
                            "EdgesWithoutHomologous-" + //$NON-NLS-1$
                            "UnmatchedNode") //$NON-NLS-1$
                    + nbSansHomologuePbNoeud * HUNDRED / nbTot
                    + "%nb, " + //$NON-NLS-1$
                    Math.round(longSansHomologuePbNoeud * HUNDRED / longTot)
                    + "%long)"); //$NON-NLS-1$
            LOGGER.debug("      " + nbSansHomologuePbPCC + I18N //$NON-NLS-1$
                    .getString(
                            "Appariement." + //$NON-NLS-1$
                            "EdgesWithoutHomologous-" + //$NON-NLS-1$
                            "NoShortestPath") //$NON-NLS-1$
                    + nbSansHomologuePbPCC * HUNDRED / nbTot
                    + "%nb, " //$NON-NLS-1$
                    + Math.round(longSansHomologuePbPCC * HUNDRED / longTot)
                    + "%long)"); //$NON-NLS-1$
        }
     ...
     * @param param matching parameters
     */
    private static void calculePoids(
            final ArcApp arcRef, final GroupeApp tousCandidats,
            final ParametresApp param) {
        ...
        while (itCandidats.hasNext()) {
            ...
            double poids = Distances.ecartSurface(
                    arcComp.getGeometrie(),
                    arcRef.getGeometrie());
     ...
     * recherche des arcs ou noeuds du réseau 2 appariés avec
     * plusieurs objets du réseau 1.
     * @param reseau1 network 1
     * @param reseau2 network 2
     * @param liens links
     * @param param matching parameters
     */
    private static void controleGlobal(
            final CarteTopo reseau1, final CarteTopo reseau2,
        ...
        while (itArcs.hasNext()) {
            ...
            if (param.varianteFiltrageImpassesParasites) {
                ...
                if (arcComp.longueur() < param.distanceNoeudsMax) {
                    ...
                    if (impasse) { //on nettoie
                        arcComp.setResultatAppariement(I18N.getString(
                                "Appariement.UnmatchedDeadend")); //$NON-NLS-1$
                        itGroupes = arcComp.getListeGroupes().iterator();
                        while (itGroupes.hasNext()) {
                            GroupeApp groupe = (GroupeApp) itGroupes.next();
                            ...
                            groupe.getListeNoeuds().remove(
                                    arcComp.getNoeudFin());
                            if (groupe.getListeArcs().size() == 0) {
                                liens.removeAll(groupe.getLiens(
                                        liens.getElements()));
                            }
                        }
                        arcComp.getListeGroupes().clear();
            ...
            while (itLiens.hasNext()) {
                ...
                lien.affecteEvaluationAuxObjetsLies(ZERO_POINT_FIVE, I18N
                        .getString("Appariement." + //$NON-NLS-1$
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.Network2Edges" //$NON-NLS-1$
                            ) + nb + "):"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.CorrectMatchedEdges")//$NON-NLS-1$
                    + nbOK + " (" + (nbOK * HUNDRED / nb) //$NON-NLS-1$
                    + "%, " //$NON-NLS-1$
                    + Math.round(longOK * HUNDRED / longTotal)
                    + "%long)"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.UncertainMatchedEdges"//$NON-NLS-1$
                            ) + nbDouteux
                            + " (" + (nbDouteux * HUNDRED / nb) //$NON-NLS-1$
                            + "%, " //$NON-NLS-1$
                            + Math.round(longDouteux * HUNDRED / longTotal)
                            + "%long)"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.UnmatchedEdges") //$NON-NLS-1$
                    + nbSansCorresp
                    + " (" + (nbSansCorresp * HUNDRED / nb) //$NON-NLS-1$
                    + "%, " //$NON-NLS-1$
                    + Math.round(longSansCorresp * HUNDRED / longTotal)
                    + "%long)"); //$NON-NLS-1$
        }
        ...
        while (itNoeuds.hasNext()) {
            ...
            if (liensObjet.size() == 0) {
                noeud.setResultatAppariement(I18N
                        .getString("Appariement.Unmatched")); //$NON-NLS-1$
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.Network2Nodes") //$NON-NLS-1$
                    + nb + "):"); //$NON-NLS-1$
            LOGGER.debug(I18N
            ...
            LOGGER.debug(I18N.
                    ...
                    + nbDouteux + " (" + (nbDouteux //$NON-NLS-1$
                            * HUNDRED / nb) + "%)"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.UnmatchedNodes") //$NON-NLS-1$
                    + nbSansCorresp
                    + " (" + (nbSansCorresp * HUNDRED / nb) //$NON-NLS-1$
                    + "%)"); //$NON-NLS-1$
        }
        ...
        while (itArcs.hasNext()) {
            ...
            if (arc.getResultatAppariement().startsWith(I18N
                    .getString("Appariement.Unmatched"))) { //$NON-NLS-1$
                nbSansCorresp++;
            ...
            if (arc.getResultatAppariement().startsWith(I18N
                    .getString("Appariement.Uncertain"))) { //$NON-NLS-1$
                nbDouteux++;
                longDouteux = longDouteux + arc.longueur();
                continue;
            }
            if (arc.getResultatAppariement().startsWith(I18N
                    .getString("Appariement.Matched"))) { //$NON-NLS-1$
                nbOK++;
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(I18N.getString(
                "Appariement.UnexpectedResultOnEdge")//$NON-NLS-1$
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.Network1Edges") //$NON-NLS-1$
                            + nb + "):"); //$NON-NLS-1$
            LOGGER.debug(I18N
                    .getString("Appariement.CorrectMatchedEdges") //$NON-NLS-1$
                    + nbOK + " (" + (nbOK * HUNDRED / nb) //$NON-NLS-1$
                    ...
                    + "%long)"); //$NON-NLS-1$
            LOGGER.debug(I18N.getString(
                    "Appariement.UncertainMatchedEdges" //$NON-NLS-1$
                    ) + nbDouteux
                    + " (" + (nbDouteux * HUNDRED / nb) //$NON-NLS-1$
                    + "%, " + //$NON-NLS-1$
                    Math.round(longDouteux * HUNDRED / longTotal)
        ...
        while (itNoeuds.hasNext()) {
            NoeudApp noeud = (NoeudApp) itNoeuds.next();
            if (noeud.getResultatAppariement().startsWith(I18N
                    .getString("Appariement.Unmatched"))) { //$NON-NLS-1$
                nbSansCorresp++;
                continue;
            }
            ...
            if (LOGGER.isDebugEnabled()) { LOGGER.debug(I18N.getString(
                    "Appariement.UnexpectedResultOnNode") //$NON-NLS-1$
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(I18N
                    .getString("Appariement.Network1Nodes") //$NON-NLS-1$
                    + nb + "):"); //$NON-NLS-1$
            LOGGER.debug(I18N.
                    getString("Appariement.CorrectMatchedNodes") //$NON-NLS-1$
                    + nbOK + " (" //$NON-NLS-1$
                    + (nbOK * HUNDRED / nb) + "%)"); //$NON-NLS-1$
            LOGGER.debug(I18N.
                    getString("Appariement.UncertainMatchedNodes")//$NON-NLS-1$
                    + nbDouteux + " (" + (nbDouteux //$NON-NLS-1$
                            * HUNDRED / nb) + "%)"); //$NON-NLS-1$
            LOGGER.debug(I18N.
                    ...
                    + (nbSansCorresp * HUNDRED / nb)
                    + "%)"); //$NON-NLS-1$
        }
    }

    /**
     * Les noeuds de référence non appariés par les 'liens' sont projetés
     * sur le réseau comp de manière à introduire un noeud dans le réseau
     * Comp aux endroits qui pourraient correspondre à ces noeuds Ref
     * non appariés.
     ...
     * @param param matching parameters
     */
    public static void decoupeNoeudsNonApparies(
            final ReseauApp ref, final ReseauApp comp,
            final EnsembleDeLiens liens, final ParametresApp param) {
    ...
    /**
     * Découpe les arcs du reseau1 'reseauADecouper' non appariés par les
     * 'liens' de manière à introduire un noeud dans le reseauADecouper aux
     * endroits où il s'éloigne du réseau2 'reseauDecoupant'.
     *
     *  Remarque: utilisé pour les GR par exemple pour traiter le cas des GR
     *  hors sentier.
     *
     ...
     * @param param matching parameters
     */
    public static void decoupeNonApparies(
        ...
        while (itArcsDecoupes.hasNext()) {
            ...
            arcsDecoupantsRes2 =
                reseauDecoupant.getPopArcs().select(
            ...
            while (itArcsDecoupants.hasNext()) {
                ...
                GM_LineString lineStringDecoupant =
                    arcDecoupant.getGeometrie();
                for (DirectPosition dp : arcADecouper.getGeometrie().coord().
                        getList()) {
                    lineStringDecoupant =
                        Operateurs.projectionEtInsertion(
                                dp, lineStringDecoupant);
                }
                // initalisation des compteurs
                indiceDernierPtProche = 0;
                boolean proche = false;
                if (Distances.distance(
                        lineStringDecoupant.getControlPoint(0),
                        arcADecouper.getGeometrie()) <= distanceMaxNoeudArc) {
                    proche = true;
                }
                // parcour des points de l'arc découpant
                for (int i = 1;
                    i < lineStringDecoupant.getControlPoint().size();
                    i++) {
                    DirectPosition ptCourantArcDecoupant =
                        lineStringDecoupant.getControlPoint(i);
                    distancePtCourantVersArcADecoupe =
                        Distances.distance(
                                ptCourantArcDecoupant,
                                arcADecouper.getGeometrie());
                    if (proche) {
                        if (distancePtCourantVersArcADecoupe
                                > distanceMaxNoeudArc) {
                    ...
                    if (distancePtCourantVersArcADecoupe
                        ...
                        DirectPosition ptDecoupage = Operateurs.projection(
                                lineStringDecoupant.getControlPoint(
                                        indiceDernierPtProche),
                                        arcADecouper.getGeometrie());
                        pointsDeDecoupage.add(new GM_Point(ptDecoupage));
                        continue;
        ...
        reseauADecouper.projete(
                pointsDeDecoupage, distanceMaxNoeudArc, distanceMaxNoeudArc);
        reseauADecouper.instancieAttributsNuls(param);
        reseauDecoupant.projete(
                pointsDeDecoupage, distanceMaxNoeudArc, distanceMaxNoeudArc);
    }
}
