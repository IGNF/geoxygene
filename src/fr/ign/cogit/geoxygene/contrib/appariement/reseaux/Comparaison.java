...
public class Comparaison {
     ...
     * Cette approximation grossière évalue pour tout point de réseau1 son écart
     * à réseau2
     * (point le plus proche quelconque), sans réaliser d'appariement de réseau.
     * 
     * @param reseau1
     *            Un réseau, typiquement un réseau peu précis dont on veut
     *            estimer la qualité par rapport à reseau2.
     * @param reseau2
     *            Un autre réseau, typiquement un réseau de bonne qualité qui
     *            sert de référence.
     * @param distanceMax
     *            Sert à éliminer les aberrations dans le calcul de la moyenne:
     *            les arcs de reseau1 situés au moins en un point à plus de
     *            distanceMax de reseau2
     *            ne sont pas pris en compte dans le calcul.
     * @return
     *         L'écart moyen. Il est calculé comme la moyenne des distances
     *         entre les points
     *         des arcs de reseau1 et reseau2. Cette moyenne est pondérée par la
     *         longueur
    ...
    public static double approximationEcartPlaniMoyen(CarteTopo reseau1,
        ...
        while (itArcsRef.hasNext()) { // pour chaque arc de this
            ...
            for (int i = 0; i < dp.size(); i++) { // pour chaque point d'un arc
                ...
                while (itArcsComp.hasNext()) {
                    ...
                    GM_LineString geomComp = (GM_LineString) objetComp
                            .getGeom();
                    DirectPosition ptProjete = Operateurs.projection(ptRef,
                            geomComp);
        ...
        System.out.println(I18N
                .getString("Comparaison.MaxDistanceBetweenEdges") + dmax); //$NON-NLS-1$
     ...
     * réseau à étudier réseau11
     * et un réseau de référence réseau2.
     * Cette évaluation se fait en s'appuyant sur le calcul des écarts entre
     * chaque point
     * de reseau1 et sa projection au plus près sur le réseau reseau2
     * (et non sur un réel appariement d'arcs).
     * Les moyennes sont pondérées par la longueur des segments entourant les
     * points,
     ...
     *            - Les arcs de reseau1 situés en au moins un point à plus de
     *            distanceMax
     *            de reseau2 ne sont pas pris en compte dans le calcul des
     *            indicateurs sur les arcs.
     *            - Les noeuds de reseau1 situés à plus de distanceMax d'un
     *            noeud
     *            de reseau2 ne sont pas pris en compte dans le calcul des
     *            indicateurs sur les noeuds
     * @return
     *         Liste (de 'double') contenant un ensemble d'indicateurs sur
     *         l'écart entre les réseaux :
     *         ESTIMATEURS SUR LES ARCS
     *         liste(0): longueur des arcs du réseau "this" total
     *         liste(1): longueur des arcs du réseau "this" pris en compte dans
     *         les calculs d'évaluation de l'écart
     *         liste(2): longueur des arcs du réseau "reseau"
     *         liste(3): nombre d'arcs du réseau "this" total
     *         liste(4): nombre d'arcs du réseau "this" pris en compte dans les
     *         calculs d'évaluation de l'écart
     *         liste(5): nombre d'arcs du réseau "reseau"
     *         liste(6): estimation du biais systématique en X sur les arcs
     *         (valeur en X de la moyenne des vecteurs d'écart entre un point de
     *         "this" et son projeté sur "reseau")
     *         liste(7): estimation du biais systématique en Y sur les arcs
     *         (valeur en Y de la moyenne des vecteurs d'écart entre un point de
     *         "this" et son projeté sur "reseau")
     *         liste(8): estimation de l'écart moyen sur les arcs
     *         (moyenne des longueurs des vecteurs d'écart entre un point de
     *         "this" et son projeté sur "reseau")
     ...
     *         point de "this" et son projeté sur "reseau")
     *         liste(10): estimation de l'écart type sur les arcs, i.e.
     *         précision une fois le biais corrigé
     *         ( racine(ecart moyen quadratique^2 - biais^2)
     *         liste(11): histogramme de répartition des écarts sur tous les
     *         points (en nb de points intermédiaires sur les arcs)
     ...
     *         (valeur en X de la moyenne des vecteurs d'écart entre un noeud de
     *         "this" et le noeud le plus proche de "reseau")
     ...
     *         (moyenne des longueurs des vecteurs d'écart entre un noeud de
     *         "this" et le noeud le plus proche de "reseau")
     *         liste(18): estimation de l'écart moyen quadratique sur les arcs
     *         (moyenne quadratique des longueurs des vecteurs d'écart entre un
     *         noeud de "this" et le noeud le plus proche de "reseau")
     *         liste(19): estimation de l'écart type sur les noeuds, i.e.
     *         précision une fois le biais corrigé
     *         ( racine(ecart moyen quadratique^2 - biais^2)
    ...
    public static List<?> evaluationEcartPosition(CarteTopo reseau1,
        ...
        if (affichage)
            System.out.println(I18N
                    .getString("Comparaison.GlobalNetworkComparison")); //$NON-NLS-1$
        resultats.add(new Double(longTotal1));
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.TotalEdgeLengthNetwork1") + Math.round(longTotal1 / 1000)); //$NON-NLS-1$
        resultats.add(new Double(longPrisEnCompte1));
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.TotalEdgeLengthNetwork1Used") + Math.round(longPrisEnCompte1 / 1000)); //$NON-NLS-1$
        resultats.add(new Double(longTotal2));
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.TotalEdgeLengthNetwork2") + Math.round(longTotal2 / 1000)); //$NON-NLS-1$
        resultats.add(new Double(nbArcsTotal1));
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.NumberOfEdgesNetwork1") + nbArcsTotal1); //$NON-NLS-1$
        resultats.add(new Double(nbArcsPrisEnCompte1));
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.NumberOfEdgesNetwork1Used") + nbArcsPrisEnCompte1); //$NON-NLS-1$
        resultats.add(new Double(nbArcsTotal2));
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.NumberOfEdgesNetwork2") + nbArcsTotal2); //$NON-NLS-1$

        if (affichage)
            System.out.println(""); //$NON-NLS-1$
        if (affichage)
            System.out.println(I18N
                    .getString("Comparaison.DeviationEstimation")); //$NON-NLS-1$
        ...
        if (affichage)
            System.out
                    .println(I18N
                            .getString("Comparaison.QuadraticAverageDeviation") + ecartQuadratiqueTotal); //$NON-NLS-1$
