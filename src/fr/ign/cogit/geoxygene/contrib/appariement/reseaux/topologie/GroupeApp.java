...
public class GroupeApp extends Groupe {
	...
	public void setResultatAppariementGlobal (String resultat) {
		...
		Iterator itComposants ;
		itComposants = this.getListeArcs().iterator();
		while (itComposants.hasNext()) {
			ArcApp arc = (ArcApp) itComposants.next();
			arc.setResultatAppariement(resultat);
		}
		itComposants = this.getListeNoeuds().iterator();
		while (itComposants.hasNext()) {
			NoeudApp noeud = (NoeudApp) itComposants.next();
    ...
    public List noeudsEntree(NoeudApp noeudref, EnsembleDeLiens liensPreappArcs) {
        List arcsRef = new ArrayList();
        List arcs = new ArrayList();
        List arcsOK = new ArrayList();
    ...
    public List noeudsSortie(NoeudApp noeudref, EnsembleDeLiens liensPreappArcs) {
        List arcsRef = new ArrayList();
        List arcs = new ArrayList();
        List arcsOK = new ArrayList();
        List noeuds = new ArrayList();
	...
	public GroupeApp plusCourtChemin(List noeudsDepart, List noeudsArrivee, double longMax) {
		...
		for(int j=0;j<noeudsDepart.size() ;j++) {
			for(int k=0;k<noeudsArrivee.size() ;k++) {
				iniComp = (Noeud)noeudsDepart.get(j);
				finComp = (Noeud)noeudsArrivee.get(k);
    ...
    * Elimination des impasses de BD comp */
    private void filtrageImpasses() {
        int i;
        ArcApp arcComp;
        List impassesDebut = new ArrayList();
        List impassesFin = new ArrayList();
        ...
        for(i=0; i<impassesDebut.size(); i++) {
            arcComp = (ArcApp)impassesDebut.get(i);
        ...
        for(i=0; i<impassesFin.size(); i++) {
            arcComp = (ArcApp)impassesFin.get(i);
            this.getListeArcs().remove(arcComp);
            arcComp.getListeGroupes().remove(this);
    ...
    * n'a pas d'autre troncon apparié avec des arcs de la BD Ref (par le préappariement) */
    private void filtrageImpassesNonApparie(EnsembleDeLiens liensPreappArcs) {
        ...
        List adjacentsComp = new ArrayList();
        List impassesDebut = new ArrayList();
        List impassesFin = new ArrayList();
    ...
    * a tous ses troncons appariés avec le même arc de BDcomp. */
    private void filtrageImpassesTousApparies(EnsembleDeLiens liensPreappArcs) {
        int i, j;
        List arcsCommuns = new ArrayList();
        List arcsNouveaux = new ArrayList();
        List arcsCompCommuniquants = new ArrayList(); 
        ArcApp arcComp;
        boolean communs;
        List impassesDebut = new ArrayList();
        List impassesFin = new ArrayList();
    ...
    * Elimination des impasses de BD comp */
    private void filtrageImpassesComplets(NoeudApp noeudRef, EnsembleDeLiens liensPreappArcs) {
        ...
        ArcApp arccomp;
        int correspondance;
        List arcsATester = new ArrayList();
    ...
    private void filtragePlusCourtChemin(NoeudApp noeudRef, EnsembleDeLiens liensPreappArcs) {
        ...
        List noeudsIn = new ArrayList();
        List noeudsOut = new ArrayList();
        List arcsOK = new ArrayList();
        List noeudsOK = new ArrayList();
        List noeudsAEnlever = new ArrayList();
    ...
    public GM_LineString compileArcs(Arc arcRef) {
        ...
        premierArc = (ArcApp)this.getListeArcs().get(0);
