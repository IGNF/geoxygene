 ...
 * L'appariement de réseaux à des échelles différentes abouti à de nombreux
 * appariements 1-n,
...
public class GroupeApp extends Groupe {
     ...
     * La notion d'entrée / sortie est relative au préappariement d'arcs et au
     * noeud ref passé en paramètre.
     ...
     * La notion d'entrée / sortie est relative au préappariement d'arcs et
     * au noeud ref passé en paramètre.
    ...
    public void enleveExtremites() {
        ...
        if (this.getListeNoeuds().size() == 0)
            return;
        ext = (NoeudApp) this.getListeNoeuds().get(
                this.getListeNoeuds().size() - 1);
        this.getListeNoeuds().remove(ext);
        ext.getListeGroupes().remove(this);
    }

    /**
     * Plus court chemin dans this pour relier les noeudsDepart aux
     * noeudsArrivee,
    ...
    public GroupeApp plusCourtChemin(List<Noeud> noeudsDepart,
        ...
        for (int j = 0; j < noeudsDepart.size(); j++) {
            for (int k = 0; k < noeudsArrivee.size(); k++) {
                ...
                pcc = (GroupeApp) iniComp.plusCourtChemin(finComp, this,
                        longMax);
                if (pcc == null)
                    continue;
     ...
     * D'après thèse de Thomas Devogèle (1997), avec ajouts de Sébastien
     * Mustière (2002).
     * S'appuie principalement sur des recherches d'impasses et des calculs de
     * plus courts chemins.
     ...
     * l'extérieure du groupe)
     * n'a pas d'autre troncon apparié avec des arcs de la BD Ref (par le
     * préappariement)
     */
    ...
    private void filtrageImpassesComplets(NoeudApp noeudRef,
        ...
        for (i = 0; i < arcsATester.size(); i++) {
            ...
            if (arccomp.impasseDebut(this)) {
                ...
                arccomp.getListeGroupes().remove(this);
                this.getListeNoeuds().remove(arccomp.getNoeudIni());
                ((NoeudApp) arccomp.getNoeudIni()).getListeGroupes().remove(
                        this);
                correspondance = noeudRef.correspCommunicants(this,
                        liensPreappArcs);
                if (correspondance == 1)
                    return;
            ...
            if (arccomp.impasseFin(this)) {
                ...
                arccomp.getListeGroupes().remove(this);
                this.getListeNoeuds().remove(arccomp.getNoeudFin());
                ((NoeudApp) arccomp.getNoeudFin()).getListeGroupes().remove(
                        this);
                correspondance = noeudRef.correspCommunicants(this,
                        liensPreappArcs);
                if (correspondance == 1)
    ...
    // ////////////////////////////////////////////////////////////////////////////////////
    /**
     * Pour l'export : mise bout à bout des arcs du groupe, dans le bon ordre.
     * NB: n'est valable que pour un groupe
     * où les arcs sont bout à bout, et dans le bon ordre, comme ceux issus du
     * plus_court_chemin
     * Cas particuliers :
     * si le goupe est vide, renvoie null;
     * si le goupe ne contient que des points, renvoie une ligne dégénérée
     * contenant 2 fois le premier point.
     */
    public GM_LineString compileArcs(Arc arcRef) {
        ...
        d2 = chemin.startPoint().distance(arcRef.getGeometrie().endPoint())
                + chemin.endPoint()
                        .distance(arcRef.getGeometrie().startPoint());
        if (d1 < d2)
            chemin2 = chemin;
        else {
