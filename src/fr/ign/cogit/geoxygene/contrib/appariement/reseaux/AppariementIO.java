...
public class AppariementIO {
	 ...
	 * 
	 * @return 
	 * L'ensemble des liens en sortie (de la classe EnsembleDeLiens). 
	 * 
	 * @param paramApp
	 * Les paramètres de l'appariement (seuls de distance, préparation topologique des données...)
	 * 
	 * @param cartesTopo
	 * Liste en entrée/sortie qui permet de récupérer en sortie les graphes intermédiaires 
	 * créés pendant le calcul (de type Reseau_App, spécialisation de CarteTopo).
	 ...
	 */
	public static EnsembleDeLiens AppariementDeJeuxGeo(ParametresApp paramApp, List cartesTopo) {
	...
	private static ReseauApp Import(ParametresApp paramApp, boolean ref) {
		ReseauApp reseau;
		Iterator itPopArcs, itPopNoeuds, itElements; 
		...
		while ( itPopArcs.hasNext() ) {
			popGeo = (FT_FeatureCollection)itPopArcs.next();
...
//				else noeud.setTaille(tailleNoeud(element, paramApp));
	 ...
	 */
	public static List scindeSelonValeursResultatsAppariement(ReseauApp reseauRef, List valeursClassement) {
		List cartesTopoClassees = new ArrayList();
		...
		while (itArcs.hasNext()) {
			...
			for(i=0;i<valeursClassement.size();i++) {
				...
				if (arc.getResultatAppariement().startsWith((String)valeursClassement.get(i))) {
					arcClasse = (ArcApp)((ReseauApp)cartesTopoClassees.get(i)).getPopArcs().nouvelElement();
		...
		while (itNoeuds.hasNext()) {
			...
			for(i=0;i<valeursClassement.size();i++) {
				...
				if (noeud.getResultatAppariement().startsWith((String)valeursClassement.get(i))) {
					noeudClasse = (NoeudApp)((ReseauApp)cartesTopoClassees.get(i)).getPopNoeuds().nouvelElement();
