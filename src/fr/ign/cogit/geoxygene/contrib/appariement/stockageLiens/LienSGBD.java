...
public class LienSGBD extends FT_Feature {
     ...
     */
    public final String getObjetsRef() { return this.objetsRef; }
    /**
     ...
     */
    public final String getObjetsComp() { return this.objetsComp; }
    /**
     ...
     * @return indicators
     */
    public final String getIndicateurs() { return this.indicateurs; }
    /**
     ...
     */
    public final void setNom(final String aNom) { this.nom = aNom; }
    ...
    public final LienSGBD conversionLiensVersSGBD(final Lien lien) {
        List<?> listeObjetsRef = lien.getObjetsRef(),
        listeObjetsComp = lien.getObjetsComp(), indic = lien.getIndicateurs();
        Iterator<?> itRef = listeObjetsRef.iterator(),
        itComp = listeObjetsComp.iterator(), itIndic = indic.iterator();
        ...
        while (itRef.hasNext()) {
            ...
            if (formatRef.contains(classe) && formatRef.length() != 0) {
                formatRef = formatRef.replaceAll(classe, classe
                        + " " + feature.getId()); //$NON-NLS-1$
            } else {
                formatRef = formatRef + classe
                + " " + feature.getId() + "|"; //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (formatIndic.length() > 0) {
            this.setIndicateurs(formatIndic.substring(0,
                    formatIndic.length() - 1));
        } else {
            this.setIndicateurs(I18N.
    ...
    public final Lien conversionSGBDVersLiens() {
        ...
        //reference
        String formatRef = this.getObjetsRef(), valeurRef, valeurRefClass,
        valeurRefIds, valeurRefId;
        StringTokenizer tokenRef =
            new StringTokenizer(formatRef, "|"); //$NON-NLS-1$
        StringTokenizer tokenRefId;
        Population populationCourante;
        FT_Feature feature;
        List<Population> liste =
            this.getEnsembleLiensSGBD().getListePopulations();
            ...
            while (it.hasNext()) {
                ...
                if (valeurRefClass.equals(populationCourante.getNomClasse())) {
                    while (tokenRefId.hasMoreElements()) {
                        ...
                        Iterator<FT_Feature> itPop =
                            populationCourante.getElements().iterator();
                        while (itPop.hasNext()) {
                            feature = itPop.next();
        ...
        //comparaison
        String formatComp = this.getObjetsComp(), valeurComp, valeurCompClass,
        valeurCompIds, valeurCompId;
        ...
        while (tokenComp.hasMoreElements()) {
            ...
            while (it.hasNext()) {
                populationCourante = it.next();
                if (valeurCompClass.equals(
                        populationCourante.getNomClasse())) {
        ...
        StringTokenizer tokenIndic = new StringTokenizer(
                formatIndic, "|"); //$NON-NLS-1$
     ...
     */
    public final void setEnsembleLiensSGBD(
            final EnsembleDeLiensSGBD ensemble) {
