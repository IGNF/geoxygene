...
/** 
 * Cluster (used e.g. in the minimal spanning tree clustering algorithm
...
public class Cluster {
	 ...
	 * (size 4 by default) if the cluster contains only a node (i.e. a Point geometry type),
	 * or two nodes (i.e. a Segment geometry type). If the cluster contains strictly more
	 * than two nodes, the polygon is created from the convex hull of the cluster,
	 * except the case where all nodes in the cluster are in the same straight line.
	 * In this last problematic case, the geometry is computed with a buffer (size 4 by default).
	 * 
	 * 
	 * @return a polygon which represents graphically the cluster in order to correctly display it.
	 */
	...
	public Population<DefaultFeature> getPopulationNoeuds() {
		...
		Population<DefaultFeature> population = new Population<DefaultFeature>(
				false,
				"Nodes of the cluster " + this.getClusterId(), //$NON-NLS-1$
	 ...
	 * Returns the general orientation of the cluster computed using a PCA method
	 ...
	 * -- Principal Component Analysis --).
	 * 
	 * @param orientation the general orientation of the cluster computed using a PCA method
	 */
	...
	/**
	 * Returns true if the cluster has all its points at the same position,
	 * false otherwise.
	 * 
	 * @param line LineString to process
	 * @return true if the cluster has all its points at the same position, false otherwise
	 */
    ...
    /**
     * Define the list of the NodeSpecific in relation, and update the inverse relation.
     * 
    ...
    /** 
     * Returns the node at the specified position in the list of nodes in relation with the cluster.
     * 
     * @param i index of the node to return
     * @return the node at the specified position in the list of nodes in relation with the cluster
     */
    ...
    /**
     * Appends the specified node to the end of the list of nodes in relation with the cluster
     * and update the inverse relation.
     * 
    ...
    /**
     * Appends the specified list of nodes to the end of the list of nodes in relation
     * with the cluster and update the inverse relation.
     * 
    ...
    /**
     * Removes the specified node in the list of nodes in relation with the cluster
     * and update the inverse relation.
	 *
    ...
    /**
     * Removes the specified list of nodes in the list of nodes in relation with the cluster
     * and update the inverse relation.
	 *
    ...
    /** 
     * Empty the list of nodes in relation with the cluster and update the inverse relation.
     */
