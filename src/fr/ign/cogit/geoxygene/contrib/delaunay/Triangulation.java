 ...
 * Triangulation class used on top of Jonathan Richard Shewchuk's Triangle
 * library.
 * @author Bonin
...
public class Triangulation extends CarteTopo {
    ...
    private void convertJinSegments() {
        ArrayList<FT_Feature> noeuds =
            new ArrayList<FT_Feature>(this.getListeNoeuds());
        ArrayList<FT_Feature> aretes =
            new ArrayList<FT_Feature>(this.getListeArcs());
        ...
        for (int i = 0; i < this.jin.numberofsegments; i++) {
            this.jin.segmentlist[2 * i] =
                noeuds.indexOf(((ArcDelaunay) aretes.get(i)).getNoeudIni());
            this.jin.segmentlist[2 * i + 1] =
                noeuds.indexOf(((ArcDelaunay) aretes.get(i)).getNoeudFin());
        }
    }
    ...
    private void convertJout() {
        if (logger.isDebugEnabled()) {
            ...
            "Triangulation.ExportStart")); //$NON-NLS-1$
        }
        try {
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                "Triangulation.NodeExportStart")); //$NON-NLS-1$
            }
            for (int i = this.jin.numberofpoints;
            i < this.jout.numberofpoints; i++) {
                this.getPopNoeuds().nouvelElement().setCoord(
                        new DirectPosition(
                                this.jout.pointlist[2 * i],
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                "Triangulation.EdgeExportStart")); //$NON-NLS-1$
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                "Triangulation.TriangleExportStart")); //$NON-NLS-1$
            }
            ...
            if (this.getOptions().indexOf('v') != -1) {
                if (logger.isDebugEnabled()) {
                    logger.debug(I18N.getString(
                    "Triangulation.VoronoiDiagramExportStart")); //$NON-NLS-1$
                }
     ...
     * true Delaunay triangulation.  The Voronoi output is usually meaningless
     * (and may contain crossing edges and other pathology) if the output is a
     * CDT or CCDT, or if it has holes or concavities.  If the triangulation
     * is convex and has no holes, this can be fixed by using the -L switch to
     * ensure a conforming Delaunay triangulation is constructed.
     ...
     * Delaunay refinement algorithm.  Adds vertices to the mesh to ensure that
     * no angles smaller than 20 degrees occur.  An alternative minimum angle 
    ...
    public void triangule(String trianguleOptions) throws Exception {
        if (this.getPopNoeuds().size() < 3) {
            logger.error(I18N.getString(
                    "Triangulation.Cancelled") //$NON-NLS-1$
                    + this.getPopNoeuds().size()
                    + I18N.getString(
                    "Triangulation.NeedsAtLeast3Points")); //$NON-NLS-1$
            return;
        }
        if (logger.isDebugEnabled()) {
            logger.debug(I18N.getString(
            "Triangulation.StartedWithOptions") //$NON-NLS-1$
            + trianguleOptions);
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(I18N.getString(
            "Triangulation.Finished")); //$NON-NLS-1$
        }
    }
