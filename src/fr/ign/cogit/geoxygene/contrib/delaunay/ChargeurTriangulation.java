...
public class ChargeurTriangulation extends Chargeur {
    ...
    public static void importLigneEnPoints(String nomClasseGeo, Triangulation carte) throws Exception {
        ...
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportLinesAsPoints")); //$NON-NLS-1$
        FT_FeatureCollection<?> listeFeatures = DataSet.db.loadAllFeatures(clGeo);
        for(i=0 ; i<listeFeatures.size() ; i++) {
            if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.NumberOfImportedLines") + i); //$NON-NLS-1$
            ...
            if ( objGeo.getGeom() instanceof fr.ign.cogit.geoxygene.spatial.coordgeom.GM_LineString ) {
                ...
                for (j=0; j<listePoints.size(); j++) {
                    if ( (j % 100) == 0 ) {
                        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.NumberOfPointsCreated") + j); //$NON-NLS-1$
                    }
        ...
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportedLinesAsPoints")); //$NON-NLS-1$
    }
    ...
    public static void importLigneEnPoints(FT_FeatureCollection<?> listeFeatures, Triangulation carte) throws Exception {
        ...
        for(int i=0 ; i<listeFeatures.size() ; i++) {
            ...
            if ( objGeo.getGeom() instanceof fr.ign.cogit.geoxygene.spatial.coordgeom.GM_LineString ) {
                ...
                if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.NumberOfPointsCreated") + listePoints.size()); //$NON-NLS-1$
    ...
    public static void importSegments(String nomClasseGeo, Triangulation carte) throws Exception {
        ...
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.DoubleNodesFiltering")); //$NON-NLS-1$
        ...
        while (it.hasNext()) {
            ...
            if (Operateurs.indice2D(tableau,dp) != -1) {
                if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.DoubleNodeRemoval")); //$NON-NLS-1$
        ...
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportedSegments")); //$NON-NLS-1$
    }
    ...
    public static void importSegments(FT_FeatureCollection listeFeatures, Triangulation carte) throws Exception {
        ...
        for(int i=0 ; i<listeFeatures.size() ; i++) {
            ...
            if ( objGeo.getGeom() instanceof fr.ign.cogit.geoxygene.spatial.coordgeom.GM_Polygon ) {
                ...
                if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.Polygon")+i+" "+listePoints); //$NON-NLS-1$ //$NON-NLS-2$
                List<Noeud> listeTemp = new ArrayList<Noeud>();
                for (int j=0; j<listePoints.size(); j++) {
                    if ( (j % 100) == 0 ) {
                        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.NumberOfPointsCreated") + j); //$NON-NLS-1$
        ...
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportedSegments")); //$NON-NLS-1$
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.NodeList")+carte.getListeNoeuds().size()); //$NON-NLS-1$
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.EdgeList")+carte.getListeArcs().size()); //$NON-NLS-1$
    }
    ...
    public static void importPolygoneEnPoints(FT_FeatureCollection listeFeatures, Triangulation carte) throws Exception {
        ...
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportPolygonsAsPoints")); //$NON-NLS-1$
        for(i=0 ; i<listeFeatures.size() ; i++) {
            if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.NumberOfImportedPolygons") + i); //$NON-NLS-1$
            objGeo = listeFeatures.get(i);
    ...
    public static void importCentroidesPolygones(FT_FeatureCollection listeFeatures, Triangulation carte) throws Exception {
        ...
        for(i=0 ; i<listeFeatures.size() ; i++) {
            ...
            if ( objGeo.getGeom() instanceof fr.ign.cogit.geoxygene.spatial.coordgeom.GM_Polygon ) {
                ...
                if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportedCentroid") + p); //$NON-NLS-1$
            }
        }
        if (logger.isDebugEnabled()) logger.debug(I18N.getString("ChargeurTriangulation.ImportedPolygonCentroids")); //$NON-NLS-1$
    }
}
