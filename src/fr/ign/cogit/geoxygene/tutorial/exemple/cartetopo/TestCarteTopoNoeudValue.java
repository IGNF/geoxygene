 ...
 *  Utilisation de noeud valué pour réaliser une cartographie de leur
 *  valuation.
 *
...
public class TestCarteTopoNoeudValue {
    ...
    public static void main(String[] args) {
        ...
        //données BDTopo
        FT_FeatureCollection<BdTopoTrRoute> tronconsBDT =
            geodb.loadAllFeatures(BdTopoTrRoute.class);		

        //création de la carte topologique
        CarteTopo carteTopo =
            CarteTopoFactory.creeCarteTopoEtendue(tronconsBDT);

        //Affichage
        //Initiatlisation du visualisateur GeOxygene
        ObjectViewer viewer = new ObjectViewer();
        viewer.addFeatureCollection(tronconsBDT,
                "Tronçon routier"); //$NON-NLS-1$
        viewer.addFeatureCollection(carteTopo.getPopNoeuds(),
                "Noeuds valués"); //$NON-NLS-1$
    }
}
