...
public class CarteTopoFactory {
  ...
  public static CarteTopo creeCarteTopoDefaut(
    ...
    Iterator<FT_Feature> it = (Iterator<FT_Feature>) collection.getElements()
        .iterator();
    FT_Feature feature;
    Arc arc;
    ...
    while (it.hasNext()) {
      ...
      if (feature.getGeom().isLineString()) {
        lineStrings = CarteTopoFactory.getLineStrings((GM_LineString) feature
            .getGeom());
      } else {
        if (feature.getGeom().isMultiCurve()) {
          lineStrings = CarteTopoFactory
              .getLineStrings((GM_LineString) JtsAlgorithms
                  .union(((GM_MultiCurve<GM_LineString>) feature.getGeom())
                      .getList()));
        }
      }
      if (lineStrings != null) {
    ...
    CarteTopoFactory.logger.info("Fusion des noeuds"); //$NON-NLS-1$
  ...
  public static CarteTopo creeCarteTopoEtendue(
    ...
    CarteTopoFactory.logger.info("Valuation des noeuds"); //$NON-NLS-1$
