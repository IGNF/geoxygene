...
public class GeodatabaseOjbFactory {
    ...
    private final static String POSTGRES_PRODUCT_NAME
    ...
    /**
     * Constructeur de GeodatabaseOjb (Oracle ou Postgis) a partir d'un alias
     * de connection defini dans le fichier de configuration
    ...
    public static Geodatabase newInstance(String jcdAlias) {
        ...
        try {
            if (ojb._conn.getMetaData().getDatabaseProductName()
                    .compareToIgnoreCase(ORACLE_PRODUCT_NAME) == 0) {
                try {
                    Class<?> geodatabaseClass
                    = Class.forName(GEODATABASE_OJB_ORACLE_CLASS_NAME);
                    geodatabaseConstructor = geodatabaseClass.getConstructor(
                            new Class[] { GeodatabaseOjb.class });
                } catch (Exception notfound) {
                    logger.fatal("Class " //$NON-NLS-1$
                            + GEODATABASE_OJB_ORACLE_CLASS_NAME
                    ...
                    .compareToIgnoreCase(POSTGRES_PRODUCT_NAME) == 0) {
                try {
                    Class<?> geodatabaseClass
                    = Class.forName(GEODATABASE_OJB_POSTGIS_CLASS_NAME);
                    geodatabaseConstructor = geodatabaseClass.getConstructor(
                            new Class[] { GeodatabaseOjb.class });
                } catch (Exception notfound) {
                    logger.fatal("Class " //$NON-NLS-1$
                            + GEODATABASE_OJB_POSTGIS_CLASS_NAME
                            + " not found in CLASSPATH"); //$NON-NLS-1$
                    logger.fatal("Program will stop"); //$NON-NLS-1$
                    logger.fatal(notfound.getMessage());
                    notfound.printStackTrace();
