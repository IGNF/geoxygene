 ...
 *  un parametre connection ajoute dans la signature de getValuesForObject
 *  (ligne 352),
 ...
 *
 * AB 11 juillet 2005 :
 * <br> Utilisation des noms de classes et de la réflection pour permettre
 * la compilation séparée pour Oracle.
...
public class GeOxygeneBrokerHelper {
    ...
    = Logger.getLogger(GeOxygeneBrokerHelper.class.getName());
    ...
    public GeOxygeneBrokerHelper(PersistenceBroker broker)
    {
        ...
        this.m_platform = PlatformFactory.getPlatformFor(
                this.m_broker.serviceConnectionManager()
     ...
     * to get user/password from the jdbc-connection-descriptor matching the
     * given PBKey.getAlias().
     */
    ...
    throws PersistenceBrokerException {
        /*
         * arminw
         * Check it out. Because the isProxyClass method is costly and most
         * objects aren't proxies, I add a instanceof check before.
         * Is every Proxy a instance of Proxy?
         */
        if ((objectOrProxy instanceof Proxy)
                && Proxy.isProxyClass(objectOrProxy.getClass())) {
            IndirectionHandler handler;
            handler = (IndirectionHandler) Proxy.getInvocationHandler(
                    objectOrProxy);
            ...
            ClassDescriptor realCld = getRealClassDescriptor(cld,
                    objectOrProxy);
            return getValuesForObject(realCld.getPkFields(), objectOrProxy,
                    convertToSql, conn);
        }
    }
     ...
     */
    public ValueContainer[] getKeyValues(ClassDescriptor cld,
            Identity oid) throws PersistenceBrokerException {
     ...
     */
    public ValueContainer[] getKeyValues(ClassDescriptor cld,
            Identity oid, boolean convertToSql)
    throws PersistenceBrokerException {
     ...
     */
    public ValueContainer[] getKeyValues(ClassDescriptor cld,
            Object objectOrProxy, Connection conn)
    throws PersistenceBrokerException {
        return getKeyValues(cld, objectOrProxy, true, conn);
    }

    /**
     * Return true if aValue is regarded as null.
     * <br>
    ...
    protected Object getAutoIncrementValue(FieldDescriptor fd, Object obj,
        ...
        if (isNull(cv)) {
            ...
            try {
                ...
                // fieldconversion
                Object result = this.m_broker.serviceSequenceManager()
                .getUniqueValue(fd);
                ...
                throw new PersistenceBrokerException(
                        "Could not get key value", //$NON-NLS-1$
                        e);
            }
        }
        return cv;
    ...
    private Query getCountQuery(QueryBySQL aQuery) {
        ...
        int orderPos = countSql.toUpperCase().indexOf(
                " ORDER BY "); //$NON-NLS-1$
        if (orderPos >= 0) {
    ...
    private Query getCountQuery(QueryByCriteria aQuery) {
        ...
        FieldDescriptor[] pkFields = this.m_broker
        .getClassDescriptor(searchClass).getPkFields();
        ...
        if (aQuery instanceof MtoNQuery) {
            ...
            ReportQueryByMtoNCriteria mnReportQuery
            = new ReportQueryByMtoNCriteria(searchClass, columns, countCrit);
            mnReportQuery.setIndirectionTable(mnQuery.getIndirectionTable());
            countQuery = mnReportQuery;
        } else {
            countQuery = new ReportQueryByCriteria(searchClass, columns,
                    countCrit);
        }
        return countQuery;
    }
    ...
    public static String buildMessageString(Object obj, Object value,
            Field field) {
        String eol = SystemUtils.LINE_SEPARATOR;
        StringBuffer buf = new StringBuffer();
        buf
        .append(eol + "object class[ " //$NON-NLS-1$
