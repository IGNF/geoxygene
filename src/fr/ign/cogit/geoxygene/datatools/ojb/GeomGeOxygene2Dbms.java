 ...
 * Pour fonctionner, utiliser absolument la classe "GeOxygeneStatementManager"
 * ( à configurer dans OJB.properties).
 * Permet d'utiliser le même convertisseur pour Oracle et Postgis, et ainsi
 * les mêmes fichiers de mapping.
 *
...
public class GeomGeOxygene2Dbms implements FieldConversion {
    ...
    private static final String POSTGRES_GEOM_CLASS_NAME
    = "org.postgis.PGgeometry"; //$NON-NLS-1$
    private static final String ORACLE_GEOM_CLASS_NAME
    = "oracle.sql.STRUCT"; //$NON-NLS-1$
    ...
    public GeomGeOxygene2Dbms () {
        ...
        try {
            ...
            try {
                this.geomOracle2GeOxygeneMethod = geomGeOxygene2OracleClass
        ...
        try {
            ...
            try {
                ...
                logger.fatal("sqlToJava does not exist in " //$NON-NLS-1$
                        + this.GeomGeOxygene2Postgis_CLASS_NAME);
    ...
    public Object sqlToJava (Object geom) {
        ...
        if (geom.getClass().getName().compareTo(ORACLE_GEOM_CLASS_NAME) == 0) {
            try {
                ...
                logger.error("Returning null geometry"); //$NON-NLS-1$
                logger.error(e.getMessage());
                e.printStackTrace();
                return null;
            }
        }
        if (geom.getClass().getName().compareTo(POSTGRES_GEOM_CLASS_NAME)
                == 0) {
            try {
        ...
        logger.error("Returning null: unhandled platform"); //$NON-NLS-1$
        return null;
    }
    ...
    public Object javaToSql (Object geom) {
        logger.error("Should not be reached: returning null"); //$NON-NLS-1$
        return null;
    }
}
