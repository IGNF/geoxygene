 ...
 */
public class GeodatabaseOjbPostgis extends GeodatabaseOjb
implements Geodatabase {
    static Logger logger
    = Logger.getLogger(GeodatabaseOjbPostgis.class.getName());
    ...
    private void updateConnection() {
        try {
            /* pgConn.addDataType(String, String) is deprecated in version
             * 1.0.x of the Postgresql JDBC driver.
             * This modification breaks compatibility with version 0.x of
             * PostGIS. */
     ...
     * Attention pour POSTGIS, l'emprise et la tolerance ne sont pas
     * renseignes.
     */
    ...
    /**
     * Charge tous les FT_Feature de la classe theClass intersectant le
     * GM_Object geom, dans la classe FT_FeatureCollection.
    ...
    public <T extends FT_Feature> FT_FeatureCollection<T> loadAllFeatures(
        ...
        if ((FT_Feature.class).isAssignableFrom(featureClass)) {
            ...
            List<?> idList = PostgisSpatialQuery.loadAllFeatures(this,
                    featureClass, geom);
            ...
            if (idList.size() > 0) {
                ...
                try {
                    ...
                    if (metadata != null && metadata.getSRID() != 0) {
                         ...
                         * SGBD on Récupère le premier élément (s'il existe) et
                         * ses métadonnées.
                         */
                        if (iter.hasNext()) {
                            T feature = iter.next();
            ...
            logger.warn("loadAllFeatures(): " + //$NON-NLS-1$
    ...
    /**
     * Charge tous les FT_Feature de la classe theClass intersectant le
     * GM_Object geom, dans la classe featureListClass.
     * A appeler a l'interieur d'une transaction ouverte.
     * La classe theClass doit etre une sous-classe de FT_Feature, sinon
     * renvoie une liste vide.
    ...
    public <T> T loadAllFeatures(Class<?> featureClass,
        ...
        try {
            ...
            logger.error("Impossible to create an instance of " //$NON-NLS-1$
    ...
    /**
     * Charge tous les FT_Feature de la classe theClass a une distance dist
     * du GM_Object geom, dans la classe FT_FeatureCollection.
     * Si geom est la geometrie d'un FT_Feature de theClass, alors ce
     * FT_Feature appartiendra au resultat.
     * A appeler a l'interieur d'une transaction ouverte.
     * La classe theClass doit etre une sous-classe de FT_Feature, sinon
     * renvoie une liste vide.
     */
            ...
            if (idList.size() > 0) {
                ...
                try {
                    ...
                    if (metadata != null && metadata.getSRID() != 0) {
                        ...
                        /* si cette classe ne contient pas de métadonnées ou
                         * si c'est une classe mère de la classe stockée dans
                         * le SGBD.
                         * on Récupère le premier élément (s'il existe) et ses
                         * métadonnées.
                         */
    ...
    /**
     * Charge tous les FT_Feature de la classe theClass a une distance dist
     * du GM_Object geom, dans la classe featureListClass.
     * Si geom est la geometrie d'un FT_Feature de theClass, alors ce
     * FT_Feature appartiendra au resultat.
     * A appeler a l'interieur d'une transaction ouverte.
     * La classe theClass doit etre une sous-classe de FT_Feature, sinon
    ...
    /**
     * Cree une requete pour permettre de charger tous les objets a partir
     * d'une liste d'identifants.
     * Usage interne.
     */
     ...
     * La classe doit heriter de FT_Feature, la table doit contenir une
     * geometrie.
     */
     ...
     * ATTENTION : La classe passee en parametre doit avoir un champ
     * "id" de type int (marche pour les FT_Feature).
     * A appeler a l'interieur d'une transaction ouverte.
     */
    ...
    public void refreshRepository(File newRepository) throws Exception {
        ...
        DescriptorRepository rd = mm.readDescriptorRepository(
                newRepository.getPath());
        mm.setDescriptor(rd, true);
        begin();
        PersistenceBrokerHandle pbh
        = (PersistenceBrokerHandle) ((HasBroker)this._tx).getBroker() ;
        PoolablePersistenceBroker ppb
        = (PoolablePersistenceBroker) pbh.getDelegate();
        GeOxygenePersistenceBrokerImpl pbi
        = (GeOxygenePersistenceBrokerImpl) ppb.getDelegate();
        pbi.refresh();
        commit();
        initMetadata();
        initGeomMetadata();
