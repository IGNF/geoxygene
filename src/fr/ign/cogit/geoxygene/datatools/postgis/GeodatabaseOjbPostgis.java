...
public class GeodatabaseOjbPostgis extends GeodatabaseOjb implements
  ...
  static Logger logger = Logger
      .getLogger(GeodatabaseOjbPostgis.class.getName());

  /** Constructeur direct. */
  public GeodatabaseOjbPostgis() {
    super();
    this.updateConnection();
    this.initGeomMetadata();
  }

  /**
   * Constructeur en specialisant GeodatabaseOjb. Usage interne, appele par
   * GeodatabaseOjbFactory.
   */
  ...
  private void updateConnection() {
    try {
      /*
       * pgConn.addDataType(String, String) is deprecated in version 1.0.x of
       * the Postgresql JDBC driver. This modification breaks compatibility with
       * version 0.x of PostGIS.
       */
  ...
  /**
   * Renseigne l'attribut _metadataList. Attention pour POSTGIS, l'emprise et la
   * tolerance ne sont pas renseignes.
   */
   ...
   * geom, dans la classe FT_FeatureCollection. A appeler a l'interieur d'une
  ...
  /**
   * Charge tous les FT_Feature de la classe theClass intersectant le GM_Object
   * geom, dans la classe featureListClass. A appeler a l'interieur d'une
   * transaction ouverte. La classe theClass doit etre une sous-classe de
  ...
  public void refreshRepository(File newRepository) throws Exception {
    ...
    this.begin();
    PersistenceBrokerHandle pbh = (PersistenceBrokerHandle) ((HasBroker) this._tx)
        .getBroker();
    PoolablePersistenceBroker ppb = (PoolablePersistenceBroker) pbh
        .getDelegate();
    GeOxygenePersistenceBrokerImpl pbi = (GeOxygenePersistenceBrokerImpl) ppb
        .getDelegate();
    pbi.refresh();
    this.commit();
    this.initMetadata();
