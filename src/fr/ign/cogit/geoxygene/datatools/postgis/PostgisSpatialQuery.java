 ...
 * @version 1.3
 * 21/07/2008 : modification de la définition de la requete dans la methode
 * executeFeatureList
 * 21/07/2008 : modification de initGeomMetadata : récupération du SRID dans
 * postgis
 */
public class PostgisSpatialQuery {
    ...
    public static void spatialIndex(Geodatabase data, Class<?> clazz) {
        try {
            ...
            String tableName = data.getMetadata(clazz).getTableName()
            .toLowerCase();
            String columnName = data.getMetadata(clazz).getGeomColumnName()
            .toLowerCase();

            // on est oblige de faire ceci, sinon message d'erreur d'Oracle :
            // nom d'index trop long...
            String indexName;
            if (tableName.length() > 24) indexName = tableName.substring(0, 24)
            + "_spidx"; //$NON-NLS-1$
            else indexName = tableName + "_spidx"; //$NON-NLS-1$

            try {
                String query = "CREATE INDEX " + indexName //$NON-NLS-1$
                + " ON " + tableName //$NON-NLS-1$
                + " USING GIST (" + columnName //$NON-NLS-1$
                + " GIST_GEOMETRY_OPS)"; //$NON-NLS-1$
                stm.executeUpdate(query);
                conn.commit();
            } catch (Exception ee) {	// l'index existe
                conn.commit();
                String query = "REINDEX INDEX " + indexName //$NON-NLS-1$
                + " FORCE"; //$NON-NLS-1$
                stm.executeUpdate(query);
            }
            stm.close();
    ...
    ///////////////////////////////////////////////////////////////////////////
    public static void initGeomMetadata (List<?> metadataList,
            Connection conn) {
        try {
            int n = metadataList.size();
            Statement stm = conn.createStatement();
            String query =
                "SELECT F_TABLE_NAME, F_GEOMETRY_COLUMN," + //$NON-NLS-1$
                " SRID FROM GEOMETRY_COLUMNS"; //$NON-NLS-1$
            ...
            while (rs.next()) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (arrayTableName != null) {
                        // ceci car pour les classes abstraites, pas de table
                        // name
                        // On compare le nom de table de GEOMETRY_COLUMNS et
                        // le nom de table issu du mapping
                        if (sqlTableName.compareToIgnoreCase(arrayTableName)
                                == 0) {
                            Metadata metadataElt
                            ...
                            metadataElt.setSRID(Integer.parseInt(
                                    sqlSridColumn));
    ...
    public static List<?> loadAllFeatures(Geodatabase data, Class<?> theClass,
            GM_Object geom) {
        String query = "Intersects"; //$NON-NLS-1$
        return executeFeatureList(data,geom,theClass,query);
    }

    public static List<?> loadAllFeatures(Geodatabase data, Class<?> theClass,
            GM_Object geom, double dist) {
    ...
    private static List<?> executeFeatureList(Geodatabase data, GM_Object geom,
        ...
        try {
            ...
            String geomString = "SRID=" + geom.getCRS() //$NON-NLS-1$
            + ";" + geom.toString(); //$NON-NLS-1$
            String query = "SELECT t." + pkColumn //$NON-NLS-1$
            + " FROM " + tableName + " t ";  //$NON-NLS-1$//$NON-NLS-2$
            query = query + "WHERE t." + geomColumn //$NON-NLS-1$
            + " && '" + geomString + "'";  //$NON-NLS-1$//$NON-NLS-2$
            query = query + " AND " + theQuery //$NON-NLS-1$
            + "('" + geomString + "',t." //$NON-NLS-1$ //$NON-NLS-2$
            + geomColumn + ")"; //$NON-NLS-1$
