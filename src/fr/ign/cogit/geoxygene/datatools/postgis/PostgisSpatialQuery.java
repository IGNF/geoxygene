...
public class PostgisSpatialQuery {
  ...
  public static void spatialIndex(Geodatabase data, Class<?> clazz) {
    try {
      ...
      if (tableName.length() > 24) {
        indexName = tableName.substring(0, 24) + "_spidx"; //$NON-NLS-1$
      } else {
        indexName = tableName + "_spidx"; //$NON-NLS-1$
      }
  ...
  public static void initGeomMetadata(List<?> metadataList, Connection conn) {
    try {
      ...
      Statement stm = conn.createStatement();
      String query = "SELECT F_TABLE_NAME, F_GEOMETRY_COLUMN," + //$NON-NLS-1$
  ...
  public static List<?> loadAllFeatures(Geodatabase data, Class<?> theClass,
    ...
    return PostgisSpatialQuery.executeFeatureList(data, geom, theClass, query);
  }
  ...
  public static List<?> loadAllFeatures(Geodatabase data, Class<?> theClass,
    ...
    String query = "Intersects"; //$NON-NLS-1$
    return PostgisSpatialQuery
        .executeFeatureList(data, buffer, theClass, query);
  }
