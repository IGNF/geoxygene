...
public class GeOxygeneGeometryUserType implements UserType {
    ...
    /**
     * Converts the native geometry object to a GeOxygene
     * <code>GM_Object</code>.
     *
     * @param object
    ...
    public GM_Object convert2GM_Object(Object object) {
        ...
        if (object instanceof PGgeometry) {
            ...
            try {
                 ...
                 */
                GM_Object geOxyGeom = WktGeOxygene.makeGeOxygene(
                        pgGeom.toString().substring(pgGeom.toString().indexOf(
                                ";") + 1)); //$NON-NLS-1$
                ...
                if (geOxyGeom instanceof GM_MultiCurve) {
                    GM_MultiCurve<GM_OrientableCurve> aggr
                    = (GM_MultiCurve<GM_OrientableCurve>) geOxyGeom;
                    if (aggr.size() == 1) { return aggr.get(0); }
                }
                if (geOxyGeom instanceof GM_MultiSurface) {
                    GM_MultiSurface<GM_OrientableSurface> aggr
                    = (GM_MultiSurface<GM_OrientableSurface>) geOxyGeom;
                    if (aggr.size() == 1) { return aggr.get(0); }
                }
                return geOxyGeom;
    ...
    public Object conv2DBGeometry(GM_Object geom, Connection connection) {
        try {
            ...
            logger.warn(
            "## WARNING ## GeOxygene to Postgis returns NULL "); //$NON-NLS-1$
            e.printStackTrace();
            return null;
        }
    }
    ...
    throws HibernateException { return original; }
