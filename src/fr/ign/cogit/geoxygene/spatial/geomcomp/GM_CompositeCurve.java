...
public class GM_CompositeCurve extends GM_Composite {
    ...
    public GM_OrientableCurve getGenerator (int i) {return (GM_OrientableCurve)this.generator.get(i);}
    ...
    public void addGenerator (GM_OrientableCurve value, double tolerance) throws Exception {
        ...
        if (generator.size() > 0) {
            ...
            pt1 = ((GM_CurveBoundary)laDerniereCourbe.boundary()).getEndPoint().getPosition();
            pt2 = ((GM_CurveBoundary)value.boundary()).getStartPoint().getPosition();
    ...
    public void addGeneratorTry (GM_OrientableCurve value, double tolerance) throws Exception {
        try {
            ...
            try {
                this.addGenerator((GM_OrientableCurve)value.getNegative(),tolerance); 
            } catch (Exception e2) {
    ...
    public GM_CompositeCurve() {
        ...
        proxy[0] = (GM_Curve)primitive;
        GM_OrientableCurve proxy1 = new GM_OrientableCurve();
        proxy1.orientation = -1;
        proxy1.proxy[0] = (GM_Curve)primitive;
        proxy1.proxy[1] = proxy1;
        proxy1.primitive = new GM_Curve((GM_Curve)primitive);
        proxy[1] = proxy1;        
    }
    ...
    private void simplifyPrimitive()  {
        ...
        if (n > 0) {    
            ...
            GM_Curve prim = (GM_Curve)this.primitive;
            while (prim.sizeSegment() > 0)
                prim.removeSegment(0);
            for (int i=0; i<n; i++) {
                GM_OrientableCurve oCurve = (GM_OrientableCurve)generator.get(i);
                GM_Curve thePrimitive = (GM_Curve)oCurve.getPrimitive();
                for (int j=0; j<thePrimitive.sizeSegment(); j++) {
                        GM_CurveSegment theSegment = thePrimitive.getSegment(j);
                        ((GM_Curve)this.primitive).addSegment(theSegment);
                }
            }
        }
    }
