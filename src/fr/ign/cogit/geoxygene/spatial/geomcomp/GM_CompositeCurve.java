...
public class GM_CompositeCurve extends GM_Composite {
	...
	public void addGenerator (GM_OrientableCurve value, double tolerance) throws Exception {
		...
		if (this.generator.size() > 0) {
			...
			else throw new Exception("Rupture de chaînage avec la courbe passée en paramètre."); //$NON-NLS-1$
		}
	...
	public void addGeneratorTry (GM_OrientableCurve value, double tolerance) throws Exception {
		try {
			...
			try {
				...
				throw new Exception("Rupture de chaînage avec la courbe passée en paramètre(après avoir essayé les 2 orientations)"); //$NON-NLS-1$
			}
		}
	}
