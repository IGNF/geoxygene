 ...
 * chose mais elle a été mise pour homogénéiser avec les GM_Composite des
 * autres types de primitives.
 * Hérite de GM_Point, mais le lien n'apparaît pas explicitement (problème de
 * double héritage en java). Les méthodes et attributs ont été reportés.
 * <P> Utilisation : un GM_CompositePoint se construit exclusivement à partir
 * d'un GM_Point.
 *
...
public class GM_CompositePoint extends GM_Composite {
    ...
    /**
     * Le GM_Point constituant self.
     * C'est une référence (un nouvel objet n'est pas construit).
     */
    ...
    public void setGenerator (GM_Point value) {
        ...
        this.position =
            new DirectPosition(value.getPosition().getCoordinate());
    }
    ...
    // la methode setPosition n'existe pas, pour obliger a passer par le
    // generator pour affecter un point.

    // Dans la norme, on passe un GM_Position en parametre. Je prefere faire
    // 2 methodes pour simplifier,
    // l'une avec un GM_Point en parametre, l'autre avec un DirectPosition.
    /**
     * NON IMPLEMENTE (renvoie null).
     * Direction entre self et le GM_Point passé en paramètre, en suivant une
     * courbe qui dépend du système de coordonnées (courbe géodésique par
     * exemple).
    ...
    /**
     * NON IMPLEMENTE (renvoie null).
     * Direction entre self et le DirectPosition passé en paramètre, en suivant
     * une courbe qui dépend du système de coordonnées (courbe géodésique par
     * exemple).
     * Le bearing retourné est un vecteur.
     */
    ...
    public GM_CompositePoint(GM_Point thePoint) {
        ...
        this.position =
            new DirectPosition(thePoint.getPosition().getCoordinate());
    }
}
