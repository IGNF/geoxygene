...
public class TP_Node extends TP_DirectedNode  {
	...
	public void setId(int Id) {
		...
		if (Id<0) System.out.println("TP_Node::setId(id) : L'identifiant doit être positif"); //$NON-NLS-1$
	}
	...
	public TP_DirectedNode asTP_DirectedTopo(int sign) {
		if (sign == +1) return this.proxy[0];
			...
			System.out.println("TP_Node::asTP_DirectedTopo(sign) : Passer +1 ou -1 en paramètre."); //$NON-NLS-1$
			return null;
		}
	}
	...
	private void ordonne (List<TP_DirectedEdge> theDirEdges) {
		...
		for (int i=0; i<theDirEdges.size(); i++) {
			...
			if (diredge.getId()<=0) geom_ = (GM_LineString)diredge.topo().getGeom();
			else geom_ = (GM_LineString)((GM_LineString)(diredge.topo().getGeom())).reverse();
			listOfAngles[i] = calculeAngle(geom_);
		}
