...
/**
 * Représente des TP_DirectedEdge connectés en un cycle. L'anneau doit être
 * orienté pour que la face soit à sa gauche.
 * 
...
public class TP_Ring extends TP_Expression {
  ...
  /**
   * Constructeur à partir de plusieurs TP_DirectedEdge. La liste doit contenir
   * au moins 1 element. Le constructeur réorganise la liste pour que les brins
   * orientés soient chaînés. Il renvoie une exception si ça ne boucle pas.
   */
  public TP_Ring(List<TP_DirectedEdge> sdt) throws Exception {
    ...
    // probleme si on commence par un arc pendant, dans ce cas on en prend un
    // autre
    ...
    while (sdt.size() > 0) {
      ...
      // pas de probleme !
        ...
        // if compteur > 1 : presence d'arc pendant (caracterise par left face =
        // right face)
      } else {
        // c'est ceux la qu'on met en premier dans la liste, pour revenir au
        // point initial
        for (int j = 0; j < sdt.size(); j++) {
          TP_DirectedEdge dt = sdt.get(j);
          int IDStartNode = dt.startNode().topo().getId();
          ...
          if ((IDEndNode == IDStartNode)
            ...
            boolean flag = true;
            if (dt.topo().equals(dt0.topo())) { // cas particulier : il faut
