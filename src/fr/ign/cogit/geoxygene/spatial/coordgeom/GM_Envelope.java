...
public class GM_Envelope {
	...
	public boolean contains (double x, double y) {
		if (x < this.lowerCorner.getX()) return false;
		else if (x > this.upperCorner.getX()) return false;
		else if (y < this.lowerCorner.getY()) return false;
		else if (y > this.upperCorner.getY()) return false;
		return true;
	}
	...
	public void expand (DirectPosition thePoint) {
		if (!this.contains(thePoint)) {
			...
			for (int i=0; i<n; i++) {
				...
				double theMin = this.lowerCorner.getCoordinate(i);
				double theMax = this.upperCorner.getCoordinate(i);
				if (theCoord > theMax) this.upperCorner.setCoordinate(i,theCoord);
				else if (theCoord < theMin) this.lowerCorner.setCoordinate(i,theCoord);
			}
		}
	}
	...
	public void expand (double x, double y) {
		if (!this.contains(x,y)) {
			...
			double Ymin = this.lowerCorner.getY();
			double Ymax = this.upperCorner.getY();
			if (y > Ymax) this.upperCorner.setY(y);
			else if (y < Ymin) this.lowerCorner.setY(y);
			if (x > Xmax) this.upperCorner.setX(x);
			else if (x < Xmin) this.lowerCorner.setX(x);
		}
	}
	...
	public void expand (double x, double y, double z) {
		if (!this.contains(x,y,z)) {
			...
			double Zmin = this.lowerCorner.getZ();
			double Zmax = this.upperCorner.getZ();
			if (z > Zmax) this.upperCorner.setZ(z);
			else if (z < Zmin) this.lowerCorner.setZ(z);
			if (y > Ymax) this.upperCorner.setY(y);
			else if (y < Ymin) this.lowerCorner.setY(y);
			if (x > Xmax) this.upperCorner.setX(x);
			else if (x < Xmin) this.lowerCorner.setX(x);
	...
	public DirectPositionList samplePoint() {
		...
		if (!Double.isNaN(this.lowerCorner.getZ()))
			dp = new DirectPosition(this.upperCorner.getX(), this.lowerCorner.getY(), this.lowerCorner.getZ() ); // a revoir
			else
		...
		if (!Double.isNaN(this.upperCorner.getZ()))
			dp = new DirectPosition(this.lowerCorner.getX(), this.upperCorner.getY(), this.upperCorner.getZ() ); // a revoir
		else
