...
public class GM_Polygon extends GM_SurfacePatch {
	...
	/** NON IMPLEMENTE.
	 * Constructeur à partir d'une frontiere et d'une surface.
	 * @param boundary
	 * @param spanSurf
	 */
	...
	/** Constructeur à partir d'une GM_LineString fermee sans verifier la fermeture.
	...
	public GM_SurfacePatch reverse() {
		...
		for (int i=0; i<n; i++) {
			...
			if (oriCurve.getOrientation() == +1) newRing.addGenerator(oriCurve.getNegative());
			else if (oriCurve.getOrientation() == -1) newRing.addGenerator(oriCurve.getPositive());
		}
	...
	@Override
	public Object clone() {
		GM_Polygon poly = new GM_Polygon((GM_Ring)getExterior().clone());
		for(int i=0; i<this.sizeInterior(); i++) poly.addInterior((GM_Ring)getInterior(i).clone());
		return poly;
	}
