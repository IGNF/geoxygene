...
public class DirectPosition {
	...
	/** Constructeur d'un DirectPosition à n dimensions : cree un tableau de coordonees à n dimensions, vide.*/
	/*public DirectPosition(int n) {
        coordinate = new double[n];
        dimension = n;
    }*/

	/** Constructeur à partir d'un tableau de coordonnees.
	 * Si le tableau passe en parametre est 2D, la 3ieme coordonnee du DirectPosition vaudra NaN.
	 * Le tableau est recopie et non passe en reference. */
	...
	/** 
	 * Affecte les coordonnees d'un tableau des coordonnees (2D ou 3D).
	 * Si le tableau passe en parametre est 2D, la 3ieme coordonnee du DirectPosition vaudra NaN.
	 * Le tableau est recopie et non passe en reference. */
	...
	/** Affecte la position d'un point geometrique. Le point passe en parametre doit avoir la meme dimension que this.*/
	public void setCoordinate(GM_Point thePoint) {
		DirectPosition pt = thePoint.getPosition();
		double[] coord = pt.getCoordinate();
		setCoordinate(coord);
	}
	/** Affecte une valeur à la i-eme coordonnees (i=0 pour X, i=1 pour Y, i=2 pour Z.). */
	public void setCoordinate(int i, double x)  {this.coordinate[i] = x;}
	...
	public void setX(double x) {this.coordinate[0] = x;}
	/** Affecte une valeur à Y (2ieme coordonnee du tableau). */
	public void setY(double y) {this.coordinate[1] = y;}
	/** Affecte une valeur à Z (3ieme coordonnee du tableau). */
	public void setZ(double z)  {this.coordinate[2] = z;}

	//////////////////////////////////////////////////////////////////////////////////////////
	// Methodes move /////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////
	/** Deplace le point suivant toutes les dimensions. Le point passe en parametre doit avoir la meme dimension que this.*/
	...
	public void move(double offsetX, double offsetY, double offsetZ) {
		this.coordinate[0] += offsetX;
		this.coordinate[1] += offsetY;
		if (this.coordinate.length==3) this.coordinate[2] += offsetZ;
	}
	...
	/**
	/** Indique si self et le point passe en parametre sont egaux.
	 * Si les 2 points ont une troisieme dimension affectee, on teste cette dimension.
	 * @param pt un point
	...
	/** Indique si self et le point passe en parametre sont egaux, à une tolerance pres.
	 * Si les 2 points ont une troisieme dimension affectee, on teste cette dimension.
	 * Tolerance est un double qui doit etre > 0. 
	 * @param pt un point
	 * @param tolerance tolerance entre this et le point passe en parametre
	 * @return vrai si le point this est egal au point passe en parametre à la tolerance pres
	 * @see #equals(Object)
	...
	/**
	 * Indique si self et le point passe en parametre sont egaux, à une tolerance pres.
	 * La comparaison est effectuee en 2D, i.e. la troisieme dimension est ignoree.
	 * Tolerance est un double qui doit etre > 0. 
	 * @param pt un point
	 * @param tolerance tolerance entre this et le point passe en parametre
	 * @return vrai si le point this est egal au point passe en parametre à la tolerance pres
	...
	public String toString () {
		if (Double.isNaN(this.getZ()))
			return new String("DirectPosition - X : "+this.getX()+"     Y : "+this.getY()); //$NON-NLS-1$ //$NON-NLS-2$
		return new String("DirectPosition - X : "+this.getX()+"     Y : "+this.getY()+"     Z : "+this.getZ());  //$NON-NLS-1$ //$NON-NLS-2$//$NON-NLS-3$
	}
}
