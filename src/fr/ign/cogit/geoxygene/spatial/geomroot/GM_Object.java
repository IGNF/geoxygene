 ...
 * 19107.
 * Cette classe définit les opérations communes aux différents objets
 * géométriques qui en héritent.
 * Toutes les opérations doivent se faire dans le même système de coordonnées.
 * Un objet géométrique est en fait une combinaison d'un système de référence
 * de coordonnées (CRS), et d'une géométrie munie de coordonnées (CoordGeom).
 * <p>
 * Les methodes geometriques font par defaut appel a la bibliotheque JTS,
 * via des appels aux methodes de la classe {@link JtsAlgorithms}. Attention,
 * bien souvent, ces méthodes ne fonctionnent que sur des primitives ou des
 * agrégats homogènes GM_MultiPrimitive.
 * <p>
 * Historiquememt, les methodes faisaient appel aux fonctions geometriques
 * d'Oracle et a la bibliotheque fournie par Oracle sdoapi.zip,
 ...
 * une table TEMP_REQUETE, avec une colonne GID (NUMBER) et une colonne GEOM
 * (SDO_GEOMETRY).
...
abstract public class GM_Object implements Cloneable {
     ...
     * Dans la norme ISO, cet attribut est une relation qui pointe vers la
     * classe SC_CRS (non implémentée)
     */
    protected int CRS = -1;
     ...
     * Cette collection d'objets a une structure de GM_Boundary, qui est un
     * sous-type de GM_Complex.
     */
     ...
     *  Si l'objet est dans un GM_Complex, alors la frontière du GM_Complex
     *  retourné doit être dans le même complexe ;
     ...
     * Si append = true, et que le fichier existe deja, la geometrie est ajoutee
     * a la fin du fichier;
     ...
     * Le format de l'image (.jpg ou .png par defaut) est determine par l'extension
     * du nom de fichier, a mettre dans le parametre "path".
     * Le tableau de couleur permet d'affecter des couleurs differentes aux
     * geometries.
     * <BR> Exemple : 
     * GM_Object.exportImage(new GM_Object[] {geom1, geom2},
     * "/home/users/truc/essai.jpg",
     ...
     * Donner dans la variable "path" le chemin et le nom du fichier (avec
     * l'extension .svgz).
     * Le tableau de couleur permet d'affecter des couleurs differentes aux
     * geometries.
     * <BR> Exemple :
     * GM_Object.exportSvgz(new GM_Object[] {geom1, geom2},
     * "/home/users/truc/essai.jpg",
     * new Color[] {Color.RED, Color.BLUE}, Color.WHITE, 150, 80) */
    public static void exportSvgz(GM_Object[] geoms, String path,
            Color foreground[], Color background, int width, int height) {
        try {
            ImgUtil.saveSvgz (geoms, path, foreground, background, width,
                height);
        } catch (Exception e) { e.printStackTrace(); }
    }
    /////////////////////////////////////////////////////////////////
    ...
    /////////////////////////////////////////////////////////////////
    /**
     * Rectangle englobant minimum de l'objet (en 2D) sous forme de
     * GM_Envelope.
     * @return envelope
     */
     ...
     * Tout autre implementation serait possible : le but serait de supporter
     * des methodes d'indexation qui n'utilisent pas les rectangles minimaux
     * englobants.  
    ...
    /**
     * Teste l'intersection stricte entre la geometrie manipulee et celle
     * passee en parametre, i.e. l'intersection sans les cas oe les geometries
     * sont simplement adjacentes (intersection = point ou ligne) ou sont
     * contenues l'une dans dans l'autre.
     * @param geom geometry
     * @return boolean
     */
     ...
     * Le nombre de segments utilises pour approximer les parties courbes du
     * buffer est celui par defaut de JTS, i.e. 8.
     * La forme du "chapeau" (cap) utilsee est celle par defaut de JTS, i.e.
     ...
     * segments approximant la partie courbe.
     * Les distances negatives sont acceptees (pour faire une erosion).
     * La forme du "chapeau" (cap) utilsee est celle par defaut de JTS, i.e.
     * CAP_ROUND : une courbe.
     * @param distance distance utilisee pour le calcul du buffer
     ...
     * do not make up "geom".
     * @param geom geometry
     * @return a Geometry representing the points making up this Geometry that
     * do not make up "geom"
     */
     ...
     * La difference symetrique (operateur boolean XOR) est la difference de
     * l'union avec l'intersection.
     *  Returns a set combining the points in this Geometry not in other, and
     *  the points in other not in this Geometry.
     * @param geom geometry
     * @return  a set combining the points in this Geometry not in other, and
     *  the points in other not in this Geometry
     */
     ...
     * @return vrai si les deux geometries sont egales (if the DE-9IM
     ...
     * exactement les memes coordonnees dans le meme ordre (avec JTS).
     * Ce predicat est plus stricte que {@link #equals(GM_Object)}
     * @param geom geometrie à comparer à this
     * @return vrai si les deux geometries ont la meme classe et sont
     * strictement egales
    ...
    /** This et l'objet passe en parametre appartiennent a la meme classe et
     * ont exactement les memes coordonnees à une tolerance pres (avec JTS)
     * Ce predicat est plus stricte que {@link #equals(GM_Object)} et moins que
     * {@link #equalsExact(GM_Object)}
     * @param geom geometrie à comparer à this
     * @return vrai si les deux geometries ont la meme classe et sont
     * strictement egales à une tolerance pres
     */
     ...
     * Returns true if the DE-9IM intersection matrix for the two Geometries
     * is T*T***T** (for two points or two surfaces), or 1*T***T** (for two
     * curves).
     * @param geom geometry
     * @return true if the DE-9IM intersection matrix for the two Geometries
     * is T*T***T** (for two points or two surfaces), or 1*T***T** (for two
     ...
     * d'auto-tangence (avec JTS).
     * Cette operation n'est pas applicable aux objets fermes (ceux pour
     * lesquels isCycle() = TRUE).
    ...
    /**
     * Renvoie TRUE si la geometrie est valide au sens JTS. Utile pour
     * debugger.
     * @return true if this is a valid geometry
     */
    public boolean isValid() { return new JtsAlgorithms().isValid(this); }
    /**
     * Returns the minimum distance between this Geometry and the Geometry
     * geom.
     * <p>
     ...
     */
    public GM_Object translate(final double tx, final double ty,
            final double tz)  {
