 ...
 * on fait hériter GM_CurveSegment de GM_Curve.
...
/*implements GM_GenericCurve*/ {
	...
	public void setSegment (int i, GM_CurveSegment value) {
		if ((GM_CurveSegment.class).isAssignableFrom(this.getClass()))
			if (i != 0)
				logger.error("Affection d'un segment avec i<>0 alors qu'un GM_CurveSegment ne contient qu'un segment qui est lui-meme. La méthode ne fait rien."); //$NON-NLS-1$
	...
	public void addSegment (GM_CurveSegment value)  {
		if ((GM_CurveSegment.class).isAssignableFrom(this.getClass()))
			if (sizeSegment() > 0)
				logger.error("Ajout d'un segment alors qu'un GM_CurveSegment ne contient qu'un segment qui est lui-meme. La méthode ne fait rien."); //$NON-NLS-1$
	...
	public void addSegment(GM_CurveSegment value, double tolerance) throws Exception {
		if ((GM_CurveSegment.class).isAssignableFrom(this.getClass()))
			...
			if (this.sizeSegment() == 0) this.segment.add(value);
				...
				else throw new Exception("Rupture de chaînage avec le segment passée en paramètre"); //$NON-NLS-1$
			}
		}
	}
		...
		if ((GM_CurveSegment.class).isAssignableFrom(this.getClass()))
			...
			try {
				...
				try {
					...
					throw new Exception("Rupture de chaînage avec le segment passée en paramètre(après avoir essayé de le retourner)."); //$NON-NLS-1$
				}
			}
		}
	}
	...
	public void addSegment (int i, GM_CurveSegment value) {
		if ((GM_CurveSegment.class).isAssignableFrom(this.getClass()))
			if (i != 0)
				logger.error("Ajout d'un segment avec i<>0 alors qu'un GM_CurveSegment ne contient qu'un segment qui est lui-meme. La méthode ne fait rien."); //$NON-NLS-1$
			else this.segment.add(value);
	 ...
	 * sans imposer que cette paramétrisation représente la longueur de la courbe, et sans imposer 
	...
	/** NON IMPLEMENTE.
	 * Paramètre au startPoint pour une courbe paramétrée, c'est-à-dire : 
	...
	/** NON IMPLEMENTE.
	 * Paramètre au endPoint pour une courbe paramétrée, c'est-à-dire : 
	 ...
	 * Elimine les points doublons consécutifs
	...
	public GM_LineString asLineString(double spacing, double offset, double tolerance)   {
		...
		if ((spacing != 0.0) || (offset != 0.0)) {
			logger.error("GM_Curve::asLineString() : Spacing et Offset ne sont pas implémentés. Passer (0.0, 0.0, tolerance) en paramètres"); //$NON-NLS-1$
			return null;
		} 
