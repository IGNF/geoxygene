...
public class GM_OrientableSurface extends GM_OrientablePrimitive {
   ...
   * GM_Surface s = (GM_Surface)this.getPrimitive(); int n = s.sizePatch(); if
   * (n == 1) { GM_Polygon poly = (GM_Polygon)s.getPatch(0); GM_SurfaceBoundary
   * bdy = new GM_SurfaceBoundary(); bdy.exterior = poly.getExterior();
   * bdy.interior = poly.getInterior(); return bdy; } else if (n == 0) {
   * System.out
   * .println("GM_OrientableSurface::boundary() : la surface est vide "); return
   * null; } else { // Dans le cas des polygones composes de plusieurs patchs,
   * on va utiliser Oracle pour calculer la frontiere // On convertit les patchs
   * en surface et on va en calculer l'union. // Puis on renvoie la frontiere de
   * cette union. GM_Surface union = new
   * GM_Surface(((GM_Surface)this.getPrimitive()).getPatch(0)); for (int i=1;
   * i<n; i++) { GM_Surface surf = new
   * GM_Surface(((GM_Surface)this.getPrimitive()).getPatch(i)); union =
   * (GM_Surface)union.union(data,0.0000000001,surf); } return union.boundary();
   * } }
   */
