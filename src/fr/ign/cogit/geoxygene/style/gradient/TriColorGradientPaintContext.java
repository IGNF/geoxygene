...
public class TriColorGradientPaintContext implements PaintContext {
  ...
  static synchronized Raster getCachedRaster(ColorModel cm, int w, int h) {
    if (cm == TriColorGradientPaintContext.cachedModel) {
      if (TriColorGradientPaintContext.cached != null) {
        Raster ras = TriColorGradientPaintContext.cached.get();
        if (ras != null && ras.getWidth() >= w && ras.getHeight() >= h) {
          TriColorGradientPaintContext.cached = null;
          return ras;
        }
      }
    }
  ...
  static synchronized void putCachedRaster(ColorModel cm, Raster ras) {
    if (TriColorGradientPaintContext.cached != null) {
      Raster cras = TriColorGradientPaintContext.cached.get();
      if (cras != null) {
    ...
    TriColorGradientPaintContext.cachedModel = cm;
    TriColorGradientPaintContext.cached = new WeakReference<Raster>(ras);
  }
  ...
  public TriColorGradientPaintContext(ColorModel cm, Point2D p1, Point2D p2,
    ...
    Point2D h1 = this.projection(dp1, dp2, dp3);
    Point2D h2 = this.projection(dp2, dp1, dp3);
    Point2D h3 = this.projection(dp3, dp1, dp2);
    this.hx1 = h1.getX();
    this.hy1 = h1.getY();
    this.hx2 = h2.getX();
    this.hy2 = h2.getY();
  ...
  private Point2D projection(Point2D p1, Point2D p2, Point2D p3) {
    ...
    return new Point2D.Double(p2.getX() + lambda * p2p3x, p2.getY() + lambda
        * p2p3y);
  }
  ...
  public void dispose() {
    if (this.saved != null) {
      TriColorGradientPaintContext.putCachedRaster(this.model, this.saved);
      this.saved = null;
    }
  }
  ...
  public Raster getRaster(int x, int y, int w, int h) {
    ...
    if (rast == null || rast.getWidth() < w || rast.getHeight() < h) {
      rast = TriColorGradientPaintContext.getCachedRaster(this.model, w, h);
      this.saved = rast;
    }
    int[] pixels = new int[w * h * 4];
    this.clipFillRaster(pixels, x, y, w, h);
    WritableRaster r = (WritableRaster) rast;
    r.setPixels(0, 0, w, h, pixels);
    return rast;
  }

  void clipFillRaster(int[] pixels, int x, int y, int w, int h) {
    int off = 0;
    int minx = x;
    int maxx = x + w;
    int currenty = y;
    ...
    do {
      int currentx = minx;
      do {
        double dx1 = currentx - this.x1;
        double dy1 = currenty - this.y1;
        double a1 = 1 - (dx1 * this.dhx1 + dy1 * this.dhy1)
            / (this.dhx1 * this.dhx1 + this.dhy1 * this.dhy1);
        double dx2 = currentx - this.x2;
        double dy2 = currenty - this.y2;
        double a2 = 1 - (dx2 * this.dhx2 + dy2 * this.dhy2)
            / (this.dhx2 * this.dhx2 + this.dhy2 * this.dhy2);
        double dx3 = currentx - this.x3;
        double dy3 = currenty - this.y3;
        double a3 = 1 - (dx3 * this.dhx3 + dy3 * this.dhy3)
            / (this.dhx3 * this.dhx3 + this.dhy3 * this.dhy3);
        double a = a1 + a2 + a3;
