...
/**
 * The ColorimetricColor class represent a color by different colorimetric systems.
 * It aims to deal with the RGB, CIELab, CIEXYZ, Java and COGIT reference systems.
 * 
...
@XmlType(name = "ColorimetricColor", propOrder = {
    "idColor",
    "hue",
    "lightness",
    "usualName",
...
public class ColorimetricColor {
	...
	//////////////////////////////////////////////////////////////////////////
	/**
	 * Common hue of the colors of a {@link ColorSlice}.
	 * It is expressed in French.
	 * For example BLEU
	 */
	@XmlElement(name = "Hue")
	...
	/**
	 * Return the corresponding {@link ColorSlice} in the {@link ColorReferenceSystem} given in parameter.
	...
	/**
	 * Return the corresponding {@link ColorWheel} in the {@link ColorReferenceSystem} given in parameter.
	 * @param crs The Color Reference System.
	...
	/**
	 * Lightness level of the color. It range from 1 to 7.
	 * All the levels does not exist for all the slices. 
	 */
	...
	/**
	 * Usual name of the color.
	 * It is made up of the hue of the corresponding {@link ColorSlice}
	 * and a literal transcription of the lightness level.
	 * It is expressed in French.
	 * For example: BLEU TRES FONCE
	 */
	...
	/**
	 * Key of the color in the COGIT Reference System.
	 * It is an other identifier of the color.
	 * It is made up of the initial of the hue of the corresponding {@link ColorSlice} and
	 * the lightness level of the color ranged from 1 to 7.
	...
	//////////////////////////////////////////////////////////////////////////
	/**
	 * Red component in the RGB color system.
	 * It range from 0 to 255.
	 */
	...
	/**
	 * Green component in the RGB color system.
	 * It range from 0 to 255.
	 */
	...
	/**
	 * Blue component in the RGB color system.
	 * It range from 0 to 255.
	 */
	 ...
	 * @param usualName The usual name of the color in the COGIT ColorReferenceSystem
	 **/ 
	...
	/**
	 * Constructor with the RGB color component
	 * The boolean specify if the color must be in the COGIT Color Reference System.
	 * @param r The Red component in the RGB color system.
	 * @param g The Green component in the RGB color system.
	 * @param b The Blue component in the RGB color system.
	 * @param reference true if the color must be in the COGIT Color Reference System, false else.
	 **/ 
	public ColorimetricColor(int r, int g, int b, boolean reference) {
		if (reference) {
			ColorimetricColor nearestColor = ColorReferenceSystem.searchColor(new Color(r, g, b));
    ...
    /**
     * Colorimetric function of the CIELab system.
     * Used to calculate the CIELab system color components
	 * from CIEXYZ components.
	 ...
	 */
	public static float contrastLightnessCIELab(ColorimetricColor c1, ColorimetricColor c2) {
	 ...
	 */
	public static float contrastHueCIELab(ColorimetricColor c1, ColorimetricColor c2) {
	...
	public static void basicColorsComponents() {
		...
		System.out.println("rouge\t"+rouge.getRedRGB()+"\t"
				...
				+rouge.getCIELabA()+"\t"
				+rouge.getCIELabB());
		System.out.println("vert\t"+vert.getRedRGB()+"\t"
				+vert.getGreenRGB()+"\t"
				+vert.getBlueRGB()+"\t"
				+vert.getCIELabL()+"\t"
				+vert.getCIELabA()+"\t"
				+vert.getCIELabB());
		System.out.println("bleu\t"+bleu.getRedRGB()+"\t"
				+bleu.getGreenRGB()+"\t"
				+bleu.getBlueRGB()+"\t"
				+bleu.getCIELabL()+"\t"
				+bleu.getCIELabA()+"\t"
		...
		System.out.println("jaune\t"+jaune.getRedRGB()+"\t"
				...
				+jaune.getCIELabA()+"\t"
				+jaune.getCIELabB());
		System.out.println("cyan\t"+cyan.getRedRGB()+"\t"
				+cyan.getGreenRGB()+"\t"
				+cyan.getBlueRGB()+"\t"
				+cyan.getCIELabL()+"\t"
				+cyan.getCIELabA()+"\t"
		...
		System.out.println("blanc\t"+blanc.getRedRGB()+"\t"
				...
				+blanc.getCIELabA()+"\t"
				+blanc.getCIELabB()+"\t");
		System.out.println("noir\t"+noir.getRedRGB()+"\t"
				+noir.getGreenRGB()+"\t"
				+noir.getBlueRGB()+"\t"
				+noir.getCIELabL()+"\t"
				+noir.getCIELabA()+"\t"
	...
	public static void referenceCOGITColorComponents() {
		ColorReferenceSystem crs = ColorReferenceSystem.unmarshall(
				ColorimetricColor.class.getResource("/color/ColorReferenceSystem.xml").getPath());
		
		List<ColorimetricColor> cogitColors = crs.getAllColors();
		
		for (ColorimetricColor couleurXml : cogitColors) {
			System.out.println(
				couleurXml.getUsualName()+"\t"
				+couleurXml.getRedRGB()+"\t"
				+couleurXml.getGreenRGB()+"\t"
				+couleurXml.getBlueRGB()+"\t"
				+couleurXml.getCIELabL()+"\t"
				+couleurXml.getCIELabA()+"\t"
				+couleurXml.getCIELabB());
		}
	}
}