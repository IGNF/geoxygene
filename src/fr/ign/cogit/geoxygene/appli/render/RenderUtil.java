    ...
    public static void draw(final GM_Object geometry, final Viewport viewport,
        ...
        if (geometry.isPolygon()) {
            ...
            for (int i = 0; i < polygon.sizeInterior(); i++) {
                try {
                    Shape shape =
                        viewport.toShape(polygon.interiorLineString(i));
                    if (shape != null) { graphics.draw(shape); }
                } catch (NoninvertibleTransformException e) {
                    e.printStackTrace();
                }
            ...
            if (geometry.isMultiSurface() || geometry.isMultiCurve()) {
                GM_Aggregate<GM_Object> aggregate =
                    (GM_Aggregate<GM_Object>) geometry;
                for (GM_Object element : aggregate) {
                    draw(element, viewport, graphics);
                }
            } else {
                try {
        ...
        if (geometry.isPolygon()) {
            ...
            if (geometry.isMultiSurface()) {
                GM_Aggregate<GM_Object> aggregate =
                    (GM_Aggregate<GM_Object>) geometry;
                for (GM_Object element : aggregate) {
                    fill(element, viewport, graphics);
                }
            }
        }
    }
