...
public class ScalePaintListener implements PaintListener {
  ...
  public void paint(final LayerViewPanel layerViewPanel, Graphics graphics) {
    ...
    graphics.fillRect(0, layerViewPanel.getHeight() - 3 * shift - barWidth,
        barLength + 4 * shift, barWidth + 3 * shift);
    graphics.fillRect(layerViewPanel.getWidth() - rightShift, layerViewPanel
        .getHeight()
        - 3 * shift, rightShift, 3 * shift);
    graphics.setColor(Color.BLACK);
    graphics.drawRect(0, layerViewPanel.getHeight() - 3 * shift - barWidth,
        barLength + 4 * shift, barWidth + 3 * shift - 1);
    graphics.drawRect(layerViewPanel.getWidth() - rightShift, layerViewPanel
        .getHeight()
        - 3 * shift, rightShift - 1, 3 * shift - 1);
    graphics
        .drawString(
            Double.toString(dist) + " m", shift + 1, layerViewPanel.getHeight() - shift - barWidth - 1); //$NON-NLS-1$
    graphics.fillRect(shift, layerViewPanel.getHeight() - shift - barWidth,
        barLength, barWidth);
    int scale = (int) (1.0d / (layerViewPanel.getViewport().getScale() * LayerViewPanel
        .getMETERS_PER_PIXEL()));
    graphics
        .drawString(
