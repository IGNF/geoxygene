 ...
 *
 */
public class ProjectFrame extends JInternalFrame implements FeatureCollectionListener {
	...
	this.layerLegendPanel.setLayout(new BoxLayout(this.layerLegendPanel, BoxLayout.PAGE_AXIS));
     ...
     * @param name
     */
    public void addFeatureCollection(Population<DefaultFeature> population, String name) {
	Layer layer = this.sld.createLayer(name, population.getFeatureType().getGeometryType());
	this.addLayer(layer);
    }
	/**
	 * @param layer
    ...
    public List<Layer> getLayers() {return this.sld.getLayers();}
	...
	else this.layerViewPanel.repaint(layer,(event.getFeature().getGeom()!=null)?event.getFeature().getGeom().union(event.getGeometry()):event.getGeometry());
	/*
	GM_Envelope envelope = event.getFeature().getGeom().envelope();
	try {
	    ...
	    this.layerViewPanel.repaint(layer, (int)lowerCorner.getX(),(int)upperCorner.getY(),(int)(upperCorner.getX()-lowerCorner.getX()),(int)(lowerCorner.getY()-upperCorner.getY()));
