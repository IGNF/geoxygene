...
/**
 * permet à partir d'une classe java choisie par l'utilisateur de générer automatiquement
...
class JavaToSql extends JPanel {
	...
	protected static void action() {
		...
		try {
			...
			GUIConfigSqlOJBXML configuration = new GUIConfigSqlOJBXML(geoxygeneDirectory,geOxygeneMapping,mappingFileName);
			...
			mappingFileName = selectedValues[3];

			if (javaFilePath == null || mappingDirectory == null || tableName == null
					|| mappingFileName == null) return;
			SQLXMLGenerator generator = new SQLXMLGenerator(data,javaFilePath,mappingDirectory,tableName,mappingFileName);
			generator.writeAll();
