...
public class ExportData extends JPanel {
    ...
    protected void action() {
        ...
        try {
            ...
            /*
			try {
				File tryFileData = new File(fileMapping.getParentFile()
						.getParentFile().getParentFile(), "data");
				if (tryFileData.exists()) {
					geoxygeneDirectory = tryFileData.getParentFile().getPath();
				} else {
					tryFileData = new File(fileMapping.getParentFile()
							.getParentFile().getParentFile().getParentFile(),
							"data");
					if (tryFileData.exists()) {
						geoxygeneDirectory = tryFileData.getParentFile()
								.getPath();
					}
				}
				if (!tryFileData.exists()) {
            ...
            ResultSet rs = data.getConnection().getMetaData().getColumns(null, null, tableName, "*"); //$NON-NLS-1$
            ...
            for (int i = 1 ; i < nbCol+1 ; i++) {
                ...
                if (rs.getMetaData().getColumnName(i).equalsIgnoreCase("COLUMN_NAME")) //$NON-NLS-1$
                    columnNameIndex = i;
                if (rs.getMetaData().getColumnTypeName(i).equalsIgnoreCase("TYPE_NAME")) //$NON-NLS-1$
                    typeNameIndex = i;
            }
            rs.absolute(1);
            ...
            for (int i = 0 ; i< fields.length ; i++) {
                if (fields[i]!=null) {
                    System.out.println("\t\t "+fields[i].getAttributeName()+ " -- "+fields[i].getColumnName());  //$NON-NLS-1$//$NON-NLS-2$
                    columnNames.put(fields[i].getAttributeName(), fields[i].getColumnName());
                }
            }
            ...
            viewer.addFeatureCollection(ftfc, I18N.getString("ExportData.ExportedData")); //$NON-NLS-1$
