	...
	protected void action() {
		...
		try {
			...
			for (int i = 1 ; i < nbCol+1 ; i++) {
				System.out.println(i+ " columnName = "+rs.getMetaData().getColumnName(i));
			...
			System.out.println("table "+tableName+" - classe "+javaClassName);
			...
			for (int i = 0 ; i< fields.length ; i++) {
				if (fields[i]!=null) {
					System.out.println("\t\t "+fields[i].getAttributeName()+ " -- "+fields[i].getColumnName());
			...
			System.out.println("ExportData failed...");
			e.printStackTrace();
		}
	}
