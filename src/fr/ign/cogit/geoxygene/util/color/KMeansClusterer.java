...
public class KMeansClusterer {
  ...
  public KMeansClusterer(int theNumberOfClusters, List<float[]> theData) {
    ...
    for (int count = 0; count < this.numberOfClusters - 1; count++) {
      ...
      for (int i = 0; i < this.data.size(); i++) {
        float distance = this.sqDistanceFromClusters(this.data.get(i));
    ...
    boolean hasChanged = this.assignDataPointsToClusters();
    while (hasChanged) {
      hasChanged = this.assignDataPointsToClusters();
    }
    t = System.currentTimeMillis() - t;
    KMeansClusterer.logger.info("KMeansClusterer with " + this.numberOfClusters
        + " clusters and " + this.data.size() + " data points took " + t
        + " ms to finish");
  }
  ...
  private boolean assignDataPointsToClusters() {
    ...
    for (float[] dataPoint : this.data) {
      ...
      for (Cluster cluster : this.clusters) {
        float distance = this.sqDistance(dataPoint, cluster.location);
  ...
  private float sqDistanceFromClusters(float[] fs) {
    ...
    for (Cluster cluster : this.clusters) {
      minDistance = Math
          .min(minDistance, this.sqDistance(fs, cluster.location));
    }
    return minDistance;
  }
  ...
  private float sqDistance(float[] fs, float[] location) {
    ...
    for (int i = 0; i < location.length; i++) {
      distance += (location[i] - fs[i]) * (location[i] - fs[i]);
    }
    return distance;
  }
  ...
  class Cluster {
    ...
    public Cluster(float[] theLocation) {
    ...
    public void computeCentroid() {
      ...
      for (float[] dataPoint : this.dataPoints) {
        for (int i = 0; i < this.location.length; i++) {
          this.location[i] += dataPoint[i] / this.dataPoints.size();
        }
      }
    }
  }
