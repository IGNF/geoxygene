...
public class OctreeQuantizer {
  ...
  // the reduceList contains a chain of all trees which contain at least 2
  // children for each level
  private OctreeNode reduceList[] = new OctreeNode[OctreeQuantizer.MAX_DEPTH + 1];
  private int k;
  ...
  public BufferedImage getRemappedImage() {
    return this.remappedImage;
  }

  // Getters and Setters
  public static int getMAXDEPTH() {
    return OctreeQuantizer.MAX_DEPTH;
  }
  ...
  public void setLeafLevel(int leafLevel) {
    this.leafDepth = leafLevel;
  }

  public OctreeNode getTree() {
    return this.tree;
  }
  ...
  public Color[] getColorLookUpTable() {
  ...
  public OctreeQuantizer(BufferedImage image) {
    ...
    OctreeQuantizer.logger.info(numberOfColors + " colors found"); //$NON-NLS-1$
    this.k = numberOfColors;
    this.quantize();
  }

  /**
   * Constructor. Runs the quantization using parameters ki as the number of
   * colors.
   * @param image image to quantize
  ...
  private void quantize() {
    ...
    this.setColors();
    OctreeQuantizer.logger
        .info("Octree Quantization took " + (System.currentTimeMillis() - t) + " ms"); //$NON-NLS-1$ //$NON-NLS-2$
    t = System.currentTimeMillis();
    this.reMap(this.image);
    OctreeQuantizer.logger
        .info("Remapping took " + (System.currentTimeMillis() - t) + " ms"); //$NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  public void reMap(BufferedImage imageToReMap) {
    this.remappedImage = new BufferedImage(imageToReMap.getWidth(),
    ...
    for (int y = 0; y < imageToReMap.getHeight(); y++) {
      for (int x = 0; x < imageToReMap.getWidth(); x++) {
        ...
        int id = this.findColor(this.tree, color);
  ...
  /**
   * Builds the octree used for the quantization using parameter k as the number
   * of colors.
   */
  public void setColors() {
    ...
    this.tree = this.delete(this.tree);
    // Set the number of leaves of the tree to 0
    this.size = 0;
    // Set the current node depth to the maximum depth (maximum image depth is
    // 24 bits)
    this.currentDepth = OctreeQuantizer.MAX_DEPTH;
    // Set the leaf level to the current node depth+1 (all leaves are on the
    // same level)
    ...
    for (int y = 0; y < this.image.getHeight(); y++) {
      for (int x = 0; x < this.image.getWidth(); x++) {
        ...
        if (added == null) {
          ...
          this.tree = this.insertNode(this.tree, color, 0);
          // writeDebugImage();
          // if we have more leaves than the target number of colors, reduce the
          // tree
          if (this.size > this.k) {
            this.reduceTree();
          }
    ...
    int index[] = new int[1];
    index[0] = 0;
    this.initColorLookUpTable(this.tree, index);
  }
  ...
  private void writeDebugImage() {
    BufferedImage debugImage = this.buildOctreeImage(50);
  ...
  /**
   * Compute the branch index for a certain color at a depth It is a 3 bit
   * integer (i.e. from 0 to 7) to determine at which branch to insert the new
   * color. The first (leftmost) bit is taken from the red component, the second
   * (middle) bit is taken from the green and the last from the blue. The bit
   * taken from each component is chosen according to the depth of the depth
   * into which the color is inserted into the octree. The highest bits are
   * taken for the topmost depth. Basically, we reproduce the same ordering as
   * in sRGB...
  ...
  private int branchIndex(Color color, int level) {
    return ((color.getRed() >> (OctreeQuantizer.MAX_DEPTH - level)) & 1) << 2
        | ((color.getGreen() >> (OctreeQuantizer.MAX_DEPTH - level)) & 1) << 1
        | (color.getBlue() >> (OctreeQuantizer.MAX_DEPTH - level)) & 1;
  }
  ...
  public int findColor(OctreeNode octreeNode, Color color) {
    if (octreeNode.leaf) {
      return octreeNode.colorIndex;
    }
    OctreeNode treeNode = octreeNode.children[this.branchIndex(color,
        octreeNode.depth)];
    return (treeNode != null) ? this.findColor(treeNode, color) : this
        .findNearestEntry(color);
  }
  ...
  public int findNearestEntry(Color color) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      // compute the distance e between the color we're looking for and the
      // current one
  ...
  public OctreeNode insertNode(OctreeNode octreeNode, Color color, int level) {
    ...
    // logger.debug("insertNode "+color+" at depth "+depth);
    ...
    if (node == null) {
      ...
      if (node.leaf) {
        this.size++; // increment the number of leaves
      }
    }
    ...
    if (!(node.leaf) && (level < this.leafDepth)) {
      int branch = this.branchIndex(color, level);
      if (node.children[branch] == null) {
        // add a new child to the node
        node.nbChildren++;
        if (node.nbChildren == 2) {
          // replace the tree for the current depth by the current tree
          // and add the previous one to the current tree as its next reduceable
          // tree
          // this way, the reduceList contains a chain of all trees which
          // contain at least 2 children for each level
          node.nextReduceable = this.reduceList[level];
          this.reduceList[level] = node;
        }
      }
      // insert the color as a child of the current node
      node.children[branch] = this.insertNode(node.children[branch], color,
          level + 1);
    }
    return node;
  }
  ...
  /**
   * Delete the given octree node
   * @param octreeNode the node to delete
   * @return null
   */
  public OctreeNode delete(OctreeNode octreeNode) {
    ...
    for (int i = 0; i < 8; i++) {
      octreeNode.children[i] = this.delete(octreeNode.children[i]);
    }
    this.numNodes--;
    return null;
  }
  ...
  /**
   * Reduce the depth of the tree by deleting children of a certain depth The
   * first tree deleted is the last one encounterd when adding colors
   */
  public void reduceTree() {
    int newLevel = this.currentDepth;
    // find the lowest level where we have reduceable trees, i.e. trees with
    // at least 2 children
    while (this.reduceList[newLevel] == null) {
      newLevel--;
    }
    this.reduceNode(newLevel);
  }

  public static final int MIN_STANDARD_DEVIATION = 0;
  public static final int MIN_DISTANCE = 1;
  public static int reductionMethod = OctreeQuantizer.MIN_DISTANCE;

  /**
   * @param octreeNode
   */
  private void reduceNode(int newLevel) {
    if (OctreeQuantizer.reductionMethod == OctreeQuantizer.MIN_STANDARD_DEVIATION) {
      this.reduceNodeWithMinimumStandardDeviation(newLevel);
    }
    if (OctreeQuantizer.reductionMethod == OctreeQuantizer.MIN_DISTANCE) {
      this.reduceNodeWithMinimumMaximumDistance(newLevel);
    }
  }
  ...
  private void reduceNodeWithMinimumDistance(int newLevel) {
    ...
    int minChildIndex = 0;
    // go through all the reduceable octree node of the level to find the best
    // choice
    ...
    if (octreeNode.nbChildren == 2) {
      if (previousOctreeNode != null) {
        previousOctreeNode.nextReduceable = octreeNode.nextReduceable;
      } else {
        this.reduceList[newLevel] = octreeNode.nextReduceable;
      }
      // delete the reduceable octree we chose and replace it by the next
      // reduceable octree of the same level if there is one
      ...
      this.delete(octreeNode.children[minChildIndex]);
  ...
  public void initColorLookUpTable(OctreeNode octreeNode, int index[]) {
    if (octreeNode != null) {
      // if tree is a leaf, add its average color to the look-up table and
      // affect it with its color index
      // otherwise, treat its children
      if (octreeNode.leaf || octreeNode.depth == this.leafDepth) {
        if (this.colorLookUpTable[index[0]] == null) {
          this.colorLookUpTable[index[0]] = octreeNode.getColor();
        }
        octreeNode.colorIndex = index[0]++;
        octreeNode.leaf = true;
      } else {
        for (int octant = 0; octant < 8; octant++) {
          this.initColorLookUpTable(octreeNode.children[octant], index);
        }
      }
    }
  }
  ...
  public void writeOctreeImage(int sizeElement, String imageName) {
    BufferedImage octreeImage = this.buildOctreeImage(sizeElement);
  ...
  public BufferedImage buildOctreeImage(int sizeElement) {
    int leafLevel = Math.min(this.getLeafLevel() + 1, 3);
    int radius = (leafLevel) * sizeElement;
    BufferedImage octreeImage = new BufferedImage(2 * radius, 2 * radius,
    ...
    this.drawNode(graphics, this.getTree(), 0, radius, leafLevel);
    return octreeImage;
  }
   ...
   */
  public void drawNode(Graphics2D graphics, OctreeNode node, double startAngle,
      int radius, int leafLevel) {
    if ((node == null) || (node.depth > leafLevel)) {
      return;
    }
    // shift the the beginning of the current arc
    int x = OctreeQuantizer.shift(leafLevel, node.depth, radius);
    // radius of the current arc
    int r = 2 * OctreeQuantizer.radius(leafLevel, node.depth, radius);
    // angle for each child element
    double angle = 360.0 / Math.pow(8.0, node.depth);
    if (OctreeQuantizer.logger.isDebugEnabled()) {
      OctreeQuantizer.logger.debug(node.depth + "-" + angle); //$NON-NLS-1$
    }
    /*
     * graphics.setColor(Color.black); graphics.drawLine(x+r/2, x+r/2,
     * (int)(x+(r/2)*(1+Math.cos(Math.toRadians(startAngle)))),
     * (int)(x+(r/2)*(1+Math.sin(Math.toRadians(startAngle)))));
     * graphics.drawLine(x+r/2, x+r/2,
     * (int)(x+(r/2)*(1+Math.cos(Math.toRadians(startAngle+angle)))),
     * (int)(x+(r/2)*(1+Math.sin(Math.toRadians(startAngle+angle)))));
     * graphics.drawArc(x,x,r,r,(int)startAngle,(int)(angle+0.5));
     */
    // draw the children
    double angleChildren = angle / 8.0;
    for (int index = 0; index < 8; index++) {
      this.drawNode(graphics, node.children[index], startAngle + index
          * angleChildren, radius, leafLevel);
    }
   ...
   * @return the shilt
   */
  public static int shift(int leafLevel, int depth, int radius) {
    return (int) ((new Double(leafLevel - depth).doubleValue() * radius) / new Double(
        leafLevel).doubleValue());
  }
   ...
   * @return the radius
   */
  public static int radius(int leafLevel, int depth, int radius) {
    return (int) (new Double(depth * radius).doubleValue() / new Double(
        leafLevel).doubleValue());
  }
  ...
  class OctreeNode {
    ...
    public void addComponents(Color color) {
      float[] newComponents = (ColorUtil.getColorSpace() == ColorUtil.RGB) ? color
          .getColorComponents(null)
          : (ColorUtil.getColorSpace() == ColorUtil.LAB) ? ColorUtil
              .toLab(color) : ColorUtil.toXyz(color);
      for (int i = 0; i < 3; i++) {
        this.components[i] += newComponents[i];
      }
    }
    ...
    public Color getColor() {
      ...
      for (int c = 0; c < 3; c++) {
        newComponents[c] = this.components[c] / Math.max(1, this.nbColor);
      }
      if (OctreeQuantizer.logger.isDebugEnabled()) {
        OctreeQuantizer.logger.debug(newComponents[0]
            + " " + newComponents[1] + " " + newComponents[2]); //$NON-NLS-1$ //$NON-NLS-2$
      }
      return (ColorUtil.getColorSpace() == ColorUtil.RGB) ? new Color(
          ColorSpace.getInstance(ColorSpace.CS_sRGB), newComponents, 1f)
          : (ColorUtil.getColorSpace() == ColorUtil.LAB) ? ColorUtil
              .toColor(newComponents) : new Color(ColorSpace
              .getInstance(ColorSpace.CS_CIEXYZ), newComponents, 1f);
    }
  }
}
