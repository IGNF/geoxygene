...
public class ImageColorClusterGraph {
  static Logger logger = Logger.getLogger(ImageColorClusterGraph.class
      .getName());
  ...
  public ImageColorClusterGraph(BufferedImage clusterImage) {
    ...
    ImageColorClusterGraph.logger.info(clusterMap.size() + " clusters");
    ...
    for (int y = 0; y < clusterImage.getHeight(); y++) {
      for (int x = 0; x < clusterImage.getWidth(); x++) {
        List<Integer> neighbors = this
            .identicalNeighbors(clusterImage, x, y, 1);
        int index = x + y * clusterImage.getWidth();
        for (Integer neighbor : neighbors) {
          this.merge(clusterIndices, index, neighbor.intValue(), clusterMap);
        }
      }
    }
    ImageColorClusterGraph.logger.info(clusterMap.size() + " clusters left");
    ...
    for (Entry<Integer, List<Integer>> entry : clusterMap.entrySet()) {
      ...
      for (Integer index : entry.getValue()) {
        ...
        neighborhood.addAll(this.neighborhood(clusterImage, x, y, 1));
      }
      neighborhood.removeAll(entry.getValue());
      for (Integer index : neighborhood) {
        Edge edge = new Edge(this.vertices.get(entry.getKey()), this.vertices
            .get(new Integer(clusterIndices[index.intValue()])));
        this.edges.add(edge);
      }
    }
    ImageColorClusterGraph.logger.info("Clustering finished in "
        + (System.currentTimeMillis() - t) + " ms");
    t = System.currentTimeMillis();
    this.stats();
    ImageColorClusterGraph.logger.info("End of Stats in "
        + (System.currentTimeMillis() - t) + " ms");
  }
   ...
   * @return
   */
  private List<Integer> identicalNeighbors(BufferedImage clusterImage, int x,
      int y, int neighborhoodSize) {
    ...
    for (int i = -neighborhoodSize; i <= neighborhoodSize; i++) {
      for (int j = -neighborhoodSize; j <= neighborhoodSize; j++) {
        ...
        if ((x + i >= 0) && (x + i < clusterImage.getWidth()) && (y + j >= 0)
            && (y + j < clusterImage.getHeight())
  ...
  private List<Integer> neighborhood(BufferedImage clusterImage, int x, int y,
  ...
  /*
   * private Integer bestNeighbor(BufferedImage clusterImage, int x, int y, int
   * neighborhoodSize) { Map<Integer,Integer> neighborArity = new
   * HashMap<Integer,Integer>(); for (int i = -neighborhoodSize ; i <=
   * neighborhoodSize ; i++) { for (int j = -neighborhoodSize ; j <=
   * neighborhoodSize ; j++) { if ((i==0)&&(j==0)) continue; if ( (x+i>=0) &&
   * (x+i<clusterImage.getWidth()) && (y+j>=0) && (y+j<clusterImage.getHeight())
   * && (clusterImage.getRGB(x, y) != clusterImage.getRGB(x+i, y+j)) ) { int
   * neighborIndex = x+i+(y+j)*clusterImage.getWidth(); if
   * (neighborArity.get(neighborIndex)==null)
   * neighborArity.put(neighborIndex,1); else neighborArity.put(neighborIndex,
   * neighborArity.get(neighborIndex)+1); } } } int maxValue = 0; Integer max=0;
   * for (Entry<Integer,Integer> neighborEntry:neighborArity.entrySet()) {
   * if(neighborEntry.getValue()>maxValue) { maxValue =
   * neighborEntry.getValue(); max = neighborEntry.getKey(); } } return max; }
   */

  private boolean merge(int[] clusterIndices, int i1, int i2,
      Map<Integer, List<Integer>> clusterMap) {
    if (clusterIndices[i1] < clusterIndices[i2]) {
      this.replaceIndices(clusterIndices, i1, i2, clusterMap);
    } else if (clusterIndices[i1] > clusterIndices[i2]) {
      this.replaceIndices(clusterIndices, i2, i1, clusterMap);
    } else {
      return false;
    }
    return true;
   ...
   * @param clusterMap
   */
  private void replaceIndices(int[] clusterIndices, int i1, int i2,
      Map<Integer, List<Integer>> clusterMap) {
    ...
    for (Integer i : pixels) {
      clusterIndices[i.intValue()] = clusterIndices[i1];
    }
    /*
     * logger.info(clusterIndices[i1]);
     * logger.info(clusterMap.get(clusterIndices[i1])); logger.info(pixels);
     */
  ...
  private void stats() {
    ...
    for (Color color : occurrenceMap.keySet()) {
      ...
      for (Vertex vertex : this.vertices.values()) {
        if (vertex.color.equals(color)) {
          ...
          if (vertex.count > 10) {
            sizes.add(new float[] { vertex.count });
          }
        }
      }
      this.colorVertices.put(color, newColorVertices);
      this.colorNumberOfClusters.put(color,
          new Integer(newColorVertices.size()));
      ...
      this.colorProportion.put(color, new Double(100
          * occurrenceMap.get(color).intValue() / (double) nbPixels));
      this.totalCountSum += countSum;
      this.totalNumberOfClusters += newColorVertices.size();
    }
    this.clusterSizeAverage = this.totalCountSum
        / (double) this.totalNumberOfClusters;
    this.clusterSizeStandardDeviation = MathUtil.ecartType(this.clusterSize,
        this.clusterSizeAverage);
    ImageColorClusterGraph.logger.info("The average cluster is "
        + this.clusterSizeAverage + " pixels large");
    ImageColorClusterGraph.logger
        .info("The standard deviation in the cluster size is " + this.clusterSizeStandardDeviation + " pixels"); //$NON-NLS-2$

    KMeansClusterer clusterer = new KMeansClusterer(this.numberOfClusters,
        sizes);
    ...
    for (float size : this.clusterSizes) {
      ImageColorClusterGraph.logger.info("size " + size);
    }

    this.clusterColors = new List[this.clusterSizes.length];

    for (Color color : occurrenceMap.keySet()) {
      ImageColorClusterGraph.logger.info(this.colorNumberOfClusters.get(color)
          + " clusters for color " + Integer.toHexString(color.getRGB())
          + " with an average of " + this.colorClusterAverageSize.get(color)
          + " pixels per cluster    ---   " + occurrenceMap.get(color)
          + " pixels   --- " + this.colorProportion.get(color) + " %");
      int[] listNumbers = this.getNumbers(this.colorVertices.get(color));
      int[] listSurfaces = this.getSurfaces(this.colorVertices.get(color));
      ImageColorClusterGraph.logger.info("It is made of:");
      for (int index = 0; index < this.clusterSizes.length; index++) {
        ImageColorClusterGraph.logger.info(listNumbers[index]
            + " clusters with a total area of " + listSurfaces[index]);
        if (listNumbers[index] > 0) {
          if (this.clusterColors[index] == null) {
            this.clusterColors[index] = new ArrayList<Color>();
          }
    ...
    for (Vertex vertex : this.vertices.values()) {
      ...
      if (sizeMap.get(new Integer(count)) == null) {
        sizeMap.put(new Integer(count), new Integer(1));
      } else {
        sizeMap.put(new Integer(count), new Integer(sizeMap.get(
    ...
    int maxOccurence = 0;
    for (Integer occurence : sizeMap.values()) {
      maxOccurence = Math.max(maxOccurence, occurence.intValue());
    }

    ImageColorClusterGraph.logger.info(sizeList[sizeList.length - 1] + " x "
        + maxOccurence);
    BufferedImage graphImage = new BufferedImage(sizeList[sizeList.length - 1]
        .intValue(), maxOccurence / 10, BufferedImage.TYPE_INT_RGB);
    ...
    for (Entry<Integer, Integer> entry : sizeMap.entrySet()) {
      graphics.drawLine(entry.getKey().intValue(), maxOccurence / 10 - 1, entry
          .getKey().intValue(), maxOccurence / 10 - 1
          - entry.getValue().intValue() / 10);
      // ColorUtil.writeImage(graphImage,
      // "/home/julien/Desktop/occurenceMap.png");
    }
  }
  ...
  private int[] getSurfaces(List<Vertex> list) {
    ...
    for (Vertex vertex : list) {
      int index = this.getIndexOfClosestCluster(vertex.count);
  ...
  public BufferedImage buildGraphImage() {
    int vertexSize = 1;
    BufferedImage graphImage = new BufferedImage(this.image.getWidth()
        * vertexSize, this.image.getHeight() * vertexSize,
    ...
    for (int y = 0; y < this.image.getHeight(); y++) {
      for (int x = 0; x < this.image.getWidth(); x++) {
        ...
        graphics.fillRect(x * vertexSize, y * vertexSize, vertexSize,
            vertexSize);
      }
    }
    graphics.setColor(Color.white);
    ImageColorClusterGraph.logger.info("drawing " + this.edges.size()
        + " edges");
    for (Edge e : this.edges) {
      graphics.drawLine(e.initialVertex.x * vertexSize + vertexSize / 2,
          e.initialVertex.y * vertexSize + vertexSize / 2, e.finalVertex.x
              * vertexSize + vertexSize / 2, e.finalVertex.y * vertexSize
              + vertexSize / 2);
    }
    ImageColorClusterGraph.logger.info("drawing "
        + this.vertices.values().size() + " vertices");
    for (Vertex v : this.vertices.values()) {
      graphics.setColor(v.color);
      // int countSize = 2*(v.count-1);
      graphics.fillOval(v.x * vertexSize, v.y * vertexSize, vertexSize - 1,
          vertexSize - 1);
      graphics.setColor(Color.black);
      graphics.drawOval(v.x * vertexSize, v.y * vertexSize, vertexSize - 1,
          vertexSize - 1);
      graphics.drawString(
          "" + v.count, v.x * vertexSize, v.y * vertexSize + vertexSize - 1); //$NON-NLS-1$

    }
    ImageColorClusterGraph.logger.info("drawing finished");
    return graphImage;
  }
  ...
  public BufferedImage buildGraphClusterImage() {
    ...
    for (Color color : this.colorVertices.keySet()) {
      ...
      for (int index = 0; index < this.numberOfClusters; index++) {
        clusterVertices[index] = new ArrayList<Vertex>();
      }
      for (Vertex v : theVertices) {
        int index = this.getIndexOfClosestCluster(v.count);
        clusterVertices[index].add(v);
      }
      float[] averageSize = new float[this.numberOfClusters];
      for (int i = 0; i < this.numberOfClusters; i++) {
        float totalSize = 0f;
        for (Vertex v : clusterVertices[i]) {
          totalSize += v.count;
        }
        if (totalSize == 0f) {
          continue;
    ...
    for (List<Color> list : this.clusterColors) {
      maxNumberOfColorsPerCluster = Math.max(maxNumberOfColorsPerCluster, list
          .size());
    }

    ImageColorClusterGraph.logger
        .info("sizeOfMaxCluster = " + sizeOfMaxCluster);
    ImageColorClusterGraph.logger.info("maxNumberOfColorsPerCluster = "
        + maxNumberOfColorsPerCluster);
    ...
    ImageColorClusterGraph.logger.info("Creating image of " + maxIndex * factor
        + " x " + (int) sizeOfMaxCluster * this.numberOfClusters * factor);

    BufferedImage graphImage = new BufferedImage((int) (maxIndex * factor),
        (int) (sizeOfMaxCluster * this.numberOfClusters * factor),
    ...
    for (int i = 0; i < positionAndSizeArray.length; i++) {
      for (int j = 0; j < positionAndSizeArray[i].length; j++) {
        ...
        graphics.fillOval((int) (positionAndSizeArray[i][j][0] * factor),
            (int) (positionAndSizeArray[i][j][1] * factor),
            (int) (positionAndSizeArray[i][j][2] * factor),
            (int) (positionAndSizeArray[i][j][2] * factor));
        graphics.setColor(Color.black);
        graphics.drawOval((int) (positionAndSizeArray[i][j][0] * factor),
            (int) (positionAndSizeArray[i][j][1] * factor),
            (int) (positionAndSizeArray[i][j][2] * factor),
            (int) (positionAndSizeArray[i][j][2] * factor));
      }
