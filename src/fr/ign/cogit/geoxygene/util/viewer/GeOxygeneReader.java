...
class GeOxygeneReader implements DataSource {
  ...
  // GeOxygene fields
  private IFeatureCollection<? extends IFeature> coll;
  private Class<? extends IFeature> featureClass;
  ...
  public GeOxygeneReader(IFeatureCollection<? extends IFeature> featColl) {
    this.coll = featColl;

    // guess feature class (type of the first feature ...)
    Iterator<? extends IFeature> iterator = this.coll.iterator();
    ...
    if (GM_Point.class.isAssignableFrom(geom.getClass())
      ...
      Class<? extends IGeometry> classe = ((GM_Aggregate<? extends IGeometry>) geom)
          .get(0).getClass();
      if (GM_Point.class.isAssignableFrom(geom.getClass())
  ...
  public void refreshFeature(IFeature feature) {
  ...
  private GeoPoint geOxygenePointToGeoPoint(int id, IGeometry geom) {
  ...
  private GeoLine geOxygeneLineStringToGeoLine(int id, IGeometry geom) {
    GM_LineString geOxyLineString = (GM_LineString) geom;
    IDirectPositionList dpl = geOxyLineString.coord();
    int nbPts = dpl.size();
  ...
  private GeoPolygon geOxygenePolygonToGeoPolygon(int id, IGeometry geom) {
  ...
  @SuppressWarnings("unchecked")
  private void addGeoShape(IFeature feature, IGeometry geom) {
    if (GM_Point.class.isAssignableFrom(geom.getClass())) {
  ...
  class InitUniqueShader extends Thread {
    ...
    public void run() {
      ...
      Iterator<? extends IFeature> iterator = GeOxygeneReader.this.coll
          .iterator();
      while (iterator.hasNext()) {
        IFeature feature = iterator.next();
        String value = null;
