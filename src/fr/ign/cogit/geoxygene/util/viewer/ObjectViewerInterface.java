...
class ObjectViewerInterface extends JFrame implements Observer {
	...
	protected void addAFeatureCollectionTheme(FT_FeatureCollection<? extends FT_Feature> fColl, String themeName) {
		...
		if (this.themesPropertiesList.size() > 0) {
			for (int i=0; i<this.themesPropertiesList.size(); i++) {
				...
				if (themeProp.getDataSourceType().equals (Utils.GEOXYGENE) && theme.getName().equals(themeName)) {
					...
					FT_FeatureCollection<? extends FT_Feature> fColl = geOxyRead.getFeatureCollection();
