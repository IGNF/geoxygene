 ...
 * processus qui lit les features et ajoute les objets à la population au fur
 * et à mesure.
...
public class ShapefileReader implements Runnable {
    ...
    = Logger.getLogger(ShapefileReader.class.getName());
    ...
    public ShapefileReader(String shapefileName, String populationName,
        ...
        this.reader = initSchema(shapefileName, this.schemaDefaultFeature,
                this.population, initSpatialIndex);
    }
    /**
    ...
    public static Population<DefaultFeature> read(String shapefileName) {
        return read(shapefileName, shapefileName.substring(
    ...
    public static Population<DefaultFeature> read(String shapefileName,
        ...
        Population<DefaultFeature> population
        = new Population<DefaultFeature>(populationName);
        if (dataset != null) { dataset.addPopulation(population); }
        try {
            SchemaDefaultFeature schemaDefaultFeature
            = new SchemaDefaultFeature();
            /** Initialise le schéma */
            ...
            logger.error(I18N.getString(
                    "ShapefileReader.FileURL") //$NON-NLS-1$
                    + shapefileName
                    + I18N.getString(
                    "ShapefileReader.MalformedNullResult")); //$NON-NLS-1$
            return null;
        } catch (IOException e) {
            logger.error(I18N.getString(
                    "ShapefileReader.ProblemReadingFile") //$NON-NLS-1$
                    + shapefileName
                    + I18N.getString(
                    "ShapefileReader.FileNotLoaded")); //$NON-NLS-1$
            return null;
        }
        return population;
    }
    ...
    public static Population<DefaultFeature> chooseAndReadShapefile() {
        ...
        /** crée un filtre qui n'accepte que les fichier shp ou les
         * répertoires */
        choixFichierShape.setFileFilter(new FileFilter(){
            ...
            public String getDescription() {
                ...
                "ShapefileReader.ESRIShapefiles"); //$NON-NLS-1$
            }
        });
        ...
        if(returnVal == JFileChooser.APPROVE_OPTION) {
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                        "ShapefileReader.YouChoseThisFile") //$NON-NLS-1$
     ...
     * @param initSpatialIndex vrai si on souhaite initialiser l'index
     * spatial de la population
     */
    public static Reader initSchema(String shapefileName,
        ...
        population.setCenter(new DirectPosition(
                (maxX + minX) / 2, (maxY + minY) / 2));
        if(logger.isTraceEnabled()) {
            logger.trace(I18N.getString(
                    "ShapefileReader.SpatialIndexInitialised") //$NON-NLS-1$
        ...
        for (int i = 0; i < nbFields; i++) {
            ...
            if(logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                        "ShapefileReader.AddingAttribute") + i //$NON-NLS-1$
                        + " = " + nomField); //$NON-NLS-1$
            }
        }
        /** création d'un schéma associé au featureType */
        newFeatureType.setGeometryType((reader.getShapeType() == null)
                ? GM_Object.class : reader.getShapeType());
        if(logger.isDebugEnabled()){
            logger.debug("shapeType = " + reader.getShapeType() //$NON-NLS-1$
                    + I18N.getString(
                    "ShapefileReader.GeometryType") //$NON-NLS-1$
    ...
    public void run() {
        try {
            read(this.reader, this.schemaDefaultFeature, this.population);
        } catch (IOException e) {
            logger.error(I18N.getString(
                    "ShapefileReader.ProblemReadingFile") //$NON-NLS-1$
                    + this.shapefileName
...
class Reader {
    ...
    public Reader(String shapefileName) throws MalformedURLException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                "ShapefileReader.File") + shapefileName //$NON-NLS-1$
                + I18N.getString(
                "ShapefileReader.NotFound")); //$NON-NLS-1$
            }
            return;
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                "ShapefileReader.PrjFile") + shapefileName //$NON-NLS-1$
                + I18N.getString(
                "ShapefileReader.NotFound")); //$NON-NLS-1$
            }
        } catch (ShapefileException e) {
            if (logger.isDebugEnabled()) {
                logger.debug(I18N.getString(
                "ShapefileReader.ErrorReadingPrjFile") //$NON-NLS-1$
                + shapefileName);
            }
        } catch (IOException e) {
            if (logger.isDebugEnabled()) {
        ...
        this.shapeType
        = geometryType(shapefileReader.getHeader().getShapeType());
        ...
        for(int i = 0; i < this.nbFields; i++) {
            ...
            this.fieldClasses[i]
                              = dbaseFileReader.getHeader().getFieldClass(i);
        }
        // FIXME gère le SRID
        ...
        /*
         try {
         System.out.println("SRS="+CRS.lookupIdentifier(prjFileReader
    ...
    private static Class<? extends GM_Object> geometryType(ShapeType type) {
        if(logger.isDebugEnabled()) {
            logger.debug("shapeType = " + type); //$NON-NLS-1$
        }
