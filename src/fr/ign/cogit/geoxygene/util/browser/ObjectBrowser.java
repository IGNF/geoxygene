 ...
 *  <br>Elle instancie le "Modèle" du navigateur d'objet de GeOxygene,
 *  conformement à l'architecture à Modèle séparable de Sun Microsystems.
 *  Elle pilote la construction de l'interface graphique (vue) représentant
 *  l'objet Java (classe ObjectBrowserGUI).
 *  <br><br> Cette classe utilise intensivement le package reflection du J2SDK
 *  afin de rendre possible la représentation graphique et la navigation au
 *  sein de n'importe quel schéma de classes Java.
 *
...
public class ObjectBrowser {
    ...
    /** Fixe le comportement par défaut lors d'une demande de
     * rafraichissement de la représentation graphique d'un objet.
     * Par défaut, l'ancienne représentation de l'objet reste visible et une
    ...
    /**
     * Teste si l'objet passé en paramètre est une instance de Collection
     * (au sens Java du terme).
     * @param obj l'objet à tester.
     * @return vrai si l'objet passé en paramètre est une instance de
     * Collection, faux sinon.
     */
    ...
    /**
     * Si classObject est de type Array, cette méthode permet de déterminer
     * le type de contenu de ce "tableau".
     *
     * @param classObject la classe de type tableau dont on cherche à
     * déterminer le type de contenu.
     * @return ObjectBrowser.PRIMITIVE dans le cas où le contenu du tableau
     * est de type primitif,
    ...
    /**
     * méthode statique permettant de tester si la classe passée en argument
     * est de type tableau d'un type primitif (int, short, long, boolean, char,
     * etc.).
     *
     * @param classObject la classe à tester.
     * @return vrai si la classe passée en argument est de type tableau d'un
     * type primitif, faux sinon.
     */
    private static boolean isArrayClassComponentTypePrimitive(
            Class<?> classObject) {
        // Faudrait peut-être intercepter le fait que ce soit pas un Array
    ...
    /**
     * méthode statique renvoyant la dimension de l'objet de type tableau
     * passé en argument.
    ...
    /**
     * méthode statique retournant la dimension de l'attribut field sur
     * l'objet obj.
     *
     * @param field l'attribut dont on cherche la dimension (tableau).
     * @param obj l'objet qui porte l'attribut field.
     * @return la dimension de l'attribut field porté par obj ou 0 s'il ne
     * s'agit pas d'un attribut de type tableau (Array).
     */
    ...
    private static int getArrayComponentType(Field field) {
        ...
        case 'L' :
            ...
            if (underlyingFieldTypeName.equals(
    ...
    /**
     * méthode statique renvoyant le nom du type ou de la classe de
     * l'objet tableau passé en paramètre.
     *
    ...
    /**
     * Si l'attribut field passé en paramètre est de type tableau, cette
     * méthode statique renvoie le nom du type ou de la classe qualifiant
     * le contenu du tableau.
     *
     * @param field le champ de type tableau dont on cherche à déterminer le
     * type de contenu.
     * @return le nom de la classe ou du type de contenu du tableau.
     */
    ...
    /**
     * Pour l'attribut field de type Collection sur l'objet obj, retourne le
     * type du contenu de la Collection.
     *
     ...
     * Collection,
     * ObjectBrowser.PRIMITIVE (resp. ObjectBrowser.OBJECT) s'il s'agit d'une
     * Collection d'éléments de type primitif (resp. de type objet).
     */
     ...
     * @param obj l'objet portant l'attribut field.
     * @return suivant le type, une valeur entière parmi
     * ObjectBrowser.PRIMITIVE,
    ...
    private static int getFieldType(Field field, Object obj) {
        ...
        try {
            ...
            if (fieldType.isPrimitive()) {
                ...
                if (fieldValueString.indexOf(fieldClass
                        + "@") > -1) { //$NON-NLS-1$
                    return OBJECT;
                }
     ...
     * @param field l'attribut à tester.
     * @param obj l'objet portant l'attribut field.
     * @return vrai si l'attribut est de type chaîne de caractères (String),
     * faux sinon.
     */
     ...
     * SHOW_PROTECTED_ATTRIBUTES, renvoie l'ensemble des attributs publics
     * et/ou protected accessibles de la classe classObj passée en argument.
     *
     * @param classObj la classe sur laquelle on cherche les attributs
     * publics et/ou protected accessibles.
     * @return un tableau (Field[]) contenant l'ensemble des attributs
     ...
     * retrieveProtectedFields, renvoie l'ensemble des attributs publics
     * et/ou protected accessibles de la classe classObj passée en argument.
     *
     * @param classObj la classe sur laquelle on cherche les attributs publics
     * et/ou protected accessibles.
     * @param retrievePublicFields si vrai, l'ensemble des champs publics de
     * la classe sera retourné par la méthode.
     * @param retrieveProtectedFields si vrai, l'ensemble des champs protected
     * de la classe sera retourné par la méthode.
     * @return un tableau (Field[]) contenant l'ensemble des attributs
     * accessibles de la classe.
     */
     ...
     * SHOW_PROTECTED_METHODS, renvoie l'ensemble des méthodes publiques
     * et/ou protected locales et héritées, ne prenant aucun argument et
     * accessibles de la classe classObj passée en argument.
     *
     * @param classObj la classe sur laquelle on cherche les méthodes
     * locales et héritées, publiques et/ou protected.
     * @return un tableau (Method[]) contenant l'ensemble des méthodes
     * publiques et/ou protected, locales et héritées, ne prenant aucun
     * argument et accessibles de la classe.
     */
    @SuppressWarnings("unused")
     ...
     * retrieveProtectedMethods, renvoie l'ensemble des méthodes publiques
     * et/ou protected, locales et héritées, ne prenant aucun argument et
     * accessibles de la classe classObj passée en argument.
     *
     ...
     * locales et héritées, portées par la classe classObj sera retourné par la
     * méthode.
     * @param retrieveProtectedMethods si vrai, l'ensemble des méthodes
     * protected, locales et héritées, portées par la classe classObj sera
     ...
     * SHOW_PROTECTED_METHODS) de la représentation graphique de l'objet obj
     * passé en argument.
     *
     * @param obj l'objet dont on souhaite obtenir une représentaion par
     * défaut dans le navigateur d'objet de GeOxygene.
     */
    ...
    /**
     * Lance l'affichage de la représentation graphique de l'objet obj passé
     * en argument.
     *
     * @param obj obj l'objet dont on souhaite obtenir une représentaion dans
     * le navigateur d'objet de GeOxygene.
     * @param showClassName si vrai, affiche dans l'interface un bandeau avec
     * le nom du type de l'objet.
     * @param showPublicAttributes si vrai, affiche dans l'interface les
     * attributs publics portés par l'objet.
     ...
     * @param showProtectedMethods si vrai, affiche dans l'interface les
     * méthodes protected, locales et héritées, portées par l'objet.
     */
        ...
        try {
            ...
            try {
                ...
                if (isCollectionClass(obj)) {
                    ...
                    int nbCollectionElements =
                        arrayFromCollectionObject.length;
                    ...
                    if (isArrayClassComponentTypePrimitive(
                            arrayFromCollectionObject
                            .getClass())) {
     ...
     * représentation de l'objet disparaît-elle ?) est fixé par la valeur de
     * la constante ObjectBrowser.HIDE_WHEN_REFRESH.
     * <p>
     * De plus, Le nouvel affichage de l'objet est régi par les valeurs des
     * constantes SHOW_CLASSNAME, SHOW_PUBLIC_ATTRIBUTES,
     * SHOW_PROTECTED_ATTRIBUTES, SHOW_PUBLIC_METHODS et
     * SHOW_PROTECTED_METHODS.</p>
     *
    ...
    public static void refresh(Object obj, boolean dispose) {
        refresh(obj, dispose, SHOW_CLASSNAME, SHOW_PUBLIC_ATTRIBUTES,
                SHOW_PROTECTED_ATTRIBUTES, SHOW_PUBLIC_METHODS,
                SHOW_PROTECTED_METHODS);
    }
