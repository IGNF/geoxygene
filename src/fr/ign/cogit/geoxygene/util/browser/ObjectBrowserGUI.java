 ...
 * séparable de Sun Microsystems. Les méthodes mises à disposition par cette
 * classe sont appelées depuis la classe ObjectBrowser (Modèle).
 * 
...
public class ObjectBrowserGUI extends JFrame {
  ...
  /**
   * Insets définissant l'espacement entre les différents composants graphiques
   * de l'interface.
   */
  ...
  /**
   * L'objet dont l'interface courante du navigateur d'objet de GeOxygene est la
   * représentation.
   */
   ...
   * 
   * @param browsedObject l'objet que l'interface du navigateur que l'on cherche
   *          à construire doit représenter.
   * @param showClassName l'affichage d'un bandeau comportant le nom du type de
   ...
   *          héritées de l'objet est-il demandé ?
   */
  public ObjectBrowserGUI(Object browsedObject, boolean showClassName,
  ...
  public ObjectBrowserGUI(Object browsedObject, boolean showClassName,
      boolean showPublicAttributes, boolean showProtectedAttributes,
      boolean showPublicMethods, boolean showProtectedMethods, String className) {
    super();
    this.currentLocale = Locale.getDefault();
    this.i18nLanguageFile = ResourceBundle.getBundle(
        ObjectBrowserGUI.I18N_LANGUAGE_FILE_LOCATION, this.currentLocale);
  ...
  /**
   * Ajoute un bandeau à l'interface contenant l'affichage du type (className)
   * de l'objet représenté par l'instance courante du navigateur d'objet.
   * 
   * @param className le type de l'objet représenté par l'instance courante du
   *          navigateur d'objet.
   */
  ...
  /**
   * Ajoute à l'interface du navigateur, la représentation d'un attribut de type
  ...
  /**
   * Ajoute à l'interface du navigateur, un composant graphique représentant le
   ...
   * </p>
   * @param attrib_label le nom de l'attribut.
   * @param attrib_values les valeurs du tableau ou de la collection portés par
   *          l'attribut.
   */
  public void addAttributeList(String attrib_label, Vector<Object> attrib_values) {
    this.addObjectAttributeList(attrib_label, attrib_values);
  }

  /**
   * Ajoute à l'interface du navigateur, un composant graphique représentant le
   * contenu d'un tableau ou d'une collection.
  ...
  /**
   * Ajoute à l'interface du navigateur, un composant graphique représentant le
   * contenu d'un attribut de type tableau ou collection.
   * @param attrib_label le nom de l'attribut.
   * @param attrib_objects les valeurs du tableau ou de la collection portés par
   *          l'attribut.
   */
  ...
  public void addObjectAttributeList(String attrib_label,
    ...
    for (int i = 0; i < NbElemAttribList; i++) {
      ...
      if (isTextPrintableField) {
        ...
        if (i < ObjectBrowserGUI.NBVISIBLEROW) {
    ...
    ObjectBrowserListRuler lruler = new ObjectBrowserListRuler(
        NbElemAttribList, (int) attribListPanel.getPreferredSize().getHeight());
    lruler.setPreferredHeight((int) (attribListPanel.getPreferredSize()
        .getHeight()));
  ...
  /**
   * Ajoute à l'interface du navigateur, un composant graphique représentant une
   * méthode.
   * 
  ...
  public void addMethod(Object obj, Method method) {
    ...
    method_button
        .addActionListener(new ObjectBrowserMethodListener(obj, method));
