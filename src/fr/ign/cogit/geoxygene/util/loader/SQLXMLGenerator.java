				...
				while (!classe.getName().equals("java.lang.Object")){
					...
					if (!classe.getName().equals("fr.ign.cogit.geoxygene.feature.FT_Feature")){
						...
						while(i!=attributs.length){
							query = query+", ";
							Field attribut = (Field)attributs[i];
	...
	private void querySQLPostgis(String cheminClasse, String nomTable, boolean flagHeritage) throws Exception {		
		...
		try {
			...
			if (flagGeomFeature){
				...
				case 0:
					stm.executeQuery("SELECT AddGeometrycolumn ('','"+nomTable+"','geom','-1','MULTIPOINT',"+sg.getDimensionGeometrie()+")");
					try {
						stm.executeQuery("ALTER TABLE "+nomTable+" DROP CONSTRAINT enforce_geotype_geom");						
					}
					catch (Exception e) {
					}
					stm.executeQuery("ALTER TABLE "+nomTable+" ADD CONSTRAINT enforce_geotype_geom CHECK (geometrytype(geom) = 'POINT'::text OR geometrytype(geom) = 'MULTIPOINT'::text OR isempty(geom) OR geom IS NULL)");
					break;
				case 1:
					stm.executeQuery("SELECT AddGeometrycolumn ('','"+nomTable+"','geom','-1','MULTILINESTRING',"+sg.getDimensionGeometrie()+")");
					try {
						stm.executeQuery("ALTER TABLE "+nomTable+" DROP CONSTRAINT enforce_geotype_geom");
					}
					catch (Exception e) {
					}
					stm.executeQuery("ALTER TABLE "+nomTable+" ADD CONSTRAINT enforce_geotype_geom CHECK (geometrytype(geom) = 'LINESTRING'::text OR geometrytype(geom) = 'MULTILINESTRING'::text OR isempty(geom) OR geom IS NULL)");
					break;
				case 2:
					stm.executeQuery("SELECT AddGeometrycolumn ('','"+nomTable+"','geom','-1','MULTIPOLYGON',"+sg.getDimensionGeometrie()+")");
					try {
						stm.executeQuery("ALTER TABLE "+nomTable+" DROP CONSTRAINT enforce_geotype_geom");
					}
					catch (Exception e) {
					}
					stm.executeQuery("ALTER TABLE "+nomTable+" ADD CONSTRAINT enforce_geotype_geom CHECK (geometrytype(geom) = 'POLYGON'::text OR geometrytype(geom) = 'MULTIPOLYGON'::text OR isempty(geom) OR geom IS NULL)");
					break;
				case 3:
					stm.executeQuery("SELECT AddGeometrycolumn ('','"+nomTable+"','geom','-1','GEOMETRYCOLLECTION',"+sg.getDimensionGeometrie()+")");
					stm.executeQuery("ALTER TABLE "+nomTable+" DROP CONSTRAINT enforce_geotype_geom");
					break;
				default:
					break;
