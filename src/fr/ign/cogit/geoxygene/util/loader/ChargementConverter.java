...
/**
 * Classe servant à convertir un chargement en XML et un fichier XML en chargement.
 * @see Chargement
 * 
...
public class ChargementConverter implements Converter {
	...
	public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
		...
		while(reader.hasMoreChildren()) {
			...
			if (reader.getNodeName().equalsIgnoreCase("DataSet")) {
						...
						while(reader.hasMoreChildren()) {
							...
							if (reader.getNodeName().equalsIgnoreCase("Nom")) {
								...
								if (!population.getNomClasse().isEmpty()) {
									try {population.setClasse((Class<FT_Feature>) Class.forName(population.getNomClasse()));}
				...
				while(reader.hasMoreChildren()) {
					reader.moveDown();
					if (reader.getNodeName().equalsIgnoreCase("NomPopulation")) {nomPopulation=reader.getValue();}
					else if (reader.getNodeName().equalsIgnoreCase("NomFichier")) {nomFichier=reader.getValue();}
					reader.moveUp();
				}
				if (!nomFichier.isEmpty()) chargement.getFichiers().put(nomPopulation, nomFichier);
			}
			reader.moveUp();
		}
		return chargement;
	...
	@SuppressWarnings("unchecked")
	@Override
	public boolean canConvert(Class classe) {return classe.equals(Chargement.class);}
}
