 ...
 * ensemble de populations, elles mêmes constituées d'un ensemble
 * d'éléments.
...
public class DataSet {
     ...
     * <p>
     * Ce paramètre est très utilisé dans GeOxygene TODO Remplacer cet
     * attribut statique non protété par un singleton
     */
    ...
    /**
     * Constructeur par défaut, recopiant les champs de métadonnées du
     * DataSet
     ...
     *
     * @param geom
     *            géométrie utilisée pour l'extraction géométrique.
     */
     ...
     * Chargement des instances des populations persistantes d'un jeu de
     * données
     * qui intersectent une géométrie donnée. ATTENTION: les tables qui
     * stockent
     * les éléments doivent avoir été indexées dans Oracle. ATTENTION
     * AGAIN:
     * seules les populations avec une géométrie sont chargées.
     *
     * @param zoneExtraction
     *            zone utilisée pour l'extraction géométrique
     */
     ...
     * qui : - intersectent une géométrie donnée (extraction géométrique),
     * - ET
     * qui appartiennent à certains thèmes et populations précisés en
     * entrée.
     *
     * @param geom
     *            définit la zone d'extraction.
     * @param themes
     *            définit les sous-DS du DS à charger. Pour le DS lui-même,
     *            et
     *            pour chaque sous-DS, on précise également quelles
     *            populations
     *            sont chargées. Ce paramètre est une liste de liste de String
     *            composée comme suit (si la liste est nulle on charge tout) :
     *            <ul>
     *            <li>1/ Le premier élément est soit null (on charge alors
     *            toutes les populations directement sous le DS), soit une liste
     *            contenant les noms des populations directement sous le DS que
     *            l'on charge (si la liste est vide, on ne charge rien).
     *            <li>2/ Tous les autres éléments sont des listes (une pour
     *            chaque sous-DS) qui contiennent chacune d'abord le nom d'un
     *            sous-DS que l'on veut charger, puis soit rien d'autre si on
     *            charge toutes les populations du sous-DS, soit le nom des
     *            populations du sous-DS que l'on veut charger.
     *            </ul>
     *            <b>NB :</b> Attention aux majuscules et aux accents.
     *            <p>
     *            <b>EXEMPLE</b> de parametre themes pour un DS représentant la
     *            BDCarto, et spécifiant qu'on ne veut charger que les troncon
     *            et les noeud du thème routier, et les troncons du thème
     *            hydro, mais tout le thème ferré.
     *            <p>
     *            <b>theme = {null, liste1, liste2, liste3}</b>, avec :
     *            <ul>
     *            <li>null car il n'y a pas de population directement sous le DS
     *            BDCarto,
     *            <li>liste1 = {"Routier","Tronçons de route",
     *            "Noeuds routier"},
    ...
    public void chargeExtractionThematiqueEtSpatiale(GM_Object geom,
        ...
        for (DataSet DS : this.getComposants()) {
            ...
            if (aCharger && DS.getPersistant()) {
                DS.chargeExtractionThematiqueEtSpatiale(geom,
                                    populationsACharger);
            }
        }
    ...
    /**
     * Pour un jeu de données persistant, détruit le jeu de données, ses
     * thèmes
     * et ses objets populations.
     * <p>
     * <b>ATTENTION :</b> ne détruit pas les éléments des populations (pour
     * cela vider les tables Oracle).
     */
    ...
    /**
     * Booléen spécifiant si le thème est persistant ou non (vrai par
     * défaut).
     * <p>
     * <b>NB :</b> si un jeu de données est non persistant, tous ses thèmes
     * sont non persistants. Mais si un jeu de données est persistant, certains
     * de ses thèmes peuvent ne pas l'être.
     * <p>
     * <b>ATTENTION :</b> pour des raisons propres à OJB, même si la classe
     * DataSet est concrète, il n'est pas possible de créer un objet
     * PERSISTANT de cette classe, il faut utiliser les sous-classes.
     *
     ...
     *
     * @param b
     *            vrai si le jeu de donné est persistant, faux sinon
     */
    ...
    // /////////////////////////////////////////////////////
    /**
     * Un DataSet se décompose récursivement en un ensemble de DataSet. Le
     * lien
     * de DataSet vers lui-même est un lien 1-n. Les méthodes get (sans
     * indice)
     * et set sont nécessaires au mapping. Les autres méthodes sont là
     * seulement
    ...
    /**
     * enlève un élément de la liste DataSet composant this, et met à jour
     * la
     * relation inverse.
     */
    ...
    public DataSet getComposant(String nomComposant) {
        for (DataSet dataset : this.getComposants())
            if (dataset.getNom().equals(nomComposant)) return dataset;
        logger
    ...
    public void setAppartientA(DataSet O) {
        ...
        if (O != null) {
            ...
            if (!(O.getComposants().contains(this))) O.getComposants()
                        .add(this);
     ...
     * sont nécessaires au mapping. Les autres méthodes sont là seulement
     * pour
     * faciliter l'utilisation de la relation.
     * <p>
     * <b>ATTENTION :</b> Pour assurer la bidirection, il faut modifier les
     * listes uniquement avec ces methodes.
     * <p>
     ...
     */
    protected List<Population<? extends FT_Feature>> populations
    = new ArrayList<Population<? extends FT_Feature>>();
    ...
    /**
     * Ajoute un objet à la liste des populations en relation, et met à jour
     * la
     * relation inverse.
     */
    ...
    /**
     * enlève un élément de la liste des populations en relation, et met à
     * jour
     * la relation inverse.
     *
     * @param O
     ...
     * <ul>
     * <li>relié à un produit, donc potentiellement à de nombreuses
     * métadonnées</li>
     * <li>relié à 0 ou 1 schémaConceptuelJeu (un schémaConceptuelJeu est
     * associé à 0 ou 1 jeu)</li>
     ...
     * sont pas destinées à être persistantes. Elles peuvent être
     * initialisées à
     * partir du schéma conceptuel, qui lui est persitent, grâce à la
     * méthode
     ...
     *
     * @param schema
     *            le schema conceptuel correspondant au jeu de donnees
     */
     ...
     * donnees
     *
     * @return liste des contraintes d'integrite s'appliquant a ce jeu de
     *         donnees
     */
    // @OneToMany
    @Transient
     ...
     *
     * @param contraintes
     *            la liste des contraintes d'integrite s'appliquant a ce jeu de
     *            donnees
     */
    ...
    /***************************************************************************
     * Partie Utilisation du DataSet : les données peuvent être accedées via
     * des
    ...
    public Population<? extends FT_Feature> getPopulationByFeatureTypeName(
        ...
        for (int i = 0; i < this.getPopulations().size(); i++) {
            if (this.getPopulations().get(i).getFeatureType().getTypeName()
                        .equals(nomFeatureType)) { return this.getPopulations()
                        .get(i); }
        }
        logger
    ...
    public static DataSet getInstance() {
        if (dataSet == null) synchronized (DataSet.class) {
