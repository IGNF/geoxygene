...
public abstract class FT_Feature implements Cloneable {
    ...
    = Logger.getLogger(FT_Feature.class.getName());
    /**
     * @return the logger
     */
     ...
     * l'attributeType donné en paramètre. Il est inutile de connaêtre la
     * classe d'implémentation du feature ni le nom de la méthode setter à
     * invoquer.
     *
     * @param attribute
     * @param valeur
     ...
     * n'est pas toujours le cas avec OJB : pour des raisons de performances,
     * le concepteur du fichier de mapping y représente parfois les relations
     * de façon unidirectionnelle. Si la méthode renvoie une liste vide,
     * vérifiez votre fichier de mapping. Si vous ne souhaitez pas le modifier,
     * explorez la relation dans l'autre sens (par exemple, avec un fichier de
     * mapping donné, le role "troncon route a pour noeud initial" sera
     * explorable mais pas le role "noeud routier à pour arcs sortants".
     * 
    ...
    public List<? extends FT_Feature> getRelatedFeatures(
        ...
        try {
            ...
            // cas ou il y a une classe association
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("classe association : " //$NON-NLS-1$
                            + role.getAssociationType().getTypeName());
                }
                List<FT_Feature> listInstancesAsso =
                    new ArrayList<FT_Feature>();
                ...
                for (int i = 0; i < listRolesAGarder.size(); i++) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("role exploré = " //$NON-NLS-1$
                    ...
                    } else { nomFieldAssoMaj = Character.toUpperCase(
                            roleExplore.getNomFieldAsso().charAt(0))
                            + roleExplore.getNomFieldAsso().substring(1); }
                    nomGetMethod = "get" + nomFieldAssoMaj; //$NON-NLS-1$
                    methodGetter = classeAsso.getDeclaredMethod(
                            nomGetMethod, (Class[]) null);
                    if (logger.isDebugEnabled()) {
                        logger.debug("methode de la classe-asso pour recuperer les instances en relation = " //$NON-NLS-1$
                    ...
                    for (int j = 0; j < listInstancesAsso.size(); j++) {
                        if (logger.isDebugEnabled()) {
                            logger.debug("j = " + j); //$NON-NLS-1$
                            logger.debug("instance = " //$NON-NLS-1$
                                    + listInstancesAsso.get(j).getId());
                            logger.debug("class  " //$NON-NLS-1$
                                    + listInstancesAsso.get(j).getClass());
                            logger.debug(methodGetter.invoke(
                                    listInstancesAsso.get(j),
                                    (Object[]) null));
                        }
     ...
     * éléments en relation ont été chargés en mémoire. Ce n'est pas toujours
     * le cas avec OJB : pour des raisons de performances, le concepteur du
     * fichier de mapping y représente parfois les relations de façon
     * unidirectionnelle.
     ...
     * @param nomRole
     * @return la liste des features en relation avec nomFeatureType via
     * nomRole
     */
    public List<? extends FT_Feature> getRelatedFeatures(
        ...
        if (role == null) {
            logger.error("Le FeatureType " + nomFeatureType //$NON-NLS-1$
