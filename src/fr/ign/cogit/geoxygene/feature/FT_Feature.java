...
public abstract class FT_Feature implements Cloneable {
  ...
  public void setGeom(GM_Object g) {
    ...
    synchronized (this.featurecollections) {
      FT_FeatureCollection<FT_Feature>[] collections = this.featurecollections
      ...
      for (FT_FeatureCollection<FT_Feature> fc : collections) {
        ...
        if ((previousGeometry != null)
            && ((this.geom == null) || !(previousGeometry == this.geom))) {
   ...
   * aucune population. NB : normalement, this appartient à une seule
   * collection. Si ce n'est pas le cas, une seule des collections est renvoyée
   * au hasard (la Première de la liste).
   */
  // @SuppressWarnings("unchecked")
  ...
  public List<FT_Feature> getCorrespondants() {
  ...
  public void clearCorrespondants() {
    for (FT_Feature O : this.correspondants) {
      O.getCorrespondants().remove(this);
    }
   ...
   */
  public List<FT_Feature> getCorrespondants(
      FT_FeatureCollection<? extends FT_Feature> pop) {
  ...
  public Population<FT_Feature> getPopulation() {
    ...
    synchronized (this.featurecollections) {
      FT_FeatureCollection<FT_Feature>[] collections = this.featurecollections
          .toArray(new FT_FeatureCollection[0]);
      for (FT_FeatureCollection<FT_Feature> f : collections) {
        if (f instanceof Population<?>) {
          return (Population<FT_Feature>) f;
        }
      }
    }
    return null;
  ...
  /**
   * Methode reflexive pour Récupèrer la valeur d'un attribut donné en paramètre
   * 
   * @param attribute
  ...
  public Object getAttribute(AttributeType attribute) {
    ...
    while (!classe.equals(Object.class)) {
      try {
        ...
        if (FT_Feature.logger.isDebugEnabled()) {
          FT_Feature.logger
              .debug("SecurityException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      } catch (IllegalArgumentException e) {
        if (FT_Feature.logger.isDebugEnabled()) {
          FT_Feature.logger
              .debug("IllegalArgumentException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      } catch (IllegalAccessException e) {
        if (FT_Feature.logger.isDebugEnabled()) {
          FT_Feature.logger
              .debug("IllegalAccessException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      } catch (InvocationTargetException e) {
        if (FT_Feature.logger.isDebugEnabled()) {
          FT_Feature.logger
              .debug("InvocationTargetException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
          FT_Feature.logger.debug(e.getCause());
        }
      }
    ...
    while (!classe.equals(Object.class)) {
      try {
        ...
        if (FT_Feature.logger.isTraceEnabled()) {
          FT_Feature.logger
              .trace("SecurityException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      } catch (IllegalArgumentException e) {
        if (FT_Feature.logger.isTraceEnabled()) {
          FT_Feature.logger
              .trace("IllegalArgumentException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      } catch (IllegalAccessException e) {
        if (FT_Feature.logger.isTraceEnabled()) {
          FT_Feature.logger
              .trace("IllegalAccessException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      } catch (InvocationTargetException e) {
        if (FT_Feature.logger.isTraceEnabled()) {
          FT_Feature.logger
              .trace("InvocationTargetException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
      classe = classe.getSuperclass();
    }
    FT_Feature.logger
        .error("Echec de l'appel au getter de l'attribut " + attribute.getNomField() + " sur l'objet " + this); //$NON-NLS-1$ //$NON-NLS-2$
    return null;
  }
    ...
    if (attribute.getMemberName().equals("geom")) { //$NON-NLS-1$
      ...
      try {
        ...
        try {
          Method methodSetter = this.getClass().getDeclaredMethod(
              nomSetFieldMethod,
              FT_Feature.class2PrimitiveClass(valeur.getClass()));
          methodSetter.invoke(this, valeur);
        } catch (Exception e1) {
          e1.printStackTrace();
        }
  ...
  public List<? extends FT_Feature> getRelatedFeatures(FeatureType ftt,
    ...
    try {
      ...
      // cas ou il y a une classe association
        ...
        if (FT_Feature.logger.isDebugEnabled()) {
          FT_Feature.logger.debug("classe association : " //$NON-NLS-1$
        ...
        if (FT_Feature.logger.isDebugEnabled()) {
          FT_Feature.logger.debug("cardMax de " //$NON-NLS-1$
        ...
        if (!role.getCardMax().equals("1")) { //$NON-NLS-1$
          if (FT_Feature.logger.isDebugEnabled()) {
            FT_Feature.logger
                .debug("invocation de " + methodGetter.getName() + " sur le feature " + this.getId()); //$NON-NLS-1$ //$NON-NLS-2$
          }
          listInstancesAsso.addAll((List<FT_Feature>) methodGetter.invoke(this,
              (Object[]) null));
          if (FT_Feature.logger.isDebugEnabled()) {
            FT_Feature.logger
                .debug("nb instances d'association = " + listInstancesAsso.size()); //$NON-NLS-1$
          }
        } else {
        ...
        for (int i = 0; i < listRolesAGarder.size(); i++) {
          ...
          if (FT_Feature.logger.isDebugEnabled()) {
            FT_Feature.logger.debug("role exploré = " //$NON-NLS-1$
          ...
          if (FT_Feature.logger.isDebugEnabled()) {
            FT_Feature.logger
                .debug("methode de la classe-asso pour recuperer les instances en relation = " //$NON-NLS-1$
          ...
          for (int j = 0; j < listInstancesAsso.size(); j++) {
            if (FT_Feature.logger.isDebugEnabled()) {
              FT_Feature.logger.debug("j = " + j); //$NON-NLS-1$
              FT_Feature.logger.debug("instance = " //$NON-NLS-1$
                  + listInstancesAsso.get(j).getId());
              FT_Feature.logger.debug("class  " //$NON-NLS-1$
                  + listInstancesAsso.get(j).getClass());
              FT_Feature.logger.debug(methodGetter.invoke(listInstancesAsso
                  .get(j), (Object[]) null));
            }
  ...
  /**
   * Methode pour Récupèrer les features en relation par l'intermédiaire du role
  ...
  public List<? extends FT_Feature> getRelatedFeatures(String nomFeatureType,
    ...
    if (role == null) {
      FT_Feature.logger
          .error("Le FeatureType " + nomFeatureType //$NON-NLS-1$
              + " ou l'AssociationRole " + nomRole //$NON-NLS-1$
  ...
  public String toString() {
    return this.getClass().getSimpleName() + " " + this.getGeom(); //$NON-NLS-1$
  }
}
