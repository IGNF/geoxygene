 ...
 * des propriet�s (attributs, r�les, opérations) et des contraintes.
 *
 * Dans GeoTools "schema" designe la structure d'un feature et non pas d'un jeu de don�es.
 */
...
public class SchemaConceptuelJeu implements SchemaConceptuel {
	 ...
	 * (le schéma resultant est int�gralement conforme au schéma du produit)
	 */
	public SchemaConceptuelJeu(SchemaConceptuelProduit schemaProduit){
		...
		while (itFT.hasNext()){
			...
			while (itAttrib.hasNext()){
				...
				if (currentFCFA.getValueDomainType()){
					//System.out.println("\nattribut "+ currentFCFA.getName()+" �num�r�");
		...
		while (itASS.hasNext()){
			...
			while (itRoles.hasNext()){
				...
				//on recupere le FeatureType li� à ce AssociationRole
	...
	/**
	 * jeu de données caract�ris�e par ce schémaJeu
	 */
	...
	public void createFeatureType(String nomClasse) {
		...
		// V�rification qu'aucune classe du même nom n'existe dans le schéma
	...
	public void removeFeatureType(FeatureType ft) {
		...
		while (iTatt.hasNext()) {
			...
			if (attbd.getValueDomainType()) {
				//On crée une liste locale des valeurs d'attributs �num�r�s
		...
		while (iTasso.hasNext()) {
			...
			//On Récupère la liste des roles jou�s par les ft dans cette asso
	...
	public void createFeatureAttribute(FeatureType ft, String nomAtt, String type, boolean valueDomainType) {
		...
		/*
		 * V�rification qu'aucun attribut du même nom n'existe pour cette classe
	...
	public void createGeneralisation(FeatureType classeCurr, FeatureType classeMere) {
		...
		if (classeCurr.getSchema() != classeMere.getSchema()) {
			...
			.showMessageDialog(
					null,
					"Erreur : On ne peut �tablir de relation de généralisation entre classes issues de BD différentes.",
	...
	public void createSpecialisation(FeatureType classeCurr, FeatureType classeFille) {
		...
		if (classeCurr.getSchema() != classeFille.getSchema()) {
			...
			.showMessageDialog(
					null,
					"Erreur : On ne peut �tablir de relation de spécialisation entre classes issues de BD différentes.",
					"Erreur!", JOptionPane.ERROR_MESSAGE);
			return;
		} else {
		}
	...
	public void createFeatureAssociation(String nomAsso, FeatureType ft1, FeatureType ft2, String role1, String role2) {
		...
		if (ft1.getSchema() != ft2.getSchema()) {
			...
			.showMessageDialog(
					null,
					"Erreur : On ne peut �tablir de lien entre classes issues de schémas différents.",
					"Erreur!", JOptionPane.ERROR_MESSAGE);
			return;
		} else {
		}
	...
	/*
	 * *********************************************************
	 * méthodes hérit�es de Schema pour lister les éléments
	 * *********************************************************
	 */
	...
	/**
	 * @return La liste de toutes les valeurs d'attributs �num�r�s du schéma
	 */
