...
public class AssociationType extends FeatureType implements	GF_AssociationType {
	 ...
	 * List memberOf; protected List featureOperations; protected List
	 * generalization = new ArrayList(); protected List specialization = new
	 * ArrayList();
	 * 
	 */
	...
	public AssociationType(fr.ign.cogit.geoxygene.schema.schemaConceptuelISOProduit.AssociationType ori) {
		this.id = ori.getId();
		...
		this.roles = new ArrayList<GF_AssociationRole>();
		this.linkBetween = new ArrayList<GF_FeatureType>();
		this.nomClasseAsso = ori.getNomClasseAsso();
	...
	public AssociationRole getRoleByName(String nomRole){
		...
		for (int i=0 ; i<listRoles.size() ; i++){
			if (((AttributeType)listRoles.get(i)).getMemberName().equals(nomRole)){
