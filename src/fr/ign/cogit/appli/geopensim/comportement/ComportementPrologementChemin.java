 ...
 *
 */
public class ComportementPrologementChemin extends Comportement {
	private static Logger logger=Logger.getLogger(ComportementPrologementChemin.class.getName());
	...
	public void declencher(Agent agent) {
		...
		if (agent instanceof AgentTronconChemin) {
			if (logger.isDebugEnabled()) logger.debug("Début du prolongement de la Chemin "+agent);
			AgentTronconChemin agentChemin = (AgentTronconChemin) agent;
			if (agentChemin.getZonesElementaires().size()!=1) {
				if (logger.isDebugEnabled()) logger.debug("Fin : ce n'est pas une impasse");
				return;
			}
			AgentZoneElementaireBatie agentZoneElementaireBatie = (AgentZoneElementaireBatie) agentChemin.getZonesElementaires().iterator().next();
			...
			for(Arc arc:carte.getPopArcs()) {
				if (arc.getCorrespondant(0).equals(agentChemin)) {
					arcRoute = arc;
				}
			}
			if (arcRoute==null) {
			...
			if (bestPoint!=null) {
				...
				IDirectPositionList pointList = agentChemin.getGeom().coord();
				if (start) pointList.add(0,bestPoint);
				else pointList.add(bestPoint);
				agentChemin.setGeom(new GM_LineString(pointList));
				agentChemin.setSimulated(true);
				agentChemin.setImpasse(!intersectsTroncon);
