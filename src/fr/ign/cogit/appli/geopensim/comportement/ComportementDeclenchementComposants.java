 ...
 *
 */
public class ComportementDeclenchementComposants extends Comportement {
    private static Logger logger=Logger.getLogger(ComportementDeclenchementComposants.class.getName());

    @Override
    public void declencher(Agent agent) {
	super.declencher(agent);
	if (logger.isDebugEnabled()) logger.debug("activation des composants de "+agent);
	...
	if (agent instanceof AgentMeso) {
		...
		composant.activer();
//          Moteur.getListe().add(composant);
	    }
	} else {
	    logger.error("Agent de type "+agent.getClass()+" au lieu de AgentMeso");
	}
	if (logger.isDebugEnabled()) logger.debug("Fin d'activation des composants de "+agent);
    }
