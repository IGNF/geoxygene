 ...
 *
 */
public class ProlongementDesserte extends Comportement {
	private static Logger logger=Logger.getLogger(ProlongementDesserte.class.getName());
	...
	public void declencher(Agent agent) {
		...
		if (agent instanceof AgentZoneElementaireBatie) {
			if (logger.isDebugEnabled()) logger.debug("Début du prolongement dans "+agent);
			AgentZoneElementaireBatie agentZone = (AgentZoneElementaireBatie) agent;
			AgentTroncon agentRoute = null;
			for (AgentTroncon agentTroncon:agentZone.getTroncons()) {
				if ((agentTroncon instanceof AgentTronconRoute)||(agentTroncon instanceof AgentTronconChemin)) {
					if (agentTroncon.getZonesElementaires().size()==1) {
						agentRoute = agentTroncon;
					}
				}
			}if (agentRoute == null) { return; }
