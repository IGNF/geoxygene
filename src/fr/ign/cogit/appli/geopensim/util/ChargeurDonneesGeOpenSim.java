 ...
 *
 */
public class ChargeurDonneesGeOpenSim {
	static Logger logger=Logger.getLogger(ChargeurDonneesGeOpenSim.class.getName());
	...
	public static AgentGeographiqueCollection charge() {
		...
		//DataSet.db.commit();
		if (logger.isDebugEnabled()) {
		    logger.debug("Fin du chargement en "+(System.currentTimeMillis()-time)+" ms");
		}
	 ...
	 * @param geoCollection collection contenant les éléments Géographiques déjà chargés et dans laquelle ajouter les nouveaux.
	 */
	@SuppressWarnings("unchecked")
	private static void charge(String fileName, String javaClassName, AgentGeographiqueCollection geoCollection) {
		BasicRepresentationFactory factory = new BasicRepresentationFactory();
		ElementRepresentation representation = factory.creerElementRepresentation(javaClassName);
		if (representation==null) {
			logger.warn("Classe "+javaClassName+" non trouvée");
			return;
		}
		Class<? extends ElementRepresentation> classe = representation.getClass();
		...
		*/
		...
		try {
		    for(IFeature feature:population) {
		        // essaye de Déterminer la date de la saisie à partir du champ source
		        String source = (String) feature.getAttribute("SOURCE");
		        //logger.info("Source = "+source);
		        int index = source.lastIndexOf('_');
		        Integer dateInteger = -1;
		        if (index<0) index = source.length()-4; // la chaine de caractère ne contient pas "_"
		        else index++;
		        if (index<0) {
		            // la date n'est pas dans l'attribut source
		            if (feature.getAttribute("DATESOURCE") instanceof Integer)
		                dateInteger = (Integer) feature.getAttribute("DATESOURCE");
		            else
		                dateInteger = Integer.parseInt((String) feature.getAttribute("DATESOURCE"));
		        } else {
		            try {
		                ...
		                // la date n'est pas dans l'attribut source
		                // logger.info(feature.getAttribute("DATESOURCE")+" "+feature.getGeom());
		                try {
		                    if (feature.getAttribute("DATESOURCE") instanceof Number) {
		                        dateInteger = ((Number) feature.getAttribute("DATESOURCE")).intValue();
		                    } else {
		                        dateInteger = Integer.parseInt((String) feature.getAttribute("DATESOURCE"));
		                    }
		...
		if (dates.isEmpty()) {
            ...
            if (index<0) {
                index = fileName.length()-4; // la chaine de caractère ne contient pas "_"
            } else { index++; }
		...
		int date = dates.last();
		if (dates.size()!=1) {
			logger.warn("Plusieurs dates ont été trouvées dans le fichier "+fileName);
			logger.warn("On affecte la dernière date trouvée à tous les objets de ce fichier : "+date);
		}
		...
		FeatureType featureType = population.getFeatureType();
		for (int index = 0 ; index < featureType.sizeFeatureAttributes() ; index++){
			if (logger.isDebugEnabled()) logger.debug("Attribute "+index+" name = "+featureType.getFeatureAttributeI(index).getMemberName()+ " type = "+featureType.getFeatureAttributeI(index).getValueType());
			if (mapping.containsKey(featureType.getFeatureAttributeI(index).getMemberName())) {
				attributeIndices.add(index);
				attributeNames.add(mapping.get(featureType.getFeatureAttributeI(index).getMemberName()));
				if (logger.isDebugEnabled()) logger.debug(" --- attribute ajouté "+mapping.get(featureType.getFeatureAttributeI(index).getMemberName())+ " de type "+mappingType.get(featureType.getFeatureAttributeI(index).getMemberName()));
			}
		}

		for(IFeature feature:population) {
			ElementRepresentation elementRepresentation = factory.creerElementRepresentation(javaClassName);
			//ft_feature.setIdRep(idRep++);
			AgentGeographique geo = null;
			for (int i = 0 ; i < attributeIndices.size() ; i++) {
				Class<?> type = mappingType.get(featureType.getFeatureAttributeI(attributeIndices.get(i)).getMemberName());
				...
				if (type.equals(AgentGeographique.class)) {
					//l'attribut contient l'identifiant de l'élément Géographique
					try {
					    	Object attribute = ((DefaultFeature) feature).getAttribute(attributeIndices.get(i));
					    	//logger.info(attribute);
						int id_Geo = (attribute instanceof Number)?((Number)attribute).intValue():Integer.parseInt((String) attribute);
						geo = getAgentGeographique(id_Geo,javaClassName,geoCollection);
					} catch (NumberFormatException e) {
						geo = null;
					}
				} else {
					...
					try {
						...
						if (type.equals(int.class)) {// l'attribut est un entier
							...
							Object attributeValue = ((DefaultFeature) feature).getAttribute(attributeIndices.get(i));
							if (attributeValue instanceof Number) {
								methode.invoke(elementRepresentation, ((Number)attributeValue).intValue());
							} else methode.invoke(elementRepresentation, Integer.parseInt((String) attributeValue));
						} else if (type.equals(String.class)) {// l'attribut est une chaine de caractère
							methode.invoke(elementRepresentation, ((DefaultFeature) feature).getAttribute(attributeIndices.get(i)));
						}
						...
						logger.warn("\""+((DefaultFeature) feature).getAttribute(attributeIndices.get(i))+"\" n'est pas convertible en entier");
						e.printStackTrace();
			...
			try {
				if (feature.getGeom().isMultiSurface()) {
					if (!((GM_MultiSurface<GM_Polygon>)feature.getGeom()).isEmpty())
						elementRepresentation.setGeom(((GM_MultiSurface<GM_Polygon>)feature.getGeom()).get(0));
				} else if (feature.getGeom().isMultiCurve()) {
					if (!((GM_MultiCurve)feature.getGeom()).isEmpty())
						elementRepresentation.setGeom(((GM_MultiCurve)feature.getGeom()).get(0));
				} else elementRepresentation.setGeom(feature.getGeom());
				if (elementRepresentation.getGeom()!=null) elementRepresentation.getGeom().setCRS(srid);
			} catch (Exception e) {
