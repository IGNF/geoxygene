...
public class OsmGeometryConversion {
		...
		for (long index : way.getVertices()) {
			...
			if (vertex != null) {
				IDirectPosition pt = CRSConversion
						.changeCRS(new GM_Point(vertex.getPosition()), "4326", epsg, false, false).coord().get(0);
				coord.add(pt);
			}
		}
	...
	public static ILineString convertOSMLineToLambert93(OSMWay way, Collection<OSMResource> nodes) {
		...
		for (long index : way.getVertices()) {
			...
			if (vertex != null) {
				IDirectPosition pt = CRSConversion.wgs84ToLambert93(vertex.getLatitude(), vertex.getLongitude());
				coord.add(pt);
			}
		}
		return new GM_LineString(coord);
	...
	public static IPoint convertOsmPointToLambert93(OSMNode node) {
		IDirectPosition pt = CRSConversion.wgs84ToLambert93(node.getLatitude(), node.getLongitude());
		return pt.toGM_Point();
	}

	public IPoint convertOsmPoint(OSMNode node) throws Exception {
		IDirectPosition pt = CRSConversion.changeCRS(new GM_Point(node.getPosition()), "4326", epsg, false, false)
				.coord().get(0);
		return pt.toGM_Point();
	}
}
