...
public class Extrusion3DObject {
  ...
  private final static Logger logger = Logger.getLogger(Extrusion3DObject.class
      .getName());

  /**
   * Convertit des géométries 3D d'une dimension N en géométrie à dimension N +1
   * en les extrudant suivant un attribut hauteur pouvant être négatif
   * 
   * @param geom la geometrie qui sera extrudee
   * @param heigth la hauteur que l'on appliquera. La geometrie renvoyee sera
  ...
  public static IGeometry conversionFromGeom(IGeometry geom, double heigth) {
    ...
    if (geom instanceof IPolygon) {
      ...
      geomFinale = Extrusion3DObject.convertitFromPolygon((GM_Polygon) geom,
          heigth);
      ...
      if (multiS.size() == 1) {
        ...
        geomFinale = Extrusion3DObject.convertitFromPolygon(
            (GM_Polygon) multiS.get(0), heigth);

      } else {

        geomFinale = Extrusion3DObject
            .convertitFromMultiPolygon(multiS, heigth);

      }

    } else if (geom instanceof ILineString) {

      geomFinale = Extrusion3DObject.convertitFromLine((GM_LineString) geom,
          heigth);

    } else if (geom instanceof IMultiCurve<?>) {

      geomFinale = Extrusion3DObject.convertitFromMultiLineString(
          (GM_MultiCurve<?>) geom, heigth);

    } else if (geom instanceof IPoint) {

      geomFinale = Extrusion3DObject
          .convertitFromPoint((GM_Point) geom, heigth);
   ...
   * 
   * @param multiP le multipoint que l'on souhaite extruder
   * @param heigth la hauteur d'extrusion
   * @return l'objet renvoye l'objet lui meme si la hauteur vaut 0 ou est
   *         invalide. Sinon, il s'agit d'une extrusion verticale
   */
  private static IGeometry convertitFromMultiPoint(IMultiPoint multiP,
      double heigth) {
    ...
    if (heigth == 0 || Double.isNaN(heigth)) {
      ...
      for (int i = 0; i < nbPoints; i++) {
        multiC.add((GM_LineString) Extrusion3DObject.convertitFromPoint(
            multiP.get(i), heigth));
   ...
   * 
   * @param point le point que l'on souhaite extruder
   * @param heigth la hauteur d'extrusion
   * @return l'objet renvoye l'objet lui meme si la hauteur vaut 0 ou est
   *         invalide. Sinon, il s'agit d'une extrusion verticale
   */
  ...
  /**
   * Renvoie l'extrusion d'un GM_MultiLineString 3D suivant une hauteur hauteur.
   * Le résultat est soit un GM_MultiSurface soit un GM_MultiLineString
   * 
   * @param multiLS la geometrie que l'on souhaite extruder
   ...
   */
  @SuppressWarnings("unchecked")
  private static IGeometry convertitFromMultiLineString(IMultiCurve<?> multiLS,
      double heigth) {
    ...
    if (heigth == 0 || Double.isNaN(heigth)) {
      ...
      for (int indpoly = 0; indpoly < nbLine; indpoly++) {
        ...
        mSurface.addAll((GM_MultiSurface<GM_Polygon>) Extrusion3DObject
            .convertitFromLine(p, heigth));
      }
   ...
   * 
   * @param multiS le multipolygone 3D initial
   * @param heigth la hauteur que l'on applique pour obtenir un solide
   * @return un solide obtenu par une extrusion droite d'un hauteur hauteur
   */
  public static IGeometry convertitFromMultiPolygon(
      IMultiSurface<IOrientableSurface> multiS, double heigth) {
    // Recuperation des coordonnees du contour
    ...
    if (heigth == 0 || Double.isNaN(heigth)) {
      ...
      for (int indpoly = 0; indpoly < nbPolygon; indpoly++) {
        ...
        mSolid
            .add((GM_Solid) Extrusion3DObject.convertitFromPolygon(p, heigth));
   ...
   * 
   * @param polyIni le polygone 3D initial
   * @param heigth la hauteur que l'on applique pour obtenir un solide. Si la
  ...
  public static IGeometry convertitFromPolygon(IPolygon polyIni, double heigth) {
    ...
    // On recupere le z constant
    double zcons = 0;

    if (heigth < 0) {
      Extrusion3DObject.logger.debug(Messages
          .getString("FenetreShapeFile3D.NegativeExtrusion"));
      zcons = b.getURDP().getZ() + heigth;
    } else if (heigth > 0) {
      Extrusion3DObject.logger.debug(Messages
          .getString("FenetreShapeFile3D.PositiveExtrusion"));
      zcons = b.getURDP().getZ() + heigth;
    }
