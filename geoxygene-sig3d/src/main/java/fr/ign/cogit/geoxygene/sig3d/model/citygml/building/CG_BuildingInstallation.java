...
public class CG_BuildingInstallation extends CG_CityObject {
  ...
  public CG_BuildingInstallation(BuildingInstallation bI) {
    ...
    if (bI.isSetLod2Geometry()) {
      this.setLod2Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(bI
          .getLod2Geometry()));
    }
    if (bI.isSetLod3Geometry()) {
      this.setLod3Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(bI
          .getLod3Geometry()));
    }

    if (bI.isSetLod4Geometry()) {
      this.setLod4Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(bI
          .getLod4Geometry()));
    }
  ...
  public CityObject export() {
    ...
    BuildingInstallation bIOut = new BuildingInstallationImpl();

    if (this.isSetLod2Geometry()) {

      bIOut.setLod2Geometry(ConvertToCityGMLGeometry
          .convertGeometryProperty(this.getLod2Geometry()));

    }

    if (this.isSetLod3Geometry()) {

      bIOut.setLod3Geometry(ConvertToCityGMLGeometry
          .convertGeometryProperty(this.getLod3Geometry()));

    }

    if (this.isSetLod4Geometry()) {

      bIOut.setLod4Geometry(ConvertToCityGMLGeometry
          .convertGeometryProperty(this.getLod4Geometry()));
