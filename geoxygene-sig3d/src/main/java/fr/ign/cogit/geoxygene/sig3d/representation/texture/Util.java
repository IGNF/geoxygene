...
public class Util {
  ...
  public static GeometryInfo geometryWithTexture(IGeometry geom,
      double imageLength, double imageHeigth) {
    ...
    if (geom instanceof ISolid) {
      ...
      Util.logger.warn(Messages.getString("Representation.GeomUnk") + " : "
          + geom.getClass());
      return null;
    }
    ...
    for (int i = 0; i < nbFacet; i++) {
      ...
      for (int k = 0; k < nbContributions; k++) {
        ...
        for (int j = 0; j < n; j++) {
          ...
          texCoord[elementajoute] = new TexCoord2f(
              (float) ((vectTemp.prodScalaire(axe)) / imageLength),
