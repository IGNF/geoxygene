 ...
 *  @version 0.1
 * 
 * Classe permettant de charger de manière générique une table géométrique de
 * PostGIS Class to load postGIS Geometries
 */
public class PostgisManager {
  private final static Logger logger = Logger.getLogger(PostgisManager.class
      .getName());
   ...
   */
  public static List<String> tableWithGeom(String host, String port,
    ...
    try {
      ...
      ResultSet r = s.executeQuery("select " + PostgisManager.NAME_TABLE
          + " from " + PostgisManager.NAME_TABLE_SPATIALREF);
   ...
   * @throws Exception
   */
  public static IFeatureCollection<IFeature> loadGeometricTableWhereClause(String host,
      String port, String database, String user, String pw, String table,String whereClause)
      throws Exception {
      ...
      while (r1.next()) {
        ...
        if (nomField.equals(nomColonneGeom)) {
          ...
          String valueType = PostgisManager.fromSQLTypeToJava(r2.getMetaData()
              .getColumnType(compt + shiftIni)); // on invite le
                                                 // asewkt,
          // les indexes
          // commencent à 1
          ...
          attLookup.put(new Integer(compt),
              new String[] { nomField, memberName });
   ...
   * @return les entités de la table avec les attributs renseignés. Null renvoyé
   *         pour table non-géométriques
   */
  public static IFeatureCollection<IFeature> loadGeometricTable(String host,
   ...
   * considère que toutes les entités utilisent le même schéma geo (nous prenons
   * le schéma de la première entité).
   * 
   * @param host hote (localhost accepté)
   * @param port port d'écoute
   * @param database nom de la pase de données
   ...
   * @param pw mot de passe
   * @param table le nom de la table que l'on souhaite charger
   * @param featColl la collection que l'on souhaite sauver
   * @param erase indique si l'on souhaite écraser la table (si elle existe)
   * @return indique si l'opération s'est déroulée avec succès
   * @throws Exception
   */
  public static boolean saveGeometricTable(String host, String port,
      String database, String user, String pw, String table,
      IFeatureCollection<IFeature> featColl, boolean erase) throws Exception {
    ...
    try {
      ...
      ResultSet r = s
          .executeQuery("SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='"
              + table.toLowerCase() + "'");
      ...
      if (r.next()) {
        ...
        if (erase) {
          ...
          PostgisManager.logger.debug(Messages.getString("Sauvegarde") + " : "
              + sqlSupp);
          s.execute(sqlSupp);
          // On supprime la colonne géométrique
          String sqlSuppGeom = "delete  from geometry_columns where f_table_name = '"
              + table + "';";
          PostgisManager.logger.debug(Messages
              .getString("Sauvegarde.DeleteGeomColumn") + " : " + sqlSuppGeom);
      ...
      PostgisManager.logger.debug(Messages.getString("Sauvegarde.CreateTable")
          + " : " + sqlCreat);
      s.execute(sqlCreat);
      ...
      + PostgisManager.NAME_COLUMN_GEOM + "', " + CRS
      + ", 'GEOMETRY', " + ((GM_Object) the_geom).coordinateDimension() + " )";
      ...
      if (lAtt != null) {
        ...
        for (int i = 0; i < nbAttribut; i++) {
          ...
          PostgisManager.logger.info(Messages.getString("Sauvegarde.AddColum")
              + " : " + sql);
   ...
   * 
   * @param sqlType l'entier représentant le type SQL voire classe
   *          java.sql.Types
  ...
  /**
   * Convertit un type GeOxygene (String, Double, Integer, Boolean) en type SQL
   * par des constantes de type String
   * 
   * @param javaType le type GeOxygene
  ...
  public static int fromJavaTypeToSQL(String javaType) {
    ...
    } else if (javaType.equalsIgnoreCase(PostgisManager.GE_DOUBLE)
