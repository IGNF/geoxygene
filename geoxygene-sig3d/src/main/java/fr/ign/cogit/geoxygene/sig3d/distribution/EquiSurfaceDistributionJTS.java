...
public class EquiSurfaceDistributionJTS {
  ...
  public EquiSurfaceDistributionJTS(IGeometry geom) throws Exception {
    ...
    for(IOrientableSurface surf : lTriangles){
    	this.lTriangles.add(AdapterFactory.toGeometry(gf,
					surf));
    }
  ...
  private DirectPosition randomPointOnTriangles(Geometry triangle,
    ...
    p1.y * (1 - sqrtAleaX) + p2.y * sqrtAleaX * (1 - aleaY)
        + p3.y * aleaY * sqrtAleaX, p1.z * (1 - sqrtAleaX)
        + p2.z * sqrtAleaX * (1 - aleaY) + p3.z * aleaY * sqrtAleaX);
  ...
  public IDirectPosition inversample(double x, double y) {
    ...
    if (tOut == null) {
	 ...
	 Coordinate[] coord =  tOut.getCoordinates();
    ...
    double val2 = (E1 * (x - p1.x) - E0 * (y - p1.y))
        / (F0 * E1 - F1 * E0);
