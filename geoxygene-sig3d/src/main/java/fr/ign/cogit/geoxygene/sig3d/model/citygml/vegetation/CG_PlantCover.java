...
public class CG_PlantCover extends CG_AbstractVegetationObject {
  ...
  public CG_PlantCover(PlantCover pC) {
    ...
    if (pC.isSetLod1MultiSurface()) {
      this.setLod1MultiSurface(ConvertyCityGMLGeometry
          .convertGMLMultiSurface(pC.getLod1MultiSurface()));
    }

    if (pC.isSetLod2MultiSurface()) {
      this.setLod2MultiSurface(ConvertyCityGMLGeometry
          .convertGMLMultiSurface(pC.getLod2MultiSurface()));
    }

    if (pC.isSetLod3MultiSurface()) {
      this.setLod3MultiSurface(ConvertyCityGMLGeometry
          .convertGMLMultiSurface(pC.getLod3MultiSurface()));
    }

    if (pC.isSetLod4MultiSurface()) {
      this.setLod4MultiSurface(ConvertyCityGMLGeometry
          .convertGMLMultiSurface(pC.getLod4MultiSurface()));
    }

    if (pC.isSetLod1MultiSolid()) {
      this.setLod1MultiSolid(ConvertyCityGMLGeometry.convertGMLMultiSolid(pC
          .getLod1MultiSolid().getMultiSolid()));
    }

    if (pC.isSetLod2MultiSolid()) {
      this.setLod2MultiSolid(ConvertyCityGMLGeometry.convertGMLMultiSolid(pC
          .getLod2MultiSolid().getMultiSolid()));
    }

    if (pC.isSetLod3MultiSolid()) {
      this.setLod3MultiSolid(ConvertyCityGMLGeometry.convertGMLMultiSolid(pC
          .getLod3MultiSolid().getMultiSolid()));
    }
   ...
   * 
   * @param value allowed object is {@link IMultiSolid<ISolid> }
   * 
   */
