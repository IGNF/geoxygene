...
public class CG_GenericCityObject extends CG_CityObject {
  ...
  public CG_GenericCityObject(GenericCityObject gCO) {
    ...
    if (gCO.isSetLod0Geometry()) {
      ...
      this.lod0Geometry = ConvertyCityGMLGeometry.convertGMLGeometry(gCO
          .getLod0Geometry());
    }

    if (gCO.isSetLod1Geometry()) {

      this.lod1Geometry = ConvertyCityGMLGeometry.convertGMLGeometry(gCO
          .getLod1Geometry());
    }

    if (gCO.isSetLod2Geometry()) {

      this.lod2Geometry = ConvertyCityGMLGeometry.convertGMLGeometry(gCO
          .getLod2Geometry());
    }

    if (gCO.isSetLod3Geometry()) {

      this.lod3Geometry = ConvertyCityGMLGeometry.convertGMLGeometry(gCO
          .getLod3Geometry());
    }

    if (gCO.isSetLod4Geometry()) {

      this.lod4Geometry = ConvertyCityGMLGeometry.convertGMLGeometry(gCO
          .getLod4Geometry());
    }

    if (gCO.isSetLod0ImplicitRepresentation()) {
      System.out
          .println("Non gestion des géométries implicites : CG_GenericCityObject");
    }
