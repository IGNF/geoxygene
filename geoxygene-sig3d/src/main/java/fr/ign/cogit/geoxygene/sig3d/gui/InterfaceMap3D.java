...
public class InterfaceMap3D extends JPanel {
  ...
  private final static Logger logger = Logger.getLogger(InterfaceMap3D.class
      .getName());
   ...
   * @param mainWindow La fenetre dans laquelle s'affiche l'interface
   */
  public InterfaceMap3D(int width, int height, MainWindow mainWindow) {
    ...
    if (ConstantRepresentation.visualisingFPS) {
      ...
      BoundingSphere boundes = new BoundingSphere(origin,
    ...
    this.pickCanvas = new PickCanvas(this.canvas3D.getOffscreenCanvas3D(),
        this.scene);
   ...
   * 
   * @param transGroup le TransformGroup qui sera mis à jour avec les
   *          déplacements
   * @return
   */
  private BranchGroup interscene(TransformGroup transGroup) {
    ...
    echelleZ.setScale(new Vector3d(1.0, 1.0,
   ...
   * 
   * @param layer la couche que l'on souhaite ajouter
   */
  public void addLayerInterface(Layer layer) {
    ...
    if (this.translate == null) {
    ...
    } else if (this.map.getLayerList().size() == 0) {
      ...
      this.initViewpoint(5);
      this.moveLight(b.getURDP().getX() * 2, b.getURDP().getY() * 2, b
          .getURDP().getZ() * 2, 0);
    }

    AbstractTableModel abstm = (AbstractTableModel) this.mainWindow
        .getContentPanel().getLayersListTable().getModel();
   ...
   * 
   * @param axis Un entier correspondant à l'axe
   ...
   * @param y
   * @param z
   * @param direction Il s'agit de la direction dans laquelle est regardée le
  ...
  public void zoomOn(double x, double y, double z, Vecteur direction) {
    ...
    Point3d eyePos = new Point3d(x + this.translate.x + direction.getX(), y
        + this.translate.y + direction.getY(), z + this.translate.z
        + direction.getZ());
   ...
   * 
   * @param x nouvelle coordonnées x
   * @param y nouvelle coordonnées y
   * @param z nouvelle coordonnées z
   * @param ind l'index de la lumière que l'on déplace
   ...
   * 
   * @param feat l'entité que l'on souhaite sélectionner
   */
  ...
  public void setSelection(IFeatureCollection<IFeature> feats) {
    ...
    if (nbelem != 0) {
      ...
      for (int i = 0; i < nbelem; i++) {
        ...
        if (rep == null) {
          InterfaceMap3D.logger.warn(Messages
              .getString("Representation.RepNulle"));
          continue;
        }
        ...
        if (rep instanceof I3DRepresentation) {
          ...
          InterfaceMap3D.logger.warn(Messages
              .getString("Representation.RepUnk"));
        }
