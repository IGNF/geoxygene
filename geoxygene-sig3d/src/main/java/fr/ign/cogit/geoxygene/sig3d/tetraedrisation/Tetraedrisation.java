 ...
 * @version 0.1
 * 
 * Classe permettant de gérer la décomposition de solides en tétraèdres ou en
 * triangles. Cette classe fait appel à la bibliothèque TetGen via JNI
 * http://tetgen.berlios.de/ Pour procéder à une tetraedrisation, il faut 1)
 * créer un objet de type tétraèdrision 2) faire appel à la méthode tetraedrise,
 * tetraedriseContrainte ou tetraedriseNonContrainte Classe for solid
 * decompositions using TetGen library : http://tetgen.berlios.de/
 * 
 */

public class Tetraedrisation {

  private final static Logger logger = Logger.getLogger(Tetraedrisation.class
      .getName());

  protected List<GM_Solid> tetraedres = new ArrayList<GM_Solid>();
  protected List<IOrientableSurface> triangles = new ArrayList<IOrientableSurface>();
  ...
  /**
   * @return renvoie les triangles générés par la décomposition (si ils
   *         existent)
   */
  public List<IOrientableSurface> getTriangles() {
   ...
   * Constructeur
   * 
   * @param feat l'entité que l'on souhaite décomposer (dimension 3 obligatoire)
   */
   ...
   * 
   * @param sol le solide que l'on souhaite décomposer
   */
   ...
   * tétraèdrisation contrainte ne fonctionne pas Il faut absolument utiliser
   * des triangles
   * 
   * @param constraint indique si l'on fait une décomposition contrainte par les
   *          faces du solide ou nom (le résultat sera la forme convexe de
   *          l'objet initial)
   */
   ...
   * 
   * @param options les options d'appel à la fonction (voir documentation
   *          http://tetgen.berlios.de/)
   * @param jin l'objet paramètre décrivant l'objet à décomposer
   * @param jout l'objet en sortie décrivant l'objet décomposé
   */
   ...
   * 
   * @param constraint indique si l'on contraint la tétraèdrisation
    ...
    try {
      ...
      for (int i = 0; i < this.jTetgenioOut.numberofpoints; i++) {
        DirectPosition dp = new DirectPosition(
            this.jTetgenioOut.pointlist[3 * i],
      ...
      for (int i = 0; i < this.jTetgenioOut.numberoftrifaces; i++) {
        ...
        IDirectPosition n1 = lPoints
            .get(this.jTetgenioOut.trifacelist[3 * i + 1]);
        IDirectPosition n2 = lPoints
            .get(this.jTetgenioOut.trifacelist[3 * i + 2]);
        ...
        GM_Ring tri = new GM_Ring(ls.getNegative());

        this.triangles.add(new GM_Polygon(tri));
      }
      ...
      if (!mesh) {
        for (int i = 0; i < this.jTetgenioOut.numberoftetrahedra; i++) {
          ...
          IDirectPosition n0 = lPoints
              .get(this.jTetgenioOut.tetrahedronlist[4 * i]);
          IDirectPosition n1 = lPoints
              .get(this.jTetgenioOut.tetrahedronlist[4 * i + 1]);
          IDirectPosition n2 = lPoints
              .get(this.jTetgenioOut.tetrahedronlist[4 * i + 2]);
          IDirectPosition n3 = lPoints
              .get(this.jTetgenioOut.tetrahedronlist[4 * i + 3]);
          ...
          ArrayList<IOrientableSurface> lTriangles = new ArrayList<IOrientableSurface>(
              4);
          lTriangles.add(tri1);
          lTriangles.add(tri2);
          lTriangles.add(tri3);
