...
public abstract class CG_AbstractReliefComponent extends CG_CityObject {
  ...
  public CG_AbstractReliefComponent(ReliefComponent rC) {
    super(rC);
    this.setLod(rC.getLod());

    if (rC.isSetExtent()) {
      GM_Polygon pol = ConvertyCityGMLGeometry.convertGMLPolygon(rC.getExtent()
          .getPolygon());
  ...
  public static CG_AbstractReliefComponent generateReliefComponentType(
      ReliefComponent rC) {
    ...
    if (rC instanceof TINRelief) {
      ...
      System.out.println("Classe non gérée :"
          + rC.getClass().getCanonicalName());
    }
   ...
   * 
   * @param value allowed object is {@link GM_Envelope }
   * 
   */
