 ...
 * 
 *
 *          Classe permettant de définir des équations de plan de calculer
 *          l'equation d'un plan ou la véritable équation pour un polygone. Il
 *          ne s'agit pas comme dans la classe EquationPlan de déterminer une
 *          équation a partir des 3 premiers points
 * 
 *          Cette équation donne le plan optimale les points ne sont pas
 *          coplanaires Class to dertermine plan equation
 * 
 */
public class ApproximatedPlanEquation extends PlanEquation {
	 ...
	 * 
	 * @param dpl
	 *            liste de points pour construire un plan
	 */
	public ApproximatedPlanEquation(IDirectPositionList dpl) {
		...
		for (int i = 1; i < nbPoints; i++) {
			...
			this.coeffa = this.coeffa
					+ ((dpAct.getY() - yini) - (dpSuiv.getY() - yini)) * ((zact - zini) + (zsuiv - zini));
			this.coeffb = this.coeffb
					+ ((zact - zini) - (zsuiv - zini)) * ((dpAct.getX() - xini) + (dpSuiv.getX() - xini));
			this.coeffc = this.coeffc + ((dpAct.getX() - xini) - (dpSuiv.getX() - xini))
		...
		/*
		 * if (Math.abs(coeffa) < 0.01) { coeffa = 0; } if (Math.abs(coeffb) <
		 * 0.01) { coeffb = 0; } if (Math.abs(coeffc) < 0.01) { coeffc = 0; }
		 */
		...
		this.equation = new String(
				this.coeffa + "x +" + this.coeffb + "y +" + this.coeffc + "z +" + this.coeffd + " = 0");
	}
	 ...
	 * 
	 * @param oS
	 *            la surface dont on veut calculer l'équation
	 */
