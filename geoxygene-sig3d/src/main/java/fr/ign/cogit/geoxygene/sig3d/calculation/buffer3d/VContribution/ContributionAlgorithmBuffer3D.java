 ...
 * contributions Cet algorithme est décrit dans Bakri[2009] L'algorithme permet
 * l'affichage dans une carte 3D Ne fonctionne qu'avec des solides
 * mathématiquement justes TODO : Etendre la fonction pour des objets ponctuels,
 * linéaires et surfaciques TODO : Nettoyer la géométrie finale Barki H., Denis
 * F, Dupont Contributing F., Vertices-based Minkowski sum of a non-convex
 * polyhedron without fold and a convex polyhedron. . Dans IEEE International
 * Conference on Shape Modeling and Applications (SMI), IEEE Computer Society
 * Press ed. Beijing, China. 2009 A visual 3D buffer algorithm for solids
 * 
 */
public class ContributionAlgorithmBuffer3D {

  private final static Logger logger = Logger
      .getLogger(ContributionAlgorithmBuffer3D.class.getName());
   ...
   * 
   * @param feat l'entité qui se verra appliqué un buffer
   * @param val la largeur du buffer
   * @param detail le détail des sphères permettant le calcul
   * @param map3D la carte qui sera utilisée pour récupérer le réulstat
   */
  public static void offsetting(IFeature feat, double val, int detail,
    ...
    MinkowskiSum sum = new MinkowskiSum(feat, new DirectPosition(0, 0, 0), val,
        detail);

    ContributionAlgorithmBuffer3D.logger.info("Time :"
    ...
    ContributionAlgorithmBuffer3D.addRelations(sum.lRelationFaceVertex,
        ContributionAlgorithmBuffer3D.C_FACE_VERTEX, map3D);
    ContributionAlgorithmBuffer3D.addLTriangles(sum.lRelationEdgeFace,
        ContributionAlgorithmBuffer3D.C_EDGE_FACE, map3D);
    ContributionAlgorithmBuffer3D.addLTriangles(sum.lRelationVertexFace,
        ContributionAlgorithmBuffer3D.C_VERTEX_FACE, map3D);
   ...
   * @return
   */
  private static List<IOrientableSurface> addLTriangles(
      List<Triangle> lTriangles, String name, Map3D map3D) {

    int nbElements = lTriangles.size();

    ArrayList<IOrientableSurface> lOS2 = new ArrayList<IOrientableSurface>(
        nbElements);
    ...
    if (map3D.getLayer(name) != null) {

      map3D.removeLayer(name);
    ...
    map3D.addLayer(new VectorLayer(ftcoll, name, true, c, 1, true));
  ...
  /**
   * Permet d'ajouter une relation FV à la carte Translate le triangle de cette
   * relation suivant le point
   * 
   * @param rFV1
   * @param name
   * @param map3D
  ...
  private static List<GM_OrientableSurface> addRelations(List<RelationFV> rFV1,
      String name, Map3D map3D) {
    ...
    for (int i = 0; i < nbElem1; i++) {
      ...
      for (int j = 0; j < nbsomest; j++) {
        ...
        ftColl1.add(new DefaultFeature((tTemp.translateTriangle(rel
            .getLVertex().get(j))).toGeoxygeneSurface()));
    ...
    Color c = new Color((int) (Math.random() * 255),
        (int) (Math.random() * 255), (int) (Math.random() * 255));
    // On ajoute les triangles à la carte
    map3D.addLayer(new VectorLayer(ftColl1, name, true, c, 1, true));
