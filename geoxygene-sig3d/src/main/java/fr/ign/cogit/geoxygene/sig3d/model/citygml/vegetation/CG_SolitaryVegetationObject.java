...
public class CG_SolitaryVegetationObject extends CG_AbstractVegetationObject {
  ...
  public CG_SolitaryVegetationObject(SolitaryVegetationObject sV) {
    ...
    if (sV.isSetLod1Geometry()) {
      this.setLod1Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(sV
          .getLod1Geometry()));
    }

    if (sV.isSetLod2Geometry()) {
      this.setLod2Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(sV
          .getLod2Geometry()));
    }

    if (sV.isSetLod3Geometry()) {
      this.setLod3Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(sV
          .getLod3Geometry()));
    }

    if (sV.isSetLod4Geometry()) {
      this.setLod4Geometry(ConvertyCityGMLGeometry.convertGMLGeometry(sV
          .getLod4Geometry()));
    }

    if (sV.isSetLod1ImplicitRepresentation()) {

      this.setLod1ImplicitRepresentation(ConvertyCityGMLGeometry
          .convertGMLGeometry(sV.getLod1ImplicitRepresentation()
              .getImplicitGeometry().getRelativeGMLGeometry().getGeometry()));

    }

    if (sV.isSetLod2ImplicitRepresentation()) {
      this.setLod2ImplicitRepresentation(ConvertyCityGMLGeometry
          .convertGMLGeometry(sV.getLod2ImplicitRepresentation()
              .getImplicitGeometry().getRelativeGMLGeometry().getGeometry()));

    }

    if (sV.isSetLod3ImplicitRepresentation()) {
      this.setLod3ImplicitRepresentation(ConvertyCityGMLGeometry
          .convertGMLGeometry(sV.getLod3ImplicitRepresentation()
              .getImplicitGeometry().getRelativeGMLGeometry().getGeometry()));

    }

    if (sV.isSetLod4ImplicitRepresentation()) {
      this.setLod4ImplicitRepresentation(ConvertyCityGMLGeometry
          .convertGMLGeometry(sV.getLod4ImplicitRepresentation()
              .getImplicitGeometry().getRelativeGMLGeometry().getGeometry()));
