...
public class AlphaShape {
  ...
  public static GM_Solid generateSol(IDirectPositionList dpl) {
     ...
     * 100; i++) { double teta = 2 * Math.PI * Math.random(); double phi = 2 *
     * Math.PI * Math.random(); DirectPosition dp = new
  ...
  private static IDirectPosition determineSphereCenter(Triangle tri,
      double rayon) {
    return AlphaShape.determineSphereCenter(tri.getCorners(0).getDirect(), tri
        .getCorners(1).getDirect(), tri.getCorners(2).getDirect(), rayon);
  }

  /**
   * On calcul le centre d'une sphère passant par 3 points et de rayon rayon. Ce
   * calcul est obtenu grâce à l'intersection entre les sphères passant par
   ...
   */
  private static IDirectPosition determineSphereCenter(IDirectPosition dp1,
      IDirectPosition dp2, IDirectPosition dp3, double rayon) {
    ...
    /*
     * double dist1 = Math.sqrt((x1 - xFinal) * (x1 - xFinal) + (y1 - yFinal)
     * (y1 - yFinal) + (z1 - zFinal) * (z1 - zFinal)); double dist2 =
     * Math.sqrt((x2 - xFinal) * (x2 - xFinal) + (y2 - yFinal) (y2 - yFinal) +
     * (z2 - zFinal) * (z2 - zFinal)); double dist3 = Math.sqrt((x3 - xFinal) *
     * (x3 - xFinal) + (y3 - yFinal) (y3 - yFinal) + (z3 - zFinal) * (z3 -
     * zFinal)); System.out.println("Distance 1 " + dist1 + " Distance 2 " +
     * dist2 + " Distance 3 " + dist3);
     */

    return pt0;
  }

  public static FT_FeatureCollection<IFeature> processAlphaShape(
    ...
    for (int i = 0; i < nbTetra; i++) {
      ...
      for (int j = 0; j < 4; j++) {
        ...
        IDirectPosition dp = AlphaShape.determineSphereCenter(tri,
            AlphaShape.alpha);
        ...
        if (dp != null) {
          ...
          if (!AlphaShape.isNearerThan(dpl, dp, AlphaShape.alpha - 0.5)) {
            ...
            featTemp.setRepresentation(new ObjectCartoon(featTemp));

            ftFeat.add(featTemp);
            /*
             * //Affiche quelques sphères if (i < 50) { featTemp = new
             * DefaultFeature(new Sphere(dp, alpha, 10));
             * featTemp.setRepresentation(new ObjectCartoon( featTemp,
             * Color.red)); ftFeat.add(featTemp); }
             */
  ...
  private static boolean isNearerThan(IDirectPositionList dpl,
      IDirectPosition dp, double distance) {
