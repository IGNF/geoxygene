...
public class TestTetra {
  ...
  public void processTest() {
    ...
    Tetraedrisation tet = new Tetraedrisation(feat);
    tet.tetraedrise(false, true);

    int nbTri = tet.getTriangles().size();
    System.out.println("Nombre de triangles : " + nbTri);

    Assert.assertEquals(12, nbTri);

    tet = new Tetraedrisation(new DefaultFeature(new GM_Solid(
        tet.getTriangles())));
    tet.tetraedrise(true, false);

    int nbTetra = tet.getTetraedres().size();

    System.out.println("Nombre de tétraèdres : " + nbTetra);
  ...
  private static IFeature createCube(double x0, double y0, double z0,
