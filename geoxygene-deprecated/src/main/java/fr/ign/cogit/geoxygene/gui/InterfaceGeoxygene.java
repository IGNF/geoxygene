...
public class InterfaceGeoxygene extends JFrame {
    ...
    public InterfaceGeoxygene() {
        ...
        JSplitPane splitPaneGauche = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, this.getPanelVisu(), this.getPanelDroit());
    ...
    public void chargeShapefiles() {
        FrameChargement chargeur = new FrameChargement(this.getPanelVisu().getDefaultSld());
            ...
            for (int i = 0; i < shapeFiles.size(); i++) {
                ...
                if (shapeFileLayers.get(i) == 0) {
                    ...
                    if (this.getPanelVisu().getDataset().getPopulation(populationName) != null) {
                        ...
                        int n = 2;
                        while (this.getPanelVisu().getDataset().getPopulation(populationName + " (" + n + ")") != null) {
                            n++;
                        }
    ...
    public void chargeShapefile(String shapefileName, String populationName) {
         ...
         */
        ShapefileReader reader = new ShapefileReader(shapefileName, populationName, this.getPanelVisu().getDataset(), true);
        /**
         * Ajoute le panel de visualisation au listeners de la population. Il
         * sera
        ...
        /**
         * Ajoute un layer au SLD du panel de visualisation. On essaye de
         * récupérer
         * les styles existants depuis le sld courant ou depuis le sld par
         * défaut
         */
        Layer layer = this.getPanelVisu().getSld().getLayer(populationName);
        /**
         * On n'a pas trouvé la couche dans le SLD courant, on cherche dans le
         * sld
         * par défaut
         */
        if (layer == null) {
            if (InterfaceGeoxygene.logger.isDebugEnabled()) {
                InterfaceGeoxygene.logger.debug("Layer " + populationName + " non trouvé dans le sld courant");
            }
            ...
            if (layer == null) {
                if (InterfaceGeoxygene.logger.isDebugEnabled()) {
                    InterfaceGeoxygene.logger.debug("Layer " + populationName + " non trouvé dans le sld par défaut");
                }
                LayerFactory factory = new LayerFactory(this.getPanelVisu().getSld());
                layer = factory.createLayer(populationName, reader.getPopulation().getFeatureType().getGeometryType());
            }
    ...
    public void addPopulation(Population<? extends FT_Feature> population, String populationName, Color color) {
        if (population.isEmpty()) {
            InterfaceGeoxygene.logger.error("Aucun élément à afficher : collection vide");
            return;
        }
        ...
        Layer layer = this.getPanelVisu().getSld().getLayer(populationName);
        /**
         * On n'a pas trouvé la couche dans le SLD courant, on cherche dans
         * le
         * sld
         * par défaut
         */
        if (layer == null) {
            synchronized (this.getPanelVisu().getSld().getLayers()) {
                if (InterfaceGeoxygene.logger.isDebugEnabled()) {
                    InterfaceGeoxygene.logger.debug("Layer " + populationName + " non trouvé dans le sld courant");
                }
                layer = this.getPanelVisu().getDefaultSld().getLayer(populationName);
                /**
                 * On n'a pas trouvé la couche dans le SLD, on en crée une
                 * nouvelle
                 */
                if (layer == null) {
                    if (InterfaceGeoxygene.logger.isDebugEnabled()) {
                    ...
                    if (population.getFeatureType() == null) {
                        layer = this.getPanelVisu().getSld().createLayer(populationName, population.get(0).getGeom().getClass(), color);
                    } else {
                        layer = this.getPanelVisu().getSld().createLayer(populationName, population.getFeatureType().getGeometryType(), color);
                    }
                }
                this.getPanelVisu().getSld().getLayers().add(layer);
            }
