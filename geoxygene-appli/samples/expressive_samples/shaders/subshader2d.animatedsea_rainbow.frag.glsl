...
vec4 computeAnimatedRainbowColor( DataGradient fragmentData ) {
    float dmax = 4000;
    float distance = fragmentData.worldUV.y * fragmentData.worldUVRange.y;
    float dd = 1.0 - smoothstep( 0, dmax, distance );
    float sec = mod( dd - time / 10000.0, 1.0 );
    return vec4( hsv2rgb( vec3(sec * 3.141592654357, dd, dd) ) * (dd) + (1-dd) * vec3(1.0,1.0,1.0) , 1.0 );
}


vec4 computeColor( DataGradient fragmentData ) {
    return computeAnimatedRainbowColor( fragmentData );
}
