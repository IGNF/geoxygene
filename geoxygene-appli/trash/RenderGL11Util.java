...
public final class RenderGL11Util {
    ...
    /**
     * Entry point for drawing anything. This method dispatches to other paint
     * methods depending on the Symbolizer class type.
    ...
    public static void paint(final LineSymbolizer symbolizer, final IFeature feature, final Viewport viewport, final double opacity) {
        ...
        if (symbolizer.getStroke().getGraphicType() == null) {
            ...
            if (shapes != null) {
                if (symbolizer.getColorMap() != null) {
                    try {
                        Color color = getColorWithOpacity(
                                new Color(symbolizer.getColorMap().getColor(
                                        Double.parseDouble(feature.getAttribute(symbolizer.getColorMap().getPropertyName()).toString()))), opacity);
    ...
    private static void graphicStrokeLineString(final LineSymbolizer symbolizer, final Shape shape, final Image image, final float size, final double opacity) {
    ...
    /**
     * Outline a MultiSurface Object by looping over all Orientable surfaces of
     * the list and outlining them
     * 
     * @param symbolizer
     * @param multiSurface
     * @param viewport
    ...
    private static void paint(final LinePlacement linePlacement, final String text, final Font font, final Color fillColor, final Color haloColor,
        ...
        logger.warn("Replace by a GL function");
        Stroke s = new TextStroke(text, font, false, linePlacement.isRepeated(), false, linePlacement.getInitialGap() * (float) scale, linePlacement.getGap()
                * (float) scale);
     ...
     * @throws NoninvertibleTransformException
     */
    private static void paint(final PointPlacement pointPlacement, final String text, final Color fillColor, final Color haloColor, final float haloRadius,
            final IDirectPosition position, final Viewport viewport, final double scale) throws NoninvertibleTransformException {
        logger.warn("Replace by a GL function");
     ...
     * 
     * @param outerShape
     *            Simple polyline describing the outer limit of the polygon
     * @param innerShapes
     *            list of simple polylines describing the inner limits of the
     *            polygon
     */
     ...
     * colors depending on segment types.
     ...
     * 
     * @param shape
     *            shape to extract the collection of points
     * @return point list
     *         TODO: if there is a MOVE_TO in the middle of the shape, we should
     *         return a list of list of points instead of a simple list of
     *         points...
     */
