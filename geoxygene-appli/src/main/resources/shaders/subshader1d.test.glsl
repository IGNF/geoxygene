...
/************************************************************************************/
vec4 computeFragmentColorRainbow( in vec4 brushColor, in vec4 paperColor, in DataPainting fragmentData ) {
...
vec4 computeFragmentColorCurvature( in vec4 brushColor, in vec4 paperColor, in DataPainting fragmentData ) {
	float curv = fragmentData.curvature * 3.0;
	vec3 col = vec3( curv, 0.0, 0.0 );
	if ( curv < 0.0 ) {
		col = vec3( 0.0, 0.0, -curv );
	}
	return vec4( col, 1.0 );
}
