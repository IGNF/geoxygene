...
public class OSMPlugin
    ...
    public void initialize(GeOxygeneApplication application) {
        ...
        for (OSMFile recentDoc : recentFiles) {
            menuRecent.add(
                    new JMenuItem(new ImportOSMNamedFileAction(recentDoc)));
        }
        menu.add(menuRecent);
        menu.addSeparator();
    ...
    public class ImportOSMFileAction extends AbstractAction
        ...
        public void actionPerformed(ActionEvent arg0) {
            ImportOsmFileFrame dialogImportOsmFrame = new ImportOsmFileFrame(
                    this);
            if (dialogImportOsmFrame.getAction().equals("OK")) {
                ...
                String name = null;
                if (doc.getName() == null) {
                    name = osmFile.getName().substring(0,
                            osmFile.getName().length() - 4);
                    doc.setName(name);
                ...
                application.getMainFrame().getGui().setCursor(
        ...
        class ImportOsmFileFrame extends JDialog implements ActionListener {
            ...
            public ImportOsmFileFrame(ImportOSMFileAction importPlugin) {
                ...
                setIconImage(new ImageIcon(GeOxygeneApplication.class
                        .getResource("/images/icons/map_add.png")).getImage());
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if (source == cancelBtn) {
                    ...
                    txtEpsg.setText(currentProjections
                            .get(usedCombo.getSelectedItem()));
                    ...
                    fc.setCurrentDirectory(new File(
                            application.getProperties().getLastOpenedFile()));
            ...
            private void initPanel() {
                ...
                currentProjections.put("Gauss-Krueger zone 4 (East Germany)",
                        "31468");
                ...
                browseBtn = new JButton(new ImageIcon(this.getClass()
                        .getResource("/images/icons/magnifier.png")));
                ...
                usedCombo = new JComboBox(
                        currentProjections.keySet().toArray());
    ...
    class BrowseTagsAction extends AbstractAction {
        ...
        */
        ...
        @Override
        public void actionPerformed(ActionEvent arg0) {
            int x = (int) CartagenApplication.getInstance().getFrame()
                    .getVisuPanel().getBounds().getCenterX();
            int y = (int) CartagenApplication.getInstance().getFrame()
                    .getVisuPanel().getBounds().getCenterY();
                        ...
                        for (int i = 0; i < ((IMultiSurface<IOrientableSurface>) union)
                            ...
                            if (i == 0) {
                                ...
                                OsmBuilding newBuilding = new OsmBuilding(
                                        remainingB.getContributor(), newGeom,
                                        remainingB.getId(),
                                        remainingB.getChangeSet(),
                                        remainingB.getVersion(),
                                        remainingB.getUid(),
                                        remainingB.getDate());
                                pop.add(newBuilding);
                            }
        ...
        */
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (ISimpleLandUseArea parcel : CartAGenDoc.getInstance()
                ...
                Collection<IRailwayLine> inside = railways
                        .select(parcel.getGeom());
                if (inside == null)
                    continue;
            ...
            if (e.getActionCommand().equals("cancel")) {
                ...
                for (IGeneObj obj : iterable) {
                    ...
                    try {
                        ...
                        // map
                        section.setGeom(
                                section.getCorrespondants().get(0).getGeom());
                        ...
                        constr = classObj.getConstructor(String.class,
                                IGeometry.class, int.class, int.class,
                                int.class, int.class, Date.class);

                        for (int i = 1; i < section.getCorrespondants()
                                .size(); i++) {
                            ILineString newLine = (ILineString) section
                                    .getCorrespondants().get(i).getGeom();
                            OsmNetworkSection newObj = (OsmNetworkSection) constr
                                    .newInstance(section.getContributor(),
                                            newLine, section.getId(),
                                            section.getChangeSet(),
                                            section.getVersion(),
                                            section.getUid(),
                                            section.getDate());
    ...
    class LodSensitivityAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (Combination comb : combSet
                    .getAllCombinationsOfNOrMoreElements(4)) {
                ...
                dataset.incrementValue(1.0, "Number of inferences",
                        LoDCategory.valueOf(decision.getCategory()).ordinal()
                                + 1);
            }
            JFreeChart chart = ChartFactory.createBarChart(
                    "LoD Inference Sensitivity", "LoD Category",
                    "Number of inferences", dataset, PlotOrientation.VERTICAL,
                    false, false, false);
    ...
    class HarmonisationFrameAction extends AbstractAction {
        ...
        */
        ...
        public HarmonisationFrameAction() {
            this.putValue(Action.SHORT_DESCRIPTION,
                    "Launch the harmonisation frame");
            ...
            try {
                ...
                for (OsmGeneObj obj : selectedObjs) {
                    ...
                    for (Criterion crit : electreMethod.getCriteria()) {
                        Map<String, Object> param = LoDMultiCriteria
                                .initParameters(obj, crit);
                        valeursCourantes.put(crit.getName(),
                                new Double(crit.value(param)));
                    }
                    ClassificationResult decision = electreMethod.decision(
                            electreMethod.getCriteria(), valeursCourantes,
                            conclusion);
                    txtArea.append(obj.getClass().getSimpleName() + ":"
                            + obj.toString() + ": "
                            + decision.getCategory().toString()
                            + " (robustness="
    ...
    public void addOsmDatabaseToFrame() throws JAXBException {
        ...
        CartAGenPlugin.getInstance().getMapDbFrame().put(database.getName(),
                frame);
        frame.getSld().setDataSet(database.getDataSet());
        frame.getLayerViewPanel().getRenderingManager()
                .setHandlingDeletion(true);
            ...
            if (frame.getSld().getLayer(populationName) == null) {
                ...
                if (defaultSld.getLayer(populationName) != null) {
                    ...
                    fts.getRules().add(LayerFactory.createRule(
                            geoClass.getGeometryType(), fillColor.darker(),
        ...
        */
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (Layer layer : sld.getLayers()) {
                ...
                GF_FeatureType ft = layer.getFeatureCollection()
                        .getFeatureType();
                // add the default metadata as attribute
                ft.addFeatureAttribute(new AttributeType("osmId", "Long"));
                ft.addFeatureAttribute(
                        new AttributeType("contributor", "String"));
                ft.addFeatureAttribute(
                        new AttributeType("changeSet", "Integer"));
    ...
    private void loadRecentFiles() {
        ...
        if (params.hasParameter("Recent OSM file 1")) {
            String path = (String) params
                    .getParameterValue("Recent OSM file 1");
            ...
            this.recentFiles.add(
                    new OSMFile(new File(path), epsg, createDb, tagFilter));
        }
        if (params.hasParameter("Recent OSM file 2")) {
            String path = (String) params
                    .getParameterValue("Recent OSM file 2");
            Map<String, String> attrs = params
        ...
        if (params.hasParameter("Recent OSM file 3")) {
            String path = (String) params
                    .getParameterValue("Recent OSM file 3");
            Map<String, String> attrs = params
        ...
        if (params.hasParameter("Recent OSM file 4")) {
            String path = (String) params
                    .getParameterValue("Recent OSM file 4");
            Map<String, String> attrs = params
        ...
        if (params.hasParameter("Recent OSM file 5")) {
            String path = (String) params
                    .getParameterValue("Recent OSM file 5");
            Map<String, String> attrs = params
    ...
    private void createProgressDialog() {
        ...
        taskLabel = new JLabel(
                loader.getCurrentTask().getLabel() + " loading...");
    ...
    class OSMFile {
        ...
        public OSMFile(File file, String epsg, boolean createDb,
                String tagFilter) {
            super();
            this.file = file;
            this.epsg = epsg;
    ...
    class ShowUsersAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            schema.getAttLookup().put(0,
                    new String[] { "name", "nbContributions" });
            ...
            Layer layer = pFrame.getSld().createLayer("contributors",
                    IPoint.class, Color.RED);
            StyledLayerDescriptor defaultSld;
            try {
                defaultSld = StyledLayerDescriptor.unmarshall(
                        IGeneObj.class.getClassLoader().getResourceAsStream(
                                "sld/sld_osm_contributors.xml"));
                layer.getStyles().addAll(
                        defaultSld.getLayer("contributors").getStyles());
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (OSMContributor contributor : contributors) {
                if (contributor.getName().equals(contribName)) {
                    ...
                    for (OsmGeneObj obj : contributor
                            .getWeekEndContributions()) {
                        DefaultFeature feat = new DefaultFeature(
                                obj.getGeom().centroid().toGM_Point());
                        feat.setFeatureType(ft);
            ...
            ProjectFrame pFrame = application.getMainFrame()
                    .getSelectedProjectFrame();
            Layer layerWeek = pFrame.getSld().createLayer(layerNameWeek,
                    IPoint.class, Color.GREEN);
            ...
            IPopulation<IFeature> popWeekend = new Population<>(
                    layerNameWeekend);
