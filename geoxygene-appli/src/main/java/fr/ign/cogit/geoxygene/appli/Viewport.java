...
/**
 * Viewport associated with a {@link LayerViewPanel}. This class is responsible for the transformation between view coordinates and model coordinates.
 * 
...
public class Viewport {
  ...
  /** The number of pixels used to approximate a curve. It is used both when transforming a curve to a linestring (especially for rendering). */
  private double spacingInPixels = 10;

  /** @return The number of pixels used to approximate a curve. It is used both when transforming a curve to a linestring (especially for rendering). */
  public double getSpacingInPixels() {
    return this.spacingInPixels;
  }

  /**
   * Set the number of pixels used to approximate a curve. It is used both when transforming a curve to a linestring (especially for rendering).
  ...
  /**
   * Taille d'un pixel en m (la longueur d'un cote de pixel de l'ecran) utilise pour le calcul de l'echelle courante de la vue. Elle est calculée à
   * partir de la résolution de l'écran en DPI. par exemple si la résolution est 90DPI, c'est: 90 pix/inch = 1/90 inch/pix = 0.0254/90 meter/pix.
   */
  ...
  /** @return Taille d'un pixel en m (la longueur d'un cote de pixel de l'ecran) utilise pour le calcul de l'echelle courante de la vue. */
   ...
   * @throws NoninvertibleTransformException throws an exception when the transformation fails
   * @see #update()
   */
   ...
   * @return a DirectPositionList of DirectPosition in the screen coordinate system corresponding to the given DirectPositionList in model coordinate
   *         system
   * @throws NoninvertibleTransformException throws an exception when the transformation fails
   */
  public final IDirectPositionList toViewDirectPositionList(final IDirectPositionList modelDirectPositionList) throws NoninvertibleTransformException {
    ...
    for (int i = 0; i < numberOfModelPoints; i++) {
      ...
      // threshold
      if (xDifference >= threshold || yDifference >= threshold || numberOfPoints < Viewport.MINIMUM_NUMBER_OF_POINTS
  ...
  public final void zoom(final Point2D p, final double factor) throws NoninvertibleTransformException {
    ...
    IEnvelope zoomModelEnvelope = new GM_Envelope(zoomPoint.getX() - Viewport.ZERO_POINT_FIVE * (width / factor) - dx, zoomPoint.getX()
        + Viewport.ZERO_POINT_FIVE * (width / factor) - dx, zoomPoint.getY() - Viewport.ZERO_POINT_FIVE * (height / factor) - dy, zoomPoint
        .getY()
  ...
  /**
   * Transform an envelope in view coordinates to an envelope in model coordinates.
   ...
   * @throws NoninvertibleTransformException throws an exception when the transformation fails
   */
  private IEnvelope toModelEnvelope(final double xMin, final double xMax, final double yMin, final double yMax)
      throws NoninvertibleTransformException {
  ...
  /**
   * Transform a point in view coordinates to a direct position in model coordinates.
