...
public class OSMActivityAreaPlugin
    implements ProjectFramePlugin, GeOxygeneApplicationPlugin, ActionListener {
  ...
  public void initialize(GeOxygeneApplication application) {
    ...
    activityByContributorMenu
        .add(new JMenuItem(new LoadContributorAreaAction()));
  ...
  /**
   * Load contributor nodes in the selected extent, and creates a new layer with
   * the features.
   * 
   * @author QTTruong
   * 
    ...
    */
    ...
    public LoadContributorAreaAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Load OSM contributor nodes and add as a new layer");
      this.putValue(Action.NAME, "Load OSM contributor nodes ");
    }
  }

  class LoadContributorAreaActionFrame extends JFrame
    ...
    private JTextField txtHost, txtPort, txtDb, txtUser, txtPwd, txtSchema,
        txtMinTemp, txtMaxTemp, txtMinLon, txtMinLat, txtMaxLon, txtMaxLat,
        txtUid;
    ...
    private void loadData() {
      ...
      try {
        osmContributorResource = ActivityArea
            .selectNodesByUid(Long.parseLong(txtUid.getText()), bbox, timespan);
      ...
      try {
        actArea = ActivityArea.getActivityAreas(osmContributorResource);
      } catch (Exception e1) {
        // TODO Auto-generated catch block
        e1.printStackTrace();
      }
      ...
      for (OSMResource resource : osmContributorResource) {
        ...
        IPoint ipoint = new GM_Point(
            CRSConversion.wgs84ToLambert93(latitude, longitude));

        points.add(new OSMDefaultFeature(resource.getContributeur(), ipoint,
            (int) resource.getId(), resource.getChangeSet(),
            resource.getVersion(), resource.getUid(), resource.getDate(),
            resource.getTags()));
      }
      // put the nodes in a new layer
      ProjectFrame pFrame = application.getMainFrame()
          .getSelectedProjectFrame();
      Layer layer = pFrame.getSld().createLayer(CONTRIBUTOR_NODE_LAYER,
      ...
      Layer layerArea = pFrame.getSld().createLayer(CONTRIBUTOR_AREA_LAYER,
          GM_Polygon.class, new Color(0.5f, 1.f, 0.5f), Color.green, 0.5f, 4);
      PolygonSymbolizer symbolizerArea = (PolygonSymbolizer) layerArea
          .getSymbolizer();
