...
/**
 * Text Strokes are used to draw text symbolizers along a path. The path is flattened using the static FLATNESS attribute.
 * <p>
 * This class is a modified version of Jerry Huxtable's TextStroke available at <a
 * href=" URL#http://www.jhlabs.com/java/java2d/strokes/TextStroke.java ">http ://www.jhlabs.com/java/java2d/strokes/TextStroke.java</a>
 */
public class TextStroke implements Stroke {
  ...
  /** If true, allow partial text, otherwise, only allow for the entire text to be drawn. */
   ...
   * @param text the text to draw
   * @param font the font used to draw the text
   * @param stretchToFit true if the text should be stretched on the entire geometry
   * @param repeat true if the text should be repeat along the geometry
   * @param partial true if partial text should be drawn, false if only the text can only be drawn if there is enough room for the entire text
   * @param initialGap initial gap at the beginning of the label when repeating the text
   * @param gap gap between labels when repeating the text
   */
  public TextStroke(String text, Font font, boolean stretchToFit, boolean repeat, boolean partial, float initialGap, float gap) {
    this.text = text;
    this.font = font;
  ...
  public Shape createStrokedShape(Shape shape) {
    ...
    float factor = this.stretchToFit ? TextStroke.length(shape) / (float) glyphVector.getLogicalBounds().getWidth() : 1.0f;
    PathIterator it = new FlatteningPathIterator(shape.getPathIterator(null), TextStroke.FLATNESS);
    ...
    while (currentCharIndex < numberOfGlyphs && !it.isDone()) {
      ...
      case PathIterator.SEG_MOVETO:
        ...
        next = nextAdvance = glyphVector.getGlyphMetrics(currentCharIndex).getAdvance() * 0.5f;
      ...
      case PathIterator.SEG_LINETO:
        ...
        if (segmentLength >= next) {
          ...
          while (currentCharIndex < numberOfGlyphs && segmentLength >= next) {
            Shape currentGlyph = glyphVector.getGlyphOutline(currentCharIndex);
            Point2D currentPosition = glyphVector.getGlyphPosition(currentCharIndex);
            ...
            nextAdvance = this.repeat || currentCharIndex < numberOfGlyphs - 1 ? glyphVector.getGlyphMetrics((currentCharIndex + 1) % numberOfGlyphs)
                .getAdvance() * 0.5f : 0;
