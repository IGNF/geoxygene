...
public class TabbedMainFrame extends AbstractMainFrame implements ChangeListener {
     ...
     */
    public TabbedMainFrame(final String title, final GeOxygeneApplication application) {
     ...
     *            frame to add
     */
    void addProjectFrame(final JTabbedPane currentDesktop, final ProjectFrame frame) {
        if (frame instanceof TabbedProjectFrame) {
            TabbedProjectFrame tabbedFrame = (TabbedProjectFrame) frame;

            VTextIcon textIcon = new VTextIcon(this.getDesktopTabbedPane(), tabbedFrame.getTitle(), VTextIcon.ROTATE_LEFT);
            CompositeIcon icon = new CompositeIcon(tabbedFrame.getIconImage(), textIcon);
            currentDesktop.addTab(null, icon, tabbedFrame.getGui());
            currentDesktop.setSelectedComponent(tabbedFrame.getGui());
        } else {
            logger.error("Cannot add a " + frame.getClass().getSimpleName() + " into a " + this.getClass().getSimpleName());
        }
    }
    ...
    public final void dispose() {
        ...
        if (previous != null) {
            this.getApplication().getProperties().setLastOpenedFile(previous.getAbsolutePath());
            this.getApplication().getProperties().marshall(this.getApplication().getPropertiesFile().getFile());
        }
    ...
    public final ProjectFrame getSelectedProjectFrame() {
        ...
        try {
            ...
            logger.error("Selected GUI frame has not be inserted into the list of ProjectFrame ! (" + this.getDesktopProjectFrames().length + " elements)");
        }
        return null;
    }
