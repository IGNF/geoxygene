...
public class SemioPlugin implements GeOxygeneApplicationPlugin, ActionListener {
  ...
  public void initialize(final GeOxygeneApplication application) {
    ...
    JMenuItem menuCercleChrom = new JMenuItem(
        I18N.getString("SemioPlugin.VisuWheels")); //$NON-NLS-1$
    ...
    JMenuItem menuContrastOnChromWheels = new JMenuItem(
        I18N.getString("SemioPlugin.VisuContrast")); //$NON-NLS-1$
    menuContrastOnChromWheels
    ...
    application
        ...
        .getMenuBar()
        .add(menuSemio,
            application.getMainFrame().getMenuBar().getComponentCount() - 1);
  }
  ...
  public void afficherCercles() {
    ...
    JDialog dialog = JColorChooser.createDialog(SemioPlugin.this.application
        .getMainFrame().getGui(),
  ...
  @Override
  public void actionPerformed(ActionEvent e) {
    ProjectFrame projectFrame = SemioPlugin.this.application.getMainFrame()
    ...
    if (e.getSource() == SemioPlugin.this.contrastMenuItem) {
      if (contrastMenuItem.isSelected()) {
        ...
        if (projectFrame.getMenuBar() == null) {
          JMenuBar menuBar = new JMenuBar();
          projectFrame.setMenuBar(menuBar);
        }
        projectFrame.getMenuBar().add(contrastAnalysisToolBar);
      } else {
        projectFrame.getMenuBar().remove(contrastAnalysisToolBar);
      }
      projectFrame.getGui().revalidate();
      projectFrame.getGui().repaint();
    }
    ...
    if (e.getSource() == semioMenuItem) {
      if (semioMenuItem.isSelected()) {
        ...
        projectFrame.getMenuBar().add(semioToolBar, 0);
      } else {
        projectFrame.getMenuBar().remove(semioToolBar);
      }
      projectFrame.getGui().revalidate();
      projectFrame.getGui().repaint();
    }
     ...
     * something like that... projectFrame.getGui().pack();
     * 
     * try { projectFrame.setMaximum(true); } catch (PropertyVetoException e1) {
     * e1.printStackTrace(); }
