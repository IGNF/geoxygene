...
public class FloatingMainFrame extends AbstractMainFrame implements InternalFrameListener {
    ...
    void addProjectFrame(final JDesktopPane desktop, final ProjectFrame project) {
        if (project instanceof FloatingProjectFrame) {
            ...
            logger.error("Cannot add a " + project.getClass().getSimpleName() + " into a " + this.getClass().getSimpleName());
        }
    }
    ...
    synchronized public final ProjectFrame newProjectFrame(final LayerViewPanel layerViewPanel) {
        ...
        if (currentDesktop == null) {
            ...
            currentDesktop = (JDesktopPane) this.createNewDesktop("Desktop #" + index);
        }

        projectFrame.getInternalFrame().setSize((int) this.getSize().getWidth() / 3 * 2, (int) this.getSize().getHeight() / 3 * 2);
        projectFrame.getInternalFrame().setVisible(true);
        this.addProjectFrame(currentDesktop, projectFrame);

        // Load project Plugin
        // FIXME - Ugly : test if the first pluginAppli were loaded !!
        int menuCount = this.getApplication().getMainFrame().getMenuBar().getMenuCount();
    ...
    public JComponent addComponentInFrame(String title, JComponent component) {
        throw new UnsupportedOperationException("Don't understand what this method is intended to do... [JeT]");
    }
