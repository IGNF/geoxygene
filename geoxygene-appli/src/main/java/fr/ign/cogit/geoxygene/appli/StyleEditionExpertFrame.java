...
public class StyleEditionExpertFrame extends JDialog implements ActionListener,
  ...
  private static Logger logger = Logger.getLogger(StyleEditionExpertFrame.class
      .getName());
   ...
   * 
   * @param projectFrame the project frame where the layer lies
  ...
  private void gotoSelectedLayer() {
    if (this.layerLegendPanel.getSelectedLayers().size() == 1) {
      int caret = this.getEditor().getText().toLowerCase()
          .indexOf("<name>" + this.layer.getName().toLowerCase() + "</name>");
      if (caret >= 0) {
  ...
  private static void centerLineInScrollPane(JTextComponent component) {
    Container container = SwingUtilities.getAncestorOfClass(JViewport.class,
        component);
  ...
  public void valueChanged(ListSelectionEvent e) {
    if (this.layerLegendPanel.getSelectedLayers().size() == 1) {
      Layer layer = this.layerLegendPanel.getSelectedLayers().iterator().next();
