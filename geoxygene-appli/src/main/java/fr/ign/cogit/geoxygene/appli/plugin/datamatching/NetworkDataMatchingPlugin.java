...
public class NetworkDataMatchingPlugin implements GeOxygeneApplicationPlugin,
  ...
  private static Logger LOGGER = Logger
      .getLogger(NetworkDataMatchingPlugin.class.getName());
  ...
  private final DatasetNetworkDataMatching datasetNetwork1 = new DatasetNetworkDataMatching();
  private final DatasetNetworkDataMatching datasetNetwork2 = new DatasetNetworkDataMatching();
  ...
  private void initializeParam() {
    ...
    // String filename1 =
    // "D:\\Data\\Appariement\\ESPON_DB\\Test02\\Reseau1.shp";
    // String filename2 =
    // "D:\\Data\\Appariement\\ESPON_DB\\Test02\\Reseau2.shp";
    ...
    // String filename1 =
    // "D:\\Data\\Appariement\\CG32\\Route\\Referentiel_ImaRoute_CG32.shp";
    // String filename1 =
    // "D:\\Data\\Appariement\\Kusay\\CGDep84_extraction.shp";
    ...
    paramProj1
        .setProjeteNoeuds1SurReseau2DistanceProjectionNoeud(2 * distanceNoeudsMax);
  ...
  protected void doNetworkDataMatching() {
    ...
    // Dataset
    String lastFile = "";
    for (int i = 0; i < paramPlugin.getParamFilenameNetwork1()
        .getListNomFichiersPopArcs().size(); i++) {
      String filename = paramPlugin.getParamFilenameNetwork1()
          .getListNomFichiersPopArcs().get(i);
      lastFile = filename;
      IPopulation<IFeature> reseau = ShapefileReader.read(filename);
      datasetNetwork1.addPopulationsArcs(reseau);
    }
    for (int i = 0; i < paramPlugin.getParamFilenameNetwork2()
        .getListNomFichiersPopArcs().size(); i++) {
      String filename = paramPlugin.getParamFilenameNetwork2()
          .getListNomFichiersPopArcs().get(i);
      lastFile = filename;
      IPopulation<IFeature> reseau = ShapefileReader.read(filename);
    ...
    application.getProperties().marshall(
        application.getPropertiesFile().getFile());

    // -------------------------------------------------------------------------------

    // Parse new parameter object to old parameter
    // ParametresApp paramOld =
    // paramPlugin.getParamNetworkDataMatching().paramNDMToParamApp();
    ...
    NetworkDataMatching networkDataMatchingProcess = new NetworkDataMatching(
        paramPlugin.getParamNetworkDataMatching(), datasetNetwork1,
        datasetNetwork2);
    networkDataMatchingProcess.setActions(paramPlugin.getDoRecalage(),
        paramPlugin.getDoLinkExport());
    ResultNetworkDataMatching resultatAppariement = networkDataMatchingProcess
        .networkDataMatching();

    // Logs
    LOGGER.info("Nb arcs du réseau 1 calculés : "
        + resultatAppariement.getReseauStat1().getReseauApp().getListeArcs()
            .size() + " >= " + datasetNetwork1.getPopulationsArcs().size());
    LOGGER.info("Nb arcs du réseau 2 calculés : "
        + resultatAppariement.getReseauStat2().getReseauApp().getListeArcs()
            .size() + " >= " + datasetNetwork2.getPopulationsArcs().size());
      ...
      if (geom instanceof GM_Aggregate<?>) {
        ...
        for (IGeometry lineGeom : ((GM_Aggregate<?>) geom).getList()) {
          if (lineGeom instanceof GM_LineString) {
            ...
            multiCurve.addAll(((GM_MultiCurve<GM_LineString>) lineGeom)
                .getList());
          } else if (lineGeom instanceof GM_Polygon) {
            DefaultFeature defaultFeature = new DefaultFeature();

            defaultFeature
                .setFeatureType(schemaDefaultFeature.getFeatureType());
            defaultFeature.setSchema(schemaDefaultFeature);
            defaultFeature.setGeom(lineGeom);
    ...
    // Liens generiques
    /*
     * Population<DefaultFeature> collection2 = new
     * Population<DefaultFeature>();
     ...
     * resultatAppariement.getLiensGeneriques();
     * LOGGER.trace("Liens generiques = " + liensGeneriques.size()); for (Lien
     * lien : liensGeneriques) {
     *//*
        * DefaultFeature defaultFeature = new DefaultFeature(); defaultFeature
        * .setFeatureType(schemaDefaultFeature2.getFeatureType());
        * defaultFeature.setSchema(schemaDefaultFeature2);
        * defaultFeature.setGeom(lien.getGeom());
        *//*
           * 
           * LOGGER.trace("Objets reseau 1 comme chaine = " +
           * lien.getObjetsRefAsString());
           * LOGGER.trace("Objets reseau 2 comme chaine = " +
           * lien.getCorrespondantsAsString());
           * 
           * // On ajoute l'attribut "nature" qui vaut i*0.05 et qui est de type
           * "double" // AttributeManager.addAttribute(defaultFeature,
           * "evaluation", "0.5", "Double"); //
           * AttributeManager.addAttribute(defaultFeature, "objetsRef", "44",
           * "String"); // AttributeManager.addAttribute(defaultFeature,
           * "objetsComp", "33", "String");
           *//*
              * List<GF_AttributeType> listAttributs =
              * lien.getFeatureType().getFeatureAttributes(); AttributeType aT =
              * new AttributeType(); aT.setMemberName("objetsRef");
              * aT.setNomField("objetsRef"); aT.setValueType("String");
              * listAttributs.add(aT);
              * lien.getFeatureType().setFeatureAttributes(listAttributs);
              * 
              * LOGGER.trace("Nb d'attributs = " +
              * lien.getFeatureType().getFeatureAttributes().size()); for
              * (GF_AttributeType attribute :
              * lien.getFeatureType().getFeatureAttributes()) { String
              * nomAttribut = attribute.getMemberName(); String value =
              * attribute. LOGGER.trace(nomAttribut); }
              *//*
                 * 
    ...
    // String repResultat =
    // "D:\\Data\\Appariement\\MesTests\\EXTRAITS-GPS\\R4\\";
    // ShapefileWriter.write(arcs, repResultat + "ReseauApparie.shp");
    // ShapefileWriter.write(liens, repResultat + "Liens.shp");
    // ShapefileWriter.write(resultatAppariement.getReseau1().getPopArcs(),
    // repResultat + "ArcTopoReseau1.shp");
    // ShapefileWriter.write(resultatAppariement.getReseau1().getPopNoeuds(),
    // repResultat + "NoeudTopoReseau1.shp");
    // ShapefileWriter.write(resultatAppariement.getReseau2().getPopArcs(),
    // repResultat + "ArcTopoReseau2.shp");
    // ShapefileWriter.write(resultatAppariement.getReseau2().getPopNoeuds(),
    // repResultat + "NoeudTopoReseau2.shp");
    ...
    Layer l1 = p1.addUserLayer(datasetNetwork1.getPopulationsArcs().get(0),
        "Réseau 1", null);
    l1.getSymbolizer().getStroke().setColor(network1Color);
    l1.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);
    Layer l2 = p1.addUserLayer(datasetNetwork2.getPopulationsArcs().get(0),
        "Réseau 2", null);
    ...
    if (paramPlugin.getDoRecalage()) {
      //
      CarteTopo reseauRecale = Recalage.recalage(resultatAppariement
          .getReseauStat1().getReseauApp(), resultatAppariement
          .getReseauStat2().getReseauApp(), liens);
      ...
      LOGGER.info("Nombre de noeuds de la carte topo n°1 = "
          + resultatAppariement.getReseauStat1().getReseauApp()
              .getListeNoeuds().size());

      /*
       * List<ReseauApp> cartesTopoReferenceValuees = AppariementIO
       * .scindeSelonValeursResultatsAppariement
       * (resultatAppariement.getReseau1(), valeursClassement); IPopulation<Arc>
       * arcsReferenceApparies = cartesTopoReferenceValuees.get(0).getPopArcs();
       * IPopulation<Arc> arcsReferenceIncertains =
       * cartesTopoReferenceValuees.get(1).getPopArcs(); IPopulation<Arc>
       * arcsReferenceNonApparies =
       * cartesTopoReferenceValuees.get(2).getPopArcs(); // IPopulation<Noeud>
       * noeudsReferenceApparies =
       * cartesTopoReferenceValuees.get(0).getPopNoeuds(); // IPopulation<Noeud>
       * noeudsReferenceIncertains =
       * cartesTopoReferenceValuees.get(1).getPopNoeuds(); // IPopulation<Noeud>
       * noeudsReferenceNonApparies =
       * cartesTopoReferenceValuees.get(2).getPopNoeuds();
       * 
       * l1 = p2.addUserLayer(arcsReferenceApparies, "Arcs réseau 1 appariés",
       * null); l1.getSymbolizer().getStroke().setColor(network1Color);
       * l1.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH); Layer l1D =
       * p2.addUserLayer(arcsReferenceIncertains, "Arcs réseau 1 incertains",
       * null); l1D.getSymbolizer().getStroke().setColor(network1DColor);
       * l1D.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH); Layer l1N =
       * p2.addUserLayer(arcsReferenceNonApparies, "Arcs réseau 1 non appariés",
       * null); l1N.getSymbolizer().getStroke().setColor(network1NColor);
       * l1N.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);
       */
      l1 = p2.addUserLayer(datasetNetwork1.getPopulationsArcs().get(0),
          "Réseau 1", null);
      l1.getSymbolizer().getStroke().setColor(network1Color);
      l1.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);

      l2 = p2.addUserLayer(datasetNetwork2.getPopulationsArcs().get(0),
          "Réseau 2", null);
      l2.getSymbolizer().getStroke().setColor(network2Color);
      l2.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);
    ...
    l1 = p3.addUserLayer(resultatAppariement.getReseauStat1().getReseauApp()
        .getPopArcs(), "CT 1 - Arcs", null);
    l1.getSymbolizer().getStroke().setColor(network1Color);
    l1.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);

    // Layer lp1 =
    p3.addUserLayer(resultatAppariement.getReseauStat1().getReseauApp()
        .getPopNoeuds(), "CT 1 - Noeuds", null);
    // Layer lp2 =
    p3.addUserLayer(resultatAppariement.getReseauStat2().getReseauApp()
        .getPopNoeuds(), "CT 2 - Noeuds", null);
    // lp1.getSymbolizer().getStroke().setColor(network1Color);
    // lp1.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);

    l2 = p3.addUserLayer(resultatAppariement.getReseauStat2().getReseauApp()
        .getPopArcs(), "CT 2 - Arcs", null);
    l2.getSymbolizer().getStroke().setColor(network2Color);
    l2.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);
    ...
    Layer link_doubtfull_layer = p3.addUserLayer(liensIncertains,
        "Liens incertains", null);
    link_doubtfull_layer.getSymbolizer().getStroke()
        .setColor(linkColorDoubtfull);
    link_doubtfull_layer.getSymbolizer().getStroke()
        .setStrokeWidth(LINE_WIDTH_LINK);

    /*
     * try { CoordinateReferenceSystem crs = CRS.decode("EPSG:32618");
     * ShapefileWriter.write(liensIncertains,
     * "D:\\Data\\Appariement\\Haiti\\rond-point\\liens.shp", crs); } catch
     ...
     * 
     * l1 = p4.addUserLayer(datasetNetwork1.getPopulationsArcs().get(0),
     * "Réseau 1", null);
     * l1.getSymbolizer().getStroke().setColor(network1Color);
     * l1.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH); l2 =
     * p4.addUserLayer(datasetNetwork2.getPopulationsArcs().get(0), "Réseau 2",
     * null); l2.getSymbolizer().getStroke().setColor(network2Color);
     * l2.getSymbolizer().getStroke().setStrokeWidth(LINE_WIDTH);
     * 
    ...
    DisplayLinkGeomPanel geomLinkPanel = new DisplayLinkGeomPanel(
        application.getMainFrame(), liens);
    this.application.getMainFrame().addFrameInDesktop("Résultat.",
        new DisplayLinkPanel(geomLinkPanel, liens));
    this.application.getMainFrame().addFrameInDesktop("Résultat.",
        new DisplayStatResultPanel(resultatAppariement));
    this.application.getMainFrame().addFrameInDesktop("Résultat.",
        new DisplayParamPanel(paramPlugin));
    this.application.getMainFrame().addFrameInDesktop("Résultat.",
        geomLinkPanel.getGui());
