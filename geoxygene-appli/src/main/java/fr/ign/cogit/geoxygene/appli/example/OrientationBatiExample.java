...
public class OrientationBatiExample extends AbstractGeOxygeneApplicationPlugin {
  ...
  private static final Logger LOGGER = Logger
      .getLogger(OrientationBatiExample.class.getName());
  ...
  private void createLayer(IGeometry bati01) {
    ...
    try {
      ...
      // ==============================================================================================================
      // Layer 1 : PPRE
      Population<DefaultFeature> popPPRE = new Population<DefaultFeature>(
          false, "PPRE", DefaultFeature.class, true);
      popPPRE.setFeatureType(this.featureTypePPRE);

      Geometry bati01Jts = AdapterFactory.toGeometry(new GeometryFactory(),
          bati01);
      ...
      PolygonSymbolizer polySymbolR = (PolygonSymbolizer) layerPPRE
          .getSymbolizer();
      ...
      // ==============================================================================================================
      // Layer 2 : bati
      Population<DefaultFeature> popBati = new Population<DefaultFeature>(
          false, "Bati", DefaultFeature.class, true);
      ...
      Object[] attributes = new Object[] { orientationGenerale, orientationMur,
          nbOrientation, orientationMur2 };
      n.setAttributes(attributes);

      // Affiche
      Layer layerBati = projectFrame.addUserLayer(popBati, "Bati", null);
      projectFrame.getLayerLegendPanel().getLayerViewPanel().getViewport()
          .zoomToFullExtent();

      PolygonSymbolizer polySymbol = (PolygonSymbolizer) layerBati
          .getSymbolizer();
      ...
      symbolizerP1.getGraphic().getExternalGraphics()
          .add(externalGraphicCircle1);

      layerBati.getStyles().get(0).getFeatureTypeStyles().get(0).getRules()
          .get(0).getSymbolizers().add(symbolizerP1);
      ...
      ptPlacement.setRotation(new RotationLabel());
      txtSymbolizer.setLabelPlacement(placement);
      layerBati.getStyles().get(0).getFeatureTypeStyles().get(0).getRules()
          .get(0).getSymbolizers().add(txtSymbolizer);

      // ==============================================================================================================
      // Layer 3 : centre feuille
      Population<DefaultFeature> popFeuille = new Population<DefaultFeature>(
          false, "Feuille", DefaultFeature.class, true);
      ...
      DirectPosition pos = new DirectPosition(ppreGeox.centroid().getX(),
          ppreGeox.centroid().getY() + l * 2 / 3);
      ...
      Layer layerFeuille = projectFrame.addUserLayer(popFeuille, "Feuilles",
          null);
      PointSymbolizer symbolizerP2 = (PointSymbolizer) layerFeuille
          .getSymbolizer();
      ...
      // Layer 4 : axes
      /*
       * Population<DefaultFeature> popAxes = new
       * Population<DefaultFeature>(false, "Axes", DefaultFeature.class, true);
       * popAxes.setFeatureType(this.featureTypeAxe);
       * 
       * 
       * 
       * double a1 = Math.tan(orientationMur); double b1 = pos.getY() - a1 *
       * pos.getX(); double x1 = pos.getX() + 2; if (orientationMur <= Math.PI /
       * 2) { x1 = pos.getX() + 10; } double y1 = a1 * x1 + b1; ILineString axe1
       * = new GM_LineString (pos, new DirectPosition(x1, y1)); DefaultFeature
       * featAxe1 = popAxes.nouvelElement(axe1);
       * featAxe1.setSchema(this.schemaAxe);
       * 
       * if (nbOrientation > 1) { double a2 = Math.tan(orientationMur2); // +
       * Math.PI / 2 double b2 = pos.getY() - a2 * pos.getX(); double x2 =
       * pos.getX() + 2; if (orientationMur2 <= Math.PI / 2) { x2 = pos.getX() +
       * 10; } double y2 = a2 * x2 + b2; ILineString axe2 = new GM_LineString
       * (pos, new DirectPosition(x2, y2)); DefaultFeature featAxe2 =
       * popAxes.nouvelElement(axe2); featAxe2.setSchema(this.schemaAxe); }
       * 
      ...
      // Layer roseLayerCommune = projectFrame.addUserLayer(commune75Pop,
      // "RoseChart", null);
      ...
      layerFeuille.getStyles().get(0).getFeatureTypeStyles().get(0).getRules()
          .get(0).getSymbolizers().add(rosets);
  ...
  private void createSchemas() {
    ...
    AttributeType orientationGeneraleAttribute = new AttributeType(
        "orientationGenerale", "double");
    this.featureTypeBati.addFeatureAttribute(orientationGeneraleAttribute);
    AttributeType orientationMurAttribute = new AttributeType("orientationMur",
        "double");
    this.featureTypeBati.addFeatureAttribute(orientationMurAttribute);
    AttributeType nbOrientationAttribute = new AttributeType("nbOrientation",
        "double");
    this.featureTypeBati.addFeatureAttribute(nbOrientationAttribute);
    AttributeType orientationMur2Attribute = new AttributeType(
        "orientationMur2", "double");
    ...
    attLookup.put(new Integer(0),
        new String[] { orientationGeneraleAttribute.getNomField(),
            orientationGeneraleAttribute.getMemberName() });
    attLookup.put(new Integer(1),
        new String[] { orientationMurAttribute.getNomField(),
            orientationMurAttribute.getMemberName() });
    attLookup.put(new Integer(2),
        new String[] { nbOrientationAttribute.getNomField(),
            nbOrientationAttribute.getMemberName() });
    attLookup.put(new Integer(3),
        new String[] { orientationMur2Attribute.getNomField(),
            orientationMur2Attribute.getMemberName() });
    ...
    for (int i = 0; i < 90; i++) {
      ...
      attLookupFeuille.put(new Integer(i), new String[] { anglei.getNomField(),
          anglei.getMemberName() });
    }
