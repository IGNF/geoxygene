...
public class LayerStylesPanel extends JPanel {
     ...
     */
    private final WeakReference<Layer> layer;
    ...
    public void paintComponent(Graphics g) {
        ...
        int numberOfColumns = Math.max(1, Math.round((float) Math.sqrt(map.keySet().size())));
        int numberOfRows = Math.max(1, map.keySet().size() / numberOfColumns);
        int columnsWidth = (this.getWidth() - numberOfColumns * this.margin - this.margin) / numberOfColumns;
        int rowHeight = (this.getHeight() - numberOfRows * this.margin - this.margin) / numberOfRows;
        int currentColumn = 0;
        int currentRow = 0;
        for (Entry<Filter, List<Symbolizer>> entry : map.entrySet()) {
            ...
            for (Symbolizer symbolizer : symbolizers) {
                if (symbolizer.isLineSymbolizer()) {
                    this.paintLine((LineSymbolizer) symbolizer, g2, currentColumn, currentRow, columnsWidth, rowHeight);
                } else {
                    if (symbolizer.isPointSymbolizer()) {
                        this.paintPoint((PointSymbolizer) symbolizer, g2, currentColumn, currentRow, columnsWidth, rowHeight);
                    } else {
                        if (symbolizer.isPolygonSymbolizer()) {
                            this.paintPolygon((PolygonSymbolizer) symbolizer, g2, currentColumn, currentRow, columnsWidth, rowHeight);
                        } else {
                            if (symbolizer.isRasterSymbolizer()) {
                                // TODO : deal with non 3-band raster,
                                // especially in GL mode
                                this.paintRaster((RasterSymbolizer) symbolizer, g2, currentColumn, currentRow, columnsWidth, rowHeight);
                            }
                        }
                    }
                }
     ...
     * @param rowHeight
     */
    private void paintLine(LineSymbolizer symbolizer, Graphics2D g2, int currentColumn, int currentRow, int columnsWidth, int rowHeight) {
                    ...
                    for (Graphic graphic : graphicList) {
                        for (ExternalGraphic external : graphic.getExternalGraphics()) {
                            if (external.getFormat().contains("png") || external.getFormat().contains("gif")) { //$NON-NLS-1$ //$NON-NLS-2$
                                ...
                                drawGraphicFillPolygon(image, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, rowHeight / 2
                                ...
                                if (external.getFormat().contains("svg")) { //$NON-NLS-1$
                                    ...
                                    drawGraphicFillPolygon(node, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, rowHeight / 2
                                            - symbolizer.getStroke().getStrokeWidth() / 2, 0);
                                }
                            }
                            return;
                        ...
                        for (Mark mark : graphic.getMarks()) {
                            ...
                            if (graphic.getRotation() != null) {
                                AffineTransform rotate = AffineTransform.getRotateInstance(Math.PI * Double.parseDouble(graphic.getRotation().evaluate(null).toString()) / 180.0);
                            ...
                            g.setColor(ColorUtil.getColorWithOpacity(mark.getFill().getColor(), this.getLayer().getOpacity()));
                            g.fill(tranlatedShape);
                            drawGraphicFillPolygon(buff, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, rowHeight / 2
                                    - symbolizer.getStroke().getStrokeWidth() / 2, 0);
                        }
                    }
     ...
     * @param rowHeight
     */
    private void paintPoint(PointSymbolizer symbolizer, Graphics2D g2, int currentColumn, int currentRow, int columnsWidth, int rowHeight) {
        for (Mark mark : symbolizer.getGraphic().getMarks()) {
            Shape markShape = mark.toShape();
            float size = symbolizer.getGraphic().getSize();
            AffineTransform at = AffineTransform.getTranslateInstance(this.margin + currentColumn * (columnsWidth + this.margin) + columnsWidth / 2, this.margin + currentRow
            ...
            g2.setColor((mark.getFill() == null) ? Color.gray : mark.getFill().getColor());
            g2.fill(markShape);
            if (mark.getStroke() != null) {
                g2.setStroke(mark.getStroke().toAwtStroke());
            }
            g2.setColor((mark.getStroke() == null) ? Color.black : mark.getStroke().getColor());
        ...
        for (ExternalGraphic theGraphic : symbolizer.getGraphic().getExternalGraphics()) {
            ...
            if (format.contains("svg")) { //$NON-NLS-1$
                ...
                drawGraphicFillPolygon(node, symbolizer.getGraphic().getSize(), g2, this.getLayer().getOpacity(), widthDrawn, widthDrawn, rowHeight / 2 - widthDrawn / 2, 0);
            } else if (format.contains("png") || format.contains("gif") || format.contains("jpg")) { //$NON-NLS-1$ //$NON-NLS-2$

                Image onlineImage = theGraphic.getOnlineResource();
                g2.drawImage(onlineImage, this.margin + currentColumn * (columnsWidth + this.margin) + columnsWidth / 2 - onlineImage.getWidth(null) / 2, this.margin + currentRow
                        * (rowHeight + this.margin) + rowHeight / 2 - onlineImage.getHeight(null) / 2, null);
            }
        }
    }
     ...
     * @param rowHeight
     */
    private void paintPolygon(PolygonSymbolizer symbolizer, Graphics2D g2, int currentColumn, int currentRow, int columnsWidth, int rowHeight) {
        ...
        if (symbolizer.getShadow() != null) {
            ...
            g2.fillRect(currentColumn * (columnsWidth + this.margin) + this.margin, currentRow * (rowHeight + this.margin) + this.margin, columnsWidth, rowHeight);
        }
        if (symbolizer.getFill() != null) {
            if (symbolizer.getFill().getExpressiveFill() != null) {
                ExpressiveDescriptor ef = (ExpressiveDescriptor) symbolizer.getFill().getExpressiveFill();
                PerlinNoiseTexture texture = null;

                // FIXME Since we don't know what parameter is a Texture, must
                // scan everything. Do better?
                for (ExpressiveParameter ep : ef.getExpressiveParameters()) {
                    if (!ep.isSimpleParameter() && (ep.getValue() instanceof PerlinNoiseTexture)) {
                        texture = (PerlinNoiseTexture) ep.getValue();
                        break;
                    }
                }
                if (texture != null) {
                ...
                for (Graphic graphic : graphicList) {
                    if (graphic.getExternalGraphics().size() != 0) {
                        for (ExternalGraphic external : graphic.getExternalGraphics()) {
                            if (external.getFormat().contains("png") || external.getFormat().contains("gif")) { //$NON-NLS-1$ //$NON-NLS-2$
                                Image image = external.getOnlineResource();
                                drawGraphicFillPolygon(image, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, rowHeight, 0, 0);
                            } else {
                                if (external.getFormat().contains("svg")) { //$NON-NLS-1$
                                    GraphicsNode node = external.getGraphicsNode();
                                    drawGraphicFillPolygon(node, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, rowHeight, 0, 0);
                                }
                            }
                        }
                        ...
                        for (Mark mark : graphic.getMarks()) {
                            ...
                            drawGraphicFillPolygon(buff, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, rowHeight, 0, 0);
                        }
                    }
                }
            }
        ...
        if (symbolizer.getStroke() != null) {
            if (symbolizer.getStroke().getColor() != null) {
                g2.setColor(symbolizer.getStroke().getColor());
                g2.setStroke(symbolizer.getStroke().toAwtStroke());
                g2.drawRect(currentColumn * (columnsWidth + this.margin) + this.margin, currentRow * (rowHeight + this.margin) + this.margin, columnsWidth, rowHeight);
            }
            if (symbolizer.getStroke().getGraphicType() != null) {
                if (GraphicFill.class.isAssignableFrom(symbolizer.getStroke().getGraphicType().getClass())) {
                        ...
                        if (graphic.getExternalGraphics().size() != 0) {
                            for (ExternalGraphic external : graphic.getExternalGraphics()) {
                                if (external.getFormat().contains("png") || external.getFormat().contains("gif")) { //$NON-NLS-1$ //$NON-NLS-2$
                                    ...
                                    // FIXME Le contour est représenté dans la
                                    // légende par les 4
                                    // bords du symbole de légende
                                    // Ce serait plus propre de créer une
                                    // polygon de la forme du
                                    // contour et de le remplir
                                    drawGraphicFillPolygon(image, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, symbolizer.getStroke()
                                            .getStrokeWidth() / 2 - this.margin * 2, 0);
                                    drawGraphicFillPolygon(image, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, rowHeight - this.margin
                                            * 2 + symbolizer.getStroke().getStrokeWidth() / 2, 0);
                                    drawGraphicFillPolygon(image, graphic.getSize(), g2, this.getLayer().getOpacity(), symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                            - symbolizer.getStroke().getStrokeWidth() - this.margin * 2, symbolizer.getStroke().getStrokeWidth() * 2 - this.margin * 2, -this.margin);
                                    drawGraphicFillPolygon(image, graphic.getSize(), g2, this.getLayer().getOpacity(), symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                            - symbolizer.getStroke().getStrokeWidth() - this.margin * 2, symbolizer.getStroke().getStrokeWidth() * 2 - this.margin * 2, columnsWidth - this.margin);

                                } else {
                                    if (external.getFormat().contains("svg")) { //$NON-NLS-1$
                                        GraphicsNode node = external.getGraphicsNode();

                                        // FIXME Idem, il faudrait recréer la
                                        // forme adéquate
                                        drawGraphicFillPolygon(node, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, symbolizer
                                                .getStroke().getStrokeWidth() / 2 - this.margin * 2, 0);
                                        drawGraphicFillPolygon(node, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                                - this.margin * 2 + symbolizer.getStroke().getStrokeWidth() / 2, 0);
                                        drawGraphicFillPolygon(node, graphic.getSize(), g2, this.getLayer().getOpacity(), symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                                - symbolizer.getStroke().getStrokeWidth() - this.margin * 2, symbolizer.getStroke().getStrokeWidth() * 2 - this.margin * 2, -this.margin);
                                        drawGraphicFillPolygon(node, graphic.getSize(), g2, this.getLayer().getOpacity(), symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                                - symbolizer.getStroke().getStrokeWidth() - this.margin * 2, symbolizer.getStroke().getStrokeWidth() * 2 - this.margin * 2, columnsWidth - this.margin);
                                    }
                            ...
                            for (Mark mark : graphic.getMarks()) {
                                ...
                                // FIXME Idem, il faudrait recréer la forme
                                // adéquate
                                drawGraphicFillPolygon(buff, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, symbolizer.getStroke()
                                        .getStrokeWidth() / 2 - this.margin * 2, 0);
                                drawGraphicFillPolygon(buff, graphic.getSize(), g2, this.getLayer().getOpacity(), widthDrawn, symbolizer.getStroke().getStrokeWidth() * 2, rowHeight - this.margin * 2
                                        + symbolizer.getStroke().getStrokeWidth() / 2, 0);
                                drawGraphicFillPolygon(buff, graphic.getSize(), g2, this.getLayer().getOpacity(), symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                        - symbolizer.getStroke().getStrokeWidth() - this.margin * 2, symbolizer.getStroke().getStrokeWidth() * 2 - this.margin * 2, -this.margin);
                                drawGraphicFillPolygon(buff, graphic.getSize(), g2, this.getLayer().getOpacity(), symbolizer.getStroke().getStrokeWidth() * 2, rowHeight
                                        - symbolizer.getStroke().getStrokeWidth() - this.margin * 2, symbolizer.getStroke().getStrokeWidth() * 2 - this.margin * 2, columnsWidth - this.margin);
                            }
     ...
     * @param rowHeight
     */
    private void paintRaster(RasterSymbolizer symbolizer, Graphics2D g2, int currentColumn, int currentRow, int columnsWidth, int rowHeight) {
        ...
        for (IFeature feature : this.layer.get().getFeatureCollection()) {
            ...
            if (coverage.getNumSampleDimensions() == 3) {
                ...
                BufferedImage thumb = image.getSubimage(0, 0, image.getWidth() / 10, image.getHeight() / 10);
