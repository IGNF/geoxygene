...
public class LayerStylesPanel extends JPanel {
                ...
                if (symbolizer.isLineSymbolizer()) {
                    ...
                    if (symbolizer.isPointSymbolizer()) {
                        this.paintPoint((PointSymbolizer) symbolizer, g2,
                                currentColumn, currentRow, columnsWidth,
                                rowHeight);
                    } else {
                        if (symbolizer.isPolygonSymbolizer()) {
                            this.paintPolygon((PolygonSymbolizer) symbolizer,
                                    g2, currentColumn, currentRow,
                                    columnsWidth, rowHeight);
                        } else {
                            if (symbolizer.isRasterSymbolizer()) {
                                this.paintRaster((RasterSymbolizer) symbolizer,
                                        g2, currentColumn, currentRow,
                                        columnsWidth, rowHeight);
                            }
                        }
            ...
            if (symbolizer.getStroke().getGraphicType() != null) {
                if (GraphicFill.class.isAssignableFrom(symbolizer.getStroke()
                    ...
                    for (Graphic graphic : graphicList) {
                        for (ExternalGraphic external : graphic
                                .getExternalGraphics()) {
                            if (external.getFormat().contains("png") || external.getFormat().contains("gif")) { //$NON-NLS-1$ //$NON-NLS-2$
                                ...
                                if (external.getFormat().contains("svg")) { //$NON-NLS-1$
                                    GraphicsNode node = external
                                            .getGraphicsNode();
                                    drawGraphicFillPolygon(node,
                                            graphic.getSize(), g2, this
                                                    .getLayer().getOpacity(),
                                            widthDrawn, symbolizer.getStroke()
                                                    .getStrokeWidth() * 2,
                                            rowHeight
                                                    / 2
                                                    - symbolizer.getStroke()
                        ...
                        for (Mark mark : graphic.getMarks()) {
                            ...
                            AffineTransform scaleTransform = AffineTransform
                                    .getScaleInstance(markShapeSize,
                                            markShapeSize);
                            ...
                            g.setColor(ColorUtil.getColorWithOpacity(mark
                                    .getFill().getColor(), this.getLayer()
                                    .getOpacity()));
                ...
                } else if (GraphicStroke.class.isAssignableFrom(symbolizer
                        .getStroke().getGraphicType().getClass())) {
    ...
    private void paintPoint(PointSymbolizer symbolizer, Graphics2D g2,
        ...
        for (Mark mark : symbolizer.getGraphic().getMarks()) {
            ...
            AffineTransform at = AffineTransform.getTranslateInstance(
                    this.margin + currentColumn * (columnsWidth + this.margin)
                            + columnsWidth / 2, this.margin + currentRow
            ...
            if (rotation instanceof Literal) {
                ...
                at.rotate(-Double.parseDouble(rotation.evaluate(null)
                        .toString()) * Math.PI / 180.0);
            }
            ...
            g2.setColor((mark.getStroke() == null) ? Color.black : mark
                    .getStroke().getColor());
        ...
        for (ExternalGraphic theGraphic : symbolizer.getGraphic()
            ...
            if (format.contains("svg")) { //$NON-NLS-1$
                ...
                drawGraphicFillPolygon(node, symbolizer.getGraphic().getSize(),
                        g2, this.getLayer().getOpacity(), widthDrawn,
                        widthDrawn, rowHeight / 2 - widthDrawn / 2, 0);
                ...
                g2.drawImage(onlineImage, this.margin + currentColumn
                        * (columnsWidth + this.margin) + columnsWidth / 2
                        - onlineImage.getWidth(null) / 2, this.margin
                        + currentRow * (rowHeight + this.margin) + rowHeight
    ...
    private void paintPolygon(PolygonSymbolizer symbolizer, Graphics2D g2,
        ...
        if (symbolizer.getShadow() != null) {
            ...
            g2.fillRect(currentColumn * (columnsWidth + this.margin)
                    + this.margin, currentRow * (rowHeight + this.margin)
                    + this.margin, columnsWidth, rowHeight);
        }
        if (symbolizer.getFill() != null) {
            if (symbolizer.getFill().getFill2DDescriptor() != null) {
                if (PerlinNoiseTextureDescriptor.class
                        .isAssignableFrom(symbolizer.getFill()
                                .getFill2DDescriptor().getClass())) {
                    PerlinNoiseTextureDescriptor texture = (PerlinNoiseTextureDescriptor) (symbolizer
                            .getFill().getFill2DDescriptor());
                    TextureFilter filter = new TextureFilter();
                    filter.setColormap(new LinearColormap(texture.getColor1()
                            .getRGB(), texture.getColor2().getRGB()));
                    ...
                    BufferedImage textureImg = new BufferedImage(columnsWidth,
                            rowHeight, BufferedImage.TYPE_INT_ARGB);
                ...
                for (Graphic graphic : graphicList) {
                    if (graphic.getExternalGraphics().size() != 0) {
                        for (ExternalGraphic external : graphic
                            ...
                            if (external.getFormat().contains("png") || external.getFormat().contains("gif")) { //$NON-NLS-1$ //$NON-NLS-2$
                                Image image = external.getOnlineResource();
                                drawGraphicFillPolygon(image,
                                        graphic.getSize(), g2, this.getLayer()
                                                .getOpacity(), widthDrawn,
                                        rowHeight, 0, 0);
                            } else {
                                if (external.getFormat().contains("svg")) { //$NON-NLS-1$
                                    GraphicsNode node = external
                        ...
                        for (Mark mark : graphic.getMarks()) {
                            ...
                            drawGraphicFillPolygon(buff, graphic.getSize(), g2,
                                    this.getLayer().getOpacity(), widthDrawn,
                                    rowHeight, 0, 0);
                        }
                    }
                }
        ...
        if (symbolizer.getStroke() != null) {
            ...
            if (symbolizer.getStroke().getGraphicType() != null) {
                if (GraphicFill.class.isAssignableFrom(symbolizer.getStroke()
                        .getGraphicType().getClass())) {
                    GraphicFill graphicFill = (GraphicFill) symbolizer
                    ...
                    for (Graphic graphic : graphicList) {
                        if (graphic.getExternalGraphics().size() != 0) {
                            ...
                            for (Mark mark : graphic.getMarks()) {
                                ...
                                // adéquate
                                drawGraphicFillPolygon(buff, graphic.getSize(),
                                        g2, this.getLayer().getOpacity(),
                                        widthDrawn, symbolizer.getStroke()
                                                .getStrokeWidth() * 2,
                    ...
                    g2.drawRect(currentColumn * (columnsWidth + this.margin)
                            ...
                            * (rowHeight + this.margin) + this.margin,
                            columnsWidth, rowHeight);
                }
            }
        }
    ...
    private void paintRaster(RasterSymbolizer symbolizer, Graphics2D g2,
        ...
        for (IFeature feature : this.layer.get().getFeatureCollection()) {
            ...
            BufferedImage image = PlanarImage.wrapRenderedImage(
                    coverage.coverage().getRenderedImage())
                    .getAsBufferedImage();
            BufferedImage thumb = image.getSubimage(0, 0,
