...
public final class RenderUtil {
    ...
    private static void texturePolygon(Texture texture, IFeature feature, Shape shape, Viewport viewport, Graphics2D graphics, BufferedImage img, double opacity) {
        ...
        if (imgTexture != null) {
            ...
            case VIEWPORTSPACE: {
                try {
                    double pixelscaleX = texture.getDimension().getWidth() / texture.getTextureWidth() * viewport.getScale();
                    double pixelScaleY = texture.getDimension().getHeight() / texture.getTextureHeight() * viewport.getScale();
                    ...
                    Point2D.Double displacedUpper = new Point2D.Double(envelope.getUpperCorner().getX() + texture.getDisplacement().getX(), envelope
                            .getUpperCorner().getY() + texture.getDisplacement().getY());
                    viewport.getModelToViewTransform().transform(displacedUpper, upper);
                    Point2D.Double displacedLower = new Point2D.Double(envelope.getLowerCorner().getX() + texture.getDisplacement().getX(), envelope
                            .getLowerCorner().getY() + texture.getDisplacement().getY());
     ...
     *            texture to draw
     */
    private static void drawTexture(IFeature feature, Shape shape, Viewport viewport, Graphics2D graphics, double opacity, BufferedImage imgTexture,
            AffineTransform localTransform, boolean xRepeat, boolean yRepeat) {
        graphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) opacity));
        if (xRepeat || yRepeat) {
