...
public final class RenderUtil {
    ...
    public static void paint(PointSymbolizer symbolizer, IFeature feature, Viewport viewport, Graphics2D graphics, double opacity) {
            ...
            if (symbolizer.getColorMap() != null) {
                try {
                    graphics.setColor(ColorUtil.getColorWithOpacity(
                            new Color(symbolizer.getColorMap().getColor((Double.parseDouble(feature.getAttribute(symbolizer.getColorMap().getPropertyName()).toString())))), opacity));
        ...
        for (ExternalGraphic theGraphic : symbolizer.getGraphic().getExternalGraphics()) {
            if (theGraphic.getFormat().contains("svg")) { //$NON-NLS-1$
                ...
                // * Math.PI / 180.0);
                at.rotate(-Double.parseDouble(symbolizer.getGraphic().getRotation().evaluate(feature).toString()) * Math.PI / 180.0);
                double ratio = node.getBounds().getWidth() / node.getBounds().getHeight();
                ...
                size *= scale;
                AffineTransform at = AffineTransform.getRotateInstance(-Double.parseDouble(symbolizer.getGraphic().getRotation().evaluate(feature).toString()) * Math.PI / 180.0);
                at.scale(size / onlineImage.getHeight(null), size / onlineImage.getHeight(null));
                AffineTransformOp op = new AffineTransformOp(at, AffineTransformOp.TYPE_BILINEAR);
                graphics.drawImage((BufferedImage) onlineImage, op, (int) (point.getX() - ((((double) onlineImage.getWidth(null)) / ((double) onlineImage.getHeight(null))) * (size / 2))),
    ...
    public static void paint(LineSymbolizer symbolizer, IFeature feature, Viewport viewport, Graphics2D graphics, double opacity) {
        ...
        if (symbolizer.getStroke().getColor(feature) != null) {
            ...
            if (shapes != null) {
                ...
                if (symbolizer.getColorMap() != null) {
                    try {
                        Color c = new Color(symbolizer.getColorMap().getColor((Double.parseDouble(feature.getAttribute(symbolizer.getColorMap().getPropertyName()).toString()))));
                    ...
                    Color adaptedColorTransp = new Color(adaptedColor.getRed(), adaptedColor.getGreen(), adaptedColor.getBlue(), (int) (symbolizer.getStroke().getStrokeOpacity() * 255f));
        ...
        if (symbolizer.getStroke().getGraphicType() != null) {
            ...
            } else if (symbolizer.getStroke().getGraphicType().getClass().isAssignableFrom(GraphicStroke.class)) {
                // GraphicStroke
            ...
            } else if (symbolizer.getStroke().getGraphicType().getClass().isAssignableFrom(GradientStroke.class)) {
                // GradientStroke
                List<Shape> shapes = getShapeList(symbolizer, geometry, viewport, false);
                if (shapes != null) {
                    for (Shape shape : shapes) {
                        ...
                        GradientStroke gradient = ((GradientStroke) symbolizer.getStroke().getGraphicType());
                        Color color1 = gradient.getColor1(feature);
                        Color color2 = gradient.getColor2(feature);

                        GradientPaint redtowhite = new GradientPaint(xMin, yMin, color1, xMax, yMax, color2);
    ...
    private static List<Shape> getShapeList(LineSymbolizer symbolizer, IGeometry geometry, Viewport viewport, boolean fill) {
        ...
        if (geometry.isMultiCurve()) {
            ...
            for (IOrientableCurve line : (IMultiCurve<IOrientableCurve>) geometry) {
                if (symbolizer.getPerpendicularOffset() != 0) {
                    IMultiCurve<ILineString> offsetCurve = JtsAlgorithms.offsetCurve((ILineString) line, symbolizer.getPerpendicularOffset() * scaleSymbolizerUOMToDataUOM);
                    for (ILineString l : offsetCurve) {
                        shapes.addAll(getLineStringShapeList(symbolizer.getStroke().getStrokeWidth(), l, viewport, fill, scaleSymbolizerUOMToDataUOM));
                    }
                } else {
    ...
    /**
     * @param data
     *            .getSymbolizer() a line symbolizer
     * @param line
    ...
    private static void graphicFillLineString(Shape shape, Graphic graphic, Viewport viewport, Graphics2D graphics, double opacity) {
        ...
        for (int i = 0; i < xSize; i++) {
            for (int j = 0; j < ySize; j++) {
                AffineTransform transform = AffineTransform.getTranslateInstance((i + 0.5) * size + shape.getBounds2D().getMinX(), (j + 0.5) * size + shape.getBounds2D().getMinY());
        ...
        if (fillColor != null && fillOpacity > 0f) {
            ...
            for (Shape shape : shapes) {
                ...
                if (symbolizer.getFill().getGraphicFill() != null) {
                    ...
                    texturePolygon(textureparam, feature, shape, viewport, graphics, img, opacity);
                }
            }
        }
        if (symbolizer.getStroke() != null) {
            ...
            if (strokeOpacity > 0f) {
                ...
                if (geometry.isPolygon()) {
                    drawPolygon(symbolizer, feature, (IPolygon) geometry, viewport, graphics, bs, opacity);
                } else {
                    if (GM_MultiSurface.class.isAssignableFrom(geometry.getClass())) {
                        for (IOrientableSurface surface : ((GM_MultiSurface<?>) geometry)) {
                            drawPolygon(symbolizer, feature, (IPolygon) surface, viewport, graphics, bs, opacity);
                        }
                    }
                }
            }
    ...
    private static void texturePolygon(Texture texture, IFeature feature, Shape shape, Viewport viewport, Graphics2D graphics, BufferedImage img, double opacity) {
        ;
        GLTexture builtTex = TextureManager.getTexture(texture, feature.getFeatureCollection(0), viewport);
        ;
        if (builtTex.getClass().isAssignableFrom(BasicTexture.class)) {
            ...
            if (builtTex != null && tex.getTextureImage() != null) {
                ...
                double pixelScaleX = envelope.width() / imgTexture.getWidth() * viewport.getScale();
                double pixelScaleY = envelope.length() / imgTexture.getHeight() * viewport.getScale();
                switch (texture.getTextureDrawingMode()) {
                case VIEWPORTSPACE: {
                    try {
                        Point2D upper = new Point2D.Double();
                        Point2D lower = new Point2D.Double();
                        Point2D.Double displacedUpper = new Point2D.Double(envelope.getUpperCorner().getX() + texture.getDisplacement().getX(), envelope.getUpperCorner().getY()
                                + texture.getDisplacement().getY());
                        viewport.getModelToViewTransform().transform(displacedUpper, upper);
                        Point2D.Double displacedLower = new Point2D.Double(envelope.getLowerCorner().getX() + texture.getDisplacement().getX(), envelope.getLowerCorner().getY()
                                + texture.getDisplacement().getY());
                        viewport.getModelToViewTransform().transform(displacedLower, lower);
                        ...
                        transform.scale(pixelScaleX * texture.getScaleFactor().getX(), pixelScaleY * texture.getScaleFactor().getY());
                        transform.rotate(texture.getRotation().getAngleInRadians());
                        drawTexture(feature, shape, viewport, graphics, opacity, imgTexture, transform, xRepeat, yRepeat);
                    } catch (NoninvertibleTransformException e) {
                        logger.error(e);
                        e.printStackTrace();
                    }
     ...
     *            texture to draw
     */
    private static void drawTexture(IFeature feature, Shape shape, Viewport viewport, Graphics2D graphics, double opacity, BufferedImage imgTexture, AffineTransform localTransform, boolean xRepeat,
            boolean yRepeat) {
        graphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) opacity));
        if (xRepeat || yRepeat) {
    ...
    private static void fillPolygon(PolygonSymbolizer symbolizer, Shape shape, Viewport viewport, Graphics2D graphics, double opacity) {
        ...
        if (symbolizer.getFill() != null) {
            ...
            Color color = new Color(symbolizerColorComponenents[0], symbolizerColorComponenents[1], symbolizerColorComponenents[2], symbolizerColorComponenents[3] * (float) opacity);
            graphics.setColor(color);
        }
        graphics.fill(shape);
    ...
    private static void drawPolygon(PolygonSymbolizer symbolizer, IFeature feature, IPolygon polygon, Viewport viewport, Graphics2D graphics, BasicStroke stroke, double opacity) {
    ...
    public static void paint(RasterSymbolizer symbolizer, IFeature obj, Viewport viewport, Graphics2D graphics, double opacity) {
        ...
        try {
            ...
            GridCoverageRenderer renderer = new GridCoverageRenderer(coverage.getCoordinateReferenceSystem(), renderEnvelope, viewport.getLayerViewPanels().iterator().next().getVisibleRect(), null);
            ...
            if (symbolizer.getColorMap() != null) {
                ...
                if (symbolizer.getColorMap().isInterpolate()) {
                    ...
                    type = 1;
                    labels = new String[symbolizer.getColorMap().getInterpolate().getNbInterpolationPoint()];
                    quantities = new double[symbolizer.getColorMap().getInterpolate().getNbInterpolationPoint()];
                    colors = new Color[symbolizer.getColorMap().getInterpolate().getNbInterpolationPoint()];

                    for (int i = 0; i < symbolizer.getColorMap().getInterpolate().getNbInterpolationPoint(); i++) {
                        labels[i] = "";
                        quantities[i] = symbolizer.getColorMap().getInterpolate().getInterpolationPoint(i).getData();
                        colors[i] = symbolizer.getColorMap().getInterpolate().getInterpolationPoint(i).getColor();
                    }
                } else if (symbolizer.getColorMap().isIntervals()) {
                    // "intervals" mode in geotools
                    type = 2;
                    labels = new String[symbolizer.getColorMap().getIntervals().getNbIntervalsPoint()];
                    quantities = new double[symbolizer.getColorMap().getIntervals().getNbIntervalsPoint()];
                    colors = new Color[symbolizer.getColorMap().getIntervals().getNbIntervalsPoint()];

                    for (int i = 0; i < symbolizer.getColorMap().getIntervals().getNbIntervalsPoint(); i++) {
                        labels[i] = "";
                        quantities[i] = symbolizer.getColorMap().getIntervals().getIntervalsPoint(i).getData();
                        colors[i] = symbolizer.getColorMap().getIntervals().getIntervalsPoint(i).getColor();
                    }
                } else if (symbolizer.getColorMap().isCategorize()) {
                    // "intervals" mode in geotools
                    type = 3;
                    labels = new String[symbolizer.getColorMap().getCategorize().getNbCategorizePoint()];
                    quantities = new double[symbolizer.getColorMap().getCategorize().getNbCategorizePoint()];
                    colors = new Color[symbolizer.getColorMap().getCategorize().getNbCategorizePoint()];

                    for (int i = 0; i < symbolizer.getColorMap().getCategorize().getNbCategorizePoint(); i++) {
                        labels[i] = "";
                        quantities[i] = symbolizer.getColorMap().getCategorize().getThreshold(i);
     ...
     */
    private static void paint(LinePlacement linePlacement, String text, Color fillColor, Color haloColor, float haloRadius, IGeometry geometry, Viewport viewport, Graphics2D graphics, double scale)
            throws NoninvertibleTransformException {
        ...
        Stroke s = new TextStroke(text, graphics.getFont(), false, linePlacement.isRepeated(), false, linePlacement.getInitialGap() * (float) scale, linePlacement.getGap() * (float) scale);
     ...
     * @throws NoninvertibleTransformException
     */
    private static void paint(PointPlacement pointPlacement, String text, Color fillColor, Color haloColor, float haloRadius, IDirectPosition position, Viewport viewport, Graphics2D graphics,
        ...
        for (DiagramSymbolizer s : symbolizer.getSymbolizers()) {
            ...
            if (s.getDiagramType().equalsIgnoreCase("piechart")) { //$NON-NLS-1$
                ...
                for (ThematicClass thematicClass : s.getThematicClass()) {
                    ...
                    graphics.fillArc((int) (point.getX() - size), (int) (point.getY() - size), (int) (2 * size), (int) (2 * size), (int) startAngle, (int) arcAngle);
                    graphics.setColor(Color.BLACK);
                    graphics.drawArc((int) (point.getX() - size), (int) (point.getY() - size), (int) (2 * size), (int) (2 * size), (int) startAngle, (int) arcAngle);
                    startAngle += arcAngle;
                }
                ...
                /*
                 * int width = (int) (2 * part * size /
                 * s.getThematicClass().size()); int startX = 0; for
                 * (ThematicClass thematicClass : s.getThematicClass()) { double
                 * value = ((Number)
                 * thematicClass.getClassValue().evaluate(feature
                 * )).doubleValue(); int hauteur = (int) (value * 2 * part *
                 * size / 100);
                 ...
                 * (int)(point.getY() + part*size) - hauteur, 2 * width,
                 * hauteur);
                 * 
                 * startX += width; }
                 */
