...
public final class RenderUtil {
    ...
    public static void paint(PointSymbolizer symbolizer, IFeature feature,
        ...
        for (Mark mark : symbolizer.getGraphic().getMarks()) {
            ...
            double scale = 1;
            if (!symbolizer.getUnitOfMeasure().equalsIgnoreCase(
                    Symbolizer.PIXEL)) {
                try {
            ...
            if (symbolizer.getColorMap() != null) {
                ...
                Object value = feature.getAttribute(symbolizer
                        .getCategorizedMap().getPropertyName());
                int rgb = symbolizer.getCategorizedMap().getColor(value);
                graphics.setColor(ColorUtil.getColorWithOpacity(new Color(rgb),
                        opacity));
            } else {
                Color color = (mark.getFill() == null) ? Color.gray : mark
                        .getFill().getColor();
        ...
        for (ExternalGraphic theGraphic : symbolizer.getGraphic()
            ...
            if (theGraphic.getFormat().contains("svg")) { //$NON-NLS-1$
                ...
                graphics.drawImage((BufferedImage) onlineImage, op,
                        (int) (point.getX() - ((((double) onlineImage
                                .getWidth(null)) / ((double) onlineImage
                                .getHeight(null))) * (size / 2))), (int) (point
                                .getY() - size / 2));
            }
        }
    }
            ...
            if (shapes != null) {
                ...
                if (symbolizer.getColorMap() != null) {
                    try {
                        Color c = new Color(
                                symbolizer
                                        .getColorMap()
                                        .getColor(
                                                (Double.parseDouble(feature
                                                        .getAttribute(
                                                                symbolizer
                                                                        .getColorMap()
                                                                        .getPropertyName())
                                                        .toString()))));
                        graphics.setColor(ColorUtil.getColorWithOpacity(c,
                                opacity));
                    } catch (NumberFormatException e) {
                        e.printStackTrace();
                    }
                    ...
                    Object value = feature.getAttribute(symbolizer
                            .getCategorizedMap().getPropertyName());
                    int rgb = symbolizer.getCategorizedMap().getColor(value);
                    graphics.setColor(ColorUtil.getColorWithOpacity(new Color(
                            rgb), opacity));
                    ...
                    for (Shape shape : shapes) {
                        ...
                        if (feature != null) {
                            ...
                            try {
                                ...
                                point_final = viewport.toViewPoint(coord
                                        .get(coord.size() - 1));
                            } catch (NoninvertibleTransformException e) {
                                e.printStackTrace();
                            }
                        ...
                        GradientPaint redtowhite = new GradientPaint(xMin,
                                yMin, color1, xMax, yMax, color2);
    ...
    private static List<Shape> getShapeList(LineSymbolizer symbolizer,
        ...
        if (symbolizer.getUnitOfMeasure().equalsIgnoreCase(Symbolizer.PIXEL)) {
            try {
                scaleSymbolizerUOMToDataUOM = 1 / viewport
        ...
        if (ICurve.class.isAssignableFrom(geometry.getClass())
            ...
            return getLineStringShapeList(symbolizer.getStroke()
                    .getStrokeWidth(), curve, viewport, fill,
                    scaleSymbolizerUOMToDataUOM);
        }
                    ...
                    IMultiCurve<ILineString> offsetCurve = JtsAlgorithms
                            .offsetCurve((ILineString) line,
                                    symbolizer.getPerpendicularOffset()
                                            * scaleSymbolizerUOMToDataUOM);
                    for (ILineString l : offsetCurve) {
                        shapes.addAll(getLineStringShapeList(symbolizer
                                .getStroke().getStrokeWidth(), l, viewport,
            ...
            for (IOrientableSurface surface : ((IMultiSurface<IOrientableSurface>) geometry)
                ...
                try {
                    Shape shape = viewport
                            .toShape(fill ? surface.buffer(symbolizer
    ...
    private static List<Shape> getShapeList(PolygonSymbolizer symbolizer,
        ...
        if (ICurve.class.isAssignableFrom(geometry.getClass())
            ...
            // JtsAlgorithms.offsetCurve(curve,
            // symbolizer.getPerpendicularOffset() *
            // scaleSymbolizerUOMToDataUOM);
            // List<Shape> shapes = new ArrayList<Shape>();
            // for (ILineString l : offsetCurve) {
    ...
    private static void paintShadow(LineSymbolizer symbolizer,
        ...
        if (symbolizer.getShadow() != null) {
            Color shadowColor = ColorUtil.getColorWithOpacity(symbolizer
                    .getShadow().getColor(), opacity);
            ...
            if (geometry.isLineString()) {
                try {
                    Shape shape = viewport.toShape(geometry.translate(
                ...
                if (geometry.isMultiCurve()) {
                    for (GM_OrientableCurve line : (GM_MultiCurve<GM_OrientableCurve>) geometry) {
                        try {
                            Shape shape = viewport.toShape(line.translate(
                                    translate_x, translate_y, 0));
                            if (shape != null) {
                                shapes.add(shape);
                            }
    ...
    private static void graphicFillLineString(Shape shape, Graphic graphic,
        ...
        for (Mark mark : graphic.getMarks()) {
            ...
            graphics.setColor(ColorUtil.getColorWithOpacity(mark.getFill()
                    .getColor(), opacity));
        }
        ...
        for (int i = 0; i < xSize; i++) {
            for (int j = 0; j < ySize; j++) {
                AffineTransform transform = AffineTransform
                        .getTranslateInstance((i + 0.5) * size
                                + shape.getBounds2D().getMinX(), (j + 0.5)
                                * size + shape.getBounds2D().getMinY());
    ...
    public static void paint(PolygonSymbolizer symbolizer, IFeature feature,
        ...
        if (symbolizer.getFill() != null
            ...
            fillColor = ColorUtil.getColorWithOpacity(symbolizer.getFill()
                    .getColor(), opacity);
        ...
        if (fillColor != null && fillOpacity > 0f) {
            ...
            for (Shape shape : shapes) {
                ...
                } else if (symbolizer.getFill().getFill2DDescriptor() != null
                        && symbolizer.getFill().getFill2DDescriptor() instanceof TextureDescriptor) {
                    texturePolygon(
                            (TextureDescriptor) (symbolizer.getFill()
                                    .getFill2DDescriptor()),
        ...
        if (symbolizer.getStroke() != null) {
            ...
            if (strokeOpacity > 0f) {
                ...
                // Solid color
                Color color = ColorUtil.getColorWithOpacity(symbolizer
                        .getStroke().getColor(feature), opacity);
                graphics.setColor(color);
                if (feature.getGeom().isPolygon()) {
                    drawPolygon(symbolizer, feature,
                            (GM_Polygon) feature.getGeom(), viewport, graphics,
                            bs, opacity);
                } else {
                    if (GM_MultiSurface.class.isAssignableFrom(feature
                            .getGeom().getClass())) {
                        for (IOrientableSurface surface : ((GM_MultiSurface<?>) feature
                                .getGeom())) {
                            drawPolygon(symbolizer, feature,
                                    (GM_Polygon) surface, viewport, graphics,
                                    bs, opacity);
                        }
                    }
                }
        ...
        if (imgTexture != null) {
            ...
            case VIEWPORTSPACE: {
                try {
                    ...
                    viewport.getModelToViewTransform().transform(
                            displacedUpper, upper);
    ...
    private static void graphicFillPolygon(Shape shape, Image image,
            float size, Graphics2D graphics, double opacity) {
        Double width = new Double(Math.max(1, shape.getBounds2D().getWidth()));
        Double height = new Double(Math.max(1, shape.getBounds2D().getHeight()));
        Double shapeHeight = new Double(size);
        double factor = shapeHeight / image.getHeight(null);
        Double shapeWidth = new Double(Math.max(image.getWidth(null) * factor,
                1));
        AffineTransform transform = AffineTransform.getTranslateInstance(shape
                .getBounds2D().getMinX(), shape.getBounds2D().getMinY());
        Image scaledImage = image.getScaledInstance(shapeWidth.intValue(),
            ...
            if (PointPlacement.class.isAssignableFrom(placement.getClass())) {
                ...
                try {
                    paint(pointPlacement, text, fillColor, haloColor,
                            haloRadius, geometry.centroid(), viewport,
                            graphics, scaleUOMToPixels);
                } catch (NoninvertibleTransformException e) {
                    e.printStackTrace();
                }
            } else {
                if (LinePlacement.class.isAssignableFrom(placement.getClass())) {
                    ...
                    if (IMultiCurve.class.isAssignableFrom(g.getClass())) {
                        ...
                        for (IOrientableCurve curve : multiCurve) {
                            try {
                                paint(linePlacement, text, fillColor,
                                        haloColor, haloRadius, curve, viewport,
                                        graphics, scaleUOMToPixels);
                            } catch (NoninvertibleTransformException e) {
                                e.printStackTrace();
                            }
                ...
                if (position == null || size == null) {
                    ...
                    for (Arc a : t.getPopArcs()) {
                        ((IPopulation<IFeature>) DataSet.getInstance()
                                .getPopulation("Triangulation")).add(new DefaultFeature(a.getGeometrie())); //$NON-NLS-1$
                    }
                    double maxDistance = Double.MIN_VALUE;
                    Noeud maxNode = null;
                    for (Arc a : t.getPopVoronoiEdges().select(
                            feature.getGeom())) {
                        if (!a.getGeometrie().intersectsStrictement(
                                feature.getGeom())) {
                    ...
                    for (Noeud n : t.getPopVoronoiVertices().select(
                            feature.getGeom())) {
