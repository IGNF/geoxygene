...
public final class RenderUtil {
    ...
    if (symbolizer.getShadow() != null) {
      ...
      if (feature.getGeom().isPolygon()) {
        ...
        if (GM_MultiSurface.class
            .isAssignableFrom(feature.getGeom().getClass())) {
          try {
            Shape shape = viewport.toShape(feature.getGeom().translate(
                translate_x, translate_y, 0));
            if (shape != null) {
    ...
    if (fillColor != null && fillOpacity > 0f) {
      ...
      for (Shape shape : shapes) {
        ...
        if (symbolizer.getFill().getGraphicFill() != null) {
          ...
          texturePolygon(symbolizer.getFill().getTexture(), feature, shape,
              viewport, graphics, img, opacity);
        }
      }
    }
    if (symbolizer.getStroke() != null) {
  ...
  private static void texturePolygon(Texture texture, IFeature feature,
    ...
    BufferedImage imgTexture = TextureManager.getInstance().getTextureImage(
        texture, feature.getFeatureCollection(0), viewport);

    // draw the texture image into resulting image
    if (imgTexture != null) {
      switch (texture.getTextureDrawingMode()) {
        case VIEWPORTSPACE:
          drawTextureModelSpaceCoordinates(feature, shape, viewport, graphics,
              opacity, imgTexture);
          break;
        case SCREENSPACE:
          drawTextureScreenspaceCoordinates(feature, shape, viewport, graphics,
              opacity, imgTexture);
          break;
        default:
   ...
   */
  private static void drawTextureScreenspaceCoordinates(IFeature feature,
      Shape shape, Viewport viewport, Graphics2D graphics, double opacity,
  ...
  private static void drawTextureModelSpaceCoordinates(IFeature feature,
      Shape shape, Viewport viewport, Graphics2D graphics, double opacity,
      BufferedImage imgTexture) {
    try {
      ...
      // FIXME: what do we do if the feature belongs to multiple feature
      // collections ??? (how to know which one is currently drawn??)
      viewport.getModelToViewTransform().transform(
          Viewport.toPoint2D(envelope.getUpperCorner()), upper);
      viewport.getModelToViewTransform().transform(
          Viewport.toPoint2D(envelope.getLowerCorner()), lower);
      ...
      screenTransform.scale(viewport.getModelToViewTransform().getScaleX()
          * textureXScale, viewport.getModelToViewTransform().getScaleY()
          * textureYScale);
      graphics.setClip(shape);
      graphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
          (float) opacity));
      ...
      // graphics.drawRect((int) lower.getX(), (int) upper.getY(), (int)
      // (upper.getX() - lower.getX()), (int) (lower.getY() - upper.getY()));
    } catch (NoninvertibleTransformException e) {
      e.printStackTrace();
    }
  }
