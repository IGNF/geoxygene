...
public final class RenderUtil {
            ...
            for (int i = 0; i < polygon.sizeInterior(); i++) {
                try {
                    Shape shape = viewport
                            .toShape(polygon.interiorLineString(i));
                    if (shape != null) {
                        graphics.draw(shape);
                    }
                } catch (NoninvertibleTransformException e) {
    ...
    public static void paint(PointSymbolizer symbolizer, IFeature feature,
        ...
        for (Mark mark : symbolizer.getGraphic().getMarks()) {
            ...
            AffineTransform at = AffineTransform
                    .getTranslateInstance(point.getX(), point.getY());
            at.rotate(-Double.parseDouble(symbolizer.getGraphic().getRotation()
                    .evaluate(feature).toString()) * Math.PI / 180.0);
            at.scale(size, size);
            ...
            if (symbolizer.getColorMap() != null) {
                try {
                    graphics.setColor(ColorUtil.getColorWithOpacity(
                            new Color(symbolizer.getColorMap()
                ...
                graphics.setColor(
                        ColorUtil.getColorWithOpacity(new Color(rgb), opacity));
            } else {
                Color color = (mark.getFill() == null) ? Color.gray
                        : mark.getFill().getColor();
                graphics.setColor(
                        ColorUtil.getColorWithOpacity(color, opacity));
            }
        ...
        for (ExternalGraphic theGraphic : symbolizer.getGraphic()
            ...
            if (theGraphic.getFormat().contains("svg")) { //$NON-NLS-1$
                ...
                // * Math.PI / 180.0);
                at.rotate(-Double.parseDouble(symbolizer.getGraphic()
                        .getRotation().evaluate(feature).toString()) * Math.PI
                        / 180.0);
                ...
                AffineTransform at = AffineTransform.getRotateInstance(
                        -Double.parseDouble(symbolizer.getGraphic()
                ...
                graphics.drawImage((BufferedImage) onlineImage, op, (int) (point
                        .getX()
                        - ((((double) onlineImage.getWidth(null))
                                / ((double) onlineImage.getHeight(null)))
                                * (size / 2))),
    ...
    public static IGeometry getGeometry(String propertyName, IFeature feature) {
        ...
        try {
            ...
            try {
                return WktGeOxygene.makeGeOxygene(
                        feature.getAttribute(propertyName).toString());
            } catch (Exception e1) {
                e1.printStackTrace();
                return null;
            }
    ...
    public static void paint(LineSymbolizer symbolizer, IFeature feature,
        ...
        if (!symbolizer.getUnitOfMeasure().equalsIgnoreCase(Symbolizer.PIXEL)) {
            try {
                scaleUOMToPixels = viewport.getModelToViewTransform()
                        .getScaleX();
            } catch (NoninvertibleTransformException e) {
                e.printStackTrace();
            }
                ...
                if (symbolizer.getColorMap() != null) {
                    try {
                        Color c = new Color(symbolizer.getColorMap()
                                .getColor((Double.parseDouble(feature
                                        .getAttribute(symbolizer.getColorMap()
                                                .getPropertyName())
                                        .toString()))));
                        graphics.setColor(
                                ColorUtil.getColorWithOpacity(c, opacity));
                    } catch (NumberFormatException e) {
                        e.printStackTrace();
                    }
                    // Categorized Map
                    ...
                    Color adaptedColor = new Color(
                            Integer.parseInt(feature
                                    .getAttribute(symbolizer.getProxySymbol()
                                            .getProxyColorPropertyName())
                                    .toString()));
                    ...
                    Color adaptedColorTransp = new Color(adaptedColor.getRed(),
                            ...
                            (int) (symbolizer.getStroke().getStrokeOpacity()
                                    * 255f));
                    // Color with stroke and layer opacity
                    graphics.setColor(ColorUtil
                            .getColorWithOpacity(adaptedColorTransp, opacity));
        ...
        if (symbolizer.getStroke().getGraphicType() != null) {
            if (symbolizer.getStroke().getGraphicType().getClass()
                ...
                List<Shape> shapes = getShapeList(symbolizer, geometry,
                        viewport, true);
                // GraphicFill
                List<Graphic> graphicList = ((GraphicFill) symbolizer
                        .getStroke().getGraphicType()).getGraphics();
                for (Graphic graphic : graphicList) {
                    for (Shape shape : shapes) {
                        graphicFillLineString(shape, graphic, viewport,
                                graphics, opacity);
                    }
                }
                ...
                List<Shape> shapes = getShapeList(symbolizer, geometry,
                        viewport, false);
                if (shapes != null) {
                    List<Graphic> graphicList = ((GraphicStroke) symbolizer
                            .getStroke().getGraphicType()).getGraphics();
                    for (Graphic graphic : graphicList) {
                        for (Shape shape : shapes) {
                            graphicStrokeLineString(shape, graphic, viewport,
                                    graphics, opacity, space);
                        }
                    }
                }
            } else if (symbolizer.getStroke().getGraphicType().getClass()
                ...
                if (shapes != null) {
                    for (Shape shape : shapes) {
                        ...
                        if (feature != null) {
                            IDirectPositionList coord = feature.getGeom()
                                    .coord();
                            Point2D point_init = null;
                            Point2D point_final = null;
                            try {
                                point_init = viewport.toViewPoint(coord.get(0));
                                point_final = viewport.toViewPoint(
                                        coord.get(coord.size() - 1));
                            } catch (NoninvertibleTransformException e) {
                                e.printStackTrace();
                            }
                        ...
                        GradientStroke gradient = ((GradientStroke) symbolizer
                                .getStroke().getGraphicType());
                        Color color1 = gradient.getColor1(feature);
                        Color color2 = gradient.getColor2(feature);

                        GradientPaint redtowhite = new GradientPaint(xMin, yMin,
                                color1, xMax, yMax, color2);
    ...
    private static List<Shape> getShapeList(LineSymbolizer symbolizer,
        ...
        if (ICurve.class.isAssignableFrom(geometry.getClass())
            ...
            if (symbolizer.getPerpendicularOffset() != 0) {
                IMultiCurve<ILineString> offsetCurve = JtsAlgorithms
                        .offsetCurve(curve, symbolizer.getPerpendicularOffset()
                                * scaleSymbolizerUOMToDataUOM);
                List<Shape> shapes = new ArrayList<Shape>();
                for (ILineString l : offsetCurve) {
                    shapes.addAll(getLineStringShapeList(
                            symbolizer.getStroke().getStrokeWidth(), l,
                            viewport, fill, scaleSymbolizerUOMToDataUOM));
                }
                return shapes;
            }
            return getLineStringShapeList(
                    symbolizer.getStroke().getStrokeWidth(), curve, viewport,
                    fill, scaleSymbolizerUOMToDataUOM);
        }
        if (geometry.isMultiCurve()) {
            ...
            for (IOrientableCurve line : (IMultiCurve<IOrientableCurve>) geometry) {
                if (symbolizer.getPerpendicularOffset() != 0) {
                    ...
                    shapes.addAll(getLineStringShapeList(
                            symbolizer.getStroke().getStrokeWidth(), line,
                            viewport, fill, scaleSymbolizerUOMToDataUOM));
                }
        ...
        if (geometry.isMultiSurface()) {
            ...
            for (IOrientableSurface surface : ((IMultiSurface<IOrientableSurface>) geometry)
                    .getList()) {
                try {
                    Shape shape = viewport.toShape(fill
                            ? surface.buffer(
    ...
    private static List<Shape> getShapeList(PolygonSymbolizer symbolizer,
        ...
        if (geometry.isMultiCurve()) {
            ...
            for (IOrientableCurve line : (IMultiCurve<IOrientableCurve>) geometry) {
                ...
                // } else {
                shapes.addAll(getLineStringShapeList(
                        symbolizer.getStroke().getStrokeWidth(), line, viewport,
                        fill, scaleSymbolizerUOMToDataUOM));
                // }
            }
            return shapes;
    ...
    private static List<Shape> getLineStringShapeList(float strokeWidth,
            IOrientableCurve line, Viewport viewport, boolean fill,
            double scale) {
        List<Shape> shapes = new ArrayList<Shape>();
        try {
            Shape shape = viewport.toShape(
                    fill ? line.buffer(strokeWidth * 0.5 * scale) : line);
            if (shape != null) {
                shapes.add(shape);
            }
        } catch (NoninvertibleTransformException e) {
    ...
    private static void paintShadow(LineSymbolizer symbolizer,
        ...
        if (symbolizer.getShadow() != null) {
            Color shadowColor = ColorUtil.getColorWithOpacity(
                    symbolizer.getShadow().getColor(), opacity);
    ...
    private static void graphicStrokeLineString(Shape shape, Graphic graphic,
        ...
        for (ExternalGraphic external : graphic.getExternalGraphics()) {
            if (external.getFormat().contains("png") //$NON-NLS-1$
                ...
                if (external.getFormat().contains("svg")) { //$NON-NLS-1$
                    GraphicsNode node = external.getGraphicsNode();
                    graphicStrokeLineString(shape, node, size, graphics,
                            opacity);
                }
            }
            return;
    ...
    private static List<AffineTransform> getGraphicStrokeLineStringTransforms(
        ...
        if (space == 0.0) {
            ...
            for (double curv = 0.0; curv < line.length(); curv += space
                    + shapeWidth) {
                IDirectPosition p = Operateurs.pointEnAbscisseCurviligne(line,
                        curv);
                AffineTransform transform = AffineTransform
                        .getTranslateInstance(p.getX(), p.getY());
                transform.concatenate(scaleTransform);
    ...
    public static void paint(PolygonSymbolizer symbolizer, IFeature feature,
        ...
        if (symbolizer.getShadow() != null) {
            ...
            if (geometry.isPolygon()) {
                ...
                if (GM_MultiSurface.class
                        .isAssignableFrom(geometry.getClass())) {
                    try {
                        Shape shape = viewport.toShape(geometry
                                .translate(translate_x, translate_y, 0));
                        if (shape != null) {
        ...
        if (symbolizer.getFill() != null
                && symbolizer.getFill().getColor() != null && opacity > 0f) {
        ...
        if (fillColor != null && fillOpacity > 0f) {
            ...
            for (Shape shape : shapes) {
                ...
                if (symbolizer.getFill().getGraphicFill() != null) {
                    ...
                    for (ExpressiveParameter p : symbolizer.getFill()
                            .getExpressiveFill().getExpressiveParameters()) {
                        if (p.getValue() instanceof Texture) {
                            textureparam = (Texture) p.getValue();
                            break;
                        }
                    }
                    texturePolygon(textureparam, feature, shape, viewport,
        ...
        if (symbolizer.getStroke() != null) {
            ...
            if (strokeOpacity > 0f) {
                ...
                if (geometry.isPolygon()) {
                    drawPolygon(symbolizer, feature, (IPolygon) geometry,
                            viewport, graphics, bs, opacity);
                } else {
                    if (GM_MultiSurface.class
                            .isAssignableFrom(geometry.getClass())) {
                        for (IOrientableSurface surface : ((GM_MultiSurface<?>) geometry)) {
                            drawPolygon(symbolizer, feature, (IPolygon) surface,
                                    viewport, graphics, bs, opacity);
                        }
                    }
                }
            ...
            if (symbolizer.getStroke().getGraphicType() != null) {
                if (symbolizer.getStroke().getGraphicType().getClass()
                    ...
                    if (!symbolizer.getUnitOfMeasure()
                            .equalsIgnoreCase(Symbolizer.PIXEL)) {
                        try {
                            scaleUOMToPixels = viewport
                                    .getModelToViewTransform().getScaleX();
                        } catch (NoninvertibleTransformException e) {
                            e.printStackTrace();
                        }
                    }
                    ...
                    if (shapes != null) {
                        ...
                        for (Graphic graphic : graphicList) {
                            for (Shape shape : shapes) {
                                graphicStrokeLineString(shape, graphic,
                                        viewport, graphics, opacity, space);
                            }
                        }
                    }
                }
        ...
        if (builtTex.getClass().isAssignableFrom(BasicTexture.class)) {
            ...
            if (builtTex != null && tex.getTextureImage() != null) {
                ...
                case VIEWPORTSPACE: {
                    try {
                        ...
                        viewport.getModelToViewTransform()
                                .transform(displacedUpper, upper);
                ...
                case SCREENSPACE: {
                    ...
                    double pixelDisplacementX = texture.getDisplacement()
                            .getX();
                    double pixelDisplacementY = texture.getDisplacement()
                            .getY();
                    AffineTransform transform = new AffineTransform();
    ...
    private static void fillPolygon(PolygonSymbolizer symbolizer, Shape shape,
        ...
        if (symbolizer.getFill() != null) {
            float[] symbolizerColorComponenents = symbolizer.getFill()
                    .getColor().getComponents(null);
    ...
    public static void paint(RasterSymbolizer symbolizer, IFeature obj,
        ...
        try {
            ...
            // style: Opacity
            s.setOpacity((new FilterFactoryImpl())
                    .literal(opacity * symbolizer.getOpacity()));
            ...
            if (symbolizer.getChannelSelection() != null) {
                ...
                if (symbolizer.getChannelSelection().isGrayChannel()) {
                    ...
                    ContrastEnhancement ce = sf.contrastEnhancement(
                            ff.literal(1.0), ContrastMethod.NONE);
                    SelectedChannelType sct = sf.createSelectedChannelType(
                            String.valueOf(symbolizer.getChannelSelection()
                                    .getGrayChannel().getSourceChannelName()),
                            ce);
                    ...
                    sct[0] = sf.createSelectedChannelType(
                            String.valueOf(symbolizer.getChannelSelection()
                                    .getRedChannel().getSourceChannelName()),
                            ce[0]);
                    sct[1] = sf.createSelectedChannelType(
                            String.valueOf(symbolizer.getChannelSelection()
                                    .getGreenChannel().getSourceChannelName()),
                            ce[1]);
                    sct[2] = sf.createSelectedChannelType(
                            String.valueOf(symbolizer.getChannelSelection()
                                    .getBlueChannel().getSourceChannelName()),
                            ce[2]);

                    channelSelection = sf.channelSelection(sct[0], sct[1],
                            sct[2]);
                } else {
            ...
            if (symbolizer.getColorMap() != null) {
                ...
                if (symbolizer.getColorMap().isInterpolate()) {
                    ...
                    type = 1;
                    labels = new String[symbolizer.getColorMap()
                            .getInterpolate().getNbInterpolationPoint()];
                    quantities = new double[symbolizer.getColorMap()
                            .getInterpolate().getNbInterpolationPoint()];
                    colors = new Color[symbolizer.getColorMap().getInterpolate()
                            .getNbInterpolationPoint()];

                    for (int i = 0; i < symbolizer.getColorMap()
                            .getInterpolate().getNbInterpolationPoint(); i++) {
                        labels[i] = "";
                        quantities[i] = symbolizer.getColorMap()
                                .getInterpolate().getInterpolationPoint(i)
                                .getData();
                    ...
                    labels = new String[symbolizer.getColorMap().getIntervals()
                            .getNbIntervalsPoint()];
                    quantities = new double[symbolizer.getColorMap()
                            .getIntervals().getNbIntervalsPoint()];
                    ...
                    quantities = new double[symbolizer.getColorMap()
                            .getCategorize().getNbCategorizePoint()];
                    ...
                    for (int i = 0; i < symbolizer.getColorMap().getCategorize()
                        ...
                        quantities[i] = symbolizer.getColorMap().getCategorize()
                                .getThreshold(i);
                        colors[i] = symbolizer.getColorMap().getCategorize()
                                .getColor(i);
                    }
                } else {
                    System.err.println(
                            "Requested ColorMap mode is not yet implemented");
                }
    ...
    public static void paint(TextSymbolizer symbolizer, IFeature feature,
        ...
        if (symbolizer.getLabelPlacement() != null) {
            if (symbolizer.getLabelPlacement().getPlacement() != null
                    && symbolizer.getLabelPlacement()
                            .getPlacement() instanceof PointPlacement) {
                PointPlacement pl = ((PointPlacement) symbolizer
                        .getLabelPlacement().getPlacement());
                if (pl.getRotation() != null) {
                    double rotationval = pl.getRotation()
                            .getRotationValue(feature);
        ...
        if (text != null) {
            paint(symbolizer, text, feature.getGeom(), viewport, graphics,
                    opacity);
        }
    }
    ...
    public static void paint(TextSymbolizer symbolizer, String text,
        ...
        if (labelPlacement != null && labelPlacement.getPlacement() != null) {
            ...
            if (PointPlacement.class.isAssignableFrom(placement.getClass())) {
                ...
                try {
                    paint(pointPlacement, text, fillColor, haloColor,
                            haloRadius, geometry.centroid(), viewport, graphics,
                            scaleUOMToPixels);
                } catch (NoninvertibleTransformException e) {
                    e.printStackTrace();
                }
            } else {
                if (LinePlacement.class
                        .isAssignableFrom(placement.getClass())) {
                    ...
                    if (IMultiCurve.class.isAssignableFrom(g.getClass())) {
                        ...
                        for (IOrientableCurve curve : multiCurve) {
                            try {
                                paint(linePlacement, text, fillColor, haloColor,
                                        haloRadius, curve, viewport, graphics,
                                        scaleUOMToPixels);
                            } catch (NoninvertibleTransformException e) {
                                e.printStackTrace();
                            }
                        ...
                        try {
                            paint(linePlacement, text, fillColor, haloColor,
                                    haloRadius, g, viewport, graphics,
                                    scaleUOMToPixels);
                        } catch (NoninvertibleTransformException e) {
                            e.printStackTrace();
                        }
    ...
    private static void paint(LinePlacement linePlacement, String text,
        ...
        if (haloColor != null) {
            ...
            graphics.setStroke(new BasicStroke(haloRadius,
        ...
        for (DiagramSymbolizer s : symbolizer.getSymbolizers()) {
            ...
            if (position == null || size == null) {
                ...
                for (Arc a : t.getPopVoronoiEdges().select(feature.getGeom())) {
                    if (!a.getGeometrie()
                            .intersectsStrictement(feature.getGeom())) {
                        ((Population<DefaultFeature>) DataSet.getInstance()
                                .getPopulation("MedialAxis")) //$NON-NLS-1$
                                        .add(new DefaultFeature(
                                                a.getGeometrie()));
                    }
                }
                for (Noeud n : t.getPopVoronoiVertices()
            ...
            if (s.getDiagramType().equalsIgnoreCase("piechart")) { //$NON-NLS-1$
                ...
                for (ThematicClass thematicClass : s.getThematicClass()) {
                    ...
                    graphics.fillArc((int) (point.getX() - size),
                            (int) (point.getY() - size), (int) (2 * size),
                            (int) (2 * size), (int) startAngle, (int) arcAngle);
                    graphics.setColor(Color.BLACK);
                ...
                for (ThematicClass thematicClass : s.getThematicClass()) {
                    ...
                    graphics.setColor(ColorUtil.getColorWithOpacity(
                            thematicClass.getFill().getColor(), opacity));
                    graphics.fillRect(
                            (int) (point.getX() - part * size) + startX,
                            (int) (point.getY() + part * size) - hauteur,
                            2 * width, hauteur);
                ...
                if (s.getDiagramType().equalsIgnoreCase("rosechart90")) {
                    pas = 90 / nbPas;
                } else if (s.getDiagramType()
                        .equalsIgnoreCase("rosechart180")) {
                    pas = 180 / nbPas;
                }
                double angleDegre = 0;
                for (ThematicClass thematicClass : s.getThematicClass()) {
                ...
                graphics.drawArc((int) (x0 + 1 * size / 4),
                        (int) (y0 + 1 * size / 4),
                        (int) (2 * size - 2 * size / 4),
                        (int) (2 * size - 2 * size / 4), 0, 180);
                graphics.drawArc((int) (x0 + 2 * size / 4),
                        (int) (y0 + 2 * size / 4),
                        (int) (2 * size - 4 * size / 4),
                        (int) (2 * size - 4 * size / 4), 0, 180);
                graphics.drawArc((int) (x0 + 3 * size / 4),
                        (int) (y0 + 3 * size / 4),
                        (int) (2 * size - 6 * size / 4),
                        (int) (2 * size - 6 * size / 4), 0, 180);
                // graphics.drawLine((int)(x0 + size), (int)(y0 + size),
                // (int)(x0 +
                // size), (int)(y0));
                graphics.setColor(Color.BLACK);
                graphics.setStroke(new BasicStroke(1.0f));
                graphics.drawArc((int) (x0 + 0 * size / 4),
                        (int) (y0 + 0 * size / 4),
                        (int) (2 * size - 0 * size / 4),
                        (int) (2 * size - 0 * size / 4), 0, 180);
                graphics.drawLine((int) (x0), (int) (y0 + size),
                        (int) (x0 + 2 * size), (int) (y0 + size));
                 ...
                 * value = ((Number)
                 * thematicClass.getClassValue().evaluate(feature)).doubleValue(
                 * ); int hauteur = (int) (value * 2 * part * size / 100);
    ...
    private static void drawArrow(Graphics2D g1, int x1, int y1, int x2,
            int y2) {
