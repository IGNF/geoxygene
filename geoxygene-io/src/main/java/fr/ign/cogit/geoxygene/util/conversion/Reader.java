...
public class Reader {
  ...
  public Reader(String shapefileName) throws MalformedURLException {
    ...
    try {
      shapefileReader = new ShapefileReader(shpf, true, false,
          new GeometryFactory());
      dbaseFileReader = new DbaseFileReader(shpf, true,
    ...
    this.shapeType = Reader.geometryType(shapefileReader.getHeader()
        .getShapeType());
    // LOGGER.log(Level.INFO, "ShapeType = "
    // + shapefileReader.getHeader().getShapeType());
    ...
    for (int i = 0; i < this.nbFields; i++) {
      ...
      // LOGGER.log(Level.INFO, "field " + i + " = " + this.fieldNames[i] + " ("
      // + this.fieldClasses[i] + ")");
    }
    /*
