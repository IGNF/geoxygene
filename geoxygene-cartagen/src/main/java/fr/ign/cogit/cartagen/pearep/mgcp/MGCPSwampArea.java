...
public class MGCPSwampArea extends MGCPFeature implements ISimpleLandUseArea {
  ...
  private long acc, ace_eval, ale_eval, fcsubtype, smc, src_name, upd_name,
      tid, tre, valid_stat, veg, vsp, zval_type, originform, targetscal;
  private double area;

  private MGCPLandUseType landUseType;

  public MGCPSwampArea(IPolygon poly) {
    super();
    this.setGeom(poly);
   ...
   * @param type
   */
  public MGCPSwampArea(IPolygon poly, HashMap<String, Object> attributes,
      PeaRepDbType type) {
    this(poly);
