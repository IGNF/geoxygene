...
public class DataSet implements IDataSet<SchemaConceptuelJeu> {
     ...
     * statique non protégé par un singleton
     */
    ...
    public void chargeElements() {
        if (!this.getPersistant()) {
            DataSet.LOGGER.warn("----- ATTENTION : Probleme au chargement du jeu de donnees " + this.getNom()); //$NON-NLS-1$
            DataSet.LOGGER.warn("----- Impossible de charger les elements d'un jeu de donnees non persistant"); //$NON-NLS-1$
            return;
        }
     ...
     * seules les populations avec une géométrie sont charg�es.
     * 
     * @param zoneExtraction zone utilisée pour l'extraction g�om�trique
     */
    @Override
     ...
     *            et pour chaque sous-DS, on précise également quelles
     *            populations sont chargées. Ce paramètre est une liste de liste
     *            de String composée comme suit (si la liste est nulle on charge
     *            tout) :
     *            <ul>
     ...
     *            et les noeud du thème routier, et les troncons du thème hydro,
     *            mais tout le thème ferré.
     *            <p>
    ...
    public void detruitJeu() {
        if (!this.getPersistant()) {
            DataSet.LOGGER.warn("----- ATTENTION : Probleme à la destruction du jeu de donnees " + this.getNom()); //$NON-NLS-1$
        ...
        // destruction des zones d'extraction associ�es à this
     ...
     * non persistants. Mais si un jeu de données est persistant, certains de
     * ses thèmes peuvent ne pas l'être.
     * <p>
     * <b>ATTENTION :</b> pour des raisons propres à OJB, même si la classe
     * DataSet est concrète, il n'est pas possible de créer un objet PERSISTANT
     * de cette classe, il faut utiliser les sous-classes.
     * 
    ...
    public IDataSet<?> getComposant(String nomComposant) {
        ...
        DataSet.LOGGER.warn("----- ATTENTION : DataSet composant #" + nomComposant + "# introuvable dans le DataSet "
                + this.getNom());
        return null;
    }
    ...
    // ////////////////////////////////////////////////////////////////////////////////////////////
    /**
     * méthodes permettant de cr�er un jeu de données:
     * <ul>
     * <li>relié à un produit, donc potentiellement à de nombreuses m�tadonnées</li>
    ...
    public IPopulation<? extends IFeature> getPopulationByFeatureTypeName(String nomFeatureType) {
        ...
        DataSet.LOGGER.error("La Population " + nomFeatureType + " n'a pas �t� trouv�e."); //$NON-NLS-1$//$NON-NLS-2$
        return null;
    }
