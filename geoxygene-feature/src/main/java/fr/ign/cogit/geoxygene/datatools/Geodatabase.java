...
/**
 * Represente une connection a une base de donnees geographique. Gere la
 * manipulation des donnees via un mappeur objet-relationnel (Castor, OJB).
 * 
...
public interface Geodatabase {
  ...
  // ///////////////////////////////////////////////////////////////
  /**
   * Charge l'objet d'identifiant id. A utilisser avec precaution, car ne charge
   * pas forcement toutes les relations. Passer un Integer pour id, si
  ...
  /**
   * Charge tous les objets persistants de la classe featureClass et les met
   * dans une liste. A appeler a l'interieur d'une transaction ouverte.
   */
  public <T> List<T> loadAll(Class<T> featureClass);

  /**
   * Charge tous les objets persistants de la classe featureClass dans la classe
   * featureListClass. A appeler a l'interieur d'une transaction ouverte.
   */
  public <T> T loadAll(Class<?> featureClass, Class<T> featureListClass);

  /**
   * Charge tous les FT_Feature de la classe featureClass dans la classe
   ...
   * classe featureClass doit etre une sous-classe de FT_Feature, sinon renvoie
   * une liste vide. La classe featureListClass doit etre un sous classe de
   ...
   * GM_Object geom, dans la classe FT_FeatureCollection. A appeler a
  ...
  /**
   * Charge tous les FT_Feature de la classe featureClass intersectant le
   * GM_Object geom, dans la classe featureListClass. A appeler a l'interieur
   * d'une transaction ouverte. La classe featureClass doit etre une sous-classe
   * de FT_Feature, sinon renvoie une liste vide. La classe featureListClass
   ...
   * GM_Object geom, dans la classe FT_FeatureCollection. Si geom est la
   * geometrie d'un FT_Feature de theClass, alors ce FT_Feature appartiendra au
   * resultat. A appeler a l'interieur d'une transaction ouverte. La classe
   ...
   * GM_Object geom, dans la classe featureListClass. Si geom est la geometrie
   * d'un FT_Feature de theClass, alors ce FT_Feature appartiendra au resultat.
   * A appeler a l'interieur d'une transaction ouverte. La classe featureClass
   * doit etre une sous-classe de FT_Feature, sinon renvoie une liste vide. La
...
//  public OQLQuery newOQLQuery();
  ...
  /**
   * Renvoie les metadonnees de la classe mappee avec la table theTable. Si
   * theTable est mappee avec plusieurs classes, en renvoie une.
  ...
  /**
   * Calcule DANS LE SGBD l'emprise la table mappee avec la classe (utile pour
   * Oracle ...).
   * @param clazz Cette classe doit heriter de FT_Feature, la table doit
   *          contenir une geometrie.
   */
  public void mbr(Class<?> clazz);

  /**
   * Calcule DANS LE SGBD un index spatial sur la table mappee avec la classe
   * (R-Tree).
   * @param clazz Cette classe doit heriter de FT_Feature, la table doit
   *          contenir une geometrie.
   */
   ...
   * INSERT, UPDATE, DELETE, mais pas SELECT. Utile uniquement pour debugger :
   * on ne doit pas utiliser de SQL direct avec GeOxygene !
  ...
  /**
   * Renvoie l'identifiant maximum de la classe theClass. ATTENTION : La classe
  ...
  /**
   * Renvoie l'identifiant minimum de la classe theClass. ATTENTION : La classe
   * passee en parametre doit avoir un champ "id" de type int (marche pour les
   * FT_Feature). A appeler a l'interieur d'une transaction ouverte.
   */
