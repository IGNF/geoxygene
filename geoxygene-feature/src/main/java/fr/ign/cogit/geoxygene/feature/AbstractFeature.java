...
public abstract class AbstractFeature implements IFeature {
     ...
     * Logger.
     */
    protected static final Logger logger = Logger.getLogger(AbstractFeature.class.getName());
     ...
     * pas a la création aux mêmes featureCollections que son parent ni a la
     * même
     * population.
     * 
    ...
    public void setGeom(IGeometry g) {
        ...
        synchronized (this.featurecollections) {
            IFeatureCollection<IFeature>[] collections = this.featurecollections.toArray(new IFeatureCollection[0]);
            for (IFeatureCollection<IFeature> fc : collections) {
                 ...
                 * !previousGeometry.equals(this.geom))) {
                 * fc.fireActionPerformed(new
                 * FeatureCollectionEvent(fc, this,
    ...
    @Override
    public List<IFeature> getCorrespondants(IFeatureCollection<? extends IFeature> pop) {
     ...
     * propriétés (valeurs d'attributs, opérations et objets en relation)
     * peuvent
     * être accedés de façon générique en mentionnant le nom de la propriété.
     * Pour
     ...
     * de
     * schéma logique correspondant au featureType Route) qui étend
     * FeatureCommun.
     * Les valeurs d'attributs ne sont pas initialisées.
     * 
     * @param featureType
     *            le feature type de l'objet à créer
     * @return l'objet créé
     */
    ...
    public Object getAttribute(GF_AttributeType attribute) {
        ...
        if (attribute.getMemberName() != null && attribute.getMemberName().equals("topo")) { //$NON-NLS-1$
            AbstractFeature.logger.warn("WARNING : Pour Récupèrer la primitive topologique par défaut, veuillez utiliser " //$NON-NLS-1$
        ...
        if (((AttributeType) attribute).getNomField().length() == 0) {
            ...
            nomFieldMaj = Character.toUpperCase(((AttributeType) attribute).getNomField().charAt(0)) + ((AttributeType) attribute).getNomField().substring(1);
        }
        ...
        while (!classe.equals(Object.class)) {
            try {
                ...
                if (AbstractFeature.logger.isDebugEnabled()) {
                    AbstractFeature.logger.debug("SecurityException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe);
                }
            } catch (IllegalArgumentException e) {
                if (AbstractFeature.logger.isDebugEnabled()) {
                    AbstractFeature.logger.debug("IllegalArgumentException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe);
                }
                ...
                if (AbstractFeature.logger.isDebugEnabled()) {
                    AbstractFeature.logger.debug("IllegalAccessException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe);
                }
            } catch (InvocationTargetException e) {
                if (AbstractFeature.logger.isDebugEnabled()) {
                    AbstractFeature.logger.debug("InvocationTargetException pendant l'appel de la méthode " + nomGetFieldMethod + " sur la classe " + classe);
                }
            }
        ...
        AbstractFeature.logger.error("Echec de l'appel à la méthode " + nomGetFieldMethod + " sur la classe " + this.getClass());
        return null;
    }
    ...
    public void setAttribute(GF_AttributeType attribute, Object valeur) {
        if (attribute.getMemberName().equals("geom")) { //$NON-NLS-1$
            AbstractFeature.logger.warn("WARNING : Pour affecter la primitive géométrique par défaut, veuillez utiliser " //$NON-NLS-1$
            ...
            AbstractFeature.logger.warn("WARNING : Pour affecter la primitive topologique par défaut, veuillez utiliser " //$NON-NLS-1$
                    + "la méthode IFeature.getTopo() et non pas MdFeature.getAttribute(AttributeType attribute)"); //$NON-NLS-1$
            ...
            try {
                ...
                try {
                    Method methodSetter = this.getClass().getDeclaredMethod(nomSetFieldMethod, AbstractFeature.class2PrimitiveClass(valeur.getClass()));
        ...
        try {
            ...
            if (((AssociationRole) role).getNomFieldAsso() == null) {
                ...
                if (((AssociationRole) role).getNomFieldClasse().length() == 0) {
                    ...
                    nomFieldClasseMaj = Character.toUpperCase(((AssociationRole) role).getNomFieldClasse().charAt(0))
            ...
            // cas ou il y a une classe association
                ...
                if (AbstractFeature.logger.isDebugEnabled()) {
                    AbstractFeature.logger.debug("classe association : " + role.getAssociationType().getTypeName());
                }
                ...
                if (AbstractFeature.logger.isDebugEnabled()) {
                    AbstractFeature.logger.debug("cardMax de " + role.getMemberName() + " = " + role.getCardMax());
                }
                if (!role.getCardMax().equals("1")) {
                    if (AbstractFeature.logger.isDebugEnabled()) {
                        AbstractFeature.logger.debug("invocation de " + methodGetter.getName() + " sur le feature " + this.getId());
                    }
                    listInstancesAsso.addAll((List<IFeature>) methodGetter.invoke(this, (Object[]) null));
                    if (AbstractFeature.logger.isDebugEnabled()) {
                        AbstractFeature.logger.debug("nb instances d'association = " + listInstancesAsso.size());
                    }
                } else {
                    listInstancesAsso.add((IFeature) methodGetter.invoke(this, (Object[]) null));
                    if (AbstractFeature.logger.isDebugEnabled()) {
                        AbstractFeature.logger.debug("nb instances d'association = " + listInstancesAsso.size());
                    }
                }
                ...
                for (int i = 0; i < listRolesAGarder.size(); i++) {
                    ...
                    if (AbstractFeature.logger.isDebugEnabled()) {
                        AbstractFeature.logger.debug("role exploré = " + roleExplore.getMemberName());
                    }
                    String nomFieldAssoMaj;
                    if (roleExplore.getNomFieldAsso().length() == 0) {
                        nomFieldAssoMaj = roleExplore.getNomFieldAsso();
                    } else {
                        nomFieldAssoMaj = Character.toUpperCase(roleExplore.getNomFieldAsso().charAt(0)) + roleExplore.getNomFieldAsso().substring(1);
                    }
                    nomGetMethod = "get" + nomFieldAssoMaj;
                    methodGetter = classeAsso.getDeclaredMethod(nomGetMethod, (Class[]) null);
                    if (AbstractFeature.logger.isDebugEnabled()) {
                        AbstractFeature.logger.debug("methode de la classe-asso pour recuperer les instances en relation = " + methodGetter.getName());
                    }
                    /**
                     * je vais chercher les objets en relation via chaque
                     * instance de
                     * classe-association
                     */
                    for (int j = 0; j < listInstancesAsso.size(); j++) {
                        ...
                        if (AbstractFeature.logger.isDebugEnabled()) {
                            AbstractFeature.logger.debug("instance = " + listInstancesAsso.get(j).getId());
                        }
                        if (AbstractFeature.logger.isDebugEnabled()) {
                            AbstractFeature.logger.debug("class  " + listInstancesAsso.get(j).getClass());
                        }
                        if (AbstractFeature.logger.isDebugEnabled()) {
                            AbstractFeature.logger.debug(methodGetter.invoke(listInstancesAsso.get(j), (Object[]) null));
                        }
    ...
    public List<? extends IFeature> getRelatedFeatures(String nomFeatureType, String nomRole) {
        ...
        GF_FeatureType ftt = ((FeatureType) this.getFeatureType()).getSchema().getFeatureTypeByName(nomFeatureType);
        ...
        if ((ftt == null) || (role == null)) {
            AbstractFeature.logger.error("Le FeatureType " + nomFeatureType + " ou l'AssociationRole " + nomRole
    ...
    /**
     * marqueur de suppression d'un objet (utilisé par exemple en
     * généralisation)
     */
     ...
     * @param classe
     *            classe
     * @return la classe correspondant à un type primitif ou null si le
     *         paramètre
