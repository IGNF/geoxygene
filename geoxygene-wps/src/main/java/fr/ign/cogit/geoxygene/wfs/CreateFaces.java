package fr.ign.cogit.geoxygene.wfs;
...
public class CreateFaces implements GeoServerProcess {
  private final static Logger logger = Logger.getLogger(CreateFaces.class.getName());
  ...
  public SimpleFeatureCollection execute(
    ...
    logger.info("Start Converting");
    try {
      SimpleFeatureCollection faces = GeOxygeneGeoToolsTypes.convert2FeatureCollection(map
          .getPopFaces(), features.getSchema().getCoordinateReferenceSystem());
      logger.info("End Converting");
      return faces;
    } catch (Exception e) {
      e.printStackTrace();
