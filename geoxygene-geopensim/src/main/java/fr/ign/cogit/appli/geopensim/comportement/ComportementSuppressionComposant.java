 ...
 *
 */
public class ComportementSuppressionComposant extends Comportement {
	private static Logger logger=Logger.getLogger(ComportementSuppressionComposant.class.getName());
	...
	public void declencher(Agent agent) {
		...
		if (agent instanceof AgentMeso) {
			...
			AgentGeographique composantASupprimer = null;
			for(AgentGeographique composant:agentMeso.getComposants()) {
				composant.calculerSatisfaction();
				double satisfaction = composant.getSatisfaction();
				if (satisfaction < satisfactionMin) {
					satisfactionMin = satisfaction;
					composantASupprimer = composant;
				}
			}
			if (composantASupprimer!=null) {
				composantASupprimer.remove();
				agentMeso.removeComposant(composantASupprimer);
			}
		} else {
