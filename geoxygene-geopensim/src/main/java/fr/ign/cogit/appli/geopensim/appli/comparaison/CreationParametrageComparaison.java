 ...
 *
 */
public class CreationParametrageComparaison extends JDialog implements ActionListener{


	private static final long serialVersionUID = -2410212071568927811L;
	private static final Logger logger = Logger.getLogger(CreationParametrageComparaison.class.getName());
	...
	HashMap<String,List<ParametresComparaison>> parametrage = new HashMap<String,List<ParametresComparaison>>();
	
	
	// Constructeur
	public CreationParametrageComparaison(JFrame parent, String nom,List<ParametresComparaison> listParam){
		this(parent);
		
		// Le nom du paramétrage
		nomParametrage.setText(nom);
	
		// Les méthodes de comparaison
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel(); 
		for (ParametresComparaison param:listParam){
			Object[] obj = new Object[3];
			obj[0] = param.getMethode();
			obj[1] = param.getTypeOperation();
			obj[2] = param.getPonderation().toString();
	...
	public CreationParametrageComparaison(JFrame parent){

		// La fenêtre
		super (parent,"Choix des paramètres de comparaison",true);		
	...
	public HashMap<String, List<ParametresComparaison>> getParametrage(){
		if (valider)	return parametrage;
		else return null;
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(boutonValidation)){
			...
			// Validation
			valider = true;
			parametrage.put(nouveauNom, listeParametresComparaison);
			// Fermeture de la fenêtre
			setVisible(false);	
			dispose();
