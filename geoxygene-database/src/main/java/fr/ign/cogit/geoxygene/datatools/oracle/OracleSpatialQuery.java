...
public class OracleSpatialQuery {
	static Logger logger=Logger.getLogger(OracleSpatialQuery.class.getName());
	...
	public static double length (GM_Object isoGeom)  {
		try {
			...
			if (sdoGeom.getGeometryType() == Curve.class) return ((Curve)sdoGeom).length();
				...
				logger.warn("### ATTENTION oracle.SpatialQuery.length() : mauvais type - renvoie -1 ###");
				return -1.0;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static double area (GM_Object isoGeom)  {
		try {
			...
			if (sdoGeom.getGeometryType() == Surface.class) return ((Surface)sdoGeom).area();
				...
				logger.warn("### ATTENTION oracle.SpatialQuery.area() : mauvais type - renvoie -1 ###");
				return -1.0;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static double perimeter (GM_Object isoGeom)  {
		try {
			...
			if (sdoGeom.getGeometryType() == Surface.class) return ((Surface)sdoGeom).perimeter();
				...
				logger.warn("### ATTENTION oracle.SpatialQuery.perimeter() : mauvais type - renvoie -1 ###");
				return -1.0;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static GM_Object bufferAgregat (Geodatabase data, double tolerance, double radius, GM_Object geom) {
		...
		try {
			...
			if (!(GM_Aggregate.class).isAssignableFrom(geom.getClass())) {
				logger.warn("le GM_Object doit etre un agregat (GM_Aggregate ou sous-classe)");
				logger.warn("le calcul de buffer renvoie NULL");
				return null;
			}
			...
			for(GM_Object o:aggr) {
				i++;
				STRUCT str = (STRUCT) GeometryConvertor.GM_Object2Sdo(o, conn);
	...
	public static void spatialIndex(Geodatabase data, Class<?> clazz) {
		try {
			...
			if (indexName.compareTo("") == 0) {
				if (logger.isDebugEnabled()) logger.debug("index spatial sur "+tableName+" inexistant...");
				...
				if (logger.isDebugEnabled()) logger.debug("creation index...");
				...
				stm.executeQuery(query);
				if (logger.isDebugEnabled()) logger.debug("index spatial sur "+tableName+" cree (R-Tree) - nom : "+indexName);
			} else {
				if (logger.isDebugEnabled()) logger.debug("index spatial sur "+tableName+" existant...");

				if (logger.isDebugEnabled()) logger.debug("reconstruction index...");
				query = "ALTER INDEX "+indexName+" REBUILD";
				stm.executeQuery(query);
				if (logger.isDebugEnabled()) logger.debug("index spatial sur "+tableName+" reconstruit (R-Tree) - nom : "+indexName);
			}
		...
		try {
			...
			if (sqlDiminfo == null) {
				...
				if (dim == 3) {
					...
					ResultSet Z = sqlDiminfo.getResultSet(3,1);
					Z.next();
			...
			// close and commit
			logger.warn(tableName+" : emprise de la table renseignee");
			stm.close();
			conn.commit();
