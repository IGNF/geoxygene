 ...
 * class="donnees.julien.Troncon_cours_eau"/&gt; <br/>
 * 
...
public class TestHibernate {
	...
	public void testTronconCoursEau() {
		...
		// -----------------------------------------------------------------------------
		// Test : chargement des objets déjà dans la base
		List<Troncon_cours_eau> resultTroncon_cours_eau = db.loadAll(Troncon_cours_eau.class);
		...
		IGeometry g = new GM_LineString(liste);
		g.setCRS(2154);
		t.setGeom(g);
		...
		// On relit le cours d'eau et on vérifie quelques éléments
		Troncon_cours_eau t2 = db.load(Troncon_cours_eau.class, t.getId());
		Assert.assertEquals(t.getId(), t2.getId());
		Assert.assertEquals("géométrie sauvegardée et relue",
			"LINESTRING (10000.0 10000.0 100.0, 20000.0 10000.0 10.0)",
			t2.getGeom().toString()
		);
		Assert.assertEquals("SRID sauvegardé et relu", 
			2154,
			t2.getGeom().getCRS()
		);
