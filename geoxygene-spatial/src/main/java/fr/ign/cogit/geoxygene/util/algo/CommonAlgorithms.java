...
import fr.ign.cogit.geoxygene.api.spatial.coordgeom.*;
...
public class CommonAlgorithms {
   ...
   * @return nearest point from geom1
   */
   ...
   * @return
   */
  public static IDirectPositionList getNearestPoints(IEnvelope env1, IEnvelope env2) {
  ...
  public static IGeometry buffer(IGeometry geom, int quad, int cap, int join,
    ...
    IGeometry geom_ = null;
    try {
      geom_ = AdapterFactory.toGM_Object(BufferOp.bufferOp(AdapterFactory.toGeometry(new GeometryFactory(), geom), distance, new BufferParameters(quad, cap, join, mitre)));
    } catch (Exception e) {
      e.printStackTrace();
    }
    return geom_;
