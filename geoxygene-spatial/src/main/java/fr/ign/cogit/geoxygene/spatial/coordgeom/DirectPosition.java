...
public class DirectPosition implements IDirectPosition {
    ...
    /**
     * Constructeur par défaut (3D): crée un tableau de coordonées à 3
     * dimensions,
     * vide.
     */
    ...
    @Override
    public void move(final double offsetX, final double offsetY, final double offsetZ) {
        this.coordinate[0] += offsetX;
        this.coordinate[1] += offsetY;
    ...
    public int hashCode() {
        if (this.coordinate.length == 2) {
            return Double.valueOf(this.getX()).hashCode() ^ Double.valueOf(this.getY()).hashCode();
        }
