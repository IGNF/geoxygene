...
public class MorphologyTransform {
    ...
    public IGeometry opening(IPolygon polygon) {
        ...
        // now the dilatation
        IGeometry dilatated = intBuffer.buffer(bufferSize, bufferStep, capForm, capForm);
    ...
    public IPolygon closing(IPolygon polygon) {
        try {
            IPolygon buffer = (IPolygon) polygon.buffer(bufferSize, bufferStep, capForm, capForm);
    ...
    public IMultiSurface<IPolygon> closingMultiPolygon(IMultiSurface<IPolygon> geom) {
        try {
            IMultiSurface<IPolygon> buffer = (IMultiSurface<IPolygon>) geom.buffer(bufferSize, bufferStep, capForm,
                    capForm);
            // the buffered geometry is slightly filtered to avoid small
            // geometry
            // problems
            IGeometry filtered = Filtering.DouglasPeucker(buffer, 1.0);
            IMultiSurface<IPolygon> closed = erosionMultiPolygon((IMultiSurface<IPolygon>) filtered);
            return closed;
        } catch (Exception e) {
            return geom;
        }
    ...
    public IGeometry closing(IMultiSurface<IOrientableSurface> geom) {
        try {
            IMultiSurface<IOrientableSurface> buffer = (IMultiSurface<IOrientableSurface>) geom.buffer(bufferSize,
                    bufferStep, capForm, capForm);
            // the buffered geometry is slightly filtered to avoid small
            // geometry
            // problems
            IGeometry filtered = Filtering.DouglasPeucker(buffer, 1.0);
    ...
    public IGeometry erosionNoHole(IPolygon geom) {
        ...
        // buffer the exterior
        IPolygon buffer = (IPolygon) exterior.buffer(bufferSize, bufferStep, capForm, capForm);
        ...
        if (buffer.getInterior().size() == 1) {
            ...
            IMultiSurface<IOrientableSurface> surfComp = new GM_MultiSurface<IOrientableSurface>(lOS);
            return surfComp;
        }
    }
    ...
    public IGeometry erosion(IPolygon geom) {
        ...
        for (IRing hole : geom.getInterior()) {
            ...
            // buffer this polygon
            IGeometry buffer = polygon.buffer(bufferSize, bufferStep, capForm, capForm);
     ...
     *            : the geometry to erode.
     */
    public IMultiSurface<IPolygon> erosionMultiPolygon(IMultiSurface<IPolygon> geom) {
    ...
    public IPolygon minkowskiSumWithCustomPolyCentr(IPolygon polygon, IPolygon polyToSum, IDirectPosition centroid) {
        if (polygon == null)
            return null;
        ...
        IPolygon extSum = minkowskiSumWithCustomPolyCentrNoHole(polyNoHole, polyToSum, centroid);
        // loop on the inner rings to reduce each one and add it as a hole for
        // extSum
        for (IRing inner : polygon.getInterior()) {
            IGeometry innerDiff = minkowskiDiffWithCustomPolyCentrNoHole(new GM_Polygon(inner), polyToSum, centroid);
            if (innerDiff == null)
                continue;
    ...
    public IGeometry minkowskiDiffWithCustomPolyCentr(IPolygon polygon, IPolygon polyToSum, IDirectPosition centroid) {
        // easy case: no holes in polygon
        if (polygon.getInterior().size() == 0) {
            return minkowskiDiffWithCustomPolyCentrNoHole(polygon, polyToSum, centroid);
        }

        // case with holes in polygon: the final polygon should be the minkowski
        // difference of the exterior and the minkowski sum for the inner rings.
        IPolygon polyNoHole = new GM_Polygon(polygon.getExterior());
        IGeometry extSum = minkowskiDiffWithCustomPolyCentrNoHole(polyNoHole, polyToSum, centroid);
        // loop on the inner rings to reduce each one and add it as a hole for
        // extSum
        for (IRing inner : polygon.getInterior()) {
            IGeometry innerDiff = minkowskiDiffWithCustomPolyCentrNoHole(new GM_Polygon(inner), polyToSum, centroid);
    ...
    private IPolygon minkowskiSumWithCustomPolyCentrNoHole(IPolygon polygon, IPolygon polyToSum,
        ...
        IPolygon union = (IPolygon) lineLeftDilatationFromPolyCentr(exterior, polyToSum, centroid);
        // if union contains holes, they have to be removed
        return CommonAlgorithmsFromCartAGen.removeHoles(union);
    }

    private IGeometry minkowskiDiffWithCustomPolyCentrNoHole(IPolygon polygon, IPolygon polyToSum,
            IDirectPosition centroid) {
        // get the exterior LineString of polygon
        ILineString exterior = polygon.exteriorLineString();
        // compute the dilatation of the exterior with polyToSum
        IPolygon union = lineLeftDilatationFromPolyCentr(exterior, polyToSum, centroid);
     ...
     * @return
     */
    private IPolygon lineLeftDilatationFromPolyCentr(ILineString line, IPolygon polyToSum, IDirectPosition centroid) {
        ...
        double vp = (point1.getX() - point0.getX()) * (point2.getY() - point0.getY())
        ...
        for (int i = 0; i < line.getControlPoint().size() - 1; i++) {
            ...
            IDirectPosition pointC = line.getControlPoint(i == line.getControlPoint().size() - 2 ? 1 : i + 2);
            ...
            double vectorialProduct = (pointB.getX() - pointA.getX()) * (pointC.getY() - pointA.getY())
            ...
            if (vectorialProduct < 0.0) {
                ...
                while (true) {
                    ...
                    if (afterBC || (tempPoint.equals(positionBC))) {
                        ...
                        afterBC = true;
                        IDirectPosition toAdd = new DirectPosition(pointB.getX() + centroid.getX() - tempPoint.getX(),
                                pointB.getY() + centroid.getY() - tempPoint.getY());
                        tempList.add(toAdd);
            ...
            tempList.add(new DirectPosition(pointB.getX() + centroid.getX() - positionAB.getX(),
                    pointB.getY() + centroid.getY() - positionAB.getY()));

            tempList.add(new DirectPosition(pointA.getX() + centroid.getX() - positionAB.getX(),
     ...
     * @return
     */
    private IDirectPosition getFurthestPoint(IDirectPositionList coord, double theta) {
        ...
        for (IDirectPosition toCompare : coord) {
            ...
            double xRotation = toCompare.getX() * Math.cos(-theta) - toCompare.getY() * Math.sin(-theta);
