...
public abstract class MesureOrientationV2 {
  ...
  /**
   * Orientation of the MBR of the given geometry, with angle petween -PI/2 and
   * PI/2.
   * @param geom
   * @return
   */
  public static double getGeneralOrientation(Geometry geom) {
   ...
   * @return l'orientation de la géométrie : 999.9 si le PPRE n'est pas defini,
   *         ou s'il est carré.
   */
  public static double getOrientationGenerale(Geometry geom) {

    double angle = getGeneralOrientation(geom);
    if (angle < 0)
      angle += Math.PI;
  ...
  /**
   * Qualification de la biscornuité d'un polygone à partir des orientations
   * mesurées.
   */
  public static String getBiscornuite(Geometry geom) {

    // Constantes
    double seuilComparaisonOrientation = Math.PI / 36; // (= 5°) angle en deça
                                                       // duquel l'orientation
                                                       // du batiment et
                                                       // l'orientation des
                                                       // cotes sont considérées
                                                       // identiques
    double seuilIndicateurConfiance = 0.8; // seuil de l'indicateur de confiance
                                           // à partir duquel on considére la
                                           // mesure comme fiable
    ...
    if (!pertinence)
      biscornuite = "aucune_orientation";
    else {
      int nbOrient = mesureOrientation.getNombreOrientation();
      if (nbOrient > 1) {
        double contribSecondaire = mesureOrientation
            .getContributionSecondaire();
        double contribPrincipale = mesureOrientation
            .getContributionPrincipale();
        ...
        double orientationGenerale = getGeneralOrientation(geom);
        double orientationPrincipale = mesureOrientation
            .getOrientationPrincipale();
        double diffOrientation = (Math.abs(orientationGenerale
            - orientationPrincipale));
        if (diffOrientation > (Math.PI / 4))
          diffOrientation = (0.5 * Math.PI) - diffOrientation;
        double indicateur = mesureOrientation
            .getIndicateurOrientationPrincipale();
        if (indicateur >= seuilIndicateurConfiance) { // les murs sont à peu
                                                      // près droits
          if ((diffOrientation >= seuilComparaisonOrientation)
              && (elongation < 0.4)) {
            // l'orientation du batiment est la même que celle des murs
            biscornuite = "escalier_mur_droit";
          } else {// l'orientation du batiment est différente de celle des murs
                  // et le batiment est alongé
