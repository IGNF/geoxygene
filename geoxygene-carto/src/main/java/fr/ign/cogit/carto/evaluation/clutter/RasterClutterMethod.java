...
public class RasterClutterMethod {
	...
	/**
	 * Compute the edge density clutter in a grid version of the image, where
	 * the number of columns of the grid is given (the size of the cells is
	 * computed according to the image size).
	 * 
	 * @param nbColumns
	 * @return
	 */
	public Map<Integer, Map<Integer, Double>> getGridEdgeDensityClutter(
		...
		for (int i = 0; i < nbRows; i++) {
			...
			for (int j = 0; j < nbColumns; j++) {
				...
				TiledImage subImage = image.getSubImage(x, y, cellSize,
						cellSize);
	...
	private double computeEdgeDensityClutter(PlanarImage image) {
		...
		return Math.abs(edgeTotal
				/ (edgeImage.getWidth() * edgeImage.getHeight()));
	}
}
