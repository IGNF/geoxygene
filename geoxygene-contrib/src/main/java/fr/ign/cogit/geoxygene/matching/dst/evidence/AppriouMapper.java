...
public class AppriouMapper {
  ...
  /**
   * 
   * @param hypIndex l'index de l'hypothèse au sein de la liste totale des hypothèses.
   * @param hypsize Nombre d'hypothèses totales
   * @param encoded
   * @return
   * @throws Exception
   */
  public List<Pair<byte[], Float>> mapFocalisedToGlobal(int hypIndex, int hypsize,
    ...
    List<Pair<byte[], Float>> decodeds = new ArrayList<Pair<byte[], Float>>(
        encoded.size());
    ...
    for (Pair<byte[], Float> pair : encoded) {
      ...
      // Non apparié
      else if (Arrays.equals(pair.getFirst(), new byte[] { 0, 1 })) {
