...
public class CriteriaLoader {
  ...
  public static Collection<Source<Hypothesis>> load(FactoryType type) {
    ...
    if (type == FactoryType.LINEAR) {
      path += "linear";
    } else if (type == FactoryType.SURFACE) {
      path += "surface";
    } else if (type == FactoryType.PUNCTUAL) {
      path += "point";
    }
  ...
  private static Collection<Source<Hypothesis>> load(String path) {
    ...
    File packagefile = new File(CriteriaLoader.class.getResource(realPath)
        .getFile());
    for (File file : packagefile.listFiles()) {
      ...
      try {
        ...
        System.out.println("cannot cast"
            + ((clazz == null) ? "null" : clazz.getName()) + " to Source");
        continue;
      }
    }
