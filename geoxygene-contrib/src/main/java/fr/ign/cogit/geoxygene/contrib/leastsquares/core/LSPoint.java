...
public class LSPoint extends AbstractFeature {
	...
	LSPoint(IFeature object, IDirectPosition pt, double position, GeometryType type, boolean pointExtr, boolean fixe,
	...
	private void assembleContraintesInternes() {
		boolean prems = true;
		Iterator<LSInternalConstraint> iter = this.getInternalConstraints().iterator();
		while (iter.hasNext()) {
			...
			for (IFeature obj : this.objs) {
				...
				this.getSystemeLocal()
						.setNonNullValues(this.getSystemeLocal().getNonNullValues() + systeme.getNonNullValues());
				...
				for (int i = 0; i < lignes; i++) {
					for (int j = 0; j < colonnes; j++) {
						...
						if (!(j + 1 >= this.getSystemeLocal().getUnknowns().size())) {
							if (inconnue.equals(this.getSystemeLocal().getUnknowns().get(j + 1))) {
								estX = true;
							}
						}
						...
						this.getSystemeLocal().setA(i, j, systeme.getA(i - local.getRowNumber(), colonne));
	...
	/**
	 * <p>
	 * On instancie le set des contraintes internes qui s'appliquent � ce point. Il
	 * s'agit de cr�er les objets java correspondant aux mapspecs.
	 * 
	...
	public void setContraintesInternes(MapspecsLS mapspecs, LSScheduler sched)
		...
		if (ReflectionUtil.containsClassOrSuper(mapspecs.getClassesRigides(), className)) {
			iter = mapspecs.getContraintesRigides().iterator();
		} else if (ReflectionUtil.containsClassOrSuper(mapspecs.getClassesMalleables(), className)) {
		...
		while (iter.hasNext()) {
			...
			Method applyMethod = constraintClass.getDeclaredMethod("appliesTo", LSPoint.class);
			if (!(Boolean) applyMethod.invoke(null, this)) {
				continue;
			}
			Constructor<?> constr = constraintClass.getConstructor(LSPoint.class, LSScheduler.class);
			this.internalConstraints.add((LSInternalConstraint) constr.newInstance(this, sched));
	...
	/**
	 * <p>
	 * On instancie le set des contraintes externes qui s'appliquent à ce point. Il
	 * s'agit de créer les objets java correspondant aux mapspecs.
	 * 
	 * @throws ClassNotFoundException
	 * 
	 */
	void setContraintesExternes(MapspecsLS mapspecs, LSScheduler sched) throws ClassNotFoundException {
		...
		for (IFeature obj : this.objs) {
			...
			while (iter.hasNext()) {
				...
				if ((!nomClasse1.equals(classePt)) && (!classe1.isAssignableFrom(classePt))
						&& (!classe2.equals(classePt)) && (!classe2.isAssignableFrom(classePt))) {
					continue;
				}
				...
				if (constrClass.equals(LSCoalescenceConstraint.class)) {
					...
					LSCoalescenceConstraint contr = new LSCoalescenceConstraint(this, obj, obj, sched);
					contr.seuilSep = mapspecs.getContraintesExternes().get(contrainte).doubleValue();
					this.externalConstraints.add(contr);
				} else if (constrClass.equals(LSProximityConstraint.class)) {
					// on cherche les conflits
					double distance = mapspecs.getContraintesExternes().get(contrainte).doubleValue();

					// on met la distance à l'échelle
					double dist_req = distance * sched.getMapspec().getEchelle() / 1000.0;

					Map<LSSpatialConflict, Set<IFeature>> conflits = this.rechercheConflitsTIN(dist_req, sched,
							autreClasse);
					for (LSSpatialConflict conflit : conflits.keySet()) {
						for (IFeature voisin : conflits.get(conflit)) {
							LSProximityConstraint contr = new LSProximityConstraint(this, obj, voisin, sched, conflit);
	 ...
	 * 
	 */
	public Map<LSSpatialConflict, Set<IFeature>> rechercheConflitsTIN(double distance, LSScheduler sched,
			Class<?> classeVoisin) {
		...
		for (LSSpatialConflict conflit : sched.getConflits()) {
			if (conflit.getPoint1().equals(this)) {
				...
				if (voisinsConf.size() != 0) {
					LSScheduler.logger.finest("conflict " + conflit + " has distance: " + conflit.distance());
					System.out.println("conflict " + conflit + " has distance: " + conflit.distance());
	 ...
	 * 
	 */
	public Set<IFeature> rechercheConflits(double distance, LSScheduler sched, IFeatureCollection<IFeature> neighbours)
		...
		Collection<IFeature> setVoisins = neighbours.select(this.getIniPt(), 1.25 * dist);
		...
		for (IFeature obj : this.objs) {
			if (sched.getMapspec().getClassesMalleables().contains(obj.getClass())) {
				...
				Collection<IFeature> objsInter = neighbours.select((ILineString) obj.getGeom());
				...
				for (IFeature voisin : setVoisins) {
					...
					// g�n�ralis�s
					if ((!sched.getObjsFixes().contains(voisin)) && (!sched.getObjsMalleables().contains(voisin))
					...
					if (objsInter.contains(voisin)) {
						...
						IDirectPosition interPt = CommonAlgorithmsFromCartAGen
								.getCommonVertexBetween2Lines((ILineString) geom, (ILineString) geomVoisin);
						int index1 = CommonAlgorithmsFromCartAGen.getNearestVertexPositionFromPoint(geom, interPt);
						int index2 = CommonAlgorithmsFromCartAGen.getNearestVertexPositionFromPoint(geom,
								this.getIniPt());

						// on calcule la distance entre les deux indices
						double dist2 = CommonAlgorithmsFromCartAGen.getLineDistanceBetweenIndexes((ILineString) geom,
								index1, index2);
						if (dist2 >= 2 * dist2) {
				...
				// dans les conflits
				for (IFeature voisin : setVoisins) {
					if ((!sched.getObjsRigides().contains(voisin)) && (!sched.getObjsFixes().contains(voisin))
	...
	/**
	 * Détermine si deux points sont voisins au sens de (Harrie, 2002) : même objet
	 * et à moins de 8 vertices l'un de l'autre.
	 * 
	 * @param point2
	 * @return
	 */
	public boolean estVoisin(LSScheduler sched, LSPoint point2) {
		...
		IGeometry geom = obj.getGeom();
		if (sched.getObjsMalleables().contains(obj) && geom.coord().size() < sched.getMapObjPts().get(obj).size()) {
			geom = LineDensification.densification2(geom, sched.getMapspec().getDensStep());
		}
	...
	public void setContraintesInternes(Set<LSInternalConstraint> contraintesInternes) {
