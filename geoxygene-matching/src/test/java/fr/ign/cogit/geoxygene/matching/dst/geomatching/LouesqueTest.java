...
public class LouesqueTest {
  
  private final static Logger LOGGER = Logger.getLogger(LouesqueTest.class);
  ...
  public void testAppriou1Critere() throws Exception {
    ...
    LOGGER.info("====================================================================================");
    LOGGER.info("Appariement Louesque - 1 critère (géométrique)");
    ...
    IPopulation<IFeature> oronymePop = ShapefileReader.read("./data/pic-louesque/oronyme_pic_louesque.shp", "Oronyme", null, true);
    IPopulation<IFeature> pointReliefPop = ShapefileReader.read("./data/pic-louesque/point_remarq_relief_pic_louesque.shp", "Relief", null, true);
    
    Assert.assertEquals(1, pointReliefPop.size());
    Assert.assertEquals(5, oronymePop.size());
    ...
    Assert.assertEquals(0.3782916, result.getValue(), 0.0001);
    Assert.assertEquals(1, result.getHypothesis().size());
    Assert.assertEquals("col de louesque", result.getHypothesis().get(0).getAttribute("NOM").toString().trim());
  ...
  public void testAppriou2Criteres() throws Exception {
    ...
    List<Source<IFeature, GeomHypothesis>> criteria = new ArrayList<Source<IFeature, GeomHypothesis>>();
    LOGGER.info("====================================================================================");
    LOGGER.info("Appariement Louesque - 2 critères (géométrique + toponyme)");
    ...
    Assert.assertEquals(0.565147, result.getValue(), 0.0001);
    Assert.assertEquals(1, result.getHypothesis().size());
    Assert.assertEquals("col de louesque", result.getHypothesis().get(0).getAttribute("NOM").toString().trim());
  ...
  public void testAppriou3Criteres() throws Exception {
    ...
    List<Source<IFeature, GeomHypothesis>> criteria = new ArrayList<Source<IFeature, GeomHypothesis>>();
    LOGGER.info("====================================================================================");
    LOGGER.info("Appariement Louesque - 3 critères (géométrique + toponyme + wp)");
    ...
    Assert.assertEquals(0.5234, result.getValue(), 0.0001);
    Assert.assertEquals(1, result.getHypothesis().size());
    Assert.assertEquals("soum de louesque", result.getHypothesis().get(0).getAttribute("NOM").toString().trim());
