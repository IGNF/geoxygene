...
public class LevenshteinDist extends GeoSource {
  ...
  public double[] evaluate(IFeature ref, GeomHypothesis candidate) {
    ...
    Object bdcNature = ref.getAttribute(attributeName1);
    ...
    if (bdcNature != null && bdnNature != null) {
      ...
      // DST_LOGGER.info("        distance = " + distance + "(bdcNature = " +
      // bdcNature.toString() + ", bdnNature = " + bdnNature.toString() + ")");
      DST_LOGGER.info("        distance = " + distance + " (bdc = "
          + bdcNature.toString() + ", bdn = " + bdnNature.toString() + ")");
      // System.out.println(" bdcNature = " + bdcNature.toString() +
      // ", bdnNature = " + bdnNature.toString() + " => " + distance);
      ...
      return masses;
      /*
       * for (int i = 0; i < ref.getFeatureType().getFeatureAttributes().size();
       * i++) {
       * System.out.print(ref.getFeatureType().getFeatureAttributes().get(
       * i).getMemberName()+", "); } System.out.print(" -- "); for (int i = 0; i
       * < candidate.getFeatureType().getFeatureAttributes().size(); i++) {
       * System
       * .out.print(candidate.getFeatureType().getFeatureAttributes().get(i
       * ).getMemberName()+", "); } System.out.println("");
       */
  ...
  /**
   * Mesure de similarité définie par Samal, A., Seth, S. et Cueto, K. A
   * feature-based approach to conflation of geospatial sources. IJGIS,
   * juillet-août 2004, vol. 18, n°5, p. 459-489
   * 
   * @param s
   * @param t
   * @return int as coefficient de similarité
   */
