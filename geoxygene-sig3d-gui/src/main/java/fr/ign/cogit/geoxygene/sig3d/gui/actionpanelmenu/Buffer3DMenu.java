...
import fr.ign.cogit.geoxygene.sig3d.calculation.minkowskisum.VContribution.ContributionAlgorithmBuffer3D;
import fr.ign.cogit.geoxygene.sig3d.calculation.minkowskisum.vunion.UnionAlgorithmBuffer3D;
  ...
  public void actionPerformed(ActionEvent actionevent) {
    ...
    if (source == this.ok) {
      ...
      if (ind == 0) {
    	  ...
    	  featColl=ContributionAlgorithmBuffer3D.offsetting(feat1, intTailleBuffer,
            intDetailSphere);

      } else {

       featColl= UnionAlgorithmBuffer3D.offsetting(feat1, intTailleBuffer,
            intDetailSphere);
        
  
      }

      if(featColl == null || featColl.isEmpty()){
    	  return;
      }
      
      this.iMap3D.getCurrent3DMap().removeLayer(UnionAlgorithmBuffer3D.DECOMP);
      Color c = new Color((int) (Math.random() * 255),
          (int) (Math.random() * 255), (int) (Math.random() * 255));
      // On ajoute les triangles à la carte
      this.iMap3D.getCurrent3DMap().addLayer(new VectorLayer(featColl, UnionAlgorithmBuffer3D.DECOMP, true,
          c, 1, true));
