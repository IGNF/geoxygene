...
public class LightMenu extends JPanel implements ActionListener {
	...
	private final static Logger logger = Logger.getLogger(CameraAnimation.class.getName());
	...
	public LightMenu(InterfaceMap3D iMap3D) {
		...
		// Titre
		this.setBorder(new TitledBorder(new EtchedBorder(), Messages.getString("FenetreLumiere.Title")));
		...
		labelPosDefinies.setText(Messages.getString("FenetreLumiere.PredifinedPositions"));
		...
		this.jTFX = new JTextField(String.valueOf(point.x - this.iMap3D.getTranslate().x));
		...
		this.jTFY = new JTextField(String.valueOf(point.y - this.iMap3D.getTranslate().y));
		...
		this.jTFZ = new JTextField(String.valueOf(point.z - this.iMap3D.getTranslate().z));
		...
		this.jBCLightColor
				.setBackground(new Color((int) (255 * color.x), (int) (255 * color.y), (int) (255 * color.z)));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == this.ok) {
			...
			this.iMap3D.getLights().get(LightMenu.index).setColor(new Color3f((float) couleur.getRed() / 255,
	 ...
	 * représente l'emprise de la carte et les numéros indiquent les positions 1
	 * 2 3 ________ YMAX | | | | 4 | 5 | 6 | | |________| YMIN 7 8 9 XMIN XMAX
	 * 
	 * @param pos
	 */
	 ...
	 * 
	 * @param newIndex
	 *            l'indice de l'objet sélectionné dans la liste des lumières
	...
	public void setIndex(int newIndex) {
		...
		this.jTFX.setText("" + this.tbl.getValueAt(LightMenu.index, LightsListTableModel.IND_X));
		this.jTFY.setText("" + this.tbl.getValueAt(LightMenu.index, LightsListTableModel.IND_Y));
		this.jTFZ.setText("" + this.tbl.getValueAt(LightMenu.index, LightsListTableModel.IND_Z));

		this.jBCLightColor.setBackground((Color) this.tbl.getValueAt(LightMenu.index, LightsListTableModel.IND_COL));
	}
