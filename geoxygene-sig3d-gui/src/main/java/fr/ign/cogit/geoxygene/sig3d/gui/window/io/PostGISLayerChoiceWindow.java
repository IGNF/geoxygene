 ...
 * @version 0.1
 * 
 *          Fenetre permettant de sélectionner des couches dans une BDD PostGIS
 *          Window to select geometric table in PostGIS
 */
public class PostGISLayerChoiceWindow extends JDialog implements ActionListener {
	...
	public static void main(String[] args){
	  (new PostGISLayerChoiceWindow(null, new ArrayList<String>(), "localhost", "Lyon_Databses", "5432", "public","postgres", "postgres")).setVisible(true);;
	  
  }

	/**
	 * Initialise une fenêtre permettant d'ajouter une couche PostGIS à une
	 * carte à partir d'informations nécessaires
	 * 
	 * @param iMap3D
	 *            la carte qui se veerra ajoutée la couche
	 * @param layerName
	 *            la liste des noms de couche
	 * @param host
	 *            le nom de l'hote
	 * @param database
	 ...
	 *            le mot de passe
	 */
	public PostGISLayerChoiceWindow(InterfaceMap3D iMap3D, List<String> layerName, String host, String database,
			String port, String schema, String user, String pw) {
		...
		jscrollpan.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		jscrollpan.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(this.ok)) {
			...
			if (nbCouchesSelectionnes == 0) {
				...
				JOptionPane.showMessageDialog(null, Messages.getString("FenetreChargement.PostGISNoLayer"), //$NON-NLS-1$
			...
			for (int i = 0; i < nbCouchesSelectionnes; i++) {
				...
				try {
					...
					featColl = PostgisManager.loadGeometricTable(this.host, this.port, this.database, this.schema, nomActu,
							this.user, this.pw);
				} catch (Exception ex) {
				...
				if (dimension == 2) {
					...
					try {
						(new Feature2DLoadingWindow(this.iCarte3D, featColl)).setVisible(true);
					} catch (Exception e1) {
