 ...
 * 
 * @see <a href="http://www.brucelindbloom.com/">http://www.brucelindbloom.com/
 *      </a> for the equations.<br/>
...
public class CIELchColorSpace extends ColorSpace {
	...
	/**
	 * @param use_awt_XYZ_ColorSpace
	 *            : either use the AWT XYZ color space or the custom Geoxygene
	 *            XYZ space to convert colors between sRGB and CIE L*c*h.
	 */
	 ...
	 * 
	 * @param a
	 *            color in CIE Lch(ab) coordinates.
	 * @return the color in CIE L*a*b* coordinates
	 */
	public float[] toCIELab(float[] Lch) {
		float L = Lch[0];
		double c = Lch[1];
		double h = Lch[2];
	...
	/**
	 * Convenience method to convert from Cartesian CIELab coordinates to CIE
	 * Lch polar coordinates.
	 * 
	 * @param a
	 *            color in CIE L*a*b* coordinates.
	 * @return the color in CIE Lch(ab) coordinates
	 */
	public float[] fromCIELab(float[] Lab) {
		float L = Lab[0];
		double a = Lab[1];
		double b = Lab[2];
